# Code generated by `ubx sdk gen` from google_sqladmin_v1beta4_connect. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1beta4Connect_DnsNames:
    connection_type: Any = None
    dns_scope: Any = None
    name: Any = None
    record_manager: Any = None

@dataclasses.dataclass
class V1beta4Connect_IpAddresses:
    ip_address: Any = None
    time_to_retire: Any = None
    type: Any = None

@dataclasses.dataclass
class V1beta4Connect_Nodes:
    dns_name: Any = None
    dns_names: Any = None
    ip_addresses: Any = None
    name: Any = None

@dataclasses.dataclass
class V1beta4Connect_ServerCaCert:
    # PEM representation.
    cert: Any = None
    # Serial number, as extracted from the certificate.
    cert_serial_number: Any = None
    # User supplied name. Constrained to [a-zA-Z.-_ ]+.
    common_name: Any = None
    # The time when the certificate was created in [RFC 3339](https://tools.ietf.org/html/rfc3339) format, for example `2012-11-15T16:19:00.094Z`.
    create_time: Any = None
    # The time when the certificate expires in [RFC 3339](https://tools.ietf.org/html/rfc3339) format, for example `2012-11-15T16:19:00.094Z`.
    expiration_time: Any = None
    # Name of the database instance.
    instance: Any = None
    # This is always `sql#sslCert`.
    kind: Any = None
    # The URI of this resource.
    self_link: Any = None
    # Sha1 Fingerprint.
    sha1_fingerprint: Any = None

@dataclasses.dataclass
class V1beta4ConnectConfig:
    instance: Any = None
    project: Any = None
    read_time: Any = None

@dataclasses.dataclass
class V1beta4ConnectAttrs:
    # `SECOND_GEN`: Cloud SQL database instance. `EXTERNAL`: A database server that is not managed by Google. This property is read-only; use the `tier` property in the `settings` object to determine the database type.
    backend_type: Any = None
    # Optional. Output only. Connection name of the Cloud SQL instance used in connection strings, in the format project:region:instance.
    connection_name: Any = None
    # Custom subject alternative names for the server certificate.
    custom_subject_alternative_names: Any = None
    # The database engine type and version. The `databaseVersion` field cannot be changed after instance creation. MySQL instances: `MYSQL_8_0`, `MYSQL_5_7` (default), or `MYSQL_5_6`. PostgreSQL instances: `POSTGRES_9_6`, `POSTGRES_10`, `POSTGRES_11` or `POSTGRES_12` (default), `POSTGRES_13`, or `POSTGRES_14`. SQL Server instances: `SQLSERVER_2017_STANDARD` (default), `SQLSERVER_2017_ENTERPRISE`, `SQLSERVER_2017_EXPRESS`, `SQLSERVER_2017_WEB`, `SQLSERVER_2019_STANDARD`, `SQLSERVER_2019_ENTERPRISE`, `SQLSERVER_2019_EXPRESS`, or `SQLSERVER_2019_WEB`.
    database_version: Any = None
    # The dns name of the instance.
    dns_name: Any = None
    # Output only. The list of DNS names used by this instance.
    dns_names: Any = None
    instance: Any = None
    # The assigned IP addresses for the instance.
    ip_addresses: Any = None
    # This is always `sql#connectSettings`.
    kind: Any = None
    # Optional. Output only. mdx_protocol_support controls how the client uses metadata exchange when connecting to the instance. The values in the list representing parts of the MDX protocol that are supported by this instance. When the list is empty, the instance does not support MDX, so the client must not send an MDX request. The default is empty.
    mdx_protocol_support: Any = None
    # The number of read pool nodes in a read pool.
    node_count: Any = None
    # Output only. Entries containing information about each read pool node of the read pool.
    nodes: Any = None
    project: Any = None
    # Whether PSC connectivity is enabled for this instance.
    psc_enabled: Any = None
    read_time: Any = None
    # The cloud region for the instance. e.g. `us-central1`, `europe-west1`. The region cannot be changed after instance creation.
    region: Any = None
    # SslCerts Resource
    server_ca_cert: Any = None
    # Specify what type of CA is used for the server certificate.
    server_ca_mode: Any = None

V1beta4Connect = ubx.DataSourceBinding(
    wire_type="google_sqladmin_v1beta4_connect",
    fields={
        "instance": ubx.FieldSpec(wire_name="instance"),
        "project": ubx.FieldSpec(wire_name="project"),
        "read_time": ubx.FieldSpec(wire_name="read_time"),
    },
)
