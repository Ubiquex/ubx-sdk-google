# Code generated by `ubx sdk gen` from google_compute_region_autoscaler. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy_CpuUtilization:
    predictive_method: Any = None
    target: Any = None

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy_LoadBalancingUtilization:
    target: Any = None

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy_Metric:
    filter: Any = None
    name: Any = None
    single_instance_assignment: Any = None
    target: Any = None
    type: Any = None

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy_ScaleInControl_MaxScaledInReplicas:
    fixed: Any = None
    percent: Any = None

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy_ScaleInControl:
    time_window_sec: Any = None
    max_scaled_in_replicas: Any = None

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy_ScalingSchedules:
    description: Any = None
    disabled: Any = None
    duration_sec: Any = None
    min_required_replicas: Any = None
    name: Any = None
    schedule: Any = None
    time_zone: Any = None

@dataclasses.dataclass
class RegionAutoscaler_AutoscalingPolicy:
    cooldown_period: Any = None
    max_replicas: Any = None
    min_replicas: Any = None
    mode: Any = None
    stabilization_period: Any = None
    cpu_utilization: Any = None
    load_balancing_utilization: Any = None
    metric: Any = None
    scale_in_control: Any = None
    scaling_schedules: Any = None

@dataclasses.dataclass
class RegionAutoscaler_Timeouts:
    create: Any = None
    delete: Any = None
    update: Any = None

_RegionAutoscaler_AutoscalingPolicy_CpuUtilizationFields = {
    "predictive_method": ubx.FieldSpec(wire_name="predictive_method"),
    "target": ubx.FieldSpec(wire_name="target"),
}

_RegionAutoscaler_AutoscalingPolicy_LoadBalancingUtilizationFields = {
    "target": ubx.FieldSpec(wire_name="target"),
}

_RegionAutoscaler_AutoscalingPolicy_MetricFields = {
    "filter": ubx.FieldSpec(wire_name="filter"),
    "name": ubx.FieldSpec(wire_name="name"),
    "single_instance_assignment": ubx.FieldSpec(wire_name="single_instance_assignment"),
    "target": ubx.FieldSpec(wire_name="target"),
    "type": ubx.FieldSpec(wire_name="type"),
}

_RegionAutoscaler_AutoscalingPolicy_ScaleInControl_MaxScaledInReplicasFields = {
    "fixed": ubx.FieldSpec(wire_name="fixed"),
    "percent": ubx.FieldSpec(wire_name="percent"),
}

_RegionAutoscaler_AutoscalingPolicy_ScaleInControlFields = {
    "time_window_sec": ubx.FieldSpec(wire_name="time_window_sec"),
    "max_scaled_in_replicas": ubx.FieldSpec(
        wire_name="max_scaled_in_replicas",
        kind="list",
        fields=_RegionAutoscaler_AutoscalingPolicy_ScaleInControl_MaxScaledInReplicasFields,
    ),
}

_RegionAutoscaler_AutoscalingPolicy_ScalingSchedulesFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "disabled": ubx.FieldSpec(wire_name="disabled"),
    "duration_sec": ubx.FieldSpec(wire_name="duration_sec"),
    "min_required_replicas": ubx.FieldSpec(wire_name="min_required_replicas"),
    "name": ubx.FieldSpec(wire_name="name"),
    "schedule": ubx.FieldSpec(wire_name="schedule"),
    "time_zone": ubx.FieldSpec(wire_name="time_zone"),
}

_RegionAutoscaler_AutoscalingPolicyFields = {
    "cooldown_period": ubx.FieldSpec(wire_name="cooldown_period"),
    "max_replicas": ubx.FieldSpec(wire_name="max_replicas"),
    "min_replicas": ubx.FieldSpec(wire_name="min_replicas"),
    "mode": ubx.FieldSpec(wire_name="mode"),
    "stabilization_period": ubx.FieldSpec(wire_name="stabilization_period"),
    "cpu_utilization": ubx.FieldSpec(
        wire_name="cpu_utilization",
        kind="list",
        fields=_RegionAutoscaler_AutoscalingPolicy_CpuUtilizationFields,
    ),
    "load_balancing_utilization": ubx.FieldSpec(
        wire_name="load_balancing_utilization",
        kind="list",
        fields=_RegionAutoscaler_AutoscalingPolicy_LoadBalancingUtilizationFields,
    ),
    "metric": ubx.FieldSpec(
        wire_name="metric",
        kind="list",
        fields=_RegionAutoscaler_AutoscalingPolicy_MetricFields,
    ),
    "scale_in_control": ubx.FieldSpec(
        wire_name="scale_in_control",
        kind="list",
        fields=_RegionAutoscaler_AutoscalingPolicy_ScaleInControlFields,
    ),
    "scaling_schedules": ubx.FieldSpec(
        wire_name="scaling_schedules",
        kind="set",
        fields=_RegionAutoscaler_AutoscalingPolicy_ScalingSchedulesFields,
    ),
}

_RegionAutoscaler_TimeoutsFields = {
    "create": ubx.FieldSpec(wire_name="create"),
    "delete": ubx.FieldSpec(wire_name="delete"),
    "update": ubx.FieldSpec(wire_name="update"),
}

@dataclasses.dataclass
class RegionAutoscalerConfig:
    deletion_policy: Any = None
    description: Any = None
    id: Any = None
    name: Any = None
    project: Any = None
    region: Any = None
    target: Any = None
    autoscaling_policy: Any = None
    timeouts: Any = None

RegionAutoscaler = ubx.ResourceBinding(
    wire_type="google_compute_region_autoscaler",
    fields={
        "deletion_policy": ubx.FieldSpec(wire_name="deletion_policy"),
        "description": ubx.FieldSpec(wire_name="description"),
        "id": ubx.FieldSpec(wire_name="id"),
        "name": ubx.FieldSpec(wire_name="name"),
        "project": ubx.FieldSpec(wire_name="project"),
        "region": ubx.FieldSpec(wire_name="region"),
        "target": ubx.FieldSpec(wire_name="target"),
        "autoscaling_policy": ubx.FieldSpec(
            wire_name="autoscaling_policy",
            kind="list",
            fields=_RegionAutoscaler_AutoscalingPolicyFields,
        ),
        "timeouts": ubx.FieldSpec(
            wire_name="timeouts",
            kind="object",
            fields=_RegionAutoscaler_TimeoutsFields,
        ),
    },
)
