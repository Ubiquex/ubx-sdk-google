# Code generated by `ubx sdk gen` from google_cloudsupport_cloudsupport_support_event_subscription. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class CloudsupportSupportEventSubscriptionConfig:
    # Output only. The time at which the subscription was created.
    create_time: Any = None
    # Output only. The time at which the subscription was deleted.
    delete_time: Any = None
    # Output only. Reason why subscription is failing. State of subscription must be FAILING in order for this to have a value.
    failure_reason: Any = None
    # Identifier. The resource name of the support event subscription.
    name: Any = None
    # Required. The name of the Pub/Sub topic to publish notifications to. Format: projects/{project}/topics/{topic}
    pub_sub_topic: Any = None
    # Output only. The time at which the subscription will be purged.
    purge_time: Any = None
    # Output only. The state of the subscription.
    state: Any = None
    # Output only. The time at which the subscription was last updated.
    update_time: Any = None

@dataclasses.dataclass
class CloudsupportSupportEventSubscriptionAttrs:
    # Output only. The time at which the subscription was created.
    create_time: Any = None
    # Output only. The time at which the subscription was deleted.
    delete_time: Any = None
    # Output only. Reason why subscription is failing. State of subscription must be FAILING in order for this to have a value.
    failure_reason: Any = None
    # Identifier. The resource name of the support event subscription.
    name: Any = None
    # Required. The name of the Pub/Sub topic to publish notifications to. Format: projects/{project}/topics/{topic}
    pub_sub_topic: Any = None
    # Output only. The time at which the subscription will be purged.
    purge_time: Any = None
    # Output only. The state of the subscription.
    state: Any = None
    # Output only. The time at which the subscription was last updated.
    update_time: Any = None

CloudsupportSupportEventSubscription = ubx.ResourceBinding(
    wire_type="google_cloudsupport_cloudsupport_support_event_subscription",
    fields={
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "delete_time": ubx.FieldSpec(wire_name="delete_time"),
        "failure_reason": ubx.FieldSpec(wire_name="failure_reason"),
        "name": ubx.FieldSpec(wire_name="name"),
        "pub_sub_topic": ubx.FieldSpec(wire_name="pub_sub_topic"),
        "purge_time": ubx.FieldSpec(wire_name="purge_time"),
        "state": ubx.FieldSpec(wire_name="state"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
