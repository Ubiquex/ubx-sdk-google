...
class ParametermanagerVersion_Payload:
    # Required. bytes data for storing payload.
    data: Any = None

_ParametermanagerVersion_PayloadFields = {
    "data": ubx.FieldSpec(wire_name="data"),
}

@dataclasses.dataclass
class ParametermanagerVersionConfig:
...
@dataclasses.dataclass
class ParametermanagerVersionAttrs:
    # Output only. [Output only] Create time stamp
    create_time: Any = None
    # Optional. Disabled boolean to determine if a ParameterVersion acts as a metadata only resource (payload is never returned if disabled is true). If true any calls will always default to BASIC view even if the user explicitly passes FULL view as part of the request. A render call on a disabled resource fails with an error. Default value is False.
    disabled: Any = None
...
ParametermanagerVersion = ubx.ResourceBinding(
    wire_type="google_parametermanager_parametermanager_version",
    fields={
        ...
        "payload": ubx.FieldSpec(
            wire_name="payload",
            kind="object",
            fields=_ParametermanagerVersion_PayloadFields,
        ),
