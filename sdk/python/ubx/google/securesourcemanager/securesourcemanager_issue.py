# Code generated by `ubx sdk gen` from google_securesourcemanager_securesourcemanager_issue. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class SecuresourcemanagerIssueConfig:
    # Optional. Issue body. Provides a detailed description of the issue.
    body: Any = None
    # Output only. Close timestamp (if closed). Cleared when is re-opened.
    close_time: Any = None
    # Output only. Creation timestamp.
    create_time: Any = None
    # Optional. This checksum is computed by the server based on the value of other fields, and may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding.
    etag: Any = None
    # Identifier. Unique identifier for an issue. The issue id is generated by the server. Format: `projects/{project}/locations/{location}/repositories/{repository}/issues/{issue_id}`
    name: Any = None
    # Output only. State of the issue.
    state: Any = None
    # Required. Issue title.
    title: Any = None
    # Output only. Last updated timestamp.
    update_time: Any = None

@dataclasses.dataclass
class SecuresourcemanagerIssueAttrs:
    # Optional. Issue body. Provides a detailed description of the issue.
    body: Any = None
    # Output only. Close timestamp (if closed). Cleared when is re-opened.
    close_time: Any = None
    # Output only. Creation timestamp.
    create_time: Any = None
    # Optional. This checksum is computed by the server based on the value of other fields, and may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding.
    etag: Any = None
    # Identifier. Unique identifier for an issue. The issue id is generated by the server. Format: `projects/{project}/locations/{location}/repositories/{repository}/issues/{issue_id}`
    name: Any = None
    # Output only. State of the issue.
    state: Any = None
    # Required. Issue title.
    title: Any = None
    # Output only. Last updated timestamp.
    update_time: Any = None

SecuresourcemanagerIssue = ubx.ResourceBinding(
    wire_type="google_securesourcemanager_securesourcemanager_issue",
    fields={
        "body": ubx.FieldSpec(wire_name="body"),
        "close_time": ubx.FieldSpec(wire_name="close_time"),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "etag": ubx.FieldSpec(wire_name="etag"),
        "name": ubx.FieldSpec(wire_name="name"),
        "state": ubx.FieldSpec(wire_name="state"),
        "title": ubx.FieldSpec(wire_name="title"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
