...
class BigtableadminSchemaBundle_ProtoSchema:
...
_BigtableadminSchemaBundle_ProtoSchemaFields = {
    "proto_descriptors": ubx.FieldSpec(wire_name="proto_descriptors"),
}

@dataclasses.dataclass
class BigtableadminSchemaBundleConfig:
...
@dataclasses.dataclass
class BigtableadminSchemaBundleAttrs:
    # Optional. The etag for this schema bundle. This may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding. The server returns an ABORTED error on a mismatched etag.
    etag: Any = None
    # Identifier. The unique name identifying this schema bundle. Values are of the form `projects/{project}/instances/{instance}/tables/{table}/schemaBundles/{schema_bundle}`
    name: Any = None
...
BigtableadminSchemaBundle = ubx.ResourceBinding(
    wire_type="google_bigtableadmin_bigtableadmin_schema_bundle",
    fields={
        ...
        "proto_schema": ubx.FieldSpec(
            ...
            kind="object",
            fields=_BigtableadminSchemaBundle_ProtoSchemaFields,
        ),
    },
)
