# Code generated by `ubx sdk gen` from google_apihub_apihub_deployment. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class ApihubDeployment_Attributes_EnumValues_Values:
    description: Any = None
    display_name: Any = None
    id: Any = None
    immutable: Any = None

@dataclasses.dataclass
class ApihubDeployment_Attributes_EnumValues:
    values: Any = None

@dataclasses.dataclass
class ApihubDeployment_Attributes_JsonValues:
    values: Any = None

@dataclasses.dataclass
class ApihubDeployment_Attributes:
    attribute: Any = None
    enum_values: Any = None
    json_values: Any = None
    string_values: Any = None
    uri_values: Any = None

@dataclasses.dataclass
class ApihubDeployment_Documentation:
    # Optional. The uri of the externally hosted documentation.
    external_uri: Any = None

@dataclasses.dataclass
class ApihubDeployment_SourceMetadata_PluginInstanceActionSource:
    action_id: Any = None
    plugin_instance: Any = None

@dataclasses.dataclass
class ApihubDeployment_SourceMetadata:
    original_resource_create_time: Any = None
    original_resource_id: Any = None
    original_resource_update_time: Any = None
    plugin_instance_action_source: Any = None
    source_type: Any = None

_ApihubDeployment_Attributes_EnumValues_ValuesFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "display_name": ubx.FieldSpec(wire_name="display_name"),
    "id": ubx.FieldSpec(wire_name="id"),
    "immutable": ubx.FieldSpec(wire_name="immutable"),
}

_ApihubDeployment_Attributes_EnumValuesFields = {
    "values": ubx.FieldSpec(
        wire_name="values",
        kind="list",
        fields=_ApihubDeployment_Attributes_EnumValues_ValuesFields,
    ),
}

_ApihubDeployment_Attributes_JsonValuesFields = {
    "values": ubx.FieldSpec(wire_name="values"),
}

_ApihubDeployment_AttributesFields = {
    "attribute": ubx.FieldSpec(wire_name="attribute"),
    "enum_values": ubx.FieldSpec(
        wire_name="enum_values",
        kind="object",
        fields=_ApihubDeployment_Attributes_EnumValuesFields,
    ),
    "json_values": ubx.FieldSpec(
        wire_name="json_values",
        kind="object",
        fields=_ApihubDeployment_Attributes_JsonValuesFields,
    ),
    "string_values": ubx.FieldSpec(
        wire_name="string_values",
        kind="object",
        fields=_ApihubDeployment_Attributes_JsonValuesFields,
    ),
    "uri_values": ubx.FieldSpec(
        wire_name="uri_values",
        kind="object",
        fields=_ApihubDeployment_Attributes_JsonValuesFields,
    ),
}

_ApihubDeployment_DocumentationFields = {
    "external_uri": ubx.FieldSpec(wire_name="external_uri"),
}

_ApihubDeployment_SourceMetadata_PluginInstanceActionSourceFields = {
    "action_id": ubx.FieldSpec(wire_name="action_id"),
    "plugin_instance": ubx.FieldSpec(wire_name="plugin_instance"),
}

_ApihubDeployment_SourceMetadataFields = {
    "original_resource_create_time": ubx.FieldSpec(wire_name="original_resource_create_time"),
    "original_resource_id": ubx.FieldSpec(wire_name="original_resource_id"),
    "original_resource_update_time": ubx.FieldSpec(wire_name="original_resource_update_time"),
    "plugin_instance_action_source": ubx.FieldSpec(
        wire_name="plugin_instance_action_source",
        kind="object",
        fields=_ApihubDeployment_SourceMetadata_PluginInstanceActionSourceFields,
    ),
    "source_type": ubx.FieldSpec(wire_name="source_type"),
}

@dataclasses.dataclass
class ApihubDeploymentConfig:
    # Output only. The API versions linked to this deployment. Note: A particular deployment could be linked to multiple different API versions (of same or different APIs).
    api_versions: Any = None
    # Optional. The list of user defined attributes associated with the deployment resource. The key is the attribute name. It will be of the format: `projects/{project}/locations/{location}/attributes/{attribute}`. The value is the attribute values associated with the resource.
    attributes: Any = None
    # Output only. The time at which the deployment was created.
    create_time: Any = None
    # The attribute values associated with resource.
    deployment_type: Any = None
    # Optional. The description of the deployment.
    description: Any = None
    # Required. The display name of the deployment.
    display_name: Any = None
    # Documentation details.
    documentation: Any = None
    # Required. The endpoints at which this deployment resource is listening for API requests. This could be a list of complete URIs, hostnames or an IP addresses.
    endpoints: Any = None
    # The attribute values associated with resource.
    environment: Any = None
    # The attribute values associated with resource.
    management_url: Any = None
    # Identifier. The name of the deployment. Format: `projects/{project}/locations/{location}/deployments/{deployment}`
    name: Any = None
    # Required. The resource URI identifies the deployment within its gateway. For Apigee gateways, its recommended to use the format: organizations/{org}/environments/{env}/apis/{api}. For ex: if a proxy with name `orders` is deployed in `staging` environment of `cymbal` organization, the resource URI would be: `organizations/cymbal/environments/staging/apis/orders`.
    resource_uri: Any = None
    # The attribute values associated with resource.
    slo: Any = None
    # Optional. The environment at source for the deployment. For example: prod, dev, staging, etc.
    source_environment: Any = None
    # Output only. The list of sources and metadata from the sources of the deployment.
    source_metadata: Any = None
    # Optional. The project to which the deployment belongs. For Google Cloud gateways, this will refer to the project identifier. For others like Edge/OPDK, this will refer to the org identifier.
    source_project: Any = None
    # The attribute values associated with resource.
    source_uri: Any = None
    # Output only. The time at which the deployment was last updated.
    update_time: Any = None

@dataclasses.dataclass
class ApihubDeploymentAttrs:
    # Output only. The API versions linked to this deployment. Note: A particular deployment could be linked to multiple different API versions (of same or different APIs).
    api_versions: Any = None
    # Optional. The list of user defined attributes associated with the deployment resource. The key is the attribute name. It will be of the format: `projects/{project}/locations/{location}/attributes/{attribute}`. The value is the attribute values associated with the resource.
    attributes: Any = None
    # Output only. The time at which the deployment was created.
    create_time: Any = None
    # The attribute values associated with resource.
    deployment_type: Any = None
    # Optional. The description of the deployment.
    description: Any = None
    # Required. The display name of the deployment.
    display_name: Any = None
    # Documentation details.
    documentation: Any = None
    # Required. The endpoints at which this deployment resource is listening for API requests. This could be a list of complete URIs, hostnames or an IP addresses.
    endpoints: Any = None
    # The attribute values associated with resource.
    environment: Any = None
    # The attribute values associated with resource.
    management_url: Any = None
    # Identifier. The name of the deployment. Format: `projects/{project}/locations/{location}/deployments/{deployment}`
    name: Any = None
    # Required. The resource URI identifies the deployment within its gateway. For Apigee gateways, its recommended to use the format: organizations/{org}/environments/{env}/apis/{api}. For ex: if a proxy with name `orders` is deployed in `staging` environment of `cymbal` organization, the resource URI would be: `organizations/cymbal/environments/staging/apis/orders`.
    resource_uri: Any = None
    # The attribute values associated with resource.
    slo: Any = None
    # Optional. The environment at source for the deployment. For example: prod, dev, staging, etc.
    source_environment: Any = None
    # Output only. The list of sources and metadata from the sources of the deployment.
    source_metadata: Any = None
    # Optional. The project to which the deployment belongs. For Google Cloud gateways, this will refer to the project identifier. For others like Edge/OPDK, this will refer to the org identifier.
    source_project: Any = None
    # The attribute values associated with resource.
    source_uri: Any = None
    # Output only. The time at which the deployment was last updated.
    update_time: Any = None

ApihubDeployment = ubx.ResourceBinding(
    wire_type="google_apihub_apihub_deployment",
    fields={
        "api_versions": ubx.FieldSpec(wire_name="api_versions"),
        "attributes": ubx.FieldSpec(
            wire_name="attributes",
            kind="map",
            fields=_ApihubDeployment_AttributesFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "deployment_type": ubx.FieldSpec(
            wire_name="deployment_type",
            kind="object",
            fields=_ApihubDeployment_AttributesFields,
        ),
        "description": ubx.FieldSpec(wire_name="description"),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "documentation": ubx.FieldSpec(
            wire_name="documentation",
            kind="object",
            fields=_ApihubDeployment_DocumentationFields,
        ),
        "endpoints": ubx.FieldSpec(wire_name="endpoints"),
        "environment": ubx.FieldSpec(
            wire_name="environment",
            kind="object",
            fields=_ApihubDeployment_AttributesFields,
        ),
        "management_url": ubx.FieldSpec(
            wire_name="management_url",
            kind="object",
            fields=_ApihubDeployment_AttributesFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "resource_uri": ubx.FieldSpec(wire_name="resource_uri"),
        "slo": ubx.FieldSpec(
            wire_name="slo",
            kind="object",
            fields=_ApihubDeployment_AttributesFields,
        ),
        "source_environment": ubx.FieldSpec(wire_name="source_environment"),
        "source_metadata": ubx.FieldSpec(
            wire_name="source_metadata",
            kind="list",
            fields=_ApihubDeployment_SourceMetadataFields,
        ),
        "source_project": ubx.FieldSpec(wire_name="source_project"),
        "source_uri": ubx.FieldSpec(
            wire_name="source_uri",
            kind="object",
            fields=_ApihubDeployment_AttributesFields,
        ),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
