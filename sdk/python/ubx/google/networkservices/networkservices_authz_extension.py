...
class NetworkservicesAuthzExtensionConfig:
...
@dataclasses.dataclass
class NetworkservicesAuthzExtensionAttrs:
    # Optional. The `:authority` header in the gRPC request sent from Envoy to the extension service. It is required when the `service` field points to a backend service.
    authority: Any = None
    # Output only. The timestamp when the resource was created.
    create_time: Any = None
...
NetworkservicesAuthzExtension = ubx.ResourceBinding(
    wire_type="google_networkservices_networkservices_authz_extension",
    fields={
