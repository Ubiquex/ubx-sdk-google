# Code generated by `ubx sdk gen` from google_oracledatabase_oracledatabase_odb_subnet. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class OracledatabaseOdbSubnetConfig:
    # Required. The CIDR range of the subnet.
    cidr_range: Any = None
    # Output only. The date and time that the OdbNetwork was created.
    create_time: Any = None
    # Optional. Labels or tags associated with the resource.
    labels: Any = None
    # Identifier. The name of the OdbSubnet resource in the following format: projects/{project}/locations/{location}/odbNetworks/{odb_network}/odbSubnets/{odb_subnet}
    name: Any = None
    # Required. Purpose of the subnet.
    purpose: Any = None
    # Output only. State of the ODB Subnet.
    state: Any = None

@dataclasses.dataclass
class OracledatabaseOdbSubnetAttrs:
    # Required. The CIDR range of the subnet.
    cidr_range: Any = None
    # Output only. The date and time that the OdbNetwork was created.
    create_time: Any = None
    # Optional. Labels or tags associated with the resource.
    labels: Any = None
    # Identifier. The name of the OdbSubnet resource in the following format: projects/{project}/locations/{location}/odbNetworks/{odb_network}/odbSubnets/{odb_subnet}
    name: Any = None
    # Required. Purpose of the subnet.
    purpose: Any = None
    # Output only. State of the ODB Subnet.
    state: Any = None

OracledatabaseOdbSubnet = ubx.ResourceBinding(
    wire_type="google_oracledatabase_oracledatabase_odb_subnet",
    fields={
        "cidr_range": ubx.FieldSpec(wire_name="cidr_range"),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "labels": ubx.FieldSpec(wire_name="labels"),
        "name": ubx.FieldSpec(wire_name="name"),
        "purpose": ubx.FieldSpec(wire_name="purpose"),
        "state": ubx.FieldSpec(wire_name="state"),
    },
)
