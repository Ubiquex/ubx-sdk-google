# Code generated by `ubx sdk gen` from google_ces_v1beta_ces_v1beta_tool. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1betaCesV1betaTool_AgentTool:
    # Optional. The resource name of the agent that is the entry point of the tool. Format: `projects/{project}/locations/{location}/agents/{agent}`
    agent: Any = None
    # Optional. Description of the tool's purpose.
    description: Any = None
    # Required. The name of the agent tool.
    name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ClientFunction_Parameters:
    additional_properties: Any = None
    # Optional. The value should be validated against any (one or more) of the subschemas in the list.
    any_of: Any = None
    # Optional. Default value of the data.
    default: Any = None
    # Optional. A map of definitions for use by `ref`. Only allowed at the root of the schema.
    defs: Any = None
    # Optional. The description of the data.
    description: Any = None
    # Optional. Possible values of the element of primitive type with enum format. Examples: 1. We can define direction as : {type:STRING, format:enum, enum:["EAST", NORTH", "SOUTH", "WEST"]} 2. We can define apartment number as : {type:INTEGER, format:enum, enum:["101", "201", "301"]}
    enum: Any = None
    items: Any = None
    # Optional. Maximum number of the elements for Type.ARRAY.
    max_items: Any = None
    # Optional. Maximum value for Type.INTEGER and Type.NUMBER.
    maximum: Any = None
    # Optional. Minimum number of the elements for Type.ARRAY.
    min_items: Any = None
    # Optional. Minimum value for Type.INTEGER and Type.NUMBER.
    minimum: Any = None
    # Optional. Indicates if the value may be null.
    nullable: Any = None
    # Optional. Schemas of initial elements of Type.ARRAY.
    prefix_items: Any = None
    # Optional. Properties of Type.OBJECT.
    properties: Any = None
    # Optional. Allows indirect references between schema nodes. The value should be a valid reference to a child of the root `defs`. For example, the following schema defines a reference to a schema node named "Pet": ``` type: object properties: pet: ref: #/defs/Pet defs: Pet: type: object properties: name: type: string ``` The value of the "pet" property is a reference to the schema node named "Pet". See details in https://json-schema.org/understanding-json-schema/structuring.
    ref: Any = None
    # Optional. Required properties of Type.OBJECT.
    required: Any = None
    # Optional. The title of the schema.
    title: Any = None
    # Required. The type of the data.
    type: Any = None
    # Optional. Indicate the items in the array must be unique. Only applies to TYPE.ARRAY.
    unique_items: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ClientFunction:
    # Optional. The function description.
    description: Any = None
    # Required. The function name.
    name: Any = None
    # Represents a select subset of an OpenAPI 3.0 schema object.
    parameters: Any = None
    # Represents a select subset of an OpenAPI 3.0 schema object.
    response: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ConnectorTool_Action_EntityOperation:
    # Required. ID of the entity.
    entity_id: Any = None
    # Required. Operation to perform on the entity.
    operation: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ConnectorTool_Action:
    # ID of a Connection action for the tool to use.
    connection_action_id: Any = None
    # Entity CRUD operation specification.
    entity_operation: Any = None
    # Optional. Entity fields to use as inputs for the operation. If no fields are specified, all fields of the Entity will be used.
    input_fields: Any = None
    # Optional. Entity fields to return from the operation. If no fields are specified, all fields of the Entity will be returned.
    output_fields: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ConnectorTool_AuthConfig_Oauth2AuthCodeConfig:
    # Required. Oauth token parameter name to pass through. Must be in the format `$context.variables.`.
    oauth_token: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ConnectorTool_AuthConfig_Oauth2JwtBearerConfig:
    # Required. Client parameter name to pass through. Must be in the format `$context.variables.`.
    client_key: Any = None
    # Required. Issuer parameter name to pass through. Must be in the format `$context.variables.`.
    issuer: Any = None
    # Required. Subject parameter name to pass through. Must be in the format `$context.variables.`.
    subject: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ConnectorTool_AuthConfig:
    # Oauth 2.0 Authorization Code authentication configuration.
    oauth2_auth_code_config: Any = None
    # JWT Profile Oauth 2.0 Authorization Grant authentication configuration.
    oauth2_jwt_bearer_config: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ConnectorTool:
    # Configuration of an Action for the tool to use. Note: This can be either an Action or an Operation. See https://cloud.google.com/integration-connectors/docs/entities-operation-action for details.
    action: Any = None
    # End-user authentication configuration used for Connection calls. The field values must be the names of context variables in the format `$context.variables.`.
    auth_config: Any = None
    # Required. The full resource name of the referenced Integration Connectors Connection. Format: `projects/{project}/locations/{location}/connections/{connection}`
    connection: Any = None
    # Optional. The description of the tool that can be used by the Agent to decide whether to call this ConnectorTool.
    description: Any = None
    # Optional. The name of the tool that can be used by the Agent to decide whether to call this ConnectorTool.
    name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs_BoostControlSpec_ControlPoints:
    attribute_value: Any = None
    boost_amount: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs_BoostControlSpec:
    attribute_type: Any = None
    control_points: Any = None
    field_name: Any = None
    interpolation_type: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs:
    boost: Any = None
    boost_control_spec: Any = None
    condition: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec:
    condition_boost_specs: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_BoostSpecs:
    data_stores: Any = None
    spec: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_DataStoreSource_DataStore_ConnectorConfig:
    # Resource name of the collection the data store belongs to.
    collection: Any = None
    # Display name of the collection the data store belongs to.
    collection_display_name: Any = None
    # The name of the data source. Example: `salesforce`, `jira`, `confluence`, `bigquery`.
    data_source: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_DataStoreSource_DataStore:
    # The connector config for the data store connection.
    connector_config: Any = None
    # Output only. Timestamp when the data store was created.
    create_time: Any = None
    # Output only. The display name of the data store.
    display_name: Any = None
    # Output only. The document processing mode for the data store connection. Only set for PUBLIC_WEB and UNSTRUCTURED data stores.
    document_processing_mode: Any = None
    # Required. Full resource name of the DataStore. Format: `projects/{project}/locations/{location}/collections/{collection}/dataStores/{dataStore}`
    name: Any = None
    # Output only. The type of the data store. This field is readonly and populated by the server.
    type: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_DataStoreSource:
    # A DataStore resource in Vertex AI Search.
    data_store: Any = None
    # Optional. Filter specification for the DataStore. See: https://cloud.google.com/generative-ai-app-builder/docs/filter-search-metadata
    filter: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_EngineSource:
    # Optional. Use to target specific DataStores within the Engine. If empty, the search applies to all DataStores associated with the Engine.
    data_store_sources: Any = None
    # Required. Full resource name of the Engine. Format: `projects/{project}/locations/{location}/collections/{collection}/engines/{engine}`
    engine: Any = None
    # Optional. A filter applied to the search across the Engine. Not relevant and not used if 'data_store_sources' is provided. See: https://cloud.google.com/generative-ai-app-builder/docs/filter-search-metadata
    filter: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_GroundingConfig:
    disabled: Any = None
    grounding_level: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfig_ModelSettings:
    model: Any = None
    temperature: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfig:
    disabled: Any = None
    model_settings: Any = None
    prompt: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_SnippetsConfig:
    enable_snippets: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool_ModalityConfigs:
    grounding_config: Any = None
    modality_type: Any = None
    rewriter_config: Any = None
    snippets_config: Any = None
    summarization_config: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_DataStoreTool:
    # Optional. Boost specification to boost certain documents.
    boost_specs: Any = None
    # Configuration for searching within a specific DataStore.
    data_store_source: Any = None
    # Optional. The tool description.
    description: Any = None
    # Configuration for searching within an Engine, potentially targeting specific DataStores.
    engine_source: Any = None
    # Optional. The filter parameter behavior.
    filter_parameter_behavior: Any = None
    # Optional. The modality configs for the data store.
    modality_configs: Any = None
    # Required. The data store tool name.
    name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_FileSearchTool:
    # Optional. The type of the corpus. Default is FULLY_MANAGED.
    corpus_type: Any = None
    # Optional. The tool description.
    description: Any = None
    # Optional. The corpus where files are stored. Format: projects/{project}/locations/{location}/ragCorpora/{rag_corpus}
    file_corpus: Any = None
    # Required. The tool name.
    name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_GoogleSearchTool_PromptConfig:
    # Optional. Defines the prompt used for the system instructions when interacting with the agent in chat conversations. If not set, default prompt will be used.
    text_prompt: Any = None
    # Optional. Defines the prompt used for the system instructions when interacting with the agent in voice conversations. If not set, default prompt will be used.
    voice_prompt: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_GoogleSearchTool:
    # Optional. Content will be fetched directly from these URLs for context and grounding. Example: "https://example.com/path.html". A maximum of 20 URLs are allowed.
    context_urls: Any = None
    # Optional. Description of the tool's purpose.
    description: Any = None
    # Optional. List of domains to be excluded from the search results. Example: "example.com". A maximum of 2000 domains can be excluded.
    exclude_domains: Any = None
    # Required. The name of the tool.
    name: Any = None
    # Optional. Specifies domains to restrict search results to. Example: "example.com", "another.site". A maximum of 20 domains can be specified.
    preferred_domains: Any = None
    # Prompt settings used by the model when processing or summarizing the google search results.
    prompt_config: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_ApiAuthentication_ApiKeyConfig:
    # Required. The name of the SecretManager secret version resource storing the API key. Format: `projects/{project}/secrets/{secret}/versions/{version}` Note: You should grant `roles/secretmanager.secretAccessor` role to the CES service agent `service-@gcp-sa-ces.iam.gserviceaccount.com`.
    api_key_secret_version: Any = None
    # Required. The parameter name or the header name of the API key. E.g., If the API request is "https://example.com/act?X-Api-Key=", "X-Api-Key" would be the parameter name.
    key_name: Any = None
    # Required. Key location in the request.
    request_location: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_ApiAuthentication_BearerTokenConfig:
    # Required. The bearer token. Must be in the format `$context.variables.`.
    token: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_ApiAuthentication_OauthConfig:
    # Required. The client ID from the OAuth provider.
    client_id: Any = None
    # Required. The name of the SecretManager secret version resource storing the client secret. Format: `projects/{project}/secrets/{secret}/versions/{version}` Note: You should grant `roles/secretmanager.secretAccessor` role to the CES service agent `service-@gcp-sa-ces.iam.gserviceaccount.com`.
    client_secret_version: Any = None
    # Required. OAuth grant types.
    oauth_grant_type: Any = None
    # Optional. The OAuth scopes to grant.
    scopes: Any = None
    # Required. The token endpoint in the OAuth provider to exchange for an access token.
    token_endpoint: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_ApiAuthentication_ServiceAccountAuthConfig:
    # Optional. The OAuth scopes to grant. If not specified, the default scope `https://www.googleapis.com/auth/cloud-platform` is used.
    scopes: Any = None
    # Required. The email address of the service account used for authentication. CES uses this service account to exchange an access token and the access token is then sent in the `Authorization` header of the request. The service account must have the `roles/iam.serviceAccountTokenCreator` role granted to the CES service agent `service-@gcp-sa-ces.iam.gserviceaccount.com`.
    service_account: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_ApiAuthentication:
    # Configurations for authentication with API key.
    api_key_config: Any = None
    # Configurations for authentication with a bearer token.
    bearer_token_config: Any = None
    # Configurations for authentication with OAuth.
    oauth_config: Any = None
    # Configurations for authentication using a custom service account.
    service_account_auth_config: Any = None
    # Configurations for authentication with [ID token](https://cloud.google.com/docs/authentication/token-types#id) generated from service agent.
    service_agent_id_token_auth_config: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_ServiceDirectoryConfig:
    # Required. The name of [Service Directory](https://cloud.google.com/service-directory) service. Format: `projects/{project}/locations/{location}/namespaces/{namespace}/services/{service}`. Location of the service directory must be the same as the location of the app.
    service: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_TlsConfig_CaCerts:
    cert: Any = None
    display_name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool_TlsConfig:
    # Required. Specifies a list of allowed custom CA certificates for HTTPS verification.
    ca_certs: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_McpTool:
    # Authentication information required for API calls.
    api_authentication: Any = None
    # Optional. The custom headers to send in the request to the MCP server. The values must be in the format `$context.variables.` and can be set in the session variables. See https://docs.cloud.google.com/customer-engagement-ai/conversational-agents/ps/tool/open-api#openapi-injection for more details.
    custom_headers: Any = None
    # Optional. The description of the MCP tool.
    description: Any = None
    # Represents a select subset of an OpenAPI 3.0 schema object.
    input_schema: Any = None
    # Required. The name of the MCP tool.
    name: Any = None
    # Optional. The name override of the MCP tool. This is populated if the name was overridden by a Toolset override.
    name_override: Any = None
    # Represents a select subset of an OpenAPI 3.0 schema object.
    output_schema: Any = None
    # Required. The server address of the MCP server, e.g., "https://example.com/mcp/". If the server is built with the MCP SDK, the url should be suffixed with "/mcp/". Only Streamable HTTP transport based servers are supported. This is the same as the server_address in the McpToolset. See https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http for more details.
    server_address: Any = None
    # Configuration for tools using Service Directory.
    service_directory_config: Any = None
    # Output only. The dynamic availability state of the tool on the external server.
    state: Any = None
    # The TLS configuration.
    tls_config: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_OpenApiTool:
    # Authentication information required for API calls.
    api_authentication: Any = None
    # Optional. The description of the tool. If not provided, the description of the tool will be derived from the OpenAPI schema, from `operation.description` or `operation.summary`.
    description: Any = None
    # Optional. If true, the agent will ignore unknown fields in the API response.
    ignore_unknown_fields: Any = None
    # Optional. The name of the tool. If not provided, the name of the tool will be derived from the OpenAPI schema, from `operation.operationId`.
    name: Any = None
    # Required. The OpenAPI schema in JSON or YAML format.
    open_api_schema: Any = None
    # Configuration for tools using Service Directory.
    service_directory_config: Any = None
    # The TLS configuration.
    tls_config: Any = None
    # Optional. The server URL of the Open API schema. This field is only set in tools in the environment dependencies during the export process if the schema contains a server url. During the import process, if this url is present in the environment dependencies and the schema has the $env_var placeholder, it will replace the placeholder in the schema.
    url: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_PythonFunction:
    # Output only. The description of the Python function, parsed from the python code's docstring.
    description: Any = None
    # Optional. The name of the Python function to execute. Must match a Python function name defined in the python code. Case sensitive. If the name is not provided, the first function defined in the python code will be used.
    name: Any = None
    # Optional. The Python code to execute for the tool.
    python_code: Any = None
    # Configuration for tools using Service Directory.
    service_directory_config: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_RemoteAgentTool_AgentCard_Skills:
    description: Any = None
    examples: Any = None
    id: Any = None
    input_modes: Any = None
    name: Any = None
    output_modes: Any = None
    tags: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_RemoteAgentTool_AgentCard_SupportedInterfaces:
    protocol_binding: Any = None
    protocol_version: Any = None
    tenant: Any = None
    url: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_RemoteAgentTool_AgentCard:
    # Required. A description of the agent's domain of action/solution space.
    description: Any = None
    # Required. A human-readable name for the agent.
    name: Any = None
    # Required. Skills represent a unit of ability an agent can perform. This may somewhat abstract but represents a more focused set of actions that the agent is highly likely to succeed at.
    skills: Any = None
    # Required. Ordered list of supported interfaces. The first entry is preferred.
    supported_interfaces: Any = None
    # Required. The version of the agent.
    version: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_RemoteAgentTool:
    # AgentCard conveys key information about a remote agent. It is a trimmed version of the AgentCard defined in the A2A protocol https://a2a-protocol.org/dev/specification/#441-agentcard
    agent_card: Any = None
    # Required. The description of the tool.
    description: Any = None
    # Required. The name of the tool.
    name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_SystemTool:
    # Output only. The description of the system tool.
    description: Any = None
    # Required. The name of the system tool.
    name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ToolFakeConfig_CodeBlock:
    # Required. Python code which will be invoked in tool fake mode. Expected Python function signature - To catch all tool calls: def fake_tool_call(tool: Tool, input: dict[str, Any], callback_context: CallbackContext) -> Optional[dict[str, Any]]: To catch a specific tool call: def fake_{tool_id}(tool: Tool, input: dict[str, Any], callback_context: CallbackContext) -> Optional[dict[str, Any]]: If the function returns None, the real tool will be invoked instead.
    python_code: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_ToolFakeConfig:
    # A code block to be executed instead of a real tool call.
    code_block: Any = None
    # Optional. Whether the tool is using fake mode.
    enable_fake_mode: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_WidgetTool_DataMapping:
    # Optional. A map of widget input parameter fields to the corresponding output fields of the source tool.
    field_mappings: Any = None
    # Optional. The mode of the data mapping.
    mode: Any = None
    # A Python function tool.
    python_function: Any = None
    # Deprecated: Use `python_function` instead.
    python_script: Any = None
    # Optional. The resource name of the tool that provides the data for the widget (e.g., a search tool or a custom function). Format: `projects/{project}/locations/{location}/agents/{agent}/tools/{tool}`
    source_tool_name: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_WidgetTool_TextResponseConfig:
    # Optional. The static text response to return when type is STATIC.
    static_text: Any = None
    # Optional. Instruction for the LLM on how to generate the text response. Used as the description for the text response parameter if type is LLM_GENERATED.
    text_response_instruction: Any = None
    # Optional. The strategy for providing the text response.
    type: Any = None

@dataclasses.dataclass
class V1betaCesV1betaTool_WidgetTool:
    # Configuration for mapping data from a source tool to the widget's input parameters.
    data_mapping: Any = None
    # Optional. The description of the widget tool.
    description: Any = None
    # Required. The display name of the widget tool.
    name: Any = None
    # Represents a select subset of an OpenAPI 3.0 schema object.
    parameters: Any = None
    # Configuration for the text response returned with the widget.
    text_response_config: Any = None
    # Optional. Configuration for rendering the widget.
    ui_config: Any = None
    # Optional. The type of the widget tool. If not specified, the default type will be CUSTOMIZED.
    widget_type: Any = None

_V1betaCesV1betaTool_AgentToolFields = {
    "agent": ubx.FieldSpec(wire_name="agent"),
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
}

_V1betaCesV1betaTool_ClientFunction_ParametersFields = {
    "additional_properties": ubx.FieldSpec(wire_name="additional_properties"),
    "any_of": ubx.FieldSpec(wire_name="any_of"),
    "default": ubx.FieldSpec(wire_name="default"),
    "defs": ubx.FieldSpec(wire_name="defs"),
    "description": ubx.FieldSpec(wire_name="description"),
    "enum": ubx.FieldSpec(wire_name="enum"),
    "items": ubx.FieldSpec(wire_name="items"),
    "max_items": ubx.FieldSpec(wire_name="max_items"),
    "maximum": ubx.FieldSpec(wire_name="maximum"),
    "min_items": ubx.FieldSpec(wire_name="min_items"),
    "minimum": ubx.FieldSpec(wire_name="minimum"),
    "nullable": ubx.FieldSpec(wire_name="nullable"),
    "prefix_items": ubx.FieldSpec(wire_name="prefix_items"),
    "properties": ubx.FieldSpec(wire_name="properties"),
    "ref": ubx.FieldSpec(wire_name="ref"),
    "required": ubx.FieldSpec(wire_name="required"),
    "title": ubx.FieldSpec(wire_name="title"),
    "type": ubx.FieldSpec(wire_name="type"),
    "unique_items": ubx.FieldSpec(wire_name="unique_items"),
}

_V1betaCesV1betaTool_ClientFunctionFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
    "parameters": ubx.FieldSpec(
        wire_name="parameters",
        kind="object",
        fields=_V1betaCesV1betaTool_ClientFunction_ParametersFields,
    ),
    "response": ubx.FieldSpec(
        wire_name="response",
        kind="object",
        fields=_V1betaCesV1betaTool_ClientFunction_ParametersFields,
    ),
}

_V1betaCesV1betaTool_ConnectorTool_Action_EntityOperationFields = {
    "entity_id": ubx.FieldSpec(wire_name="entity_id"),
    "operation": ubx.FieldSpec(wire_name="operation"),
}

_V1betaCesV1betaTool_ConnectorTool_ActionFields = {
    "connection_action_id": ubx.FieldSpec(wire_name="connection_action_id"),
    "entity_operation": ubx.FieldSpec(
        wire_name="entity_operation",
        kind="object",
        fields=_V1betaCesV1betaTool_ConnectorTool_Action_EntityOperationFields,
    ),
    "input_fields": ubx.FieldSpec(wire_name="input_fields"),
    "output_fields": ubx.FieldSpec(wire_name="output_fields"),
}

_V1betaCesV1betaTool_ConnectorTool_AuthConfig_Oauth2AuthCodeConfigFields = {
    "oauth_token": ubx.FieldSpec(wire_name="oauth_token"),
}

_V1betaCesV1betaTool_ConnectorTool_AuthConfig_Oauth2JwtBearerConfigFields = {
    "client_key": ubx.FieldSpec(wire_name="client_key"),
    "issuer": ubx.FieldSpec(wire_name="issuer"),
    "subject": ubx.FieldSpec(wire_name="subject"),
}

_V1betaCesV1betaTool_ConnectorTool_AuthConfigFields = {
    "oauth2_auth_code_config": ubx.FieldSpec(
        wire_name="oauth2_auth_code_config",
        kind="object",
        fields=_V1betaCesV1betaTool_ConnectorTool_AuthConfig_Oauth2AuthCodeConfigFields,
    ),
    "oauth2_jwt_bearer_config": ubx.FieldSpec(
        wire_name="oauth2_jwt_bearer_config",
        kind="object",
        fields=_V1betaCesV1betaTool_ConnectorTool_AuthConfig_Oauth2JwtBearerConfigFields,
    ),
}

_V1betaCesV1betaTool_ConnectorToolFields = {
    "action": ubx.FieldSpec(
        wire_name="action",
        kind="object",
        fields=_V1betaCesV1betaTool_ConnectorTool_ActionFields,
    ),
    "auth_config": ubx.FieldSpec(
        wire_name="auth_config",
        kind="object",
        fields=_V1betaCesV1betaTool_ConnectorTool_AuthConfigFields,
    ),
    "connection": ubx.FieldSpec(wire_name="connection"),
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
}

_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs_BoostControlSpec_ControlPointsFields = {
    "attribute_value": ubx.FieldSpec(wire_name="attribute_value"),
    "boost_amount": ubx.FieldSpec(wire_name="boost_amount"),
}

_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs_BoostControlSpecFields = {
    "attribute_type": ubx.FieldSpec(wire_name="attribute_type"),
    "control_points": ubx.FieldSpec(
        wire_name="control_points",
        kind="list",
        fields=_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs_BoostControlSpec_ControlPointsFields,
    ),
    "field_name": ubx.FieldSpec(wire_name="field_name"),
    "interpolation_type": ubx.FieldSpec(wire_name="interpolation_type"),
}

_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecsFields = {
    "boost": ubx.FieldSpec(wire_name="boost"),
    "boost_control_spec": ubx.FieldSpec(
        wire_name="boost_control_spec",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecs_BoostControlSpecFields,
    ),
    "condition": ubx.FieldSpec(wire_name="condition"),
}

_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_SpecFields = {
    "condition_boost_specs": ubx.FieldSpec(
        wire_name="condition_boost_specs",
        kind="list",
        fields=_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_Spec_ConditionBoostSpecsFields,
    ),
}

_V1betaCesV1betaTool_DataStoreTool_BoostSpecsFields = {
    "data_stores": ubx.FieldSpec(wire_name="data_stores"),
    "spec": ubx.FieldSpec(
        wire_name="spec",
        kind="list",
        fields=_V1betaCesV1betaTool_DataStoreTool_BoostSpecs_SpecFields,
    ),
}

_V1betaCesV1betaTool_DataStoreTool_DataStoreSource_DataStore_ConnectorConfigFields = {
    "collection": ubx.FieldSpec(wire_name="collection"),
    "collection_display_name": ubx.FieldSpec(wire_name="collection_display_name"),
    "data_source": ubx.FieldSpec(wire_name="data_source"),
}

_V1betaCesV1betaTool_DataStoreTool_DataStoreSource_DataStoreFields = {
    "connector_config": ubx.FieldSpec(
        wire_name="connector_config",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_DataStoreSource_DataStore_ConnectorConfigFields,
    ),
    "create_time": ubx.FieldSpec(wire_name="create_time"),
    "display_name": ubx.FieldSpec(wire_name="display_name"),
    "document_processing_mode": ubx.FieldSpec(wire_name="document_processing_mode"),
    "name": ubx.FieldSpec(wire_name="name"),
    "type": ubx.FieldSpec(wire_name="type"),
}

_V1betaCesV1betaTool_DataStoreTool_DataStoreSourceFields = {
    "data_store": ubx.FieldSpec(
        wire_name="data_store",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_DataStoreSource_DataStoreFields,
    ),
    "filter": ubx.FieldSpec(wire_name="filter"),
}

_V1betaCesV1betaTool_DataStoreTool_EngineSourceFields = {
    "data_store_sources": ubx.FieldSpec(
        wire_name="data_store_sources",
        kind="list",
        fields=_V1betaCesV1betaTool_DataStoreTool_DataStoreSourceFields,
    ),
    "engine": ubx.FieldSpec(wire_name="engine"),
    "filter": ubx.FieldSpec(wire_name="filter"),
}

_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_GroundingConfigFields = {
    "disabled": ubx.FieldSpec(wire_name="disabled"),
    "grounding_level": ubx.FieldSpec(wire_name="grounding_level"),
}

_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfig_ModelSettingsFields = {
    "model": ubx.FieldSpec(wire_name="model"),
    "temperature": ubx.FieldSpec(wire_name="temperature"),
}

_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfigFields = {
    "disabled": ubx.FieldSpec(wire_name="disabled"),
    "model_settings": ubx.FieldSpec(
        wire_name="model_settings",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfig_ModelSettingsFields,
    ),
    "prompt": ubx.FieldSpec(wire_name="prompt"),
}

_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_SnippetsConfigFields = {
    "enable_snippets": ubx.FieldSpec(wire_name="enable_snippets"),
}

_V1betaCesV1betaTool_DataStoreTool_ModalityConfigsFields = {
    "grounding_config": ubx.FieldSpec(
        wire_name="grounding_config",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_GroundingConfigFields,
    ),
    "modality_type": ubx.FieldSpec(wire_name="modality_type"),
    "rewriter_config": ubx.FieldSpec(
        wire_name="rewriter_config",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfigFields,
    ),
    "snippets_config": ubx.FieldSpec(
        wire_name="snippets_config",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_SnippetsConfigFields,
    ),
    "summarization_config": ubx.FieldSpec(
        wire_name="summarization_config",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_ModalityConfigs_RewriterConfigFields,
    ),
}

_V1betaCesV1betaTool_DataStoreToolFields = {
    "boost_specs": ubx.FieldSpec(
        wire_name="boost_specs",
        kind="list",
        fields=_V1betaCesV1betaTool_DataStoreTool_BoostSpecsFields,
    ),
    "data_store_source": ubx.FieldSpec(
        wire_name="data_store_source",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_DataStoreSourceFields,
    ),
    "description": ubx.FieldSpec(wire_name="description"),
    "engine_source": ubx.FieldSpec(
        wire_name="engine_source",
        kind="object",
        fields=_V1betaCesV1betaTool_DataStoreTool_EngineSourceFields,
    ),
    "filter_parameter_behavior": ubx.FieldSpec(wire_name="filter_parameter_behavior"),
    "modality_configs": ubx.FieldSpec(
        wire_name="modality_configs",
        kind="list",
        fields=_V1betaCesV1betaTool_DataStoreTool_ModalityConfigsFields,
    ),
    "name": ubx.FieldSpec(wire_name="name"),
}

_V1betaCesV1betaTool_FileSearchToolFields = {
    "corpus_type": ubx.FieldSpec(wire_name="corpus_type"),
    "description": ubx.FieldSpec(wire_name="description"),
    "file_corpus": ubx.FieldSpec(wire_name="file_corpus"),
    "name": ubx.FieldSpec(wire_name="name"),
}

_V1betaCesV1betaTool_GoogleSearchTool_PromptConfigFields = {
    "text_prompt": ubx.FieldSpec(wire_name="text_prompt"),
    "voice_prompt": ubx.FieldSpec(wire_name="voice_prompt"),
}

_V1betaCesV1betaTool_GoogleSearchToolFields = {
    "context_urls": ubx.FieldSpec(wire_name="context_urls"),
    "description": ubx.FieldSpec(wire_name="description"),
    "exclude_domains": ubx.FieldSpec(wire_name="exclude_domains"),
    "name": ubx.FieldSpec(wire_name="name"),
    "preferred_domains": ubx.FieldSpec(wire_name="preferred_domains"),
    "prompt_config": ubx.FieldSpec(
        wire_name="prompt_config",
        kind="object",
        fields=_V1betaCesV1betaTool_GoogleSearchTool_PromptConfigFields,
    ),
}

_V1betaCesV1betaTool_McpTool_ApiAuthentication_ApiKeyConfigFields = {
    "api_key_secret_version": ubx.FieldSpec(wire_name="api_key_secret_version"),
    "key_name": ubx.FieldSpec(wire_name="key_name"),
    "request_location": ubx.FieldSpec(wire_name="request_location"),
}

_V1betaCesV1betaTool_McpTool_ApiAuthentication_BearerTokenConfigFields = {
    "token": ubx.FieldSpec(wire_name="token"),
}

_V1betaCesV1betaTool_McpTool_ApiAuthentication_OauthConfigFields = {
    "client_id": ubx.FieldSpec(wire_name="client_id"),
    "client_secret_version": ubx.FieldSpec(wire_name="client_secret_version"),
    "oauth_grant_type": ubx.FieldSpec(wire_name="oauth_grant_type"),
    "scopes": ubx.FieldSpec(wire_name="scopes"),
    "token_endpoint": ubx.FieldSpec(wire_name="token_endpoint"),
}

_V1betaCesV1betaTool_McpTool_ApiAuthentication_ServiceAccountAuthConfigFields = {
    "scopes": ubx.FieldSpec(wire_name="scopes"),
    "service_account": ubx.FieldSpec(wire_name="service_account"),
}

_V1betaCesV1betaTool_McpTool_ApiAuthenticationFields = {
    "api_key_config": ubx.FieldSpec(
        wire_name="api_key_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ApiAuthentication_ApiKeyConfigFields,
    ),
    "bearer_token_config": ubx.FieldSpec(
        wire_name="bearer_token_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ApiAuthentication_BearerTokenConfigFields,
    ),
    "oauth_config": ubx.FieldSpec(
        wire_name="oauth_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ApiAuthentication_OauthConfigFields,
    ),
    "service_account_auth_config": ubx.FieldSpec(
        wire_name="service_account_auth_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ApiAuthentication_ServiceAccountAuthConfigFields,
    ),
    "service_agent_id_token_auth_config": ubx.FieldSpec(wire_name="service_agent_id_token_auth_config"),
}

_V1betaCesV1betaTool_McpTool_ServiceDirectoryConfigFields = {
    "service": ubx.FieldSpec(wire_name="service"),
}

_V1betaCesV1betaTool_McpTool_TlsConfig_CaCertsFields = {
    "cert": ubx.FieldSpec(wire_name="cert"),
    "display_name": ubx.FieldSpec(wire_name="display_name"),
}

_V1betaCesV1betaTool_McpTool_TlsConfigFields = {
    "ca_certs": ubx.FieldSpec(
        wire_name="ca_certs",
        kind="list",
        fields=_V1betaCesV1betaTool_McpTool_TlsConfig_CaCertsFields,
    ),
}

_V1betaCesV1betaTool_McpToolFields = {
    "api_authentication": ubx.FieldSpec(
        wire_name="api_authentication",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ApiAuthenticationFields,
    ),
    "custom_headers": ubx.FieldSpec(wire_name="custom_headers"),
    "description": ubx.FieldSpec(wire_name="description"),
    "input_schema": ubx.FieldSpec(
        wire_name="input_schema",
        kind="object",
        fields=_V1betaCesV1betaTool_ClientFunction_ParametersFields,
    ),
    "name": ubx.FieldSpec(wire_name="name"),
    "name_override": ubx.FieldSpec(wire_name="name_override"),
    "output_schema": ubx.FieldSpec(
        wire_name="output_schema",
        kind="object",
        fields=_V1betaCesV1betaTool_ClientFunction_ParametersFields,
    ),
    "server_address": ubx.FieldSpec(wire_name="server_address"),
    "service_directory_config": ubx.FieldSpec(
        wire_name="service_directory_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ServiceDirectoryConfigFields,
    ),
    "state": ubx.FieldSpec(wire_name="state"),
    "tls_config": ubx.FieldSpec(
        wire_name="tls_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_TlsConfigFields,
    ),
}

_V1betaCesV1betaTool_OpenApiToolFields = {
    "api_authentication": ubx.FieldSpec(
        wire_name="api_authentication",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ApiAuthenticationFields,
    ),
    "description": ubx.FieldSpec(wire_name="description"),
    "ignore_unknown_fields": ubx.FieldSpec(wire_name="ignore_unknown_fields"),
    "name": ubx.FieldSpec(wire_name="name"),
    "open_api_schema": ubx.FieldSpec(wire_name="open_api_schema"),
    "service_directory_config": ubx.FieldSpec(
        wire_name="service_directory_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ServiceDirectoryConfigFields,
    ),
    "tls_config": ubx.FieldSpec(
        wire_name="tls_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_TlsConfigFields,
    ),
    "url": ubx.FieldSpec(wire_name="url"),
}

_V1betaCesV1betaTool_PythonFunctionFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
    "python_code": ubx.FieldSpec(wire_name="python_code"),
    "service_directory_config": ubx.FieldSpec(
        wire_name="service_directory_config",
        kind="object",
        fields=_V1betaCesV1betaTool_McpTool_ServiceDirectoryConfigFields,
    ),
}

_V1betaCesV1betaTool_RemoteAgentTool_AgentCard_SkillsFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "examples": ubx.FieldSpec(wire_name="examples"),
    "id": ubx.FieldSpec(wire_name="id"),
    "input_modes": ubx.FieldSpec(wire_name="input_modes"),
    "name": ubx.FieldSpec(wire_name="name"),
    "output_modes": ubx.FieldSpec(wire_name="output_modes"),
    "tags": ubx.FieldSpec(wire_name="tags"),
}

_V1betaCesV1betaTool_RemoteAgentTool_AgentCard_SupportedInterfacesFields = {
    "protocol_binding": ubx.FieldSpec(wire_name="protocol_binding"),
    "protocol_version": ubx.FieldSpec(wire_name="protocol_version"),
    "tenant": ubx.FieldSpec(wire_name="tenant"),
    "url": ubx.FieldSpec(wire_name="url"),
}

_V1betaCesV1betaTool_RemoteAgentTool_AgentCardFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
    "skills": ubx.FieldSpec(
        wire_name="skills",
        kind="list",
        fields=_V1betaCesV1betaTool_RemoteAgentTool_AgentCard_SkillsFields,
    ),
    "supported_interfaces": ubx.FieldSpec(
        wire_name="supported_interfaces",
        kind="list",
        fields=_V1betaCesV1betaTool_RemoteAgentTool_AgentCard_SupportedInterfacesFields,
    ),
    "version": ubx.FieldSpec(wire_name="version"),
}

_V1betaCesV1betaTool_RemoteAgentToolFields = {
    "agent_card": ubx.FieldSpec(
        wire_name="agent_card",
        kind="object",
        fields=_V1betaCesV1betaTool_RemoteAgentTool_AgentCardFields,
    ),
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
}

_V1betaCesV1betaTool_SystemToolFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
}

_V1betaCesV1betaTool_ToolFakeConfig_CodeBlockFields = {
    "python_code": ubx.FieldSpec(wire_name="python_code"),
}

_V1betaCesV1betaTool_ToolFakeConfigFields = {
    "code_block": ubx.FieldSpec(
        wire_name="code_block",
        kind="object",
        fields=_V1betaCesV1betaTool_ToolFakeConfig_CodeBlockFields,
    ),
    "enable_fake_mode": ubx.FieldSpec(wire_name="enable_fake_mode"),
}

_V1betaCesV1betaTool_WidgetTool_DataMappingFields = {
    "field_mappings": ubx.FieldSpec(wire_name="field_mappings"),
    "mode": ubx.FieldSpec(wire_name="mode"),
    "python_function": ubx.FieldSpec(
        wire_name="python_function",
        kind="object",
        fields=_V1betaCesV1betaTool_PythonFunctionFields,
    ),
    "python_script": ubx.FieldSpec(wire_name="python_script"),
    "source_tool_name": ubx.FieldSpec(wire_name="source_tool_name"),
}

_V1betaCesV1betaTool_WidgetTool_TextResponseConfigFields = {
    "static_text": ubx.FieldSpec(wire_name="static_text"),
    "text_response_instruction": ubx.FieldSpec(wire_name="text_response_instruction"),
    "type": ubx.FieldSpec(wire_name="type"),
}

_V1betaCesV1betaTool_WidgetToolFields = {
    "data_mapping": ubx.FieldSpec(
        wire_name="data_mapping",
        kind="object",
        fields=_V1betaCesV1betaTool_WidgetTool_DataMappingFields,
    ),
    "description": ubx.FieldSpec(wire_name="description"),
    "name": ubx.FieldSpec(wire_name="name"),
    "parameters": ubx.FieldSpec(
        wire_name="parameters",
        kind="object",
        fields=_V1betaCesV1betaTool_ClientFunction_ParametersFields,
    ),
    "text_response_config": ubx.FieldSpec(
        wire_name="text_response_config",
        kind="object",
        fields=_V1betaCesV1betaTool_WidgetTool_TextResponseConfigFields,
    ),
    "ui_config": ubx.FieldSpec(wire_name="ui_config"),
    "widget_type": ubx.FieldSpec(wire_name="widget_type"),
}

@dataclasses.dataclass
class V1betaCesV1betaToolConfig:
    # Represents a tool that allows the agent to call another agent.
    agent_tool: Any = None
    # Represents a client-side function that the agent can invoke. When the tool is chosen by the agent, control is handed off to the client. The client is responsible for executing the function and returning the result as a ToolResponse to continue the interaction with the agent.
    client_function: Any = None
    # A ConnectorTool allows connections to different integrations. See: https://cloud.google.com/integration-connectors/docs/overview.
    connector_tool: Any = None
    # Output only. Timestamp when the tool was created.
    create_time: Any = None
    # Tool to retrieve from Vertex AI Search datastore or engine for grounding. Accepts either a datastore or an engine, but not both. See Vertex AI Search: https://cloud.google.com/generative-ai-app-builder/docs/enterprise-search-introduction.
    data_store_tool: Any = None
    # Output only. The display name of the tool, derived based on the tool's type. For example, display name of a ClientFunction is derived from its `name` property.
    display_name: Any = None
    # Etag used to ensure the object hasn't changed during a read-modify-write operation. If the etag is empty, the update will overwrite any concurrent changes.
    etag: Any = None
    # Optional. The execution type of the tool.
    execution_type: Any = None
    # The file search tool allows the agent to search across the files uploaded by the app/agent developer. It has presets to give relatively good quality search over the uploaded files and summarization of the retrieved results.
    file_search_tool: Any = None
    # Output only. If the tool is generated by the LLM assistant, this field contains a descriptive summary of the generation.
    generated_summary: Any = None
    # Represents a tool to perform Google web searches for grounding. See https://cloud.google.com/customer-engagement-ai/conversational-agents/ps/tool#google-search.
    google_search_tool: Any = None
    # An MCP tool. See https://modelcontextprotocol.io/specification/2025-06-18/server/tools for more details.
    mcp_tool: Any = None
    # Identifier. The resource name of the tool. Format: * `projects/{project}/locations/{location}/apps/{app}/tools/{tool}` for standalone tools. * `projects/{project}/locations/{location}/apps/{app}/toolsets/{toolset}/tools/{tool}` for tools retrieved from a toolset. These tools are dynamic and output-only; they cannot be referenced directly where a tool is expected.
    name: Any = None
    # A remote API tool defined by an OpenAPI schema.
    open_api_tool: Any = None
    # A Python function tool.
    python_function: Any = None
    # Represents a tool that allows the agent to call another remote agent.
    remote_agent_tool: Any = None
    # Pre-defined system tool.
    system_tool: Any = None
    # Optional. The timeout for the tool execution. If not set, the default timeout is 30 seconds for `SYNCHRONOUS` tools and 60 seconds for `ASYNCHRONOUS` tools.
    timeout: Any = None
    # Configuration for tool behavior in fake mode.
    tool_fake_config: Any = None
    # Output only. Timestamp when the tool was last updated.
    update_time: Any = None
    # Represents a widget tool that the agent can invoke. When the tool is chosen by the agent, agent will return the widget to the client. The client is responsible for processing the widget and generating the next user query to continue the interaction with the agent.
    widget_tool: Any = None

@dataclasses.dataclass
class V1betaCesV1betaToolAttrs:
    # Represents a tool that allows the agent to call another agent.
    agent_tool: Any = None
    # Represents a client-side function that the agent can invoke. When the tool is chosen by the agent, control is handed off to the client. The client is responsible for executing the function and returning the result as a ToolResponse to continue the interaction with the agent.
    client_function: Any = None
    # A ConnectorTool allows connections to different integrations. See: https://cloud.google.com/integration-connectors/docs/overview.
    connector_tool: Any = None
    # Output only. Timestamp when the tool was created.
    create_time: Any = None
    # Tool to retrieve from Vertex AI Search datastore or engine for grounding. Accepts either a datastore or an engine, but not both. See Vertex AI Search: https://cloud.google.com/generative-ai-app-builder/docs/enterprise-search-introduction.
    data_store_tool: Any = None
    # Output only. The display name of the tool, derived based on the tool's type. For example, display name of a ClientFunction is derived from its `name` property.
    display_name: Any = None
    # Etag used to ensure the object hasn't changed during a read-modify-write operation. If the etag is empty, the update will overwrite any concurrent changes.
    etag: Any = None
    # Optional. The execution type of the tool.
    execution_type: Any = None
    # The file search tool allows the agent to search across the files uploaded by the app/agent developer. It has presets to give relatively good quality search over the uploaded files and summarization of the retrieved results.
    file_search_tool: Any = None
    # Output only. If the tool is generated by the LLM assistant, this field contains a descriptive summary of the generation.
    generated_summary: Any = None
    # Represents a tool to perform Google web searches for grounding. See https://cloud.google.com/customer-engagement-ai/conversational-agents/ps/tool#google-search.
    google_search_tool: Any = None
    # An MCP tool. See https://modelcontextprotocol.io/specification/2025-06-18/server/tools for more details.
    mcp_tool: Any = None
    # Identifier. The resource name of the tool. Format: * `projects/{project}/locations/{location}/apps/{app}/tools/{tool}` for standalone tools. * `projects/{project}/locations/{location}/apps/{app}/toolsets/{toolset}/tools/{tool}` for tools retrieved from a toolset. These tools are dynamic and output-only; they cannot be referenced directly where a tool is expected.
    name: Any = None
    # A remote API tool defined by an OpenAPI schema.
    open_api_tool: Any = None
    # A Python function tool.
    python_function: Any = None
    # Represents a tool that allows the agent to call another remote agent.
    remote_agent_tool: Any = None
    # Pre-defined system tool.
    system_tool: Any = None
    # Optional. The timeout for the tool execution. If not set, the default timeout is 30 seconds for `SYNCHRONOUS` tools and 60 seconds for `ASYNCHRONOUS` tools.
    timeout: Any = None
    # Configuration for tool behavior in fake mode.
    tool_fake_config: Any = None
    # Output only. Timestamp when the tool was last updated.
    update_time: Any = None
    # Represents a widget tool that the agent can invoke. When the tool is chosen by the agent, agent will return the widget to the client. The client is responsible for processing the widget and generating the next user query to continue the interaction with the agent.
    widget_tool: Any = None

V1betaCesV1betaTool = ubx.ResourceBinding(
    wire_type="google_ces_v1beta_ces_v1beta_tool",
    fields={
        "agent_tool": ubx.FieldSpec(
            wire_name="agent_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_AgentToolFields,
        ),
        "client_function": ubx.FieldSpec(
            wire_name="client_function",
            kind="object",
            fields=_V1betaCesV1betaTool_ClientFunctionFields,
        ),
        "connector_tool": ubx.FieldSpec(
            wire_name="connector_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_ConnectorToolFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "data_store_tool": ubx.FieldSpec(
            wire_name="data_store_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_DataStoreToolFields,
        ),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "etag": ubx.FieldSpec(wire_name="etag"),
        "execution_type": ubx.FieldSpec(wire_name="execution_type"),
        "file_search_tool": ubx.FieldSpec(
            wire_name="file_search_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_FileSearchToolFields,
        ),
        "generated_summary": ubx.FieldSpec(wire_name="generated_summary"),
        "google_search_tool": ubx.FieldSpec(
            wire_name="google_search_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_GoogleSearchToolFields,
        ),
        "mcp_tool": ubx.FieldSpec(
            wire_name="mcp_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_McpToolFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "open_api_tool": ubx.FieldSpec(
            wire_name="open_api_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_OpenApiToolFields,
        ),
        "python_function": ubx.FieldSpec(
            wire_name="python_function",
            kind="object",
            fields=_V1betaCesV1betaTool_PythonFunctionFields,
        ),
        "remote_agent_tool": ubx.FieldSpec(
            wire_name="remote_agent_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_RemoteAgentToolFields,
        ),
        "system_tool": ubx.FieldSpec(
            wire_name="system_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_SystemToolFields,
        ),
        "timeout": ubx.FieldSpec(wire_name="timeout"),
        "tool_fake_config": ubx.FieldSpec(
            wire_name="tool_fake_config",
            kind="object",
            fields=_V1betaCesV1betaTool_ToolFakeConfigFields,
        ),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
        "widget_tool": ubx.FieldSpec(
            wire_name="widget_tool",
            kind="object",
            fields=_V1betaCesV1betaTool_WidgetToolFields,
        ),
    },
)
