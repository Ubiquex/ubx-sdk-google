# Code generated by `ubx sdk gen` from google_ces_v1beta_ces_v1beta_evaluation_expectation. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1betaCesV1betaEvaluationExpectation_LlmCriteria:
    # Required. The prompt/instructions provided to the LLM judge.
    prompt: Any = None

_V1betaCesV1betaEvaluationExpectation_LlmCriteriaFields = {
    "prompt": ubx.FieldSpec(wire_name="prompt"),
}

@dataclasses.dataclass
class V1betaCesV1betaEvaluationExpectationConfig:
    # Output only. Timestamp when the evaluation expectation was created.
    create_time: Any = None
    # Required. User-defined display name. Must be unique within the app.
    display_name: Any = None
    # Output only. Etag used to ensure the object hasn't changed during a read-modify-write operation. If the etag is empty, the update will overwrite any concurrent changes.
    etag: Any = None
    # Configuration for LLM-based evaluation criteria.
    llm_criteria: Any = None
    # Identifier. The unique identifier of this evaluation expectation. Format: `projects/{project}/locations/{location}/apps/{app}/evaluationExpectations/{evaluation_expectation}`
    name: Any = None
    # Optional. User-defined tags for expectations. Can be used to filter expectations.
    tags: Any = None
    # Output only. Timestamp when the evaluation expectation was last updated.
    update_time: Any = None

@dataclasses.dataclass
class V1betaCesV1betaEvaluationExpectationAttrs:
    # Output only. Timestamp when the evaluation expectation was created.
    create_time: Any = None
    # Required. User-defined display name. Must be unique within the app.
    display_name: Any = None
    # Output only. Etag used to ensure the object hasn't changed during a read-modify-write operation. If the etag is empty, the update will overwrite any concurrent changes.
    etag: Any = None
    # Configuration for LLM-based evaluation criteria.
    llm_criteria: Any = None
    # Identifier. The unique identifier of this evaluation expectation. Format: `projects/{project}/locations/{location}/apps/{app}/evaluationExpectations/{evaluation_expectation}`
    name: Any = None
    # Optional. User-defined tags for expectations. Can be used to filter expectations.
    tags: Any = None
    # Output only. Timestamp when the evaluation expectation was last updated.
    update_time: Any = None

V1betaCesV1betaEvaluationExpectation = ubx.ResourceBinding(
    wire_type="google_ces_v1beta_ces_v1beta_evaluation_expectation",
    fields={
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "etag": ubx.FieldSpec(wire_name="etag"),
        "llm_criteria": ubx.FieldSpec(
            wire_name="llm_criteria",
            kind="object",
            fields=_V1betaCesV1betaEvaluationExpectation_LlmCriteriaFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "tags": ubx.FieldSpec(wire_name="tags"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
