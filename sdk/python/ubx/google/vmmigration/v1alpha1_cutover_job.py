# Code generated by `ubx sdk gen` from google_vmmigration_v1alpha1_cutover_job. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineDisksTargetDetails_Disks:
    disk_uri: Any = None
    source_disk_number: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineDisksTargetDetails_VmTargetDetails:
    # Output only. The URI of the Compute Engine VM.
    vm_uri: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineDisksTargetDetails:
    # The details of each created Persistent Disk.
    disks: Any = None
    # Details for a disks-only migration.
    disks_target_details: Any = None
    # Details for the VM created VM as part of disks migration.
    vm_target_details: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails_AdaptationModifiers:
    modifier: Any = None
    value: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails_AppliedLicense:
    # The OS license returned from the adaptation module's report.
    os_license: Any = None
    # The license type that was used in OS adaptation.
    type: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeScheduling_NodeAffinities:
    key: Any = None
    operator: Any = None
    values: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeScheduling:
    automatic_restart: Any = None
    # The minimum number of virtual CPUs this instance will consume when running on a sole-tenant node. Ignored if no node_affinites are configured.
    min_node_cpus: Any = None
    # A set of node affinity and anti-affinity configurations for sole tenant nodes.
    node_affinities: Any = None
    # How the instance should behave when the host machine undergoes maintenance that may temporarily impact instance performance.
    on_host_maintenance: Any = None
    # Whether the Instance should be automatically restarted whenever it is terminated by Compute Engine (not terminated by user). This configuration is identical to `automaticRestart` field in Compute Engine create instance under scheduling. It was changed to an enum (instead of a boolean) to match the default value in Compute Engine which is automatic restart.
    restart_type: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails_Encryption:
    # Required. The name of the encryption key that is stored in Google Cloud KMS.
    kms_key: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails_NetworkInterfaces:
    external_ip: Any = None
    internal_ip: Any = None
    network: Any = None
    network_tier: Any = None
    subnetwork: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineTargetDetails:
    # Optional. Modifiers to be used as configuration of the OS adaptation process.
    adaptation_modifiers: Any = None
    # Additional licenses to assign to the VM.
    additional_licenses: Any = None
    # AppliedLicense holds the license data returned by adaptation module report.
    applied_license: Any = None
    # Optional. By default the virtual machine will keep its existing boot option. Setting this property will trigger an internal process which will convert the virtual machine from using the existing boot option to another.
    boot_conversion: Any = None
    # The VM Boot Option, as set in the source VM.
    boot_option: Any = None
    # Scheduling information for VM on maintenance/restart behaviour and node allocation in sole tenant nodes. Options for instance behavior when the host machine undergoes maintenance that may temporarily impact instance performance.
    compute_scheduling: Any = None
    # Optional. Additional replica zones of the target regional disks. If this list is not empty a regional disk will be created. The first supported zone would be the one stated in the zone field. The rest are taken from this list. Please refer to the [regional disk creation API](https://cloud.google.com/compute/docs/regions-zones/global-regional-zonal-resources) for further details about regional vs zonal disks. If not specified, a zonal disk will be created in the same zone the VM is created.
    disk_replica_zones: Any = None
    # The disk type to use in the VM.
    disk_type: Any = None
    # Optional. Defines whether the instance has integrity monitoring enabled.
    enable_integrity_monitoring: Any = None
    # Optional. Defines whether the instance has vTPM enabled.
    enable_vtpm: Any = None
    # Encryption message describes the details of the applied encryption.
    encryption: Any = None
    # The hostname to assign to the VM.
    hostname: Any = None
    # A map of labels to associate with the VM.
    labels: Any = None
    # The license type to use in OS adaptation.
    license_type: Any = None
    # The machine type to create the VM with.
    machine_type: Any = None
    # The machine type series to create the VM with.
    machine_type_series: Any = None
    # The metadata key/value pairs to assign to the VM.
    metadata: Any = None
    # List of NICs connected to this VM.
    network_interfaces: Any = None
    # A list of network tags to associate with the VM.
    network_tags: Any = None
    # The Google Cloud target project ID or project name.
    project: Any = None
    # Defines whether the instance has Secure Boot enabled. This can be set to true only if the VM boot option is EFI.
    secure_boot: Any = None
    # The service account to associate the VM with.
    service_account: Any = None
    # Optional. The storage pool used for the VM disks. If specified this will be the storage pool in which the disk is created. This is the full path of the storage pool resource, for example: "projects/my-project/zones/us-central1-a/storagePools/my-storage-pool". The storage pool must be in the same project and zone as the target disks. The storage pool's type must match the disk type.
    storage_pool: Any = None
    # The name of the VM to create.
    vm_name: Any = None
    # The zone in which to create the VM.
    zone: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_ComputeEngineVmDetails:
    # AppliedLicense holds the license data returned by adaptation module report.
    applied_license: Any = None
    # Output only. The VM Boot Option, as set in the source VM.
    boot_option: Any = None
    # Scheduling information for VM on maintenance/restart behaviour and node allocation in sole tenant nodes. Options for instance behavior when the host machine undergoes maintenance that may temporarily impact instance performance.
    compute_scheduling: Any = None
    # The disk type to use in the VM.
    disk_type: Any = None
    # The external IP to define in the VM.
    external_ip: Any = None
    # The internal IP to define in the VM. The formats accepted are: `ephemeral` \ ipv4 address \ a named address resource full path.
    internal_ip: Any = None
    # A map of labels to associate with the VM.
    labels: Any = None
    # The license type to use in OS adaptation.
    license_type: Any = None
    # The machine type to create the VM with.
    machine_type: Any = None
    # The machine type series to create the VM with.
    machine_type_series: Any = None
    # The metadata key/value pairs to assign to the VM.
    metadata: Any = None
    # The name of the VM to create.
    name: Any = None
    # The network to connect the VM to.
    network: Any = None
    # List of NICs connected to this VM.
    network_interfaces: Any = None
    # A list of network tags to associate with the VM.
    network_tags: Any = None
    # Output only. The project in which to create the VM.
    project: Any = None
    # Defines whether the instance has Secure Boot enabled. This can be set to true only if the vm boot option is EFI.
    secure_boot: Any = None
    # The service account to associate the VM with.
    service_account: Any = None
    # The subnetwork to connect the VM to.
    subnetwork: Any = None
    # The full path of the resource of type TargetProject which represents the Compute Engine project in which to create this VM.
    target_project: Any = None
    # The zone in which to create the VM.
    zone: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Error:
    # The status code, which should be an enum value of google.rpc.Code.
    code: Any = None
    # A list of messages that carry the error details. There is a common set of message types for APIs to use.
    details: Any = None
    # A developer-facing error message, which should be in English. Any user-facing error message should be localized and sent in the google.rpc.Status.details field, or localized by the client.
    message: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps_FinalSync_Steps_Replicating:
    last_thirty_minutes_average_bytes_per_second: Any = None
    last_two_minutes_average_bytes_per_second: Any = None
    replicated_bytes: Any = None
    total_bytes: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps_FinalSync_Steps:
    end_time: Any = None
    initializing_replication: Any = None
    post_processing: Any = None
    replicating: Any = None
    start_time: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps_FinalSync_Warnings_ActionItem:
    locale: Any = None
    message: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps_FinalSync_Warnings_HelpLinks:
    description: Any = None
    url: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps_FinalSync_Warnings:
    action_item: Any = None
    code: Any = None
    help_links: Any = None
    warning_message: Any = None
    warning_time: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps_FinalSync:
    cycle_number: Any = None
    end_time: Any = None
    error: Any = None
    name: Any = None
    progress: Any = None
    progress_percent: Any = None
    start_time: Any = None
    state: Any = None
    steps: Any = None
    total_pause_duration: Any = None
    warnings: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJob_Steps:
    end_time: Any = None
    final_sync: Any = None
    instantiating_migrated_vm: Any = None
    preparing_vm_disks: Any = None
    previous_replication_cycle: Any = None
    shutting_down_source_vm: Any = None
    start_time: Any = None

_V1alpha1CutoverJob_ComputeEngineDisksTargetDetails_DisksFields = {
    "disk_uri": ubx.FieldSpec(wire_name="disk_uri"),
    "source_disk_number": ubx.FieldSpec(wire_name="source_disk_number"),
}

_V1alpha1CutoverJob_ComputeEngineDisksTargetDetails_VmTargetDetailsFields = {
    "vm_uri": ubx.FieldSpec(wire_name="vm_uri"),
}

_V1alpha1CutoverJob_ComputeEngineDisksTargetDetailsFields = {
    "disks": ubx.FieldSpec(
        wire_name="disks",
        kind="list",
        fields=_V1alpha1CutoverJob_ComputeEngineDisksTargetDetails_DisksFields,
    ),
    "disks_target_details": ubx.FieldSpec(wire_name="disks_target_details"),
    "vm_target_details": ubx.FieldSpec(
        wire_name="vm_target_details",
        kind="object",
        fields=_V1alpha1CutoverJob_ComputeEngineDisksTargetDetails_VmTargetDetailsFields,
    ),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetails_AdaptationModifiersFields = {
    "modifier": ubx.FieldSpec(wire_name="modifier"),
    "value": ubx.FieldSpec(wire_name="value"),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetails_AppliedLicenseFields = {
    "os_license": ubx.FieldSpec(wire_name="os_license"),
    "type": ubx.FieldSpec(wire_name="type"),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeScheduling_NodeAffinitiesFields = {
    "key": ubx.FieldSpec(wire_name="key"),
    "operator": ubx.FieldSpec(wire_name="operator"),
    "values": ubx.FieldSpec(wire_name="values"),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeSchedulingFields = {
    "automatic_restart": ubx.FieldSpec(wire_name="automatic_restart"),
    "min_node_cpus": ubx.FieldSpec(wire_name="min_node_cpus"),
    "node_affinities": ubx.FieldSpec(
        wire_name="node_affinities",
        kind="list",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeScheduling_NodeAffinitiesFields,
    ),
    "on_host_maintenance": ubx.FieldSpec(wire_name="on_host_maintenance"),
    "restart_type": ubx.FieldSpec(wire_name="restart_type"),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetails_EncryptionFields = {
    "kms_key": ubx.FieldSpec(wire_name="kms_key"),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetails_NetworkInterfacesFields = {
    "external_ip": ubx.FieldSpec(wire_name="external_ip"),
    "internal_ip": ubx.FieldSpec(wire_name="internal_ip"),
    "network": ubx.FieldSpec(wire_name="network"),
    "network_tier": ubx.FieldSpec(wire_name="network_tier"),
    "subnetwork": ubx.FieldSpec(wire_name="subnetwork"),
}

_V1alpha1CutoverJob_ComputeEngineTargetDetailsFields = {
    "adaptation_modifiers": ubx.FieldSpec(
        wire_name="adaptation_modifiers",
        kind="list",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_AdaptationModifiersFields,
    ),
    "additional_licenses": ubx.FieldSpec(wire_name="additional_licenses"),
    "applied_license": ubx.FieldSpec(
        wire_name="applied_license",
        kind="object",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_AppliedLicenseFields,
    ),
    "boot_conversion": ubx.FieldSpec(wire_name="boot_conversion"),
    "boot_option": ubx.FieldSpec(wire_name="boot_option"),
    "compute_scheduling": ubx.FieldSpec(
        wire_name="compute_scheduling",
        kind="object",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeSchedulingFields,
    ),
    "disk_replica_zones": ubx.FieldSpec(wire_name="disk_replica_zones"),
    "disk_type": ubx.FieldSpec(wire_name="disk_type"),
    "enable_integrity_monitoring": ubx.FieldSpec(wire_name="enable_integrity_monitoring"),
    "enable_vtpm": ubx.FieldSpec(wire_name="enable_vtpm"),
    "encryption": ubx.FieldSpec(
        wire_name="encryption",
        kind="object",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_EncryptionFields,
    ),
    "hostname": ubx.FieldSpec(wire_name="hostname"),
    "labels": ubx.FieldSpec(wire_name="labels"),
    "license_type": ubx.FieldSpec(wire_name="license_type"),
    "machine_type": ubx.FieldSpec(wire_name="machine_type"),
    "machine_type_series": ubx.FieldSpec(wire_name="machine_type_series"),
    "metadata": ubx.FieldSpec(wire_name="metadata"),
    "network_interfaces": ubx.FieldSpec(
        wire_name="network_interfaces",
        kind="list",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_NetworkInterfacesFields,
    ),
    "network_tags": ubx.FieldSpec(wire_name="network_tags"),
    "project": ubx.FieldSpec(wire_name="project"),
    "secure_boot": ubx.FieldSpec(wire_name="secure_boot"),
    "service_account": ubx.FieldSpec(wire_name="service_account"),
    "storage_pool": ubx.FieldSpec(wire_name="storage_pool"),
    "vm_name": ubx.FieldSpec(wire_name="vm_name"),
    "zone": ubx.FieldSpec(wire_name="zone"),
}

_V1alpha1CutoverJob_ComputeEngineVmDetailsFields = {
    "applied_license": ubx.FieldSpec(
        wire_name="applied_license",
        kind="object",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_AppliedLicenseFields,
    ),
    "boot_option": ubx.FieldSpec(wire_name="boot_option"),
    "compute_scheduling": ubx.FieldSpec(
        wire_name="compute_scheduling",
        kind="object",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_ComputeSchedulingFields,
    ),
    "disk_type": ubx.FieldSpec(wire_name="disk_type"),
    "external_ip": ubx.FieldSpec(wire_name="external_ip"),
    "internal_ip": ubx.FieldSpec(wire_name="internal_ip"),
    "labels": ubx.FieldSpec(wire_name="labels"),
    "license_type": ubx.FieldSpec(wire_name="license_type"),
    "machine_type": ubx.FieldSpec(wire_name="machine_type"),
    "machine_type_series": ubx.FieldSpec(wire_name="machine_type_series"),
    "metadata": ubx.FieldSpec(wire_name="metadata"),
    "name": ubx.FieldSpec(wire_name="name"),
    "network": ubx.FieldSpec(wire_name="network"),
    "network_interfaces": ubx.FieldSpec(
        wire_name="network_interfaces",
        kind="list",
        fields=_V1alpha1CutoverJob_ComputeEngineTargetDetails_NetworkInterfacesFields,
    ),
    "network_tags": ubx.FieldSpec(wire_name="network_tags"),
    "project": ubx.FieldSpec(wire_name="project"),
    "secure_boot": ubx.FieldSpec(wire_name="secure_boot"),
    "service_account": ubx.FieldSpec(wire_name="service_account"),
    "subnetwork": ubx.FieldSpec(wire_name="subnetwork"),
    "target_project": ubx.FieldSpec(wire_name="target_project"),
    "zone": ubx.FieldSpec(wire_name="zone"),
}

_V1alpha1CutoverJob_ErrorFields = {
    "code": ubx.FieldSpec(wire_name="code"),
    "details": ubx.FieldSpec(wire_name="details"),
    "message": ubx.FieldSpec(wire_name="message"),
}

_V1alpha1CutoverJob_Steps_FinalSync_Steps_ReplicatingFields = {
    "last_thirty_minutes_average_bytes_per_second": ubx.FieldSpec(wire_name="last_thirty_minutes_average_bytes_per_second"),
    "last_two_minutes_average_bytes_per_second": ubx.FieldSpec(wire_name="last_two_minutes_average_bytes_per_second"),
    "replicated_bytes": ubx.FieldSpec(wire_name="replicated_bytes"),
    "total_bytes": ubx.FieldSpec(wire_name="total_bytes"),
}

_V1alpha1CutoverJob_Steps_FinalSync_StepsFields = {
    "end_time": ubx.FieldSpec(wire_name="end_time"),
    "initializing_replication": ubx.FieldSpec(wire_name="initializing_replication"),
    "post_processing": ubx.FieldSpec(wire_name="post_processing"),
    "replicating": ubx.FieldSpec(
        wire_name="replicating",
        kind="object",
        fields=_V1alpha1CutoverJob_Steps_FinalSync_Steps_ReplicatingFields,
    ),
    "start_time": ubx.FieldSpec(wire_name="start_time"),
}

_V1alpha1CutoverJob_Steps_FinalSync_Warnings_ActionItemFields = {
    "locale": ubx.FieldSpec(wire_name="locale"),
    "message": ubx.FieldSpec(wire_name="message"),
}

_V1alpha1CutoverJob_Steps_FinalSync_Warnings_HelpLinksFields = {
    "description": ubx.FieldSpec(wire_name="description"),
    "url": ubx.FieldSpec(wire_name="url"),
}

_V1alpha1CutoverJob_Steps_FinalSync_WarningsFields = {
    "action_item": ubx.FieldSpec(
        wire_name="action_item",
        kind="object",
        fields=_V1alpha1CutoverJob_Steps_FinalSync_Warnings_ActionItemFields,
    ),
    "code": ubx.FieldSpec(wire_name="code"),
    "help_links": ubx.FieldSpec(
        wire_name="help_links",
        kind="list",
        fields=_V1alpha1CutoverJob_Steps_FinalSync_Warnings_HelpLinksFields,
    ),
    "warning_message": ubx.FieldSpec(
        wire_name="warning_message",
        kind="object",
        fields=_V1alpha1CutoverJob_Steps_FinalSync_Warnings_ActionItemFields,
    ),
    "warning_time": ubx.FieldSpec(wire_name="warning_time"),
}

_V1alpha1CutoverJob_Steps_FinalSyncFields = {
    "cycle_number": ubx.FieldSpec(wire_name="cycle_number"),
    "end_time": ubx.FieldSpec(wire_name="end_time"),
    "error": ubx.FieldSpec(
        wire_name="error",
        kind="object",
        fields=_V1alpha1CutoverJob_ErrorFields,
    ),
    "name": ubx.FieldSpec(wire_name="name"),
    "progress": ubx.FieldSpec(wire_name="progress"),
    "progress_percent": ubx.FieldSpec(wire_name="progress_percent"),
    "start_time": ubx.FieldSpec(wire_name="start_time"),
    "state": ubx.FieldSpec(wire_name="state"),
    "steps": ubx.FieldSpec(
        wire_name="steps",
        kind="list",
        fields=_V1alpha1CutoverJob_Steps_FinalSync_StepsFields,
    ),
    "total_pause_duration": ubx.FieldSpec(wire_name="total_pause_duration"),
    "warnings": ubx.FieldSpec(
        wire_name="warnings",
        kind="list",
        fields=_V1alpha1CutoverJob_Steps_FinalSync_WarningsFields,
    ),
}

_V1alpha1CutoverJob_StepsFields = {
    "end_time": ubx.FieldSpec(wire_name="end_time"),
    "final_sync": ubx.FieldSpec(
        wire_name="final_sync",
        kind="object",
        fields=_V1alpha1CutoverJob_Steps_FinalSyncFields,
    ),
    "instantiating_migrated_vm": ubx.FieldSpec(wire_name="instantiating_migrated_vm"),
    "preparing_vm_disks": ubx.FieldSpec(wire_name="preparing_vm_disks"),
    "previous_replication_cycle": ubx.FieldSpec(
        wire_name="previous_replication_cycle",
        kind="object",
        fields=_V1alpha1CutoverJob_Steps_FinalSyncFields,
    ),
    "shutting_down_source_vm": ubx.FieldSpec(wire_name="shutting_down_source_vm"),
    "start_time": ubx.FieldSpec(wire_name="start_time"),
}

@dataclasses.dataclass
class V1alpha1CutoverJobConfig:
    # ComputeEngineDisksTargetDetails is a collection of created Persistent Disks details.
    compute_engine_disks_target_details: Any = None
    # ComputeEngineTargetDetails is a collection of details for creating a VM in a target Compute Engine project.
    compute_engine_target_details: Any = None
    # TargetVMDetails is a collection of details for creating a VM in a target Compute Engine project.
    compute_engine_vm_details: Any = None
    # Output only. The time the cutover job was created (as an API call, not when it was actually created in the target).
    create_time: Any = None
    # Output only. The time the cutover job had finished.
    end_time: Any = None
    # The `Status` type defines a logical error model that is suitable for different programming environments, including REST APIs and RPC APIs. It is used by [gRPC](https://github.com/grpc). Each `Status` message contains three pieces of data: error code, error message, and error details. You can find out more about this error model and how to work with it in the [API Design Guide](https://cloud.google.com/apis/design/errors).
    error: Any = None
    # Output only. The name of the cutover job.
    name: Any = None
    # Output only. The current progress in percentage of the cutover job.
    progress: Any = None
    # Output only. The current progress in percentage of the cutover job.
    progress_percent: Any = None
    # Output only. State of the cutover job.
    state: Any = None
    # Output only. A message providing possible extra details about the current state.
    state_message: Any = None
    # Output only. The time the state was last updated.
    state_time: Any = None
    # Output only. The cutover steps list representing its progress.
    steps: Any = None
    # TargetVMDetails is a collection of details for creating a VM in a target Compute Engine project.
    target_details: Any = None

@dataclasses.dataclass
class V1alpha1CutoverJobAttrs:
    # ComputeEngineDisksTargetDetails is a collection of created Persistent Disks details.
    compute_engine_disks_target_details: Any = None
    # ComputeEngineTargetDetails is a collection of details for creating a VM in a target Compute Engine project.
    compute_engine_target_details: Any = None
    # TargetVMDetails is a collection of details for creating a VM in a target Compute Engine project.
    compute_engine_vm_details: Any = None
    # Output only. The time the cutover job was created (as an API call, not when it was actually created in the target).
    create_time: Any = None
    # Output only. The time the cutover job had finished.
    end_time: Any = None
    # The `Status` type defines a logical error model that is suitable for different programming environments, including REST APIs and RPC APIs. It is used by [gRPC](https://github.com/grpc). Each `Status` message contains three pieces of data: error code, error message, and error details. You can find out more about this error model and how to work with it in the [API Design Guide](https://cloud.google.com/apis/design/errors).
    error: Any = None
    # Output only. The name of the cutover job.
    name: Any = None
    # Output only. The current progress in percentage of the cutover job.
    progress: Any = None
    # Output only. The current progress in percentage of the cutover job.
    progress_percent: Any = None
    # Output only. State of the cutover job.
    state: Any = None
    # Output only. A message providing possible extra details about the current state.
    state_message: Any = None
    # Output only. The time the state was last updated.
    state_time: Any = None
    # Output only. The cutover steps list representing its progress.
    steps: Any = None
    # TargetVMDetails is a collection of details for creating a VM in a target Compute Engine project.
    target_details: Any = None

V1alpha1CutoverJob = ubx.ResourceBinding(
    wire_type="google_vmmigration_v1alpha1_cutover_job",
    fields={
        "compute_engine_disks_target_details": ubx.FieldSpec(
            wire_name="compute_engine_disks_target_details",
            kind="object",
            fields=_V1alpha1CutoverJob_ComputeEngineDisksTargetDetailsFields,
        ),
        "compute_engine_target_details": ubx.FieldSpec(
            wire_name="compute_engine_target_details",
            kind="object",
            fields=_V1alpha1CutoverJob_ComputeEngineTargetDetailsFields,
        ),
        "compute_engine_vm_details": ubx.FieldSpec(
            wire_name="compute_engine_vm_details",
            kind="object",
            fields=_V1alpha1CutoverJob_ComputeEngineVmDetailsFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "end_time": ubx.FieldSpec(wire_name="end_time"),
        "error": ubx.FieldSpec(
            wire_name="error",
            kind="object",
            fields=_V1alpha1CutoverJob_ErrorFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "progress": ubx.FieldSpec(wire_name="progress"),
        "progress_percent": ubx.FieldSpec(wire_name="progress_percent"),
        "state": ubx.FieldSpec(wire_name="state"),
        "state_message": ubx.FieldSpec(wire_name="state_message"),
        "state_time": ubx.FieldSpec(wire_name="state_time"),
        "steps": ubx.FieldSpec(
            wire_name="steps",
            kind="list",
            fields=_V1alpha1CutoverJob_StepsFields,
        ),
        "target_details": ubx.FieldSpec(
            wire_name="target_details",
            kind="object",
            fields=_V1alpha1CutoverJob_ComputeEngineVmDetailsFields,
        ),
    },
)
