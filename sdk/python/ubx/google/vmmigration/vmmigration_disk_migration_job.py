# Code generated by `ubx sdk gen` from google_vmmigration_vmmigration_disk_migration_job. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class VmmigrationDiskMigrationJob_AwsSourceDiskDetails:
    # Optional. Output only. Disk type.
    disk_type: Any = None
    # Output only. Size in GiB.
    size_gib: Any = None
    # Optional. Output only. A map of AWS volume tags.
    tags: Any = None
    # Required. AWS volume ID.
    volume_id: Any = None

@dataclasses.dataclass
class VmmigrationDiskMigrationJob_Errors:
    code: Any = None
    details: Any = None
    message: Any = None

@dataclasses.dataclass
class VmmigrationDiskMigrationJob_Steps:
    copying_source_disk_snapshot: Any = None
    creating_source_disk_snapshot: Any = None
    end_time: Any = None
    provisioning_target_disk: Any = None
    start_time: Any = None

@dataclasses.dataclass
class VmmigrationDiskMigrationJob_TargetDetails_Encryption:
    # Required. The name of the encryption key that is stored in Google Cloud KMS.
    kms_key: Any = None

@dataclasses.dataclass
class VmmigrationDiskMigrationJob_TargetDetails_TargetDisk:
    # Optional. Target Compute Engine Disk ID. This is the resource ID segment of the Compute Engine Disk to create. In the resource name compute/v1/projects/{project}/zones/{zone}/disks/disk1 "disk1" is the resource ID for the disk.
    disk_id: Any = None
    # Required. The disk type to use.
    disk_type: Any = None
    # Optional. Replication zones of the regional disk. Should be of the form: projects/{target-project}/locations/{replica-zone} Currently only one replica zone is supported.
    replica_zones: Any = None
    # Required. The Compute Engine zone in which to create the disk. Should be of the form: projects/{target-project}/locations/{zone}
    zone: Any = None

@dataclasses.dataclass
class VmmigrationDiskMigrationJob_TargetDetails:
    # Encryption message describes the details of the applied encryption.
    encryption: Any = None
    # Optional. A map of labels to associate with the disk.
    labels: Any = None
    # Compute Engine disk target details.
    target_disk: Any = None
    # Required. The name of the resource of type TargetProject which represents the Compute Engine project in which to create the disk. Should be of the form: projects/{project}/locations/global/targetProjects/{target-project}
    target_project: Any = None

_VmmigrationDiskMigrationJob_AwsSourceDiskDetailsFields = {
    "disk_type": ubx.FieldSpec(wire_name="disk_type"),
    "size_gib": ubx.FieldSpec(wire_name="size_gib"),
    "tags": ubx.FieldSpec(wire_name="tags"),
    "volume_id": ubx.FieldSpec(wire_name="volume_id"),
}

_VmmigrationDiskMigrationJob_ErrorsFields = {
    "code": ubx.FieldSpec(wire_name="code"),
    "details": ubx.FieldSpec(wire_name="details"),
    "message": ubx.FieldSpec(wire_name="message"),
}

_VmmigrationDiskMigrationJob_StepsFields = {
    "copying_source_disk_snapshot": ubx.FieldSpec(wire_name="copying_source_disk_snapshot"),
    "creating_source_disk_snapshot": ubx.FieldSpec(wire_name="creating_source_disk_snapshot"),
    "end_time": ubx.FieldSpec(wire_name="end_time"),
    "provisioning_target_disk": ubx.FieldSpec(wire_name="provisioning_target_disk"),
    "start_time": ubx.FieldSpec(wire_name="start_time"),
}

_VmmigrationDiskMigrationJob_TargetDetails_EncryptionFields = {
    "kms_key": ubx.FieldSpec(wire_name="kms_key"),
}

_VmmigrationDiskMigrationJob_TargetDetails_TargetDiskFields = {
    "disk_id": ubx.FieldSpec(wire_name="disk_id"),
    "disk_type": ubx.FieldSpec(wire_name="disk_type"),
    "replica_zones": ubx.FieldSpec(wire_name="replica_zones"),
    "zone": ubx.FieldSpec(wire_name="zone"),
}

_VmmigrationDiskMigrationJob_TargetDetailsFields = {
    "encryption": ubx.FieldSpec(
        wire_name="encryption",
        kind="object",
        fields=_VmmigrationDiskMigrationJob_TargetDetails_EncryptionFields,
    ),
    "labels": ubx.FieldSpec(wire_name="labels"),
    "target_disk": ubx.FieldSpec(
        wire_name="target_disk",
        kind="object",
        fields=_VmmigrationDiskMigrationJob_TargetDetails_TargetDiskFields,
    ),
    "target_project": ubx.FieldSpec(wire_name="target_project"),
}

@dataclasses.dataclass
class VmmigrationDiskMigrationJobConfig:
    # Represents the source AWS Disk details.
    aws_source_disk_details: Any = None
    # Output only. The time the DiskMigrationJob resource was created.
    create_time: Any = None
    # Output only. Provides details on the errors that led to the disk migration job's state in case of an error.
    errors: Any = None
    # Output only. Identifier. The identifier of the DiskMigrationJob.
    name: Any = None
    # Output only. State of the DiskMigrationJob.
    state: Any = None
    # Output only. The disk migration steps list representing its progress.
    steps: Any = None
    # Details of the target disk in Compute Engine.
    target_details: Any = None
    # Output only. The last time the DiskMigrationJob resource was updated.
    update_time: Any = None

@dataclasses.dataclass
class VmmigrationDiskMigrationJobAttrs:
    # Represents the source AWS Disk details.
    aws_source_disk_details: Any = None
    # Output only. The time the DiskMigrationJob resource was created.
    create_time: Any = None
    # Output only. Provides details on the errors that led to the disk migration job's state in case of an error.
    errors: Any = None
    # Output only. Identifier. The identifier of the DiskMigrationJob.
    name: Any = None
    # Output only. State of the DiskMigrationJob.
    state: Any = None
    # Output only. The disk migration steps list representing its progress.
    steps: Any = None
    # Details of the target disk in Compute Engine.
    target_details: Any = None
    # Output only. The last time the DiskMigrationJob resource was updated.
    update_time: Any = None

VmmigrationDiskMigrationJob = ubx.ResourceBinding(
    wire_type="google_vmmigration_vmmigration_disk_migration_job",
    fields={
        "aws_source_disk_details": ubx.FieldSpec(
            wire_name="aws_source_disk_details",
            kind="object",
            fields=_VmmigrationDiskMigrationJob_AwsSourceDiskDetailsFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "errors": ubx.FieldSpec(
            wire_name="errors",
            kind="list",
            fields=_VmmigrationDiskMigrationJob_ErrorsFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "state": ubx.FieldSpec(wire_name="state"),
        "steps": ubx.FieldSpec(
            wire_name="steps",
            kind="list",
            fields=_VmmigrationDiskMigrationJob_StepsFields,
        ),
        "target_details": ubx.FieldSpec(
            wire_name="target_details",
            kind="object",
            fields=_VmmigrationDiskMigrationJob_TargetDetailsFields,
        ),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
