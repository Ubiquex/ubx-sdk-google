# Code generated by `ubx sdk gen` from google_contactcenterinsights_contactcenterinsights_auto_labeling_rule. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class ContactcenterinsightsAutoLabelingRule_Conditions:
    condition: Any = None
    value: Any = None

_ContactcenterinsightsAutoLabelingRule_ConditionsFields = {
    "condition": ubx.FieldSpec(wire_name="condition"),
    "value": ubx.FieldSpec(wire_name="value"),
}

@dataclasses.dataclass
class ContactcenterinsightsAutoLabelingRuleConfig:
    # Whether the rule is active.
    active: Any = None
    # Conditions to apply for auto-labeling the label_key. Representing sequential block of if .. else if .. else statements. The value of the first matching condition will be used.
    conditions: Any = None
    # Output only. The time at which this rule was created.
    create_time: Any = None
    # The description of the rule.
    description: Any = None
    # The user-provided display name of the rule.
    display_name: Any = None
    # The label key. This is also the {auto_labeling_rule} in the resource name. Only settable if label_key_type is LABEL_KEY_TYPE_CUSTOM.
    label_key: Any = None
    # The type of the label key.
    label_key_type: Any = None
    # Identifier. The resource name of the auto-labeling rule. Format: projects/{project}/locations/{location}/autoLabelingRules/{auto_labeling_rule}
    name: Any = None
    # Output only. The most recent time at which the rule was updated.
    update_time: Any = None

@dataclasses.dataclass
class ContactcenterinsightsAutoLabelingRuleAttrs:
    # Whether the rule is active.
    active: Any = None
    # Conditions to apply for auto-labeling the label_key. Representing sequential block of if .. else if .. else statements. The value of the first matching condition will be used.
    conditions: Any = None
    # Output only. The time at which this rule was created.
    create_time: Any = None
    # The description of the rule.
    description: Any = None
    # The user-provided display name of the rule.
    display_name: Any = None
    # The label key. This is also the {auto_labeling_rule} in the resource name. Only settable if label_key_type is LABEL_KEY_TYPE_CUSTOM.
    label_key: Any = None
    # The type of the label key.
    label_key_type: Any = None
    # Identifier. The resource name of the auto-labeling rule. Format: projects/{project}/locations/{location}/autoLabelingRules/{auto_labeling_rule}
    name: Any = None
    # Output only. The most recent time at which the rule was updated.
    update_time: Any = None

ContactcenterinsightsAutoLabelingRule = ubx.ResourceBinding(
    wire_type="google_contactcenterinsights_contactcenterinsights_auto_labeling_rule",
    fields={
        "active": ubx.FieldSpec(wire_name="active"),
        "conditions": ubx.FieldSpec(
            wire_name="conditions",
            kind="list",
            fields=_ContactcenterinsightsAutoLabelingRule_ConditionsFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "description": ubx.FieldSpec(wire_name="description"),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "label_key": ubx.FieldSpec(wire_name="label_key"),
        "label_key_type": ubx.FieldSpec(wire_name="label_key_type"),
        "name": ubx.FieldSpec(wire_name="name"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
