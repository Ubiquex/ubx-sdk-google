# Code generated by `ubx sdk gen` from google_pubsub_pubsub_topic. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_AwsKinesis:
    # Required. AWS role ARN to be used for Federated Identity authentication with Kinesis. Check the Pub/Sub docs for how to set up this role and the required permissions that need to be attached to it.
    aws_role_arn: Any = None
    # Required. The Kinesis consumer ARN to used for ingestion in Enhanced Fan-Out mode. The consumer must be already created and ready to be used.
    consumer_arn: Any = None
    # Required. The GCP service account to be used for Federated Identity authentication with Kinesis (via a `AssumeRoleWithWebIdentity` call for the provided role). The `aws_role_arn` must be set up with `accounts.google.com:sub` equals to this service account number.
    gcp_service_account: Any = None
    # Output only. An output-only field that indicates the state of the Kinesis ingestion source.
    state: Any = None
    # Required. The Kinesis stream ARN to ingest data from.
    stream_arn: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_AwsMsk:
    # Required. AWS role ARN to be used for Federated Identity authentication with Amazon MSK. Check the Pub/Sub docs for how to set up this role and the required permissions that need to be attached to it.
    aws_role_arn: Any = None
    # Required. The Amazon Resource Name (ARN) that uniquely identifies the cluster.
    cluster_arn: Any = None
    # Required. The GCP service account to be used for Federated Identity authentication with Amazon MSK (via a `AssumeRoleWithWebIdentity` call for the provided role). The `aws_role_arn` must be set up with `accounts.google.com:sub` equals to this service account number.
    gcp_service_account: Any = None
    # Output only. An output-only field that indicates the state of the Amazon MSK ingestion source.
    state: Any = None
    # Required. The name of the topic in the Amazon MSK cluster that Pub/Sub will import from.
    topic: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_AzureEventHubs:
    # Optional. The client id of the Azure application that is being used to authenticate Pub/Sub.
    client_id: Any = None
    # Optional. The name of the Event Hub.
    event_hub: Any = None
    # Optional. The GCP service account to be used for Federated Identity authentication.
    gcp_service_account: Any = None
    # Optional. The name of the Event Hubs namespace.
    namespace: Any = None
    # Optional. Name of the resource group within the azure subscription.
    resource_group: Any = None
    # Output only. An output-only field that indicates the state of the Event Hubs ingestion source.
    state: Any = None
    # Optional. The Azure subscription id.
    subscription_id: Any = None
    # Optional. The tenant id of the Azure application that is being used to authenticate Pub/Sub.
    tenant_id: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormat:
    # Optional. When unset, '\n' is used.
    delimiter: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_CloudStorage:
    # Configuration for reading Cloud Storage data in Avro binary format. The bytes of each object will be set to the `data` field of a Pub/Sub message.
    avro_format: Any = None
    # Optional. Cloud Storage bucket. The bucket name must be without any prefix like "gs://". See the [bucket naming requirements] (https://cloud.google.com/storage/docs/buckets#naming).
    bucket: Any = None
    # Optional. Glob pattern used to match objects that will be ingested. If unset, all objects will be ingested. See the [supported patterns](https://cloud.google.com/storage/docs/json_api/v1/objects/list#list-objects-and-prefixes-using-glob).
    match_glob: Any = None
    # Optional. Only objects with a larger or equal creation timestamp will be ingested.
    minimum_object_create_time: Any = None
    # Configuration for reading Cloud Storage data written via [Cloud Storage subscriptions](https://cloud.google.com/pubsub/docs/cloudstorage). The data and attributes fields of the originally exported Pub/Sub message will be restored when publishing.
    pubsub_avro_format: Any = None
    # Output only. An output-only field that indicates the state of the Cloud Storage ingestion source.
    state: Any = None
    # Configuration for reading Cloud Storage data in text format. Each line of text as specified by the delimiter will be set to the `data` field of a Pub/Sub message.
    text_format: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_ConfluentCloud:
    # Required. The address of the bootstrap server. The format is url:port.
    bootstrap_server: Any = None
    # Required. The id of the cluster.
    cluster_id: Any = None
    # Required. The GCP service account to be used for Federated Identity authentication with `identity_pool_id`.
    gcp_service_account: Any = None
    # Required. The id of the identity pool to be used for Federated Identity authentication with Confluent Cloud. See https://docs.confluent.io/cloud/current/security/authenticate/workload-identities/identity-providers/oauth/identity-pools.html#add-oauth-identity-pools.
    identity_pool_id: Any = None
    # Output only. An output-only field that indicates the state of the Confluent Cloud ingestion source.
    state: Any = None
    # Required. The name of the topic in the Confluent Cloud cluster that Pub/Sub will import from.
    topic: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettings:
    # Optional. The minimum severity level of Platform Logs that will be written.
    severity: Any = None

@dataclasses.dataclass
class PubsubTopic_IngestionDataSourceSettings:
    # Ingestion settings for Amazon Kinesis Data Streams.
    aws_kinesis: Any = None
    # Ingestion settings for Amazon MSK.
    aws_msk: Any = None
    # Ingestion settings for Azure Event Hubs.
    azure_event_hubs: Any = None
    # Ingestion settings for Cloud Storage.
    cloud_storage: Any = None
    # Ingestion settings for Confluent Cloud.
    confluent_cloud: Any = None
    # Settings for Platform Logs produced by Pub/Sub.
    platform_logs_settings: Any = None

@dataclasses.dataclass
class PubsubTopic_MessageStoragePolicy:
    # Optional. A list of IDs of Google Cloud regions where messages that are published to the topic may be persisted in storage. Messages published by publishers running in non-allowed Google Cloud regions (or running outside of Google Cloud altogether) are routed for storage in one of the allowed regions. An empty list means that no regions are allowed, and is not a valid configuration.
    allowed_persistence_regions: Any = None
    # Optional. If true, `allowed_persistence_regions` is also used to enforce in-transit guarantees for messages. That is, Pub/Sub will fail Publish operations on this topic and subscribe operations on any subscription attached to this topic in any region that is not in `allowed_persistence_regions`.
    enforce_in_transit: Any = None

@dataclasses.dataclass
class PubsubTopic_MessageTransforms_AiInference_UnstructuredInference:
    parameters: Any = None

@dataclasses.dataclass
class PubsubTopic_MessageTransforms_AiInference:
    endpoint: Any = None
    service_account_email: Any = None
    unstructured_inference: Any = None

@dataclasses.dataclass
class PubsubTopic_MessageTransforms_Compression:
    compression_algorithm: Any = None
    compression_mode: Any = None

@dataclasses.dataclass
class PubsubTopic_MessageTransforms_JavascriptUdf:
    code: Any = None
    function_name: Any = None

@dataclasses.dataclass
class PubsubTopic_MessageTransforms:
    ai_inference: Any = None
    compression: Any = None
    disabled: Any = None
    enabled: Any = None
    javascript_udf: Any = None

@dataclasses.dataclass
class PubsubTopic_SchemaSettings:
    # Optional. The encoding of messages validated against `schema`.
    encoding: Any = None
    # Optional. The minimum (inclusive) revision allowed for validating messages. If empty or not present, allow any revision to be validated against last_revision or any revision created before.
    first_revision_id: Any = None
    # Optional. The maximum (inclusive) revision allowed for validating messages. If empty or not present, allow any revision to be validated against first_revision or any revision created after.
    last_revision_id: Any = None
    # Required. The name of the schema that messages published should be validated against. Format is `projects/{project}/schemas/{schema}`. The value of this field will be `_deleted-schema_` if the schema has been deleted.
    schema: Any = None

_PubsubTopic_IngestionDataSourceSettings_AwsKinesisFields = {
    "aws_role_arn": ubx.FieldSpec(wire_name="aws_role_arn"),
    "consumer_arn": ubx.FieldSpec(wire_name="consumer_arn"),
    "gcp_service_account": ubx.FieldSpec(wire_name="gcp_service_account"),
    "state": ubx.FieldSpec(wire_name="state"),
    "stream_arn": ubx.FieldSpec(wire_name="stream_arn"),
}

_PubsubTopic_IngestionDataSourceSettings_AwsMskFields = {
    "aws_role_arn": ubx.FieldSpec(wire_name="aws_role_arn"),
    "cluster_arn": ubx.FieldSpec(wire_name="cluster_arn"),
    "gcp_service_account": ubx.FieldSpec(wire_name="gcp_service_account"),
    "state": ubx.FieldSpec(wire_name="state"),
    "topic": ubx.FieldSpec(wire_name="topic"),
}

_PubsubTopic_IngestionDataSourceSettings_AzureEventHubsFields = {
    "client_id": ubx.FieldSpec(wire_name="client_id"),
    "event_hub": ubx.FieldSpec(wire_name="event_hub"),
    "gcp_service_account": ubx.FieldSpec(wire_name="gcp_service_account"),
    "namespace": ubx.FieldSpec(wire_name="namespace"),
    "resource_group": ubx.FieldSpec(wire_name="resource_group"),
    "state": ubx.FieldSpec(wire_name="state"),
    "subscription_id": ubx.FieldSpec(wire_name="subscription_id"),
    "tenant_id": ubx.FieldSpec(wire_name="tenant_id"),
}

_PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormatFields = {
    "delimiter": ubx.FieldSpec(wire_name="delimiter"),
}

_PubsubTopic_IngestionDataSourceSettings_CloudStorageFields = {
    "avro_format": ubx.FieldSpec(wire_name="avro_format"),
    "bucket": ubx.FieldSpec(wire_name="bucket"),
    "match_glob": ubx.FieldSpec(wire_name="match_glob"),
    "minimum_object_create_time": ubx.FieldSpec(wire_name="minimum_object_create_time"),
    "pubsub_avro_format": ubx.FieldSpec(wire_name="pubsub_avro_format"),
    "state": ubx.FieldSpec(wire_name="state"),
    "text_format": ubx.FieldSpec(
        wire_name="text_format",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormatFields,
    ),
}

_PubsubTopic_IngestionDataSourceSettings_ConfluentCloudFields = {
    "bootstrap_server": ubx.FieldSpec(wire_name="bootstrap_server"),
    "cluster_id": ubx.FieldSpec(wire_name="cluster_id"),
    "gcp_service_account": ubx.FieldSpec(wire_name="gcp_service_account"),
    "identity_pool_id": ubx.FieldSpec(wire_name="identity_pool_id"),
    "state": ubx.FieldSpec(wire_name="state"),
    "topic": ubx.FieldSpec(wire_name="topic"),
}

_PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettingsFields = {
    "severity": ubx.FieldSpec(wire_name="severity"),
}

_PubsubTopic_IngestionDataSourceSettingsFields = {
    "aws_kinesis": ubx.FieldSpec(
        wire_name="aws_kinesis",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_AwsKinesisFields,
    ),
    "aws_msk": ubx.FieldSpec(
        wire_name="aws_msk",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_AwsMskFields,
    ),
    "azure_event_hubs": ubx.FieldSpec(
        wire_name="azure_event_hubs",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_AzureEventHubsFields,
    ),
    "cloud_storage": ubx.FieldSpec(
        wire_name="cloud_storage",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_CloudStorageFields,
    ),
    "confluent_cloud": ubx.FieldSpec(
        wire_name="confluent_cloud",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_ConfluentCloudFields,
    ),
    "platform_logs_settings": ubx.FieldSpec(
        wire_name="platform_logs_settings",
        kind="object",
        fields=_PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettingsFields,
    ),
}

_PubsubTopic_MessageStoragePolicyFields = {
    "allowed_persistence_regions": ubx.FieldSpec(wire_name="allowed_persistence_regions"),
    "enforce_in_transit": ubx.FieldSpec(wire_name="enforce_in_transit"),
}

_PubsubTopic_MessageTransforms_AiInference_UnstructuredInferenceFields = {
    "parameters": ubx.FieldSpec(wire_name="parameters"),
}

_PubsubTopic_MessageTransforms_AiInferenceFields = {
    "endpoint": ubx.FieldSpec(wire_name="endpoint"),
    "service_account_email": ubx.FieldSpec(wire_name="service_account_email"),
    "unstructured_inference": ubx.FieldSpec(
        wire_name="unstructured_inference",
        kind="object",
        fields=_PubsubTopic_MessageTransforms_AiInference_UnstructuredInferenceFields,
    ),
}

_PubsubTopic_MessageTransforms_CompressionFields = {
    "compression_algorithm": ubx.FieldSpec(wire_name="compression_algorithm"),
    "compression_mode": ubx.FieldSpec(wire_name="compression_mode"),
}

_PubsubTopic_MessageTransforms_JavascriptUdfFields = {
    "code": ubx.FieldSpec(wire_name="code"),
    "function_name": ubx.FieldSpec(wire_name="function_name"),
}

_PubsubTopic_MessageTransformsFields = {
    "ai_inference": ubx.FieldSpec(
        wire_name="ai_inference",
        kind="object",
        fields=_PubsubTopic_MessageTransforms_AiInferenceFields,
    ),
    "compression": ubx.FieldSpec(
        wire_name="compression",
        kind="object",
        fields=_PubsubTopic_MessageTransforms_CompressionFields,
    ),
    "disabled": ubx.FieldSpec(wire_name="disabled"),
    "enabled": ubx.FieldSpec(wire_name="enabled"),
    "javascript_udf": ubx.FieldSpec(
        wire_name="javascript_udf",
        kind="object",
        fields=_PubsubTopic_MessageTransforms_JavascriptUdfFields,
    ),
}

_PubsubTopic_SchemaSettingsFields = {
    "encoding": ubx.FieldSpec(wire_name="encoding"),
    "first_revision_id": ubx.FieldSpec(wire_name="first_revision_id"),
    "last_revision_id": ubx.FieldSpec(wire_name="last_revision_id"),
    "schema": ubx.FieldSpec(wire_name="schema"),
}

@dataclasses.dataclass
class PubsubTopicConfig:
    # Settings for an ingestion data source on a topic.
    ingestion_data_source_settings: Any = None
    # Optional. The resource name of the Cloud KMS CryptoKey to be used to protect access to messages published on this topic. The expected format is `projects/*/locations/*/keyRings/*/cryptoKeys/*`.
    kms_key_name: Any = None
    # Optional. See [Creating and managing labels] (https://cloud.google.com/pubsub/docs/labels).
    labels: Any = None
    # Optional. Indicates the minimum duration to retain a message after it is published to the topic. If this field is set, messages published to the topic in the last `message_retention_duration` are always available to subscribers. For instance, it allows any attached subscription to [seek to a timestamp](https://cloud.google.com/pubsub/docs/replay-overview#seek_to_a_time) that is up to `message_retention_duration` in the past. If this field is not set, message retention is controlled by settings on individual subscriptions. Cannot be more than 31 days or less than 10 minutes.
    message_retention_duration: Any = None
    # A policy constraining the storage of messages published to the topic.
    message_storage_policy: Any = None
    # Optional. Transforms to be applied to messages published to the topic. Transforms are applied in the order specified.
    message_transforms: Any = None
    # Required. Identifier. The name of the topic. It must have the format `"projects/{project}/topics/{topic}"`. `{topic}` must start with a letter, and contain only letters (`[A-Za-z]`), numbers (`[0-9]`), dashes (`-`), underscores (`_`), periods (`.`), tildes (`~`), plus (`+`) or percent signs (`%`). It must be between 3 and 255 characters in length, and it must not start with `"goog"`.
    name: Any = None
    # Optional. Reserved for future use. This field is set only in responses from the server; it is ignored if it is set in any requests.
    satisfies_pzs: Any = None
    # Settings for validating messages published against a schema.
    schema_settings: Any = None
    # Output only. An output-only field indicating the state of the topic.
    state: Any = None
    # Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing" See https://{$universe.dns_names.final_documentation_domain}/pubsub/docs/tags for more information on using tags with Pub/Sub resources.
    tags: Any = None

@dataclasses.dataclass
class PubsubTopicAttrs:
    # Settings for an ingestion data source on a topic.
    ingestion_data_source_settings: Any = None
    # Optional. The resource name of the Cloud KMS CryptoKey to be used to protect access to messages published on this topic. The expected format is `projects/*/locations/*/keyRings/*/cryptoKeys/*`.
    kms_key_name: Any = None
    # Optional. See [Creating and managing labels] (https://cloud.google.com/pubsub/docs/labels).
    labels: Any = None
    # Optional. Indicates the minimum duration to retain a message after it is published to the topic. If this field is set, messages published to the topic in the last `message_retention_duration` are always available to subscribers. For instance, it allows any attached subscription to [seek to a timestamp](https://cloud.google.com/pubsub/docs/replay-overview#seek_to_a_time) that is up to `message_retention_duration` in the past. If this field is not set, message retention is controlled by settings on individual subscriptions. Cannot be more than 31 days or less than 10 minutes.
    message_retention_duration: Any = None
    # A policy constraining the storage of messages published to the topic.
    message_storage_policy: Any = None
    # Optional. Transforms to be applied to messages published to the topic. Transforms are applied in the order specified.
    message_transforms: Any = None
    # Required. Identifier. The name of the topic. It must have the format `"projects/{project}/topics/{topic}"`. `{topic}` must start with a letter, and contain only letters (`[A-Za-z]`), numbers (`[0-9]`), dashes (`-`), underscores (`_`), periods (`.`), tildes (`~`), plus (`+`) or percent signs (`%`). It must be between 3 and 255 characters in length, and it must not start with `"goog"`.
    name: Any = None
    # Optional. Reserved for future use. This field is set only in responses from the server; it is ignored if it is set in any requests.
    satisfies_pzs: Any = None
    # Settings for validating messages published against a schema.
    schema_settings: Any = None
    # Output only. An output-only field indicating the state of the topic.
    state: Any = None
    # Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing" See https://{$universe.dns_names.final_documentation_domain}/pubsub/docs/tags for more information on using tags with Pub/Sub resources.
    tags: Any = None

PubsubTopic = ubx.ResourceBinding(
    wire_type="google_pubsub_pubsub_topic",
    fields={
        "ingestion_data_source_settings": ubx.FieldSpec(
            wire_name="ingestion_data_source_settings",
            kind="object",
            fields=_PubsubTopic_IngestionDataSourceSettingsFields,
        ),
        "kms_key_name": ubx.FieldSpec(wire_name="kms_key_name"),
        "labels": ubx.FieldSpec(wire_name="labels"),
        "message_retention_duration": ubx.FieldSpec(wire_name="message_retention_duration"),
        "message_storage_policy": ubx.FieldSpec(
            wire_name="message_storage_policy",
            kind="object",
            fields=_PubsubTopic_MessageStoragePolicyFields,
        ),
        "message_transforms": ubx.FieldSpec(
            wire_name="message_transforms",
            kind="list",
            fields=_PubsubTopic_MessageTransformsFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "satisfies_pzs": ubx.FieldSpec(wire_name="satisfies_pzs"),
        "schema_settings": ubx.FieldSpec(
            wire_name="schema_settings",
            kind="object",
            fields=_PubsubTopic_SchemaSettingsFields,
        ),
        "state": ubx.FieldSpec(wire_name="state"),
        "tags": ubx.FieldSpec(wire_name="tags"),
    },
)
