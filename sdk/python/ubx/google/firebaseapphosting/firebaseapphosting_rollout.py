...
class FirebaseapphostingRollout_Error:
...
_FirebaseapphostingRollout_ErrorFields = {
...
@dataclasses.dataclass
class FirebaseapphostingRolloutConfig:
...
@dataclasses.dataclass
class FirebaseapphostingRolloutAttrs:
    # Optional. Unstructured key value map that may be set by external tools to store and arbitrary metadata. They are not queryable and should be preserved when modifying objects.
    annotations: Any = None
    # Required. Immutable. The name of a build that already exists. It doesn't have to be built; a rollout will wait for a build to be ready before updating traffic.
    build: Any = None
...
FirebaseapphostingRollout = ubx.ResourceBinding(
    wire_type="google_firebaseapphosting_firebaseapphosting_rollout",
    fields={
        ...
        "error": ubx.FieldSpec(
            wire_name="error",
            kind="object",
            fields=_FirebaseapphostingRollout_ErrorFields,
        ),
