# Code generated by `ubx sdk gen` from google_ftp_v1alpha_ftp_v1alpha_server. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1alphaFtpV1alphaServer_ExternalConfig:
    # Optional. List of CIDR blocks that are allowed to access the Server. A CIDR range consists of an IP Address and a prefix length to construct the subnet mask. By default, the prefix length is 32 (i.e. matches a single IP address). For now, only IPV4 addresses are supported. Examples: "203.0.113.0/24" - matches with the IP addresses in the range 203.0.113.0 - 203.0.113.255. "0.0.0.0/0" - matches against any IP address. This field must contain at least one entry if the access type is EXTERNAL. The number of allowed CIDR blocks cannot exceed 500. Example: 192.168.0.0/16
    allowed_cidr_blocks: Any = None
    # Output only. IP address of the LB via which clients will connect.
    ip_address: Any = None

@dataclasses.dataclass
class V1alphaFtpV1alphaServer_GoogleManagedServerCredential:
    # Output only. Asymmetric algorithm used by the public key. Possible values (can be expanded in future): - ssh-ed25519
    asymmetric_algorithm: Any = None
    # Output only. The fingerprint is a hash of the public key, and is displayed when clients access the server for the first time to verify the server's identity.
    fingerprint: Any = None

@dataclasses.dataclass
class V1alphaFtpV1alphaServer_InternalConfig_ConsumerAcceptList:
    connection_limit: Any = None
    project: Any = None

@dataclasses.dataclass
class V1alphaFtpV1alphaServer_InternalConfig_ConsumerRejectList:
    project: Any = None

@dataclasses.dataclass
class V1alphaFtpV1alphaServer_InternalConfig_PscEndpoints:
    endpoint: Any = None
    network: Any = None
    status: Any = None

@dataclasses.dataclass
class V1alphaFtpV1alphaServer_InternalConfig:
    # Required. A list of projects that are permitted to connect. At least one project is required in the allow list.
    consumer_accept_list: Any = None
    # Optional. A list of projects that are denied connection. Format: "projects/sample_project_id" or "projects/1234567890" Projects in this list will be denied access, even if they are included in the `allow_list`. If this list is empty, no projects are explicitly rejected.
    consumer_reject_list: Any = None
    # Output only. Details of endpoints created by the customer.
    psc_endpoints: Any = None
    # Output only. The resource name of the service attachment. Format: `projects/{project}/regions/{region}/serviceAttachments/{service_attachment}`
    service_attachment: Any = None

_V1alphaFtpV1alphaServer_ExternalConfigFields = {
    "allowed_cidr_blocks": ubx.FieldSpec(wire_name="allowed_cidr_blocks"),
    "ip_address": ubx.FieldSpec(wire_name="ip_address"),
}

_V1alphaFtpV1alphaServer_GoogleManagedServerCredentialFields = {
    "asymmetric_algorithm": ubx.FieldSpec(wire_name="asymmetric_algorithm"),
    "fingerprint": ubx.FieldSpec(wire_name="fingerprint"),
}

_V1alphaFtpV1alphaServer_InternalConfig_ConsumerAcceptListFields = {
    "connection_limit": ubx.FieldSpec(wire_name="connection_limit"),
    "project": ubx.FieldSpec(wire_name="project"),
}

_V1alphaFtpV1alphaServer_InternalConfig_ConsumerRejectListFields = {
    "project": ubx.FieldSpec(wire_name="project"),
}

_V1alphaFtpV1alphaServer_InternalConfig_PscEndpointsFields = {
    "endpoint": ubx.FieldSpec(wire_name="endpoint"),
    "network": ubx.FieldSpec(wire_name="network"),
    "status": ubx.FieldSpec(wire_name="status"),
}

_V1alphaFtpV1alphaServer_InternalConfigFields = {
    "consumer_accept_list": ubx.FieldSpec(
        wire_name="consumer_accept_list",
        kind="list",
        fields=_V1alphaFtpV1alphaServer_InternalConfig_ConsumerAcceptListFields,
    ),
    "consumer_reject_list": ubx.FieldSpec(
        wire_name="consumer_reject_list",
        kind="list",
        fields=_V1alphaFtpV1alphaServer_InternalConfig_ConsumerRejectListFields,
    ),
    "psc_endpoints": ubx.FieldSpec(
        wire_name="psc_endpoints",
        kind="list",
        fields=_V1alphaFtpV1alphaServer_InternalConfig_PscEndpointsFields,
    ),
    "service_attachment": ubx.FieldSpec(wire_name="service_attachment"),
}

@dataclasses.dataclass
class V1alphaFtpV1alphaServerConfig:
    # Required. The access type of the Server.
    access_type: Any = None
    # Output only. [Output only] Create time stamp
    create_time: Any = None
    # Optional. Display name of the Server
    display_name: Any = None
    # Configuration for external server.
    external_config: Any = None
    # Represents credentials of an FTP Server.
    google_managed_server_credential: Any = None
    # Configuration for private server accessible via PSC.
    internal_config: Any = None
    # Optional. Labels as key value pairs
    labels: Any = None
    # Identifier. name of resource
    name: Any = None
    # Output only. Whether the Server satisfies Physical Zone Isolation (PZI) requirements.
    satisfies_pzi: Any = None
    # Output only. Whether the Server satisfies Physical Zone Separation (PZS) requirements.
    satisfies_pzs: Any = None
    # Output only. Service agent used to access the customer bucket.
    service_agent: Any = None
    # Output only. The state of the server.
    state: Any = None
    # Output only. [Output only] Update time stamp
    update_time: Any = None

@dataclasses.dataclass
class V1alphaFtpV1alphaServerAttrs:
    # Required. The access type of the Server.
    access_type: Any = None
    # Output only. [Output only] Create time stamp
    create_time: Any = None
    # Optional. Display name of the Server
    display_name: Any = None
    # Configuration for external server.
    external_config: Any = None
    # Represents credentials of an FTP Server.
    google_managed_server_credential: Any = None
    # Configuration for private server accessible via PSC.
    internal_config: Any = None
    # Optional. Labels as key value pairs
    labels: Any = None
    # Identifier. name of resource
    name: Any = None
    # Output only. Whether the Server satisfies Physical Zone Isolation (PZI) requirements.
    satisfies_pzi: Any = None
    # Output only. Whether the Server satisfies Physical Zone Separation (PZS) requirements.
    satisfies_pzs: Any = None
    # Output only. Service agent used to access the customer bucket.
    service_agent: Any = None
    # Output only. The state of the server.
    state: Any = None
    # Output only. [Output only] Update time stamp
    update_time: Any = None

V1alphaFtpV1alphaServer = ubx.ResourceBinding(
    wire_type="google_ftp_v1alpha_ftp_v1alpha_server",
    fields={
        "access_type": ubx.FieldSpec(wire_name="access_type"),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "external_config": ubx.FieldSpec(
            wire_name="external_config",
            kind="object",
            fields=_V1alphaFtpV1alphaServer_ExternalConfigFields,
        ),
        "google_managed_server_credential": ubx.FieldSpec(
            wire_name="google_managed_server_credential",
            kind="object",
            fields=_V1alphaFtpV1alphaServer_GoogleManagedServerCredentialFields,
        ),
        "internal_config": ubx.FieldSpec(
            wire_name="internal_config",
            kind="object",
            fields=_V1alphaFtpV1alphaServer_InternalConfigFields,
        ),
        "labels": ubx.FieldSpec(wire_name="labels"),
        "name": ubx.FieldSpec(wire_name="name"),
        "satisfies_pzi": ubx.FieldSpec(wire_name="satisfies_pzi"),
        "satisfies_pzs": ubx.FieldSpec(wire_name="satisfies_pzs"),
        "service_agent": ubx.FieldSpec(wire_name="service_agent"),
        "state": ubx.FieldSpec(wire_name="state"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
