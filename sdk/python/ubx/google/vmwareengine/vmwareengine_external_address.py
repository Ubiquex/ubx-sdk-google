# Code generated by `ubx sdk gen` from google_vmwareengine_vmwareengine_external_address. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class VmwareengineExternalAddressConfig:
    # Output only. Creation time of this resource.
    create_time: Any = None
    # User-provided description for this resource.
    description: Any = None
    # Output only. The external IP address of a workload VM.
    external_ip: Any = None
    # The internal IP address of a workload VM.
    internal_ip: Any = None
    # Output only. Identifier. The resource name of this external IP address. Resource names are schemeless URIs that follow the conventions in https://cloud.google.com/apis/design/resource_names. For example: `projects/my-project/locations/us-central1-a/privateClouds/my-cloud/externalAddresses/my-address`
    name: Any = None
    # Output only. The state of the resource.
    state: Any = None
    # Output only. System-generated unique identifier for the resource.
    uid: Any = None
    # Output only. Last update time of this resource.
    update_time: Any = None

@dataclasses.dataclass
class VmwareengineExternalAddressAttrs:
    # Output only. Creation time of this resource.
    create_time: Any = None
    # User-provided description for this resource.
    description: Any = None
    # Output only. The external IP address of a workload VM.
    external_ip: Any = None
    # The internal IP address of a workload VM.
    internal_ip: Any = None
    # Output only. Identifier. The resource name of this external IP address. Resource names are schemeless URIs that follow the conventions in https://cloud.google.com/apis/design/resource_names. For example: `projects/my-project/locations/us-central1-a/privateClouds/my-cloud/externalAddresses/my-address`
    name: Any = None
    # Output only. The state of the resource.
    state: Any = None
    # Output only. System-generated unique identifier for the resource.
    uid: Any = None
    # Output only. Last update time of this resource.
    update_time: Any = None

VmwareengineExternalAddress = ubx.ResourceBinding(
    wire_type="google_vmwareengine_vmwareengine_external_address",
    fields={
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "description": ubx.FieldSpec(wire_name="description"),
        "external_ip": ubx.FieldSpec(wire_name="external_ip"),
        "internal_ip": ubx.FieldSpec(wire_name="internal_ip"),
        "name": ubx.FieldSpec(wire_name="name"),
        "state": ubx.FieldSpec(wire_name="state"),
        "uid": ubx.FieldSpec(wire_name="uid"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
