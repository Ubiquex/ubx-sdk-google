# Code generated by `ubx sdk gen` from google_agentregistry_v1alpha_agentregistry_v1alpha_service. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaService_AgentSpec:
    # Optional. The content of the Agent spec in the JSON format. This payload is validated against the schema for the specified type. The content size is limited to `10KB`.
    content: Any = None
    # Required. The type of the agent spec content.
    type: Any = None

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaService_Interfaces:
    protocol_binding: Any = None
    url: Any = None

_V1alphaAgentregistryV1alphaService_AgentSpecFields = {
    "content": ubx.FieldSpec(wire_name="content"),
    "type": ubx.FieldSpec(wire_name="type"),
}

_V1alphaAgentregistryV1alphaService_InterfacesFields = {
    "protocol_binding": ubx.FieldSpec(wire_name="protocol_binding"),
    "url": ubx.FieldSpec(wire_name="url"),
}

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaServiceConfig:
    # The spec of the agent.
    agent_spec: Any = None
    # Output only. Create time.
    create_time: Any = None
    # Optional. User-defined description of an Service. Can have a maximum length of `2048` characters.
    description: Any = None
    # Optional. User-defined display name for the Service. Can have a maximum length of `63` characters.
    display_name: Any = None
    # The spec of the endpoint.
    endpoint_spec: Any = None
    # Optional. The connection details for the Service.
    interfaces: Any = None
    # The spec of the MCP Server.
    mcp_server_spec: Any = None
    # Identifier. The resource name of the Service. Format: `projects/{project}/locations/{location}/services/{service}`.
    name: Any = None
    # Output only. The resource name of the resulting Agent, MCP Server, or Endpoint. Format: * `projects/{project}/locations/{location}/mcpServers/{mcp_server}` * `projects/{project}/locations/{location}/agents/{agent}` * `projects/{project}/locations/{location}/endpoints/{endpoint}`
    registry_resource: Any = None
    # Output only. Update time.
    update_time: Any = None

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaServiceAttrs:
    # The spec of the agent.
    agent_spec: Any = None
    # Output only. Create time.
    create_time: Any = None
    # Optional. User-defined description of an Service. Can have a maximum length of `2048` characters.
    description: Any = None
    # Optional. User-defined display name for the Service. Can have a maximum length of `63` characters.
    display_name: Any = None
    # The spec of the endpoint.
    endpoint_spec: Any = None
    # Optional. The connection details for the Service.
    interfaces: Any = None
    # The spec of the MCP Server.
    mcp_server_spec: Any = None
    # Identifier. The resource name of the Service. Format: `projects/{project}/locations/{location}/services/{service}`.
    name: Any = None
    # Output only. The resource name of the resulting Agent, MCP Server, or Endpoint. Format: * `projects/{project}/locations/{location}/mcpServers/{mcp_server}` * `projects/{project}/locations/{location}/agents/{agent}` * `projects/{project}/locations/{location}/endpoints/{endpoint}`
    registry_resource: Any = None
    # Output only. Update time.
    update_time: Any = None

V1alphaAgentregistryV1alphaService = ubx.ResourceBinding(
    wire_type="google_agentregistry_v1alpha_agentregistry_v1alpha_service",
    fields={
        "agent_spec": ubx.FieldSpec(
            wire_name="agent_spec",
            kind="object",
            fields=_V1alphaAgentregistryV1alphaService_AgentSpecFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "description": ubx.FieldSpec(wire_name="description"),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "endpoint_spec": ubx.FieldSpec(
            wire_name="endpoint_spec",
            kind="object",
            fields=_V1alphaAgentregistryV1alphaService_AgentSpecFields,
        ),
        "interfaces": ubx.FieldSpec(
            wire_name="interfaces",
            kind="list",
            fields=_V1alphaAgentregistryV1alphaService_InterfacesFields,
        ),
        "mcp_server_spec": ubx.FieldSpec(
            wire_name="mcp_server_spec",
            kind="object",
            fields=_V1alphaAgentregistryV1alphaService_AgentSpecFields,
        ),
        "name": ubx.FieldSpec(wire_name="name"),
        "registry_resource": ubx.FieldSpec(wire_name="registry_resource"),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
