# Code generated by `ubx sdk gen` from google_agentregistry_v1alpha_agentregistry_v1alpha_binding. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaBinding_AuthProviderBinding:
    # Required. The resource name of the target AuthProvider. Format: * `projects/{project}/locations/{location}/authProviders/{auth_provider}`
    auth_provider: Any = None
    # Optional. The continue URI of the AuthProvider. The URI is used to reauthenticate the user and finalize the managed OAuth flow.
    continue_uri: Any = None
    # Optional. The list of OAuth2 scopes of the AuthProvider.
    scopes: Any = None

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaBinding_Source:
    # The identifier of the source Agent. Format: * `urn:agent:{publisher}:{namespace}:{name}`
    identifier: Any = None

_V1alphaAgentregistryV1alphaBinding_AuthProviderBindingFields = {
    "auth_provider": ubx.FieldSpec(wire_name="auth_provider"),
    "continue_uri": ubx.FieldSpec(wire_name="continue_uri"),
    "scopes": ubx.FieldSpec(wire_name="scopes"),
}

_V1alphaAgentregistryV1alphaBinding_SourceFields = {
    "identifier": ubx.FieldSpec(wire_name="identifier"),
}

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaBindingConfig:
    # The AuthProvider of the Binding.
    auth_provider_binding: Any = None
    # Output only. Timestamp when this binding was created.
    create_time: Any = None
    # Optional. User-defined description of a Binding. Can have a maximum length of `2048` characters.
    description: Any = None
    # Optional. User-defined display name for the Binding. Can have a maximum length of `63` characters.
    display_name: Any = None
    # Required. Identifier. The resource name of the Binding. Format: `projects/{project}/locations/{location}/bindings/{binding}`.
    name: Any = None
    # The source of the Binding.
    source: Any = None
    # The target of the Binding.
    target: Any = None
    # Output only. Timestamp when this binding was last updated.
    update_time: Any = None

@dataclasses.dataclass
class V1alphaAgentregistryV1alphaBindingAttrs:
    # The AuthProvider of the Binding.
    auth_provider_binding: Any = None
    # Output only. Timestamp when this binding was created.
    create_time: Any = None
    # Optional. User-defined description of a Binding. Can have a maximum length of `2048` characters.
    description: Any = None
    # Optional. User-defined display name for the Binding. Can have a maximum length of `63` characters.
    display_name: Any = None
    # Required. Identifier. The resource name of the Binding. Format: `projects/{project}/locations/{location}/bindings/{binding}`.
    name: Any = None
    # The source of the Binding.
    source: Any = None
    # The target of the Binding.
    target: Any = None
    # Output only. Timestamp when this binding was last updated.
    update_time: Any = None

V1alphaAgentregistryV1alphaBinding = ubx.ResourceBinding(
    wire_type="google_agentregistry_v1alpha_agentregistry_v1alpha_binding",
    fields={
        "auth_provider_binding": ubx.FieldSpec(
            wire_name="auth_provider_binding",
            kind="object",
            fields=_V1alphaAgentregistryV1alphaBinding_AuthProviderBindingFields,
        ),
        "create_time": ubx.FieldSpec(wire_name="create_time"),
        "description": ubx.FieldSpec(wire_name="description"),
        "display_name": ubx.FieldSpec(wire_name="display_name"),
        "name": ubx.FieldSpec(wire_name="name"),
        "source": ubx.FieldSpec(
            wire_name="source",
            kind="object",
            fields=_V1alphaAgentregistryV1alphaBinding_SourceFields,
        ),
        "target": ubx.FieldSpec(
            wire_name="target",
            kind="object",
            fields=_V1alphaAgentregistryV1alphaBinding_SourceFields,
        ),
        "update_time": ubx.FieldSpec(wire_name="update_time"),
    },
)
