// Code generated by `ubx sdk gen` from google_lustre_lustre_instance. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface LustreInstance_AccessRulesOptions_AccessRules {
  ipAddressRanges?: string[] | Computed<string[]>;
  name?: string | Computed<string>;
  squashMode?: string | Computed<string>;
}

export interface LustreInstance_AccessRulesOptions {
  /** Optional. The access rules for the instance. */
  accessRules?: LustreInstance_AccessRulesOptions_AccessRules[] | Computed<LustreInstance_AccessRulesOptions_AccessRules[]>;
  /** Optional. The user squash GID for the default access rule. This user squash GID applies to all root users connecting from clients that are not matched by any of the access rules. If not set, the default is 0 (no GID squash). */
  defaultSquashGid?: number | Computed<number>;
  /** Required. The squash mode for the default access rule. */
  defaultSquashMode?: string | Computed<string>;
  /** Optional. The user squash UID for the default access rule. This user squash UID applies to all root users connecting from clients that are not matched by any of the access rules. If not set, the default is 0 (no UID squash). */
  defaultSquashUid?: number | Computed<number>;
}

export interface LustreInstance_DynamicTierOptions {
  /** Required. Immutable. The dynamic tier mode of the instance. */
  mode?: string | Computed<string>;
}

export interface LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDate {
  day?: number | Computed<number>;
  month?: number | Computed<number>;
  year?: number | Computed<number>;
}

export interface LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_Time {
  hours?: number | Computed<number>;
  minutes?: number | Computed<number>;
  nanos?: number | Computed<number>;
  seconds?: number | Computed<number>;
}

export interface LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow {
  endDate?: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDate | Computed<LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDate>;
  startDate?: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDate | Computed<LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDate>;
  time?: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_Time | Computed<LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_Time>;
}

export interface LustreInstance_MaintenancePolicy_WeeklyMaintenanceWindows {
  dayOfWeek?: string | Computed<string>;
  startTime?: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_Time | Computed<LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_Time>;
}

export interface LustreInstance_MaintenancePolicy {
  /** Optional. The exclusion windows for the instance. Currently limited to 1 window. */
  maintenanceExclusionWindow?: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow[] | Computed<LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow[]>;
  /** Required. The weekly maintenance windows for the instance. Currently limited to 1 window. */
  weeklyMaintenanceWindows?: LustreInstance_MaintenancePolicy_WeeklyMaintenanceWindows[] | Computed<LustreInstance_MaintenancePolicy_WeeklyMaintenanceWindows[]>;
}

export interface LustreInstance_UpcomingMaintenanceSchedule {
  /** Output only. The scheduled end time for the maintenance. */
  endTime?: string | Computed<string>;
  /** Output only. The scheduled start time for the maintenance. */
  startTime?: string | Computed<string>;
}

const LustreInstance_AccessRulesOptions_AccessRulesFields: FieldMap = {
  ipAddressRanges: "ip_address_ranges",
  name: "name",
  squashMode: "squash_mode",
};

const LustreInstance_AccessRulesOptionsFields: FieldMap = {
  accessRules: {
    wireName: "access_rules",
    kind: "list",
    fields: LustreInstance_AccessRulesOptions_AccessRulesFields,
  },
  defaultSquashGid: "default_squash_gid",
  defaultSquashMode: "default_squash_mode",
  defaultSquashUid: "default_squash_uid",
};

const LustreInstance_DynamicTierOptionsFields: FieldMap = {
  mode: "mode",
};

const LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDateFields: FieldMap = {
  day: "day",
  month: "month",
  year: "year",
};

const LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_TimeFields: FieldMap = {
  hours: "hours",
  minutes: "minutes",
  nanos: "nanos",
  seconds: "seconds",
};

const LustreInstance_MaintenancePolicy_MaintenanceExclusionWindowFields: FieldMap = {
  endDate: {
    wireName: "end_date",
    kind: "object",
    fields: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDateFields,
  },
  startDate: {
    wireName: "start_date",
    kind: "object",
    fields: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_EndDateFields,
  },
  time: {
    wireName: "time",
    kind: "object",
    fields: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_TimeFields,
  },
};

const LustreInstance_MaintenancePolicy_WeeklyMaintenanceWindowsFields: FieldMap = {
  dayOfWeek: "day_of_week",
  startTime: {
    wireName: "start_time",
    kind: "object",
    fields: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindow_TimeFields,
  },
};

const LustreInstance_MaintenancePolicyFields: FieldMap = {
  maintenanceExclusionWindow: {
    wireName: "maintenance_exclusion_window",
    kind: "list",
    fields: LustreInstance_MaintenancePolicy_MaintenanceExclusionWindowFields,
  },
  weeklyMaintenanceWindows: {
    wireName: "weekly_maintenance_windows",
    kind: "list",
    fields: LustreInstance_MaintenancePolicy_WeeklyMaintenanceWindowsFields,
  },
};

const LustreInstance_UpcomingMaintenanceScheduleFields: FieldMap = {
  endTime: "end_time",
  startTime: "start_time",
};

export interface LustreInstanceConfig {
  /** IP-based access rules for the Managed Lustre instance. These options define the root user squash configuration. */
  accessRulesOptions?: LustreInstance_AccessRulesOptions | Computed<LustreInstance_AccessRulesOptions>;
  /** Required. The storage capacity of the instance in gibibytes (GiB). Allowed values depend on the `perUnitStorageThroughput`. See [Performance tiers](https://docs.cloud.google.com/managed-lustre/docs/performance-tiers) for specific minimums, maximums, and step sizes for each performance tier. */
  capacityGib?: string | Computed<string>;
  /** Output only. Timestamp when the instance was created. */
  createTime?: string | Computed<string>;
  /** Optional. A user-readable description of the instance. */
  description?: string | Computed<string>;
  /** Dynamic tier options for a Managed Lustre instance. */
  dynamicTierOptions?: LustreInstance_DynamicTierOptions | Computed<LustreInstance_DynamicTierOptions>;
  /** Required. Immutable. The filesystem name for this instance. This name is used by client-side tools, including when mounting the instance. Must be eight characters or less and can only contain letters and numbers. */
  filesystem?: string | Computed<string>;
  /** Optional. Deprecated: No longer required for GKE instance creation. Indicates whether you want to enable support for GKE clients. By default, GKE clients are not supported. */
  gkeSupportEnabled?: boolean | Computed<boolean>;
  /** Optional. Immutable. The Cloud KMS key name to use for data encryption. If not set, the instance will use Google-managed encryption keys. If set, the instance will use customer-managed encryption keys. The key must be in the same region as the instance. The key format is: projects/{project}/locations/{location}/keyRings/{key_ring}/cryptoKeys/{key} */
  kmsKey?: string | Computed<string>;
  /** Optional. Labels as key value pairs. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Defines a maintenance policy for a resource. */
  maintenancePolicy?: LustreInstance_MaintenancePolicy | Computed<LustreInstance_MaintenancePolicy>;
  /** Output only. Mount point of the instance in the format `IP_ADDRESS@tcp:/FILESYSTEM`. */
  mountPoint?: string | Computed<string>;
  /** Identifier. The name of the instance. */
  name?: string | Computed<string>;
  /** Required. Immutable. The full name of the VPC network to which the instance is connected. Must be in the format `projects/{project_id}/global/networks/{network_name}`. */
  network?: string | Computed<string>;
  /** Optional. The throughput of the instance in MBps per TiB. Valid values are 0, 125, 250, 500, 1000. See [Performance tiers](https://docs.cloud.google.com/managed-lustre/docs/performance-tiers) for more information. If the instance is using the Dynamic tier, this field must not be set or must be set to zero. */
  perUnitStorageThroughput?: string | Computed<string>;
  /** Optional. The placement policy name for the instance in the format of projects/{project}/locations/{location}/resourcePolicies/{resource_policy} */
  placementPolicy?: string | Computed<string>;
  /** Output only. The state of the instance. */
  state?: string | Computed<string>;
  /** Output only. The reason why the instance is in a certain state (e.g. SUSPENDED). */
  stateReason?: string | Computed<string>;
  /** Output only. Unique ID of the resource. This is unrelated to the access rules which allow specifying the root squash uid. */
  uid?: string | Computed<string>;
  /** Represents a scheduled maintenance event. */
  upcomingMaintenanceSchedule?: LustreInstance_UpcomingMaintenanceSchedule | Computed<LustreInstance_UpcomingMaintenanceSchedule>;
  /** Output only. Timestamp when the instance was last updated. */
  updateTime?: string | Computed<string>;
}

export interface LustreInstanceAttrs {
  /** IP-based access rules for the Managed Lustre instance. These options define the root user squash configuration. */
  accessRulesOptions: LustreInstance_AccessRulesOptions;
  /** Required. The storage capacity of the instance in gibibytes (GiB). Allowed values depend on the `perUnitStorageThroughput`. See [Performance tiers](https://docs.cloud.google.com/managed-lustre/docs/performance-tiers) for specific minimums, maximums, and step sizes for each performance tier. */
  capacityGib: string;
  /** Output only. Timestamp when the instance was created. */
  createTime: string;
  /** Optional. A user-readable description of the instance. */
  description: string;
  /** Dynamic tier options for a Managed Lustre instance. */
  dynamicTierOptions: LustreInstance_DynamicTierOptions;
  /** Required. Immutable. The filesystem name for this instance. This name is used by client-side tools, including when mounting the instance. Must be eight characters or less and can only contain letters and numbers. */
  filesystem: string;
  /** Optional. Deprecated: No longer required for GKE instance creation. Indicates whether you want to enable support for GKE clients. By default, GKE clients are not supported. */
  gkeSupportEnabled: boolean;
  /** Optional. Immutable. The Cloud KMS key name to use for data encryption. If not set, the instance will use Google-managed encryption keys. If set, the instance will use customer-managed encryption keys. The key must be in the same region as the instance. The key format is: projects/{project}/locations/{location}/keyRings/{key_ring}/cryptoKeys/{key} */
  kmsKey: string;
  /** Optional. Labels as key value pairs. */
  labels: Record<string, string>;
  /** Defines a maintenance policy for a resource. */
  maintenancePolicy: LustreInstance_MaintenancePolicy;
  /** Output only. Mount point of the instance in the format `IP_ADDRESS@tcp:/FILESYSTEM`. */
  mountPoint: string;
  /** Identifier. The name of the instance. */
  name: string;
  /** Required. Immutable. The full name of the VPC network to which the instance is connected. Must be in the format `projects/{project_id}/global/networks/{network_name}`. */
  network: string;
  /** Optional. The throughput of the instance in MBps per TiB. Valid values are 0, 125, 250, 500, 1000. See [Performance tiers](https://docs.cloud.google.com/managed-lustre/docs/performance-tiers) for more information. If the instance is using the Dynamic tier, this field must not be set or must be set to zero. */
  perUnitStorageThroughput: string;
  /** Optional. The placement policy name for the instance in the format of projects/{project}/locations/{location}/resourcePolicies/{resource_policy} */
  placementPolicy: string;
  /** Output only. The state of the instance. */
  state: string;
  /** Output only. The reason why the instance is in a certain state (e.g. SUSPENDED). */
  stateReason: string;
  /** Output only. Unique ID of the resource. This is unrelated to the access rules which allow specifying the root squash uid. */
  uid: string;
  /** Represents a scheduled maintenance event. */
  upcomingMaintenanceSchedule: LustreInstance_UpcomingMaintenanceSchedule;
  /** Output only. Timestamp when the instance was last updated. */
  updateTime: string;
}

export const LustreInstance: ResourceBinding<LustreInstanceConfig, LustreInstanceAttrs> = {
  wireType: "google_lustre_lustre_instance",
  fields: {
    accessRulesOptions: {
      wireName: "access_rules_options",
      kind: "object",
      fields: LustreInstance_AccessRulesOptionsFields,
    },
    capacityGib: "capacity_gib",
    createTime: "create_time",
    description: "description",
    dynamicTierOptions: {
      wireName: "dynamic_tier_options",
      kind: "object",
      fields: LustreInstance_DynamicTierOptionsFields,
    },
    filesystem: "filesystem",
    gkeSupportEnabled: "gke_support_enabled",
    kmsKey: "kms_key",
    labels: "labels",
    maintenancePolicy: {
      wireName: "maintenance_policy",
      kind: "object",
      fields: LustreInstance_MaintenancePolicyFields,
    },
    mountPoint: "mount_point",
    name: "name",
    network: "network",
    perUnitStorageThroughput: "per_unit_storage_throughput",
    placementPolicy: "placement_policy",
    state: "state",
    stateReason: "state_reason",
    uid: "uid",
    upcomingMaintenanceSchedule: {
      wireName: "upcoming_maintenance_schedule",
      kind: "object",
      fields: LustreInstance_UpcomingMaintenanceScheduleFields,
    },
    updateTime: "update_time",
  },
};
