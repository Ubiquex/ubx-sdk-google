// Code generated by `ubx sdk gen` from google_agentregistry_v1alpha_agentregistry_v1alpha_skill. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface V1alphaAgentregistryV1alphaSkill_Frontmatter {
  /** Optional. Environmental dependencies or local sidecars. */
  compatibility?: string | Computed<string>;
  /** Required. Functional description. */
  description?: string | Computed<string>;
  /** Optional. License. */
  license?: string | Computed<string>;
  /** Optional. Extensible flattened map mapping custom tags, authors, and version parameters. */
  metadata?: Record<string, string> | Computed<Record<string, string>>;
  /** Required. The name of the skill. */
  name?: string | Computed<string>;
}

export interface V1alphaAgentregistryV1alphaSkill_InitialRevision_ArchiveUploadSource {
  /** Required. Input only. Write-only raw ZIP/TAR archive payload bytes containing the skill package. */
  archiveContent?: string | Computed<string>;
}

export interface V1alphaAgentregistryV1alphaSkill_InitialRevision_GcsSource {
  /** Optional. Cloud Storage object generation ID. If not specified, the latest generation is used. */
  generation?: string | Computed<string>;
  /** Required. Cloud Storage object URI. Format: `gs://{bucket_name}/{object_name}` */
  uri?: string | Computed<string>;
}

export interface V1alphaAgentregistryV1alphaSkill_InitialRevision {
  /** Direct write-only raw archive payload upload. */
  archiveUploadSource?: V1alphaAgentregistryV1alphaSkill_InitialRevision_ArchiveUploadSource | Computed<V1alphaAgentregistryV1alphaSkill_InitialRevision_ArchiveUploadSource>;
  /** Output only. Revision creation timestamp. */
  createTime?: string | Computed<string>;
  /** Structured metadata attributes extracted from the package's local SKILL.md frontmatter. */
  frontmatter?: V1alphaAgentregistryV1alphaSkill_Frontmatter | Computed<V1alphaAgentregistryV1alphaSkill_Frontmatter>;
  /** Specifications for Cloud Storage objects. */
  gcsSource?: V1alphaAgentregistryV1alphaSkill_InitialRevision_GcsSource | Computed<V1alphaAgentregistryV1alphaSkill_InitialRevision_GcsSource>;
  /** Identifier. Resource name of the SkillRevision. Format: `projects/{project}/locations/{location}/skills/{skill}/revisions/{revision}` */
  name?: string | Computed<string>;
  /** Output only. Cryptographic SHA-256 integrity and deduplication digest of the payload zip. */
  sha256Hash?: string | Computed<string>;
  /** Output only. Size of the compiled zip payload in bytes (assists client download progress). */
  sizeBytes?: string | Computed<string>;
  /** Output only. The system-managed lifecycle state of this revision. */
  state?: string | Computed<string>;
  /** Output only. Universally unique identifier (UUID4) for the skill revision. */
  uid?: string | Computed<string>;
}

const V1alphaAgentregistryV1alphaSkill_FrontmatterFields: FieldMap = {
  compatibility: "compatibility",
  description: "description",
  license: "license",
  metadata: "metadata",
  name: "name",
};

const V1alphaAgentregistryV1alphaSkill_InitialRevision_ArchiveUploadSourceFields: FieldMap = {
  archiveContent: "archive_content",
};

const V1alphaAgentregistryV1alphaSkill_InitialRevision_GcsSourceFields: FieldMap = {
  generation: "generation",
  uri: "uri",
};

const V1alphaAgentregistryV1alphaSkill_InitialRevisionFields: FieldMap = {
  archiveUploadSource: {
    wireName: "archive_upload_source",
    kind: "object",
    fields: V1alphaAgentregistryV1alphaSkill_InitialRevision_ArchiveUploadSourceFields,
  },
  createTime: "create_time",
  frontmatter: {
    wireName: "frontmatter",
    kind: "object",
    fields: V1alphaAgentregistryV1alphaSkill_FrontmatterFields,
  },
  gcsSource: {
    wireName: "gcs_source",
    kind: "object",
    fields: V1alphaAgentregistryV1alphaSkill_InitialRevision_GcsSourceFields,
  },
  name: "name",
  sha256Hash: "sha256_hash",
  sizeBytes: "size_bytes",
  state: "state",
  uid: "uid",
};

export interface V1alphaAgentregistryV1alphaSkillConfig {
  /** Output only. Create time. */
  createTime?: string | Computed<string>;
  /** Optional. The full resource name of the revision currently served by default (floating track). Format: `projects/{project}/locations/{location}/skills/{skill}/revisions/{revision}` */
  defaultRevision?: string | Computed<string>;
  /** Optional. Brief summary describing the capabilities of the skill. Maximum length is 2048 characters. */
  description?: string | Computed<string>;
  /** Required. Human-readable display name of the skill. Maximum length is 128 characters. */
  displayName?: string | Computed<string>;
  /** Structured metadata attributes extracted from the package's local SKILL.md frontmatter. */
  frontmatter?: V1alphaAgentregistryV1alphaSkill_Frontmatter | Computed<V1alphaAgentregistryV1alphaSkill_Frontmatter>;
  /** Represents an immutable, versioned snapshot of a Skill package. */
  initialRevision?: V1alphaAgentregistryV1alphaSkill_InitialRevision | Computed<V1alphaAgentregistryV1alphaSkill_InitialRevision>;
  /** Identifier. Resource name of the Skill. Format: `projects/{project}/locations/{location}/skills/{skill}` The `{skill}` segment acts as the resource ID. If the skill is associated with a Publisher, this segment typically uses a hyphenated namespace prefix corresponding to the publisher (e.g., `google-workspace-create-docs`). */
  name?: string | Computed<string>;
  /** Optional. The publisher resource associated with this skill. Format: `projects/{project}/locations/{location}/publishers/{publisher}` The publisher dictates the allowed namespace prefixes for the skill's name and logical `skill_id` (e.g., Publisher `google` authorizes the `google-*` prefix). */
  publisher?: string | Computed<string>;
  /** Output only. A stable, globally unique logical identifier for the skill. It is securely constructed by the backend by combining the associated `publisher`'s verified namespace and the skill's resource ID to enforce strict ownership. For example, the prefix `google-` is reserved exclusively for first-party Google publishers to prevent namespace squatting. Example: `urn:skill:google-workspace:create-docs` */
  skillId?: string | Computed<string>;
  /** Output only. The system-managed state of the skill. */
  state?: string | Computed<string>;
  /** Required. User-managed target state of the skill. */
  targetState?: string | Computed<string>;
  /** Required. Structural deployment type (SIMPLE leaf vs COMPOSITE bundle). */
  type?: string | Computed<string>;
  /** Output only. Universally unique identifier (UUID4) for the logical container. */
  uid?: string | Computed<string>;
  /** Output only. Update time. */
  updateTime?: string | Computed<string>;
}

export interface V1alphaAgentregistryV1alphaSkillAttrs {
  /** Output only. Create time. */
  createTime: string;
  /** Optional. The full resource name of the revision currently served by default (floating track). Format: `projects/{project}/locations/{location}/skills/{skill}/revisions/{revision}` */
  defaultRevision: string;
  /** Optional. Brief summary describing the capabilities of the skill. Maximum length is 2048 characters. */
  description: string;
  /** Required. Human-readable display name of the skill. Maximum length is 128 characters. */
  displayName: string;
  /** Structured metadata attributes extracted from the package's local SKILL.md frontmatter. */
  frontmatter: V1alphaAgentregistryV1alphaSkill_Frontmatter;
  /** Represents an immutable, versioned snapshot of a Skill package. */
  initialRevision: V1alphaAgentregistryV1alphaSkill_InitialRevision;
  /** Identifier. Resource name of the Skill. Format: `projects/{project}/locations/{location}/skills/{skill}` The `{skill}` segment acts as the resource ID. If the skill is associated with a Publisher, this segment typically uses a hyphenated namespace prefix corresponding to the publisher (e.g., `google-workspace-create-docs`). */
  name: string;
  /** Optional. The publisher resource associated with this skill. Format: `projects/{project}/locations/{location}/publishers/{publisher}` The publisher dictates the allowed namespace prefixes for the skill's name and logical `skill_id` (e.g., Publisher `google` authorizes the `google-*` prefix). */
  publisher: string;
  /** Output only. A stable, globally unique logical identifier for the skill. It is securely constructed by the backend by combining the associated `publisher`'s verified namespace and the skill's resource ID to enforce strict ownership. For example, the prefix `google-` is reserved exclusively for first-party Google publishers to prevent namespace squatting. Example: `urn:skill:google-workspace:create-docs` */
  skillId: string;
  /** Output only. The system-managed state of the skill. */
  state: string;
  /** Required. User-managed target state of the skill. */
  targetState: string;
  /** Required. Structural deployment type (SIMPLE leaf vs COMPOSITE bundle). */
  type: string;
  /** Output only. Universally unique identifier (UUID4) for the logical container. */
  uid: string;
  /** Output only. Update time. */
  updateTime: string;
}

export const V1alphaAgentregistryV1alphaSkill: ResourceBinding<V1alphaAgentregistryV1alphaSkillConfig, V1alphaAgentregistryV1alphaSkillAttrs> = {
  wireType: "google_agentregistry_v1alpha_agentregistry_v1alpha_skill",
  fields: {
    createTime: "create_time",
    defaultRevision: "default_revision",
    description: "description",
    displayName: "display_name",
    frontmatter: {
      wireName: "frontmatter",
      kind: "object",
      fields: V1alphaAgentregistryV1alphaSkill_FrontmatterFields,
    },
    initialRevision: {
      wireName: "initial_revision",
      kind: "object",
      fields: V1alphaAgentregistryV1alphaSkill_InitialRevisionFields,
    },
    name: "name",
    publisher: "publisher",
    skillId: "skill_id",
    state: "state",
    targetState: "target_state",
    type: "type",
    uid: "uid",
    updateTime: "update_time",
  },
};
