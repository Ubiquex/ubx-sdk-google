// Code generated by `ubx sdk gen` from google_certificatemanager_certificatemanager_dns_authorization. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface CertificatemanagerDnsAuthorization_DnsResourceRecord {
  /** Output only. Data of the DNS Resource Record. */
  data?: string | Computed<string>;
  /** Output only. Fully qualified name of the DNS Resource Record. e.g. `_acme-challenge.example.com` */
  name?: string | Computed<string>;
  /** Output only. Type of the DNS Resource Record. Currently always set to "CNAME". */
  type?: string | Computed<string>;
}

const CertificatemanagerDnsAuthorization_DnsResourceRecordFields: FieldMap = {
  data: "data",
  name: "name",
  type: "type",
};

export interface CertificatemanagerDnsAuthorizationConfig {
  /** Output only. The creation timestamp of a DnsAuthorization. */
  createTime?: string | Computed<string>;
  /** Optional. One or more paragraphs of text description of a DnsAuthorization. */
  description?: string | Computed<string>;
  /** The structure describing the DNS Resource Record that needs to be added to DNS configuration for the authorization to be usable by certificate. */
  dnsResourceRecord?: CertificatemanagerDnsAuthorization_DnsResourceRecord | Computed<CertificatemanagerDnsAuthorization_DnsResourceRecord>;
  /** Required. Immutable. A domain that is being authorized. A DnsAuthorization resource covers a single domain and its wildcard, e.g. authorization for `example.com` can be used to issue certificates for `example.com` and `*.example.com`. */
  domain?: string | Computed<string>;
  /** Optional. Set of labels associated with a DnsAuthorization. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Identifier. A user-defined name of the dns authorization. DnsAuthorization names must be unique globally and match pattern `projects/* /locations/* /dnsAuthorizations/*`. */
  name?: string | Computed<string>;
  /** Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing" */
  tags?: Record<string, string> | Computed<Record<string, string>>;
  /** Optional. Immutable. Type of DnsAuthorization. If unset during resource creation the following default will be used: - in location `global`: FIXED_RECORD, - in other locations: PER_PROJECT_RECORD. */
  type?: string | Computed<string>;
  /** Output only. The last update timestamp of a DnsAuthorization. */
  updateTime?: string | Computed<string>;
}

export interface CertificatemanagerDnsAuthorizationAttrs {
  /** Output only. The creation timestamp of a DnsAuthorization. */
  createTime: string;
  /** Optional. One or more paragraphs of text description of a DnsAuthorization. */
  description: string;
  /** The structure describing the DNS Resource Record that needs to be added to DNS configuration for the authorization to be usable by certificate. */
  dnsResourceRecord: CertificatemanagerDnsAuthorization_DnsResourceRecord;
  /** Required. Immutable. A domain that is being authorized. A DnsAuthorization resource covers a single domain and its wildcard, e.g. authorization for `example.com` can be used to issue certificates for `example.com` and `*.example.com`. */
  domain: string;
  /** Optional. Set of labels associated with a DnsAuthorization. */
  labels: Record<string, string>;
  /** Identifier. A user-defined name of the dns authorization. DnsAuthorization names must be unique globally and match pattern `projects/* /locations/* /dnsAuthorizations/*`. */
  name: string;
  /** Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing" */
  tags: Record<string, string>;
  /** Optional. Immutable. Type of DnsAuthorization. If unset during resource creation the following default will be used: - in location `global`: FIXED_RECORD, - in other locations: PER_PROJECT_RECORD. */
  type: string;
  /** Output only. The last update timestamp of a DnsAuthorization. */
  updateTime: string;
}

export const CertificatemanagerDnsAuthorization: ResourceBinding<CertificatemanagerDnsAuthorizationConfig, CertificatemanagerDnsAuthorizationAttrs> = {
  wireType: "google_certificatemanager_certificatemanager_dns_authorization",
  fields: {
    createTime: "create_time",
    description: "description",
    dnsResourceRecord: {
      wireName: "dns_resource_record",
      kind: "object",
      fields: CertificatemanagerDnsAuthorization_DnsResourceRecordFields,
    },
    domain: "domain",
    labels: "labels",
    name: "name",
    tags: "tags",
    type: "type",
    updateTime: "update_time",
  },
};
