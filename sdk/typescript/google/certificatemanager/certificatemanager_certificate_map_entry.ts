// Code generated by `ubx sdk gen` from google_certificatemanager_certificatemanager_certificate_map_entry. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface CertificatemanagerCertificateMapEntryConfig {
  /** Optional. A set of Certificates defines for the given `hostname`. There can be defined up to four certificates in each Certificate Map Entry. Each certificate must match pattern `projects/* /locations/* /certificates/*`. */
  certificates?: string[] | Computed<string[]>;
  /** Output only. The creation timestamp of a Certificate Map Entry. */
  createTime?: string | Computed<string>;
  /** Optional. One or more paragraphs of text description of a certificate map entry. */
  description?: string | Computed<string>;
  /** A Hostname (FQDN, e.g. `example.com`) or a wildcard hostname expression (`*.example.com`) for a set of hostnames with common suffix. Used as Server Name Indication (SNI) for selecting a proper certificate. */
  hostname?: string | Computed<string>;
  /** Optional. Set of labels associated with a Certificate Map Entry. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** A predefined matcher for particular cases, other than SNI selection. */
  matcher?: string | Computed<string>;
  /** Identifier. A user-defined name of the Certificate Map Entry. Certificate Map Entry names must be unique globally and match pattern `projects/* /locations/* /certificateMaps/* /certificateMapEntries/*`. */
  name?: string | Computed<string>;
  /** Output only. A serving state of this Certificate Map Entry. */
  state?: string | Computed<string>;
  /** Output only. The update timestamp of a Certificate Map Entry. */
  updateTime?: string | Computed<string>;
}

export interface CertificatemanagerCertificateMapEntryAttrs {
  /** Optional. A set of Certificates defines for the given `hostname`. There can be defined up to four certificates in each Certificate Map Entry. Each certificate must match pattern `projects/* /locations/* /certificates/*`. */
  certificates: string[];
  /** Output only. The creation timestamp of a Certificate Map Entry. */
  createTime: string;
  /** Optional. One or more paragraphs of text description of a certificate map entry. */
  description: string;
  /** A Hostname (FQDN, e.g. `example.com`) or a wildcard hostname expression (`*.example.com`) for a set of hostnames with common suffix. Used as Server Name Indication (SNI) for selecting a proper certificate. */
  hostname: string;
  /** Optional. Set of labels associated with a Certificate Map Entry. */
  labels: Record<string, string>;
  /** A predefined matcher for particular cases, other than SNI selection. */
  matcher: string;
  /** Identifier. A user-defined name of the Certificate Map Entry. Certificate Map Entry names must be unique globally and match pattern `projects/* /locations/* /certificateMaps/* /certificateMapEntries/*`. */
  name: string;
  /** Output only. A serving state of this Certificate Map Entry. */
  state: string;
  /** Output only. The update timestamp of a Certificate Map Entry. */
  updateTime: string;
}

export const CertificatemanagerCertificateMapEntry: ResourceBinding<CertificatemanagerCertificateMapEntryConfig, CertificatemanagerCertificateMapEntryAttrs> = {
  wireType: "google_certificatemanager_certificatemanager_certificate_map_entry",
  fields: {
    certificates: "certificates",
    createTime: "create_time",
    description: "description",
    hostname: "hostname",
    labels: "labels",
    matcher: "matcher",
    name: "name",
    state: "state",
    updateTime: "update_time",
  },
};
