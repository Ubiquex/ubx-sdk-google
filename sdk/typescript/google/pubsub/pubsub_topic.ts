// Code generated by `ubx sdk gen` from google_pubsub_pubsub_topic. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface PubsubTopic_IngestionDataSourceSettings_AwsKinesis {
  /** Required. AWS role ARN to be used for Federated Identity authentication with Kinesis. Check the Pub/Sub docs for how to set up this role and the required permissions that need to be attached to it. */
  awsRoleArn?: string | Computed<string>;
  /** Required. The Kinesis consumer ARN to used for ingestion in Enhanced Fan-Out mode. The consumer must be already created and ready to be used. */
  consumerArn?: string | Computed<string>;
  /** Required. The GCP service account to be used for Federated Identity authentication with Kinesis (via a `AssumeRoleWithWebIdentity` call for the provided role). The `aws_role_arn` must be set up with `accounts.google.com:sub` equals to this service account number. */
  gcpServiceAccount?: string | Computed<string>;
  /** Output only. An output-only field that indicates the state of the Kinesis ingestion source. */
  state?: string | Computed<string>;
  /** Required. The Kinesis stream ARN to ingest data from. */
  streamArn?: string | Computed<string>;
}

export interface PubsubTopic_IngestionDataSourceSettings_AwsMsk {
  /** Required. AWS role ARN to be used for Federated Identity authentication with Amazon MSK. Check the Pub/Sub docs for how to set up this role and the required permissions that need to be attached to it. */
  awsRoleArn?: string | Computed<string>;
  /** Required. The Amazon Resource Name (ARN) that uniquely identifies the cluster. */
  clusterArn?: string | Computed<string>;
  /** Required. The GCP service account to be used for Federated Identity authentication with Amazon MSK (via a `AssumeRoleWithWebIdentity` call for the provided role). The `aws_role_arn` must be set up with `accounts.google.com:sub` equals to this service account number. */
  gcpServiceAccount?: string | Computed<string>;
  /** Output only. An output-only field that indicates the state of the Amazon MSK ingestion source. */
  state?: string | Computed<string>;
  /** Required. The name of the topic in the Amazon MSK cluster that Pub/Sub will import from. */
  topic?: string | Computed<string>;
}

export interface PubsubTopic_IngestionDataSourceSettings_AzureEventHubs {
  /** Optional. The client id of the Azure application that is being used to authenticate Pub/Sub. */
  clientId?: string | Computed<string>;
  /** Optional. The name of the Event Hub. */
  eventHub?: string | Computed<string>;
  /** Optional. The GCP service account to be used for Federated Identity authentication. */
  gcpServiceAccount?: string | Computed<string>;
  /** Optional. The name of the Event Hubs namespace. */
  namespace?: string | Computed<string>;
  /** Optional. Name of the resource group within the azure subscription. */
  resourceGroup?: string | Computed<string>;
  /** Output only. An output-only field that indicates the state of the Event Hubs ingestion source. */
  state?: string | Computed<string>;
  /** Optional. The Azure subscription id. */
  subscriptionId?: string | Computed<string>;
  /** Optional. The tenant id of the Azure application that is being used to authenticate Pub/Sub. */
  tenantId?: string | Computed<string>;
}

export interface PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormat {
  /** Optional. When unset, '\n' is used. */
  delimiter?: string | Computed<string>;
}

export interface PubsubTopic_IngestionDataSourceSettings_CloudStorage {
  /** Configuration for reading Cloud Storage data in Avro binary format. The bytes of each object will be set to the `data` field of a Pub/Sub message. */
  avroFormat?: unknown | Computed<unknown>;
  /** Optional. Cloud Storage bucket. The bucket name must be without any prefix like "gs://". See the [bucket naming requirements] (https://cloud.google.com/storage/docs/buckets#naming). */
  bucket?: string | Computed<string>;
  /** Optional. Glob pattern used to match objects that will be ingested. If unset, all objects will be ingested. See the [supported patterns](https://cloud.google.com/storage/docs/json_api/v1/objects/list#list-objects-and-prefixes-using-glob). */
  matchGlob?: string | Computed<string>;
  /** Optional. Only objects with a larger or equal creation timestamp will be ingested. */
  minimumObjectCreateTime?: string | Computed<string>;
  /** Configuration for reading Cloud Storage data written via [Cloud Storage subscriptions](https://cloud.google.com/pubsub/docs/cloudstorage). The data and attributes fields of the originally exported Pub/Sub message will be restored when publishing. */
  pubsubAvroFormat?: unknown | Computed<unknown>;
  /** Output only. An output-only field that indicates the state of the Cloud Storage ingestion source. */
  state?: string | Computed<string>;
  /** Configuration for reading Cloud Storage data in text format. Each line of text as specified by the delimiter will be set to the `data` field of a Pub/Sub message. */
  textFormat?: PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormat | Computed<PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormat>;
}

export interface PubsubTopic_IngestionDataSourceSettings_ConfluentCloud {
  /** Required. The address of the bootstrap server. The format is url:port. */
  bootstrapServer?: string | Computed<string>;
  /** Required. The id of the cluster. */
  clusterId?: string | Computed<string>;
  /** Required. The GCP service account to be used for Federated Identity authentication with `identity_pool_id`. */
  gcpServiceAccount?: string | Computed<string>;
  /** Required. The id of the identity pool to be used for Federated Identity authentication with Confluent Cloud. See https://docs.confluent.io/cloud/current/security/authenticate/workload-identities/identity-providers/oauth/identity-pools.html#add-oauth-identity-pools. */
  identityPoolId?: string | Computed<string>;
  /** Output only. An output-only field that indicates the state of the Confluent Cloud ingestion source. */
  state?: string | Computed<string>;
  /** Required. The name of the topic in the Confluent Cloud cluster that Pub/Sub will import from. */
  topic?: string | Computed<string>;
}

export interface PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettings {
  /** Optional. The minimum severity level of Platform Logs that will be written. */
  severity?: string | Computed<string>;
}

export interface PubsubTopic_IngestionDataSourceSettings {
  /** Ingestion settings for Amazon Kinesis Data Streams. */
  awsKinesis?: PubsubTopic_IngestionDataSourceSettings_AwsKinesis | Computed<PubsubTopic_IngestionDataSourceSettings_AwsKinesis>;
  /** Ingestion settings for Amazon MSK. */
  awsMsk?: PubsubTopic_IngestionDataSourceSettings_AwsMsk | Computed<PubsubTopic_IngestionDataSourceSettings_AwsMsk>;
  /** Ingestion settings for Azure Event Hubs. */
  azureEventHubs?: PubsubTopic_IngestionDataSourceSettings_AzureEventHubs | Computed<PubsubTopic_IngestionDataSourceSettings_AzureEventHubs>;
  /** Ingestion settings for Cloud Storage. */
  cloudStorage?: PubsubTopic_IngestionDataSourceSettings_CloudStorage | Computed<PubsubTopic_IngestionDataSourceSettings_CloudStorage>;
  /** Ingestion settings for Confluent Cloud. */
  confluentCloud?: PubsubTopic_IngestionDataSourceSettings_ConfluentCloud | Computed<PubsubTopic_IngestionDataSourceSettings_ConfluentCloud>;
  /** Settings for Platform Logs produced by Pub/Sub. */
  platformLogsSettings?: PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettings | Computed<PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettings>;
}

export interface PubsubTopic_MessageStoragePolicy {
  /** Optional. A list of IDs of Google Cloud regions where messages that are published to the topic may be persisted in storage. Messages published by publishers running in non-allowed Google Cloud regions (or running outside of Google Cloud altogether) are routed for storage in one of the allowed regions. An empty list means that no regions are allowed, and is not a valid configuration. */
  allowedPersistenceRegions?: string[] | Computed<string[]>;
  /** Optional. If true, `allowed_persistence_regions` is also used to enforce in-transit guarantees for messages. That is, Pub/Sub will fail Publish operations on this topic and subscribe operations on any subscription attached to this topic in any region that is not in `allowed_persistence_regions`. */
  enforceInTransit?: boolean | Computed<boolean>;
}

export interface PubsubTopic_MessageTransforms_AiInference_UnstructuredInference {
  parameters?: Record<string, unknown> | Computed<Record<string, unknown>>;
}

export interface PubsubTopic_MessageTransforms_AiInference {
  endpoint?: string | Computed<string>;
  serviceAccountEmail?: string | Computed<string>;
  unstructuredInference?: PubsubTopic_MessageTransforms_AiInference_UnstructuredInference | Computed<PubsubTopic_MessageTransforms_AiInference_UnstructuredInference>;
}

export interface PubsubTopic_MessageTransforms_Compression {
  compressionAlgorithm?: string | Computed<string>;
  compressionMode?: string | Computed<string>;
}

export interface PubsubTopic_MessageTransforms_JavascriptUdf {
  code?: string | Computed<string>;
  functionName?: string | Computed<string>;
}

export interface PubsubTopic_MessageTransforms {
  aiInference?: PubsubTopic_MessageTransforms_AiInference | Computed<PubsubTopic_MessageTransforms_AiInference>;
  compression?: PubsubTopic_MessageTransforms_Compression | Computed<PubsubTopic_MessageTransforms_Compression>;
  disabled?: boolean | Computed<boolean>;
  enabled?: boolean | Computed<boolean>;
  javascriptUdf?: PubsubTopic_MessageTransforms_JavascriptUdf | Computed<PubsubTopic_MessageTransforms_JavascriptUdf>;
}

export interface PubsubTopic_SchemaSettings {
  /** Optional. The encoding of messages validated against `schema`. */
  encoding?: string | Computed<string>;
  /** Optional. The minimum (inclusive) revision allowed for validating messages. If empty or not present, allow any revision to be validated against last_revision or any revision created before. */
  firstRevisionId?: string | Computed<string>;
  /** Optional. The maximum (inclusive) revision allowed for validating messages. If empty or not present, allow any revision to be validated against first_revision or any revision created after. */
  lastRevisionId?: string | Computed<string>;
  /** Required. The name of the schema that messages published should be validated against. Format is `projects/{project}/schemas/{schema}`. The value of this field will be `_deleted-schema_` if the schema has been deleted. */
  schema?: string | Computed<string>;
}

const PubsubTopic_IngestionDataSourceSettings_AwsKinesisFields: FieldMap = {
  awsRoleArn: "aws_role_arn",
  consumerArn: "consumer_arn",
  gcpServiceAccount: "gcp_service_account",
  state: "state",
  streamArn: "stream_arn",
};

const PubsubTopic_IngestionDataSourceSettings_AwsMskFields: FieldMap = {
  awsRoleArn: "aws_role_arn",
  clusterArn: "cluster_arn",
  gcpServiceAccount: "gcp_service_account",
  state: "state",
  topic: "topic",
};

const PubsubTopic_IngestionDataSourceSettings_AzureEventHubsFields: FieldMap = {
  clientId: "client_id",
  eventHub: "event_hub",
  gcpServiceAccount: "gcp_service_account",
  namespace: "namespace",
  resourceGroup: "resource_group",
  state: "state",
  subscriptionId: "subscription_id",
  tenantId: "tenant_id",
};

const PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormatFields: FieldMap = {
  delimiter: "delimiter",
};

const PubsubTopic_IngestionDataSourceSettings_CloudStorageFields: FieldMap = {
  avroFormat: "avro_format",
  bucket: "bucket",
  matchGlob: "match_glob",
  minimumObjectCreateTime: "minimum_object_create_time",
  pubsubAvroFormat: "pubsub_avro_format",
  state: "state",
  textFormat: {
    wireName: "text_format",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_CloudStorage_TextFormatFields,
  },
};

const PubsubTopic_IngestionDataSourceSettings_ConfluentCloudFields: FieldMap = {
  bootstrapServer: "bootstrap_server",
  clusterId: "cluster_id",
  gcpServiceAccount: "gcp_service_account",
  identityPoolId: "identity_pool_id",
  state: "state",
  topic: "topic",
};

const PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettingsFields: FieldMap = {
  severity: "severity",
};

const PubsubTopic_IngestionDataSourceSettingsFields: FieldMap = {
  awsKinesis: {
    wireName: "aws_kinesis",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_AwsKinesisFields,
  },
  awsMsk: {
    wireName: "aws_msk",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_AwsMskFields,
  },
  azureEventHubs: {
    wireName: "azure_event_hubs",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_AzureEventHubsFields,
  },
  cloudStorage: {
    wireName: "cloud_storage",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_CloudStorageFields,
  },
  confluentCloud: {
    wireName: "confluent_cloud",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_ConfluentCloudFields,
  },
  platformLogsSettings: {
    wireName: "platform_logs_settings",
    kind: "object",
    fields: PubsubTopic_IngestionDataSourceSettings_PlatformLogsSettingsFields,
  },
};

const PubsubTopic_MessageStoragePolicyFields: FieldMap = {
  allowedPersistenceRegions: "allowed_persistence_regions",
  enforceInTransit: "enforce_in_transit",
};

const PubsubTopic_MessageTransforms_AiInference_UnstructuredInferenceFields: FieldMap = {
  parameters: "parameters",
};

const PubsubTopic_MessageTransforms_AiInferenceFields: FieldMap = {
  endpoint: "endpoint",
  serviceAccountEmail: "service_account_email",
  unstructuredInference: {
    wireName: "unstructured_inference",
    kind: "object",
    fields: PubsubTopic_MessageTransforms_AiInference_UnstructuredInferenceFields,
  },
};

const PubsubTopic_MessageTransforms_CompressionFields: FieldMap = {
  compressionAlgorithm: "compression_algorithm",
  compressionMode: "compression_mode",
};

const PubsubTopic_MessageTransforms_JavascriptUdfFields: FieldMap = {
  code: "code",
  functionName: "function_name",
};

const PubsubTopic_MessageTransformsFields: FieldMap = {
  aiInference: {
    wireName: "ai_inference",
    kind: "object",
    fields: PubsubTopic_MessageTransforms_AiInferenceFields,
  },
  compression: {
    wireName: "compression",
    kind: "object",
    fields: PubsubTopic_MessageTransforms_CompressionFields,
  },
  disabled: "disabled",
  enabled: "enabled",
  javascriptUdf: {
    wireName: "javascript_udf",
    kind: "object",
    fields: PubsubTopic_MessageTransforms_JavascriptUdfFields,
  },
};

const PubsubTopic_SchemaSettingsFields: FieldMap = {
  encoding: "encoding",
  firstRevisionId: "first_revision_id",
  lastRevisionId: "last_revision_id",
  schema: "schema",
};

export interface PubsubTopicConfig {
  /** Settings for an ingestion data source on a topic. */
  ingestionDataSourceSettings?: PubsubTopic_IngestionDataSourceSettings | Computed<PubsubTopic_IngestionDataSourceSettings>;
  /** Optional. The resource name of the Cloud KMS CryptoKey to be used to protect access to messages published on this topic. The expected format is `projects/* /locations/* /keyRings/* /cryptoKeys/*`. */
  kmsKeyName?: string | Computed<string>;
  /** Optional. See [Creating and managing labels] (https://cloud.google.com/pubsub/docs/labels). */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Optional. Indicates the minimum duration to retain a message after it is published to the topic. If this field is set, messages published to the topic in the last `message_retention_duration` are always available to subscribers. For instance, it allows any attached subscription to [seek to a timestamp](https://cloud.google.com/pubsub/docs/replay-overview#seek_to_a_time) that is up to `message_retention_duration` in the past. If this field is not set, message retention is controlled by settings on individual subscriptions. Cannot be more than 31 days or less than 10 minutes. */
  messageRetentionDuration?: string | Computed<string>;
  /** A policy constraining the storage of messages published to the topic. */
  messageStoragePolicy?: PubsubTopic_MessageStoragePolicy | Computed<PubsubTopic_MessageStoragePolicy>;
  /** Optional. Transforms to be applied to messages published to the topic. Transforms are applied in the order specified. */
  messageTransforms?: PubsubTopic_MessageTransforms[] | Computed<PubsubTopic_MessageTransforms[]>;
  /** Required. Identifier. The name of the topic. It must have the format `"projects/{project}/topics/{topic}"`. `{topic}` must start with a letter, and contain only letters (`[A-Za-z]`), numbers (`[0-9]`), dashes (`-`), underscores (`_`), periods (`.`), tildes (`~`), plus (`+`) or percent signs (`%`). It must be between 3 and 255 characters in length, and it must not start with `"goog"`. */
  name?: string | Computed<string>;
  /** Optional. Reserved for future use. This field is set only in responses from the server; it is ignored if it is set in any requests. */
  satisfiesPzs?: boolean | Computed<boolean>;
  /** Settings for validating messages published against a schema. */
  schemaSettings?: PubsubTopic_SchemaSettings | Computed<PubsubTopic_SchemaSettings>;
  /** Output only. An output-only field indicating the state of the topic. */
  state?: string | Computed<string>;
  /** Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing" See https://{$universe.dns_names.final_documentation_domain}/pubsub/docs/tags for more information on using tags with Pub/Sub resources. */
  tags?: Record<string, string> | Computed<Record<string, string>>;
}

export interface PubsubTopicAttrs {
  /** Settings for an ingestion data source on a topic. */
  ingestionDataSourceSettings: PubsubTopic_IngestionDataSourceSettings;
  /** Optional. The resource name of the Cloud KMS CryptoKey to be used to protect access to messages published on this topic. The expected format is `projects/* /locations/* /keyRings/* /cryptoKeys/*`. */
  kmsKeyName: string;
  /** Optional. See [Creating and managing labels] (https://cloud.google.com/pubsub/docs/labels). */
  labels: Record<string, string>;
  /** Optional. Indicates the minimum duration to retain a message after it is published to the topic. If this field is set, messages published to the topic in the last `message_retention_duration` are always available to subscribers. For instance, it allows any attached subscription to [seek to a timestamp](https://cloud.google.com/pubsub/docs/replay-overview#seek_to_a_time) that is up to `message_retention_duration` in the past. If this field is not set, message retention is controlled by settings on individual subscriptions. Cannot be more than 31 days or less than 10 minutes. */
  messageRetentionDuration: string;
  /** A policy constraining the storage of messages published to the topic. */
  messageStoragePolicy: PubsubTopic_MessageStoragePolicy;
  /** Optional. Transforms to be applied to messages published to the topic. Transforms are applied in the order specified. */
  messageTransforms: PubsubTopic_MessageTransforms[];
  /** Required. Identifier. The name of the topic. It must have the format `"projects/{project}/topics/{topic}"`. `{topic}` must start with a letter, and contain only letters (`[A-Za-z]`), numbers (`[0-9]`), dashes (`-`), underscores (`_`), periods (`.`), tildes (`~`), plus (`+`) or percent signs (`%`). It must be between 3 and 255 characters in length, and it must not start with `"goog"`. */
  name: string;
  /** Optional. Reserved for future use. This field is set only in responses from the server; it is ignored if it is set in any requests. */
  satisfiesPzs: boolean;
  /** Settings for validating messages published against a schema. */
  schemaSettings: PubsubTopic_SchemaSettings;
  /** Output only. An output-only field indicating the state of the topic. */
  state: string;
  /** Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing" See https://{$universe.dns_names.final_documentation_domain}/pubsub/docs/tags for more information on using tags with Pub/Sub resources. */
  tags: Record<string, string>;
}

export const PubsubTopic: ResourceBinding<PubsubTopicConfig, PubsubTopicAttrs> = {
  wireType: "google_pubsub_pubsub_topic",
  fields: {
    ingestionDataSourceSettings: {
      wireName: "ingestion_data_source_settings",
      kind: "object",
      fields: PubsubTopic_IngestionDataSourceSettingsFields,
    },
    kmsKeyName: "kms_key_name",
    labels: "labels",
    messageRetentionDuration: "message_retention_duration",
    messageStoragePolicy: {
      wireName: "message_storage_policy",
      kind: "object",
      fields: PubsubTopic_MessageStoragePolicyFields,
    },
    messageTransforms: {
      wireName: "message_transforms",
      kind: "list",
      fields: PubsubTopic_MessageTransformsFields,
    },
    name: "name",
    satisfiesPzs: "satisfies_pzs",
    schemaSettings: {
      wireName: "schema_settings",
      kind: "object",
      fields: PubsubTopic_SchemaSettingsFields,
    },
    state: "state",
    tags: "tags",
  },
};
