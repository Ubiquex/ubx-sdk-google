// Code generated by `ubx sdk gen` from google_compute_beta_host. DO NOT EDIT.
import type { Computed, FieldMap, DataSourceBinding } from "@ubx/sdk";

export interface BetaHost_Status_PhysicalTopology {
  /** The unique identifier of the capacity block within the cluster. */
  block?: string | Computed<string>;
  /** The cluster name of the reservation sub-block. */
  cluster?: string | Computed<string>;
  /** The unique identifier of the capacity host within the capacity sub-block. */
  host?: string | Computed<string>;
  /** The unique identifier of the capacity sub-block within the capacity block. */
  subBlock?: string | Computed<string>;
}

export interface BetaHost_Status {
  physicalTopology?: BetaHost_Status_PhysicalTopology | Computed<BetaHost_Status_PhysicalTopology>;
  /** Output only. The URIs of the instances currently running on this host. */
  runningInstances?: string[] | Computed<string[]>;
}

export interface BetaHostConfig {
  association: string | Computed<string>;
  host: string | Computed<string>;
  project: string | Computed<string>;
  zone: string | Computed<string>;
}

export interface BetaHostAttrs {
  /** Output only. All aliases for this resource. e.g. projects/123/zones/us-centra1-a/reservation/r1/reservationBlock/b1/hosts/h1 */
  aliasLinks: string[];
  association: string;
  /** Output only. The creation timestamp, formatted asRFC3339 text. */
  creationTimestamp: string;
  /** An optional description of this resource. */
  description: string;
  host: string;
  /** Output only. The unique identifier for this resource. This identifier is defined by the server. */
  id: string;
  /** Output only. The type of resource. Alwayscompute#host for hosts. */
  kind: string;
  /** Output only. The name of the host. */
  name: string;
  project: string;
  /** Output only. The self link of the host. */
  selfLink: string;
  /** Output only. The self link with id of the host. */
  selfLinkWithId: string;
  /** Output only. The state of the host. */
  state: string;
  status: BetaHost_Status;
  zone: string;
}

export const BetaHost: DataSourceBinding<BetaHostConfig, BetaHostAttrs> = {
  wireType: "google_compute_beta_host",
  fields: {
    association: "association",
    host: "host",
    project: "project",
    zone: "zone",
  },
};
