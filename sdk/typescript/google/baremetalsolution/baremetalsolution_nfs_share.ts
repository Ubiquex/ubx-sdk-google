// Code generated by `ubx sdk gen` from google_baremetalsolution_baremetalsolution_nfs_share. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface BaremetalsolutionNfsShare_AllowedClients {
  allowDev?: boolean | Computed<boolean>;
  allowSuid?: boolean | Computed<boolean>;
  allowedClientsCidr?: string | Computed<string>;
  mountPermissions?: string | Computed<string>;
  network?: string | Computed<string>;
  nfsPath?: string | Computed<string>;
  noRootSquash?: boolean | Computed<boolean>;
  shareIp?: string | Computed<string>;
}

const BaremetalsolutionNfsShare_AllowedClientsFields: FieldMap = {
  allowDev: "allow_dev",
  allowSuid: "allow_suid",
  allowedClientsCidr: "allowed_clients_cidr",
  mountPermissions: "mount_permissions",
  network: "network",
  nfsPath: "nfs_path",
  noRootSquash: "no_root_squash",
  shareIp: "share_ip",
};

export interface BaremetalsolutionNfsShareConfig {
  /** List of allowed access points. */
  allowedClients?: BaremetalsolutionNfsShare_AllowedClients[] | Computed<BaremetalsolutionNfsShare_AllowedClients[]>;
  /** Output only. An identifier for the NFS share, generated by the backend. This is the same value as nfs_share_id and will replace it in the future. */
  id?: string | Computed<string>;
  /** Labels as key value pairs. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Immutable. The name of the NFS share. */
  name?: string | Computed<string>;
  /** Output only. An identifier for the NFS share, generated by the backend. This field will be deprecated in the future, use `id` instead. */
  nfsShareId?: string | Computed<string>;
  /** Immutable. Pod name. Pod is an independent part of infrastructure. NFSShare can only be connected to the assets (networks, instances) allocated in the same pod. */
  pod?: string | Computed<string>;
  /** The requested size, in GiB. */
  requestedSizeGib?: string | Computed<string>;
  /** Output only. The state of the NFS share. */
  state?: string | Computed<string>;
  /** Immutable. The storage type of the underlying volume. */
  storageType?: string | Computed<string>;
  /** Output only. The underlying volume of the share. Created automatically during provisioning. */
  volume?: string | Computed<string>;
}

export interface BaremetalsolutionNfsShareAttrs {
  /** List of allowed access points. */
  allowedClients: BaremetalsolutionNfsShare_AllowedClients[];
  /** Output only. An identifier for the NFS share, generated by the backend. This is the same value as nfs_share_id and will replace it in the future. */
  id: string;
  /** Labels as key value pairs. */
  labels: Record<string, string>;
  /** Immutable. The name of the NFS share. */
  name: string;
  /** Output only. An identifier for the NFS share, generated by the backend. This field will be deprecated in the future, use `id` instead. */
  nfsShareId: string;
  /** Immutable. Pod name. Pod is an independent part of infrastructure. NFSShare can only be connected to the assets (networks, instances) allocated in the same pod. */
  pod: string;
  /** The requested size, in GiB. */
  requestedSizeGib: string;
  /** Output only. The state of the NFS share. */
  state: string;
  /** Immutable. The storage type of the underlying volume. */
  storageType: string;
  /** Output only. The underlying volume of the share. Created automatically during provisioning. */
  volume: string;
}

export const BaremetalsolutionNfsShare: ResourceBinding<BaremetalsolutionNfsShareConfig, BaremetalsolutionNfsShareAttrs> = {
  wireType: "google_baremetalsolution_baremetalsolution_nfs_share",
  fields: {
    allowedClients: {
      wireName: "allowed_clients",
      kind: "list",
      fields: BaremetalsolutionNfsShare_AllowedClientsFields,
    },
    id: "id",
    labels: "labels",
    name: "name",
    nfsShareId: "nfs_share_id",
    pod: "pod",
    requestedSizeGib: "requested_size_gib",
    state: "state",
    storageType: "storage_type",
    volume: "volume",
  },
};
