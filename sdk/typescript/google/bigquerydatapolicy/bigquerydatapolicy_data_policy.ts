// Code generated by `ubx sdk gen` from google_bigquerydatapolicy_bigquerydatapolicy_data_policy. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface BigquerydatapolicyDataPolicy_DataGovernanceTag {
  /** Optional. Tag keys are globally unique. Tag key is expected to be in the namespaced format, for example `parent-id/pii` where `parent-id` is the ID of the parent organization or project resource for this tag key. */
  key?: string | Computed<string>;
  /** Optional. Specifies the tag value as the short name, for example `sensitive`. */
  value?: string | Computed<string>;
}

export interface BigquerydatapolicyDataPolicy_DataMaskingPolicy {
  /** Optional. A predefined masking expression. */
  predefinedExpression?: string | Computed<string>;
  /** Optional. The name of the BigQuery routine that contains the custom masking routine, in the format of `projects/{project_number}/datasets/{dataset_id}/routines/{routine_id}`. */
  routine?: string | Computed<string>;
}

export interface BigquerydatapolicyDataPolicy_DataPolicy {
  /** This is a namespaced name specifying the key and the value. For example: `project-id/pii/sensitive`. */
  dataGovernanceTag?: BigquerydatapolicyDataPolicy_DataGovernanceTag | Computed<BigquerydatapolicyDataPolicy_DataGovernanceTag>;
  /** The policy used to specify data masking rule. */
  dataMaskingPolicy?: BigquerydatapolicyDataPolicy_DataMaskingPolicy | Computed<BigquerydatapolicyDataPolicy_DataMaskingPolicy>;
  /** Output only. User-assigned (human readable) ID of the data policy that needs to be unique within a project. Used as {data_policy_id} in part of the resource name. */
  dataPolicyId?: string | Computed<string>;
  /** Required. Type of data policy. */
  dataPolicyType?: string | Computed<string>;
  /** The etag for this Data Policy. This field is used for UpdateDataPolicy calls. If Data Policy exists, this field is required and must match the server's etag. It will also be populated in the response of GetDataPolicy, CreateDataPolicy, and UpdateDataPolicy calls. */
  etag?: string | Computed<string>;
  /** Optional. The list of IAM principals that have Fine Grained Access to the underlying data goverened by this data policy. Uses the [IAM V2 principal syntax](https://cloud.google.com/iam/docs/principal-identifiers#v2) Only supports principal types users, groups, serviceaccounts, cloudidentity. This field is supported in V2 Data Policy only. In case of V1 data policies (i.e. verion = 1 and policy_tag is set), this field is not populated. */
  grantees?: string[] | Computed<string[]>;
  /** Identifier. Resource name of this data policy, in the format of `projects/{project_number}/locations/{location_id}/dataPolicies/{data_policy_id}`. */
  name?: string | Computed<string>;
  /** Output only. Policy tag resource name, in the format of `projects/{project_number}/locations/{location_id}/taxonomies/{taxonomy_id}/policyTags/{policyTag_id}`. policy_tag is supported only for V1 data policies. */
  policyTag?: string | Computed<string>;
  /** Output only. The version of the Data Policy resource. */
  version?: string | Computed<string>;
}

const BigquerydatapolicyDataPolicy_DataGovernanceTagFields: FieldMap = {
  key: "key",
  value: "value",
};

const BigquerydatapolicyDataPolicy_DataMaskingPolicyFields: FieldMap = {
  predefinedExpression: "predefined_expression",
  routine: "routine",
};

const BigquerydatapolicyDataPolicy_DataPolicyFields: FieldMap = {
  dataGovernanceTag: {
    wireName: "data_governance_tag",
    kind: "object",
    fields: BigquerydatapolicyDataPolicy_DataGovernanceTagFields,
  },
  dataMaskingPolicy: {
    wireName: "data_masking_policy",
    kind: "object",
    fields: BigquerydatapolicyDataPolicy_DataMaskingPolicyFields,
  },
  dataPolicyId: "data_policy_id",
  dataPolicyType: "data_policy_type",
  etag: "etag",
  grantees: "grantees",
  name: "name",
  policyTag: "policy_tag",
  version: "version",
};

export interface BigquerydatapolicyDataPolicyConfig {
  /** Represents the label-policy binding. */
  dataPolicy?: BigquerydatapolicyDataPolicy_DataPolicy | Computed<BigquerydatapolicyDataPolicy_DataPolicy>;
  /** Required. User-assigned (human readable) ID of the data policy that needs to be unique within a project. Used as {data_policy_id} in part of the resource name. */
  dataPolicyId?: string | Computed<string>;
}

export interface BigquerydatapolicyDataPolicyAttrs {
  /** This is a namespaced name specifying the key and the value. For example: `project-id/pii/sensitive`. */
  dataGovernanceTag: BigquerydatapolicyDataPolicy_DataGovernanceTag;
  /** The policy used to specify data masking rule. */
  dataMaskingPolicy: BigquerydatapolicyDataPolicy_DataMaskingPolicy;
  /** Represents the label-policy binding. */
  dataPolicy: BigquerydatapolicyDataPolicy_DataPolicy;
  /** Required. User-assigned (human readable) ID of the data policy that needs to be unique within a project. Used as {data_policy_id} in part of the resource name. */
  dataPolicyId: string;
  /** Required. Type of data policy. */
  dataPolicyType: string;
  /** The etag for this Data Policy. This field is used for UpdateDataPolicy calls. If Data Policy exists, this field is required and must match the server's etag. It will also be populated in the response of GetDataPolicy, CreateDataPolicy, and UpdateDataPolicy calls. */
  etag: string;
  /** Optional. The list of IAM principals that have Fine Grained Access to the underlying data goverened by this data policy. Uses the [IAM V2 principal syntax](https://cloud.google.com/iam/docs/principal-identifiers#v2) Only supports principal types users, groups, serviceaccounts, cloudidentity. This field is supported in V2 Data Policy only. In case of V1 data policies (i.e. verion = 1 and policy_tag is set), this field is not populated. */
  grantees: string[];
  /** Identifier. Resource name of this data policy, in the format of `projects/{project_number}/locations/{location_id}/dataPolicies/{data_policy_id}`. */
  name: string;
  /** Output only. Policy tag resource name, in the format of `projects/{project_number}/locations/{location_id}/taxonomies/{taxonomy_id}/policyTags/{policyTag_id}`. policy_tag is supported only for V1 data policies. */
  policyTag: string;
  /** Output only. The version of the Data Policy resource. */
  version: string;
}

export const BigquerydatapolicyDataPolicy: ResourceBinding<BigquerydatapolicyDataPolicyConfig, BigquerydatapolicyDataPolicyAttrs> = {
  wireType: "google_bigquerydatapolicy_bigquerydatapolicy_data_policy",
  fields: {
    dataPolicy: {
      wireName: "data_policy",
      kind: "object",
      fields: BigquerydatapolicyDataPolicy_DataPolicyFields,
    },
    dataPolicyId: "data_policy_id",
  },
};
