...
export interface DeploymentGroup_ProvisioningError {
...
const DeploymentGroup_DeploymentUnitsFields: FieldMap = {
  dependencies: "dependencies",
  deployment: "deployment",
  id: "id",
};

const DeploymentGroup_ProvisioningErrorFields: FieldMap = {
  code: "code",
  details: "details",
  message: "message",
};
...
export interface DeploymentGroupConfig {
  ...
  deploymentUnits?: DeploymentGroup_DeploymentUnits[] | Computed<DeploymentGroup_DeploymentUnits[]>;
  ...
  provisioningError?: DeploymentGroup_ProvisioningError | Computed<DeploymentGroup_ProvisioningError>;
...
export interface DeploymentGroupAttrs {
  ...
  createTime: string;
  /** The deployment units of the deployment group in a DAG like structure. When a deployment group is being provisioned, the deployment units are deployed in a DAG order. The provided units must be in a DAG order, otherwise an error will be returned. */
  deploymentUnits: DeploymentGroup_DeploymentUnits[];
  /** Optional. User-defined metadata for the deployment group. */
  labels: Record<string, string>;
  /** Identifier. The name of the deployment group. Format: 'projects/{project_id}/locations/{location}/deploymentGroups/{deployment_group}'. */
  name: string;
  /** The `Status` type defines a logical error model that is suitable for different programming environments, including REST APIs and RPC APIs. It is used by [gRPC](https://github.com/grpc). Each `Status` message contains three pieces of data: error code, error message, and error details. You can find out more about this error model and how to work with it in the [API Design Guide](https://cloud.google.com/apis/design/errors). */
  provisioningError: DeploymentGroup_ProvisioningError;
  /** Output only. The provisioning state of the deployment group. */
