// Code generated by `ubx sdk gen` from google_netapp_v1beta1_netapp_v1beta1_backup_policy. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface V1beta1NetappV1beta1BackupPolicyConfig {
  /** Output only. The total number of volumes assigned by this backup policy. */
  assignedVolumeCount?: number | Computed<number>;
  /** Output only. The time when the backup policy was created. */
  createTime?: string | Computed<string>;
  /** Number of daily backups to keep. Note that the minimum daily backup limit is 2. */
  dailyBackupLimit?: number | Computed<number>;
  /** Description of the backup policy. */
  description?: string | Computed<string>;
  /** If enabled, make backups automatically according to the schedules. This will be applied to all volumes that have this policy attached and enforced on volume level. If not specified, default is true. */
  enabled?: boolean | Computed<boolean>;
  /** Resource labels to represent user provided metadata. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Number of monthly backups to keep. Note that the sum of daily, weekly and monthly backups should be greater than 1. */
  monthlyBackupLimit?: number | Computed<number>;
  /** Identifier. The resource name of the backup policy. Format: `projects/{project_id}/locations/{location}/backupPolicies/{backup_policy_id}`. */
  name?: string | Computed<string>;
  /** Output only. The backup policy state. */
  state?: string | Computed<string>;
  /** Number of weekly backups to keep. Note that the sum of daily, weekly and monthly backups should be greater than 1. */
  weeklyBackupLimit?: number | Computed<number>;
}

export interface V1beta1NetappV1beta1BackupPolicyAttrs {
  /** Output only. The total number of volumes assigned by this backup policy. */
  assignedVolumeCount: number;
  /** Output only. The time when the backup policy was created. */
  createTime: string;
  /** Number of daily backups to keep. Note that the minimum daily backup limit is 2. */
  dailyBackupLimit: number;
  /** Description of the backup policy. */
  description: string;
  /** If enabled, make backups automatically according to the schedules. This will be applied to all volumes that have this policy attached and enforced on volume level. If not specified, default is true. */
  enabled: boolean;
  /** Resource labels to represent user provided metadata. */
  labels: Record<string, string>;
  /** Number of monthly backups to keep. Note that the sum of daily, weekly and monthly backups should be greater than 1. */
  monthlyBackupLimit: number;
  /** Identifier. The resource name of the backup policy. Format: `projects/{project_id}/locations/{location}/backupPolicies/{backup_policy_id}`. */
  name: string;
  /** Output only. The backup policy state. */
  state: string;
  /** Number of weekly backups to keep. Note that the sum of daily, weekly and monthly backups should be greater than 1. */
  weeklyBackupLimit: number;
}

export const V1beta1NetappV1beta1BackupPolicy: ResourceBinding<V1beta1NetappV1beta1BackupPolicyConfig, V1beta1NetappV1beta1BackupPolicyAttrs> = {
  wireType: "google_netapp_v1beta1_netapp_v1beta1_backup_policy",
  fields: {
    assignedVolumeCount: "assigned_volume_count",
    createTime: "create_time",
    dailyBackupLimit: "daily_backup_limit",
    description: "description",
    enabled: "enabled",
    labels: "labels",
    monthlyBackupLimit: "monthly_backup_limit",
    name: "name",
    state: "state",
    weeklyBackupLimit: "weekly_backup_limit",
  },
};
