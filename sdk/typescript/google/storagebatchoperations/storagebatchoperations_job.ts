// Code generated by `ubx sdk gen` from google_storagebatchoperations_storagebatchoperations_job. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface StoragebatchoperationsJob_BucketList_Buckets_Manifest {
  manifestLocation?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_BucketList_Buckets_PrefixList {
  includedObjectPrefixes?: string[] | Computed<string[]>;
}

export interface StoragebatchoperationsJob_BucketList_Buckets {
  bucket?: string | Computed<string>;
  manifest?: StoragebatchoperationsJob_BucketList_Buckets_Manifest | Computed<StoragebatchoperationsJob_BucketList_Buckets_Manifest>;
  prefixList?: StoragebatchoperationsJob_BucketList_Buckets_PrefixList | Computed<StoragebatchoperationsJob_BucketList_Buckets_PrefixList>;
}

export interface StoragebatchoperationsJob_BucketList {
  /** Required. List of buckets and their objects to be transformed. You can specify only one bucket per job. If multiple buckets are specified, an error occurs. */
  buckets?: StoragebatchoperationsJob_BucketList_Buckets[] | Computed<StoragebatchoperationsJob_BucketList_Buckets[]>;
}

export interface StoragebatchoperationsJob_Counters {
  /** Output only. The number of objects that failed due to user errors or service errors. */
  failedObjectCount?: string | Computed<string>;
  /** Output only. Number of object custom contexts created. This field is only populated for jobs with the UpdateObjectCustomContext transformation. */
  objectCustomContextsCreated?: string | Computed<string>;
  /** Output only. Number of object custom contexts deleted. This field is only populated for jobs with the UpdateObjectCustomContext transformation. */
  objectCustomContextsDeleted?: string | Computed<string>;
  /** Output only. Number of object custom contexts updated. This counter tracks custom contexts where the key already existed, but the payload was modified. This field is only populated for jobs with the UpdateObjectCustomContext transformation. */
  objectCustomContextsUpdated?: string | Computed<string>;
  /** Output only. Number of objects completed. */
  succeededObjectCount?: string | Computed<string>;
  /** Output only. Number of bytes found from source. This field is only populated for jobs with a prefix list object configuration. */
  totalBytesFound?: string | Computed<string>;
  /** Output only. The total number of bytes affected by the transformation. For example, this counts bytes deleted for `DeleteObject` operations and bytes rewritten for `RewriteObject` operations. */
  totalBytesTransformed?: string | Computed<string>;
  /** Output only. Number of objects listed. */
  totalObjectCount?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_DeleteObject {
  /** Required. Controls deletion behavior when versioning is enabled for the object's bucket. If true, both live and noncurrent objects will be permanently deleted. Otherwise live objects in versioned buckets will become noncurrent and objects that were already noncurrent will be skipped. This setting doesn't have any impact on the Soft Delete feature. All objects deleted by this service can be be restored for the duration of the Soft Delete retention duration if enabled. If enabled and the manifest doesn't specify an object's generation, a `GetObjectMetadata` call is made to determine the live object generation. */
  permanentObjectDeletionEnabled?: boolean | Computed<boolean>;
}

export interface StoragebatchoperationsJob_ErrorSummaries_ErrorLogEntries {
  errorDetails?: string[] | Computed<string[]>;
  objectUri?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_ErrorSummaries {
  errorCode?: string | Computed<string>;
  errorCount?: string | Computed<string>;
  errorLogEntries?: StoragebatchoperationsJob_ErrorSummaries_ErrorLogEntries[] | Computed<StoragebatchoperationsJob_ErrorSummaries_ErrorLogEntries[]>;
}

export interface StoragebatchoperationsJob_LoggingConfig {
  /** Required. States in which Action are logged.If empty, no logs are generated. */
  logActionStates?: string[] | Computed<string[]>;
  /** Required. Specifies the actions to be logged. */
  logActions?: string[] | Computed<string[]>;
}

export interface StoragebatchoperationsJob_ProjectSource_BucketFilters {
  /** Optional. Description of the expression. This is a longer text which describes the expression, e.g. when hovered over it in a UI. */
  description?: string | Computed<string>;
  /** Textual representation of an expression in Common Expression Language syntax. */
  expression?: string | Computed<string>;
  /** Optional. String indicating the location of the expression for error reporting, e.g. a file name and a position in the file. */
  location?: string | Computed<string>;
  /** Optional. Title for the expression, i.e. a short string describing its purpose. This can be used e.g. in UIs which allow to enter the expression. */
  title?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_ProjectSource_TargetLocations {
  /** Required. REQUIRED. A list of Cloud Storage locations (e.g., `us-central1`) to include in the job. If `snapshot_time` is omitted, the job automatically defaults to the most recent snapshot timestamp that is successfully populated in BOTH the `object_attributes_view` and `bucket_attributes_view` across ALL specified locations. For details on Storage Insights dataset snapshots and views, see: https://docs.cloud.google.com/storage/docs/insights/dataset-tables-and-schemas#schema */
  locations?: string[] | Computed<string[]>;
  /** Optional. OPTIONAL. The exact Storage Insights snapshot timestamp to use for the job compatible with the RFC 3339 format (e.g., `2024-01-02T03:04:05Z`). If specified, this exact snapshot must exist in BOTH the `object_attributes_view` and `bucket_attributes_view` for every location listed in `locations`. If the snapshot is missing from either view in any of the locations, the job fails. */
  snapshotTime?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_ProjectSource {
  /** Represents a textual expression in the Common Expression Language (CEL) syntax. CEL is a C-like expression language. The syntax and semantics of CEL are documented at https://github.com/google/cel-spec. Example (Comparison): title: "Summary size limit" description: "Determines if a summary is less than 100 chars" expression: "document.summary.size() < 100" Example (Equality): title: "Requestor is owner" description: "Determines if requestor is the document owner" expression: "document.owner == request.auth.claims.email" Example (Logic): title: "Public documents" description: "Determine whether the document should be publicly visible" expression: "document.type != 'private' && document.type != 'internal'" Example (Data Manipulation): title: "Notification string" description: "Create a notification string with a timestamp." expression: "'New message received at ' + string(document.create_time)" The exact variables and functions that may be referenced within an expression are determined by the service that evaluates it. See the service documentation for additional information. */
  bucketFilters?: StoragebatchoperationsJob_ProjectSource_BucketFilters | Computed<StoragebatchoperationsJob_ProjectSource_BucketFilters>;
  /** Optional. The unique identifier of a dry run job to use as the baseline for the current job. Specifying this ID ensures the job is executed against the same set of objects validated during the dry run. The value corresponds to the {job_id} segment of the resource name: `projects/{project_id}/locations/{location}/jobs/{job_id}`. */
  dryRunJobId?: string | Computed<string>;
  /** Required. The resource identifier of the Storage Insights dataset configuration. Storage batch operations uses the latest snapshot from this dataset as the source to list and filter target objects. Format: `projects/{project_id}/locations/{location}/datasetConfigs/{dataset_config}`. */
  insightsDatasetConfig?: string | Computed<string>;
  /** Represents a textual expression in the Common Expression Language (CEL) syntax. CEL is a C-like expression language. The syntax and semantics of CEL are documented at https://github.com/google/cel-spec. Example (Comparison): title: "Summary size limit" description: "Determines if a summary is less than 100 chars" expression: "document.summary.size() < 100" Example (Equality): title: "Requestor is owner" description: "Determines if requestor is the document owner" expression: "document.owner == request.auth.claims.email" Example (Logic): title: "Public documents" description: "Determine whether the document should be publicly visible" expression: "document.type != 'private' && document.type != 'internal'" Example (Data Manipulation): title: "Notification string" description: "Create a notification string with a timestamp." expression: "'New message received at ' + string(document.create_time)" The exact variables and functions that may be referenced within an expression are determined by the service that evaluates it. See the service documentation for additional information. */
  objectFilters?: StoragebatchoperationsJob_ProjectSource_BucketFilters | Computed<StoragebatchoperationsJob_ProjectSource_BucketFilters>;
  /** Required. Project name of the objects to be transformed. e.g. projects/my-project or projects/123456. */
  project?: string | Computed<string>;
  /** Output only. The snapshot time used by the job to read the Storage Insights dataset for bucket and object discovery. This field is populated by the service and reflects the exact timestamp of the dataset snapshot used. */
  snapshotTime?: string | Computed<string>;
  /** Describes the Cloud Storage locations to include in a ProjectSource job. */
  targetLocations?: StoragebatchoperationsJob_ProjectSource_TargetLocations | Computed<StoragebatchoperationsJob_ProjectSource_TargetLocations>;
}

export interface StoragebatchoperationsJob_PutMetadata_ObjectRetention {
  /** Required. The object's retention expiration time, during which, the object is protected from being deleted or overwritten. The time must be specified in RFC 3339 format, for example `YYYY-MM-DD'T'HH:MM:SS'Z'` or `YYYY-MM-DD'T'HH:MM:SS.SS'Z'`. To clear an object's retention, both `retentionMode` and `retainUntilTime` must be left unset (omitted). Setting `retentionMode` to `RETENTION_MODE_UNSPECIFIED` is treated as a no-op. Unlike an unset field, it doesn't modify or clear the retention settings. */
  retainUntilTime?: string | Computed<string>;
  /** Required. The retention mode. */
  retentionMode?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_PutMetadata {
  /** Optional. Updates the objects `Cache-Control` fixed metadata. Unset values in the request are ignored. To clear the metadata, set an empty value. Additionally, the value for `Custom-Time` can't decrease. For details, see [Cache-Control](https://cloud.google.com/storage/docs/metadata#caching_data). */
  cacheControl?: string | Computed<string>;
  /** Optional. Updates objects `Content-Disposition` fixed metadata. Unset values in the request are ignored. To clear the metadata, set an empty value. For details, see [Content-Disposition](https://cloud.google.com/storage/docs/metadata#content-disposition). */
  contentDisposition?: string | Computed<string>;
  /** Optional. Updates the objects `Content-Encoding` fixed metadata. Unset values in the request are ignored. To clear the metadata, set an empty value. For details, see [Content-Encoding](https://cloud.google.com/storage/docs/metadata#content-encoding). */
  contentEncoding?: string | Computed<string>;
  /** Optional. Updates the objects `Content-Language` fixed metadata. Metadata values must use ISO 639-1 language codes. The maximum length for metadata values is 100 characters. Unset values in the request are ignored. To clear the metadata, set an empty value. For details, see [Content-Language](https://cloud.google.com/storage/docs/metadata#content-language). */
  contentLanguage?: string | Computed<string>;
  /** Optional. Updates objects `Content-Type` fixed metadata. Unset values in the request are ignored. To clear the metadata, set an empty value. For details, see [Content-Type](https://cloud.google.com/storage/docs/metadata#content-type). */
  contentType?: string | Computed<string>;
  /** Optional. Updates the object's custom metadata. This operation adds or sets individual custom metadata key-value pairs. Keys specified with empty values have their values cleared. Existing custom metadata keys not included in the request remain unchanged. For details, see [Custom metadata](https://cloud.google.com/storage/docs/metadata#custom-metadata). */
  customMetadata?: Record<string, string> | Computed<Record<string, string>>;
  /** Optional. Updates the objects `Custom-Time` fixed metadata. Unset values in the request are ignored. To clear the metadata, set an empty value. The time must be specified in RFC 3339 format, for example `YYYY-MM-DD'T'HH:MM:SS'Z'` or `YYYY-MM-DD'T'HH:MM:SS.SS'Z'`. For details, see [Custom-Time](https://cloud.google.com/storage/docs/metadata#custom-time). */
  customTime?: string | Computed<string>;
  /** Describes options for object retention update. */
  objectRetention?: StoragebatchoperationsJob_PutMetadata_ObjectRetention | Computed<StoragebatchoperationsJob_PutMetadata_ObjectRetention>;
}

export interface StoragebatchoperationsJob_PutObjectHold {
  /** Required. Updates object event based holds state. When object event based hold is set, object can't be deleted or replaced. Resets object's time in the bucket for the purposes of the retention period. */
  eventBasedHold?: string | Computed<string>;
  /** Required. Updates object temporary holds state. When object temporary hold is set, object can't be deleted or replaced. */
  temporaryHold?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_RewriteObject {
  /** Optional. Resource name of the Cloud KMS key that is used to encrypt the object. The Cloud KMS key must be located in same location as the object. For details, see https://cloud.google.com/storage/docs/encryption/using-customer-managed-keys#add-object-key Format: `projects/{project_id}/locations/{location}/keyRings/{keyring}/cryptoKeys/{key}` For example: `projects/123456/locations/us-central1/keyRings/my-keyring/cryptoKeys/my-key`. The object will be rewritten and set with the specified KMS key. */
  kmsKey?: string | Computed<string>;
  /** Optional. Rewrites the object to the specified storage class. Setting this field will perform a full byte copy of the object if the storage class is different from the object's current storage class. If Autoclass is enabled on the bucket, storage class changes are ignored by Cloud Storage. */
  storageClass?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates_Grants {
  entity?: string | Computed<string>;
  role?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates {
  /** Optional. Grants to add or update. If a grant for same entity exists, its role is updated. */
  grants?: StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates_Grants[] | Computed<StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates_Grants[]>;
  /** Optional. Entities for which all grants should be removed. An entity can't be in both `grants` and `remove_entities`. */
  removeEntities?: string[] | Computed<string[]>;
}

export interface StoragebatchoperationsJob_SetObjectAcls {
  /** Represents updates to existing access-control entries on an object. */
  accessControlsUpdates?: StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates | Computed<StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates>;
}

export interface StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates_Updates {
  value?: string | Computed<string>;
}

export interface StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates {
  /** Optional. Custom contexts to clear by key. A key can't be present in both `updates` and `keys_to_clear`. */
  keysToClear?: string[] | Computed<string[]>;
  /** Optional. Insert or update the existing custom contexts. */
  updates?: Record<string, StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates_Updates> | Computed<Record<string, StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates_Updates>>;
}

export interface StoragebatchoperationsJob_UpdateObjectCustomContext {
  /** If set, must be set to true and all existing object custom contexts are deleted. */
  clearAll?: boolean | Computed<boolean>;
  /** Describes a collection of updates to apply to custom contexts identified by key. */
  customContextUpdates?: StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates | Computed<StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates>;
}

const StoragebatchoperationsJob_BucketList_Buckets_ManifestFields: FieldMap = {
  manifestLocation: "manifest_location",
};

const StoragebatchoperationsJob_BucketList_Buckets_PrefixListFields: FieldMap = {
  includedObjectPrefixes: "included_object_prefixes",
};

const StoragebatchoperationsJob_BucketList_BucketsFields: FieldMap = {
  bucket: "bucket",
  manifest: {
    wireName: "manifest",
    kind: "object",
    fields: StoragebatchoperationsJob_BucketList_Buckets_ManifestFields,
  },
  prefixList: {
    wireName: "prefix_list",
    kind: "object",
    fields: StoragebatchoperationsJob_BucketList_Buckets_PrefixListFields,
  },
};

const StoragebatchoperationsJob_BucketListFields: FieldMap = {
  buckets: {
    wireName: "buckets",
    kind: "list",
    fields: StoragebatchoperationsJob_BucketList_BucketsFields,
  },
};

const StoragebatchoperationsJob_CountersFields: FieldMap = {
  failedObjectCount: "failed_object_count",
  objectCustomContextsCreated: "object_custom_contexts_created",
  objectCustomContextsDeleted: "object_custom_contexts_deleted",
  objectCustomContextsUpdated: "object_custom_contexts_updated",
  succeededObjectCount: "succeeded_object_count",
  totalBytesFound: "total_bytes_found",
  totalBytesTransformed: "total_bytes_transformed",
  totalObjectCount: "total_object_count",
};

const StoragebatchoperationsJob_DeleteObjectFields: FieldMap = {
  permanentObjectDeletionEnabled: "permanent_object_deletion_enabled",
};

const StoragebatchoperationsJob_ErrorSummaries_ErrorLogEntriesFields: FieldMap = {
  errorDetails: "error_details",
  objectUri: "object_uri",
};

const StoragebatchoperationsJob_ErrorSummariesFields: FieldMap = {
  errorCode: "error_code",
  errorCount: "error_count",
  errorLogEntries: {
    wireName: "error_log_entries",
    kind: "list",
    fields: StoragebatchoperationsJob_ErrorSummaries_ErrorLogEntriesFields,
  },
};

const StoragebatchoperationsJob_LoggingConfigFields: FieldMap = {
  logActionStates: "log_action_states",
  logActions: "log_actions",
};

const StoragebatchoperationsJob_ProjectSource_BucketFiltersFields: FieldMap = {
  description: "description",
  expression: "expression",
  location: "location",
  title: "title",
};

const StoragebatchoperationsJob_ProjectSource_TargetLocationsFields: FieldMap = {
  locations: "locations",
  snapshotTime: "snapshot_time",
};

const StoragebatchoperationsJob_ProjectSourceFields: FieldMap = {
  bucketFilters: {
    wireName: "bucket_filters",
    kind: "object",
    fields: StoragebatchoperationsJob_ProjectSource_BucketFiltersFields,
  },
  dryRunJobId: "dry_run_job_id",
  insightsDatasetConfig: "insights_dataset_config",
  objectFilters: {
    wireName: "object_filters",
    kind: "object",
    fields: StoragebatchoperationsJob_ProjectSource_BucketFiltersFields,
  },
  project: "project",
  snapshotTime: "snapshot_time",
  targetLocations: {
    wireName: "target_locations",
    kind: "object",
    fields: StoragebatchoperationsJob_ProjectSource_TargetLocationsFields,
  },
};

const StoragebatchoperationsJob_PutMetadata_ObjectRetentionFields: FieldMap = {
  retainUntilTime: "retain_until_time",
  retentionMode: "retention_mode",
};

const StoragebatchoperationsJob_PutMetadataFields: FieldMap = {
  cacheControl: "cache_control",
  contentDisposition: "content_disposition",
  contentEncoding: "content_encoding",
  contentLanguage: "content_language",
  contentType: "content_type",
  customMetadata: "custom_metadata",
  customTime: "custom_time",
  objectRetention: {
    wireName: "object_retention",
    kind: "object",
    fields: StoragebatchoperationsJob_PutMetadata_ObjectRetentionFields,
  },
};

const StoragebatchoperationsJob_PutObjectHoldFields: FieldMap = {
  eventBasedHold: "event_based_hold",
  temporaryHold: "temporary_hold",
};

const StoragebatchoperationsJob_RewriteObjectFields: FieldMap = {
  kmsKey: "kms_key",
  storageClass: "storage_class",
};

const StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates_GrantsFields: FieldMap = {
  entity: "entity",
  role: "role",
};

const StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdatesFields: FieldMap = {
  grants: {
    wireName: "grants",
    kind: "list",
    fields: StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdates_GrantsFields,
  },
  removeEntities: "remove_entities",
};

const StoragebatchoperationsJob_SetObjectAclsFields: FieldMap = {
  accessControlsUpdates: {
    wireName: "access_controls_updates",
    kind: "object",
    fields: StoragebatchoperationsJob_SetObjectAcls_AccessControlsUpdatesFields,
  },
};

const StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates_UpdatesFields: FieldMap = {
  value: "value",
};

const StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdatesFields: FieldMap = {
  keysToClear: "keys_to_clear",
  updates: {
    wireName: "updates",
    kind: "map",
    fields: StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdates_UpdatesFields,
  },
};

const StoragebatchoperationsJob_UpdateObjectCustomContextFields: FieldMap = {
  clearAll: "clear_all",
  customContextUpdates: {
    wireName: "custom_context_updates",
    kind: "object",
    fields: StoragebatchoperationsJob_UpdateObjectCustomContext_CustomContextUpdatesFields,
  },
};

export interface StoragebatchoperationsJobConfig {
  /** Describes list of buckets and their objects to be transformed. */
  bucketList?: StoragebatchoperationsJob_BucketList | Computed<StoragebatchoperationsJob_BucketList>;
  /** Output only. The time that the job was completed. */
  completeTime?: string | Computed<string>;
  /** Describes details about the progress of the job. */
  counters?: StoragebatchoperationsJob_Counters | Computed<StoragebatchoperationsJob_Counters>;
  /** Output only. The time that the job was created. */
  createTime?: string | Computed<string>;
  /** Describes options to delete an object. */
  deleteObject?: StoragebatchoperationsJob_DeleteObject | Computed<StoragebatchoperationsJob_DeleteObject>;
  /** Optional. A user-provided description for the job. Maximum length: 1024 bytes when unicode-encoded. */
  description?: string | Computed<string>;
  /** Optional. If true, the job runs in dry run mode, returning the total object count and, if the object configuration is a prefix list, the bytes found from source. No transformations are performed. */
  dryRun?: boolean | Computed<boolean>;
  /** Output only. Summarizes errors encountered with sample error log entries. */
  errorSummaries?: StoragebatchoperationsJob_ErrorSummaries[] | Computed<StoragebatchoperationsJob_ErrorSummaries[]>;
  /** Output only. If true, this job operates on multiple buckets. Multi-bucket jobs are subject to different quota limits than single-bucket jobs. */
  isMultiBucketJob?: boolean | Computed<boolean>;
  /** Specifies the Cloud Logging behavior. */
  loggingConfig?: StoragebatchoperationsJob_LoggingConfig | Computed<StoragebatchoperationsJob_LoggingConfig>;
  /** Identifier. The resource name of the job. Format: `projects/{project_id}/locations/global/jobs/{job_id}`. For example: `projects/123456/locations/global/jobs/job01`. `job_id` is unique in a given project. */
  name?: string | Computed<string>;
  /** Describes the project source where the objects satisfying the filters will be transformed. */
  projectSource?: StoragebatchoperationsJob_ProjectSource | Computed<StoragebatchoperationsJob_ProjectSource>;
  /** Describes options for object metadata update. */
  putMetadata?: StoragebatchoperationsJob_PutMetadata | Computed<StoragebatchoperationsJob_PutMetadata>;
  /** Describes options to update object hold. */
  putObjectHold?: StoragebatchoperationsJob_PutObjectHold | Computed<StoragebatchoperationsJob_PutObjectHold>;
  /** Describes options for object rewrite. */
  rewriteObject?: StoragebatchoperationsJob_RewriteObject | Computed<StoragebatchoperationsJob_RewriteObject>;
  /** Output only. The time that the job was scheduled. */
  scheduleTime?: string | Computed<string>;
  /** Describes options for setting object ACLs. */
  setObjectAcls?: StoragebatchoperationsJob_SetObjectAcls | Computed<StoragebatchoperationsJob_SetObjectAcls>;
  /** Output only. State of the job. */
  state?: string | Computed<string>;
  /** Describes options to update object custom contexts. */
  updateObjectCustomContext?: StoragebatchoperationsJob_UpdateObjectCustomContext | Computed<StoragebatchoperationsJob_UpdateObjectCustomContext>;
}

export interface StoragebatchoperationsJobAttrs {
  /** Describes list of buckets and their objects to be transformed. */
  bucketList: StoragebatchoperationsJob_BucketList;
  /** Output only. The time that the job was completed. */
  completeTime: string;
  /** Describes details about the progress of the job. */
  counters: StoragebatchoperationsJob_Counters;
  /** Output only. The time that the job was created. */
  createTime: string;
  /** Describes options to delete an object. */
  deleteObject: StoragebatchoperationsJob_DeleteObject;
  /** Optional. A user-provided description for the job. Maximum length: 1024 bytes when unicode-encoded. */
  description: string;
  /** Optional. If true, the job runs in dry run mode, returning the total object count and, if the object configuration is a prefix list, the bytes found from source. No transformations are performed. */
  dryRun: boolean;
  /** Output only. Summarizes errors encountered with sample error log entries. */
  errorSummaries: StoragebatchoperationsJob_ErrorSummaries[];
  /** Output only. If true, this job operates on multiple buckets. Multi-bucket jobs are subject to different quota limits than single-bucket jobs. */
  isMultiBucketJob: boolean;
  /** Specifies the Cloud Logging behavior. */
  loggingConfig: StoragebatchoperationsJob_LoggingConfig;
  /** Identifier. The resource name of the job. Format: `projects/{project_id}/locations/global/jobs/{job_id}`. For example: `projects/123456/locations/global/jobs/job01`. `job_id` is unique in a given project. */
  name: string;
  /** Describes the project source where the objects satisfying the filters will be transformed. */
  projectSource: StoragebatchoperationsJob_ProjectSource;
  /** Describes options for object metadata update. */
  putMetadata: StoragebatchoperationsJob_PutMetadata;
  /** Describes options to update object hold. */
  putObjectHold: StoragebatchoperationsJob_PutObjectHold;
  /** Describes options for object rewrite. */
  rewriteObject: StoragebatchoperationsJob_RewriteObject;
  /** Output only. The time that the job was scheduled. */
  scheduleTime: string;
  /** Describes options for setting object ACLs. */
  setObjectAcls: StoragebatchoperationsJob_SetObjectAcls;
  /** Output only. State of the job. */
  state: string;
  /** Describes options to update object custom contexts. */
  updateObjectCustomContext: StoragebatchoperationsJob_UpdateObjectCustomContext;
}

export const StoragebatchoperationsJob: ResourceBinding<StoragebatchoperationsJobConfig, StoragebatchoperationsJobAttrs> = {
  wireType: "google_storagebatchoperations_storagebatchoperations_job",
  fields: {
    bucketList: {
      wireName: "bucket_list",
      kind: "object",
      fields: StoragebatchoperationsJob_BucketListFields,
    },
    completeTime: "complete_time",
    counters: {
      wireName: "counters",
      kind: "object",
      fields: StoragebatchoperationsJob_CountersFields,
    },
    createTime: "create_time",
    deleteObject: {
      wireName: "delete_object",
      kind: "object",
      fields: StoragebatchoperationsJob_DeleteObjectFields,
    },
    description: "description",
    dryRun: "dry_run",
    errorSummaries: {
      wireName: "error_summaries",
      kind: "list",
      fields: StoragebatchoperationsJob_ErrorSummariesFields,
    },
    isMultiBucketJob: "is_multi_bucket_job",
    loggingConfig: {
      wireName: "logging_config",
      kind: "object",
      fields: StoragebatchoperationsJob_LoggingConfigFields,
    },
    name: "name",
    projectSource: {
      wireName: "project_source",
      kind: "object",
      fields: StoragebatchoperationsJob_ProjectSourceFields,
    },
    putMetadata: {
      wireName: "put_metadata",
      kind: "object",
      fields: StoragebatchoperationsJob_PutMetadataFields,
    },
    putObjectHold: {
      wireName: "put_object_hold",
      kind: "object",
      fields: StoragebatchoperationsJob_PutObjectHoldFields,
    },
    rewriteObject: {
      wireName: "rewrite_object",
      kind: "object",
      fields: StoragebatchoperationsJob_RewriteObjectFields,
    },
    scheduleTime: "schedule_time",
    setObjectAcls: {
      wireName: "set_object_acls",
      kind: "object",
      fields: StoragebatchoperationsJob_SetObjectAclsFields,
    },
    state: "state",
    updateObjectCustomContext: {
      wireName: "update_object_custom_context",
      kind: "object",
      fields: StoragebatchoperationsJob_UpdateObjectCustomContextFields,
    },
  },
};
