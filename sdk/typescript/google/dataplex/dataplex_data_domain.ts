// Code generated by `ubx sdk gen` from google_dataplex_dataplex_data_domain. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface DataplexDataDomain_Contacts_Identities {
  contactId?: string | Computed<string>;
  contactName?: string | Computed<string>;
  contactRole?: string | Computed<string>;
}

export interface DataplexDataDomain_Contacts {
  /** Required. Identities of the business contacts. */
  identities?: DataplexDataDomain_Contacts_Identities[] | Computed<DataplexDataDomain_Contacts_Identities[]>;
}

export interface DataplexDataDomain_PolicyMember {
  /** Output only. IAM policy binding member referring to a Google Cloud resource by user-assigned name (https://google.aip.dev/122). If a resource is deleted and recreated with the same name, the binding will be applicable to the new resource.Example: principal://parametermanager.googleapis.com/projects/12345/name/locations/us-central1-a/parameters/my-parameter */
  iamPolicyNamePrincipal?: string | Computed<string>;
  /** Output only. IAM policy binding member referring to a Google Cloud resource by system-assigned unique identifier (https://google.aip.dev/148#uid). If a resource is deleted and recreated with the same name, the binding will not be applicable to the new resourceExample: principal://parametermanager.googleapis.com/projects/12345/uid/locations/us-central1-a/parameters/a918fed5 */
  iamPolicyUidPrincipal?: string | Computed<string>;
}

const DataplexDataDomain_Contacts_IdentitiesFields: FieldMap = {
  contactId: "contact_id",
  contactName: "contact_name",
  contactRole: "contact_role",
};

const DataplexDataDomain_ContactsFields: FieldMap = {
  identities: {
    wireName: "identities",
    kind: "list",
    fields: DataplexDataDomain_Contacts_IdentitiesFields,
  },
};

const DataplexDataDomain_PolicyMemberFields: FieldMap = {
  iamPolicyNamePrincipal: "iam_policy_name_principal",
  iamPolicyUidPrincipal: "iam_policy_uid_principal",
};

export interface DataplexDataDomainConfig {
  /** Business contacts part of business context of a Data Domain. Corresponds to the Contacts Aspect in Dataplex Universal Catalog. */
  contacts?: DataplexDataDomain_Contacts | Computed<DataplexDataDomain_Contacts>;
  /** Output only. The time at which the DataDomain was created. */
  createTime?: string | Computed<string>;
  /** Optional. User-provided description of the DataDomain. */
  description?: string | Computed<string>;
  /** Required. User-friendly display name. */
  displayName?: string | Computed<string>;
  /** Optional. User-defined labels for the DataDomain. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Identifier. The relative resource name of the DataDomain, of the form: projects/{project_id_or_number}/locations/{location_id}/dataDomains/{data_domain_id} */
  name?: string | Computed<string>;
  /** Optional. Immutable. The resource name of the parent DataDomain. Empty if this is a top-level DataDomain. Format: projects/{project_id_or_number}/locations/{location}/dataDomains/{parent_data_domain_id} This field is immutable after creation. */
  parentDataDomain?: string | Computed<string>;
  /** Output-only policy member strings of a Google Cloud resource's built-in identity. */
  policyMember?: DataplexDataDomain_PolicyMember | Computed<DataplexDataDomain_PolicyMember>;
  /** Output only. System-generated globally unique ID for the DataDomain. */
  uid?: string | Computed<string>;
  /** Output only. The time at which the DataDomain was last updated. */
  updateTime?: string | Computed<string>;
}

export interface DataplexDataDomainAttrs {
  /** Business contacts part of business context of a Data Domain. Corresponds to the Contacts Aspect in Dataplex Universal Catalog. */
  contacts: DataplexDataDomain_Contacts;
  /** Output only. The time at which the DataDomain was created. */
  createTime: string;
  /** Optional. User-provided description of the DataDomain. */
  description: string;
  /** Required. User-friendly display name. */
  displayName: string;
  /** Optional. User-defined labels for the DataDomain. */
  labels: Record<string, string>;
  /** Identifier. The relative resource name of the DataDomain, of the form: projects/{project_id_or_number}/locations/{location_id}/dataDomains/{data_domain_id} */
  name: string;
  /** Optional. Immutable. The resource name of the parent DataDomain. Empty if this is a top-level DataDomain. Format: projects/{project_id_or_number}/locations/{location}/dataDomains/{parent_data_domain_id} This field is immutable after creation. */
  parentDataDomain: string;
  /** Output-only policy member strings of a Google Cloud resource's built-in identity. */
  policyMember: DataplexDataDomain_PolicyMember;
  /** Output only. System-generated globally unique ID for the DataDomain. */
  uid: string;
  /** Output only. The time at which the DataDomain was last updated. */
  updateTime: string;
}

export const DataplexDataDomain: ResourceBinding<DataplexDataDomainConfig, DataplexDataDomainAttrs> = {
  wireType: "google_dataplex_dataplex_data_domain",
  fields: {
    contacts: {
      wireName: "contacts",
      kind: "object",
      fields: DataplexDataDomain_ContactsFields,
    },
    createTime: "create_time",
    description: "description",
    displayName: "display_name",
    labels: "labels",
    name: "name",
    parentDataDomain: "parent_data_domain",
    policyMember: {
      wireName: "policy_member",
      kind: "object",
      fields: DataplexDataDomain_PolicyMemberFields,
    },
    uid: "uid",
    updateTime: "update_time",
  },
};
