// Code generated by `ubx sdk gen` from google_orgpolicy_orgpolicy_policy. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface OrgpolicyPolicy_Alternate_Spec_Rules_Condition {
  description?: string | Computed<string>;
  expression?: string | Computed<string>;
  location?: string | Computed<string>;
  title?: string | Computed<string>;
}

export interface OrgpolicyPolicy_Alternate_Spec_Rules_Values {
  allowedValues?: string[] | Computed<string[]>;
  deniedValues?: string[] | Computed<string[]>;
}

export interface OrgpolicyPolicy_Alternate_Spec_Rules {
  allowAll?: boolean | Computed<boolean>;
  condition?: OrgpolicyPolicy_Alternate_Spec_Rules_Condition | Computed<OrgpolicyPolicy_Alternate_Spec_Rules_Condition>;
  denyAll?: boolean | Computed<boolean>;
  enforce?: boolean | Computed<boolean>;
  parameters?: Record<string, unknown> | Computed<Record<string, unknown>>;
  values?: OrgpolicyPolicy_Alternate_Spec_Rules_Values | Computed<OrgpolicyPolicy_Alternate_Spec_Rules_Values>;
}

export interface OrgpolicyPolicy_Alternate_Spec {
  /** An opaque tag indicating the current version of the policySpec, used for concurrency control. This field is ignored if used in a `CreatePolicy` request. When the policy is returned from either a `GetPolicy` or a `ListPolicies` request, this entity tag (ETag) indicates the version of the current policySpec to use when executing a read-modify-write loop. When the policy is returned from a `GetEffectivePolicy` request, the ETag will be unset. */
  etag?: string | Computed<string>;
  /** Determines the inheritance behavior for this policy. If `inherit_from_parent` is true, policy rules set higher up in the hierarchy (up to the closest root) are inherited and present in the effective policy. If it is false, then no rules are inherited, and this policy becomes the new root for evaluation. This field can be set only for policies that configure list constraints. */
  inheritFromParent?: boolean | Computed<boolean>;
  /** Ignores policies set above this resource and restores the `constraint_default` enforcement behavior of the specific constraint at this resource. This field can be set in policies for either list or boolean constraints. If set, `rules` must be empty and `inherit_from_parent` must be set to false. */
  reset?: boolean | Computed<boolean>;
  /** In policies for boolean constraints, the following requirements apply: - There must be exactly one policy rule where a condition is unset. - Boolean policy rules with conditions must set `enforced` to the opposite of the policy rule without a condition. - During policy evaluation, policy rules with conditions that are true for a target resource take precedence. */
  rules?: OrgpolicyPolicy_Alternate_Spec_Rules[] | Computed<OrgpolicyPolicy_Alternate_Spec_Rules[]>;
  /** Output only. The time stamp this was previously updated. This represents the last time a call to `CreatePolicy` or `UpdatePolicy` was made for that policy. */
  updateTime?: string | Computed<string>;
}

export interface OrgpolicyPolicy_Alternate {
  /** Reference to the launch that will be used while audit logging and to control the launch. Set only in the alternate policy. */
  launch?: string | Computed<string>;
  /** Defines a Google Cloud policy specification that is used to specify constraints for configurations of Google Cloud resources. */
  spec?: OrgpolicyPolicy_Alternate_Spec | Computed<OrgpolicyPolicy_Alternate_Spec>;
}

const OrgpolicyPolicy_Alternate_Spec_Rules_ConditionFields: FieldMap = {
  description: "description",
  expression: "expression",
  location: "location",
  title: "title",
};

const OrgpolicyPolicy_Alternate_Spec_Rules_ValuesFields: FieldMap = {
  allowedValues: "allowed_values",
  deniedValues: "denied_values",
};

const OrgpolicyPolicy_Alternate_Spec_RulesFields: FieldMap = {
  allowAll: "allow_all",
  condition: {
    wireName: "condition",
    kind: "object",
    fields: OrgpolicyPolicy_Alternate_Spec_Rules_ConditionFields,
  },
  denyAll: "deny_all",
  enforce: "enforce",
  parameters: "parameters",
  values: {
    wireName: "values",
    kind: "object",
    fields: OrgpolicyPolicy_Alternate_Spec_Rules_ValuesFields,
  },
};

const OrgpolicyPolicy_Alternate_SpecFields: FieldMap = {
  etag: "etag",
  inheritFromParent: "inherit_from_parent",
  reset: "reset",
  rules: {
    wireName: "rules",
    kind: "list",
    fields: OrgpolicyPolicy_Alternate_Spec_RulesFields,
  },
  updateTime: "update_time",
};

const OrgpolicyPolicy_AlternateFields: FieldMap = {
  launch: "launch",
  spec: {
    wireName: "spec",
    kind: "object",
    fields: OrgpolicyPolicy_Alternate_SpecFields,
  },
};

export interface OrgpolicyPolicyConfig {
  /** Similar to PolicySpec but with an extra 'launch' field for launch reference. The PolicySpec here is specific for dry-run. */
  alternate?: OrgpolicyPolicy_Alternate | Computed<OrgpolicyPolicy_Alternate>;
  /** Defines a Google Cloud policy specification that is used to specify constraints for configurations of Google Cloud resources. */
  dryRunSpec?: OrgpolicyPolicy_Alternate_Spec | Computed<OrgpolicyPolicy_Alternate_Spec>;
  /** Optional. An opaque tag indicating the current state of the policy, used for concurrency control. This entity tag (ETag) is computed by the server based on the value of other fields, and may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding. */
  etag?: string | Computed<string>;
  /** Immutable. The resource name of the policy. Must be one of the following forms, where `constraint_name` is the name of the constraint that this policy configures: * `projects/{project_number}/policies/{constraint_name}` * `folders/{folder_number}/policies/{constraint_name}` * `organizations/{organization_number}/policies/{constraint_name}` For example, `projects/123/policies/compute.disableSerialPortAccess`. Note: `projects/{project_id}/policies/{constraint_name}` is also an acceptable name for API requests, but responses will return the name using the equivalent project number. */
  name?: string | Computed<string>;
  /** Defines a Google Cloud policy specification that is used to specify constraints for configurations of Google Cloud resources. */
  spec?: OrgpolicyPolicy_Alternate_Spec | Computed<OrgpolicyPolicy_Alternate_Spec>;
}

export interface OrgpolicyPolicyAttrs {
  /** Similar to PolicySpec but with an extra 'launch' field for launch reference. The PolicySpec here is specific for dry-run. */
  alternate: OrgpolicyPolicy_Alternate;
  /** Defines a Google Cloud policy specification that is used to specify constraints for configurations of Google Cloud resources. */
  dryRunSpec: OrgpolicyPolicy_Alternate_Spec;
  /** Optional. An opaque tag indicating the current state of the policy, used for concurrency control. This entity tag (ETag) is computed by the server based on the value of other fields, and may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding. */
  etag: string;
  /** Immutable. The resource name of the policy. Must be one of the following forms, where `constraint_name` is the name of the constraint that this policy configures: * `projects/{project_number}/policies/{constraint_name}` * `folders/{folder_number}/policies/{constraint_name}` * `organizations/{organization_number}/policies/{constraint_name}` For example, `projects/123/policies/compute.disableSerialPortAccess`. Note: `projects/{project_id}/policies/{constraint_name}` is also an acceptable name for API requests, but responses will return the name using the equivalent project number. */
  name: string;
  /** Defines a Google Cloud policy specification that is used to specify constraints for configurations of Google Cloud resources. */
  spec: OrgpolicyPolicy_Alternate_Spec;
}

export const OrgpolicyPolicy: ResourceBinding<OrgpolicyPolicyConfig, OrgpolicyPolicyAttrs> = {
  wireType: "google_orgpolicy_orgpolicy_policy",
  fields: {
    alternate: {
      wireName: "alternate",
      kind: "object",
      fields: OrgpolicyPolicy_AlternateFields,
    },
    dryRunSpec: {
      wireName: "dry_run_spec",
      kind: "object",
      fields: OrgpolicyPolicy_Alternate_SpecFields,
    },
    etag: "etag",
    name: "name",
    spec: {
      wireName: "spec",
      kind: "object",
      fields: OrgpolicyPolicy_Alternate_SpecFields,
    },
  },
};
