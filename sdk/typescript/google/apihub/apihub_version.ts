// Code generated by `ubx sdk gen` from google_apihub_apihub_version. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface ApihubVersion_Accreditation_EnumValues_Values {
  description?: string | Computed<string>;
  displayName?: string | Computed<string>;
  id?: string | Computed<string>;
  immutable?: boolean | Computed<boolean>;
}

export interface ApihubVersion_Accreditation_EnumValues {
  /** Required. The attribute values in case attribute data type is enum. */
  values?: ApihubVersion_Accreditation_EnumValues_Values[] | Computed<ApihubVersion_Accreditation_EnumValues_Values[]>;
}

export interface ApihubVersion_Accreditation_JsonValues {
  /** Required. The attribute values in case attribute data type is string or JSON. */
  values?: string[] | Computed<string[]>;
}

export interface ApihubVersion_Accreditation {
  /** Output only. The name of the attribute. Format: projects/{project}/locations/{location}/attributes/{attribute} */
  attribute?: string | Computed<string>;
  /** The attribute values of data type enum. */
  enumValues?: ApihubVersion_Accreditation_EnumValues | Computed<ApihubVersion_Accreditation_EnumValues>;
  /** The attribute values of data type string or JSON. */
  jsonValues?: ApihubVersion_Accreditation_JsonValues | Computed<ApihubVersion_Accreditation_JsonValues>;
  /** The attribute values of data type string or JSON. */
  stringValues?: ApihubVersion_Accreditation_JsonValues | Computed<ApihubVersion_Accreditation_JsonValues>;
  /** The attribute values of data type string or JSON. */
  uriValues?: ApihubVersion_Accreditation_JsonValues | Computed<ApihubVersion_Accreditation_JsonValues>;
}

export interface ApihubVersion_Documentation {
  /** Optional. The uri of the externally hosted documentation. */
  externalUri?: string | Computed<string>;
}

export interface ApihubVersion_SourceMetadata_PluginInstanceActionSource {
  actionId?: string | Computed<string>;
  pluginInstance?: string | Computed<string>;
}

export interface ApihubVersion_SourceMetadata {
  originalResourceCreateTime?: string | Computed<string>;
  originalResourceId?: string | Computed<string>;
  originalResourceUpdateTime?: string | Computed<string>;
  pluginInstanceActionSource?: ApihubVersion_SourceMetadata_PluginInstanceActionSource | Computed<ApihubVersion_SourceMetadata_PluginInstanceActionSource>;
  sourceType?: string | Computed<string>;
}

const ApihubVersion_Accreditation_EnumValues_ValuesFields: FieldMap = {
  description: "description",
  displayName: "display_name",
  id: "id",
  immutable: "immutable",
};

const ApihubVersion_Accreditation_EnumValuesFields: FieldMap = {
  values: {
    wireName: "values",
    kind: "list",
    fields: ApihubVersion_Accreditation_EnumValues_ValuesFields,
  },
};

const ApihubVersion_Accreditation_JsonValuesFields: FieldMap = {
  values: "values",
};

const ApihubVersion_AccreditationFields: FieldMap = {
  attribute: "attribute",
  enumValues: {
    wireName: "enum_values",
    kind: "object",
    fields: ApihubVersion_Accreditation_EnumValuesFields,
  },
  jsonValues: {
    wireName: "json_values",
    kind: "object",
    fields: ApihubVersion_Accreditation_JsonValuesFields,
  },
  stringValues: {
    wireName: "string_values",
    kind: "object",
    fields: ApihubVersion_Accreditation_JsonValuesFields,
  },
  uriValues: {
    wireName: "uri_values",
    kind: "object",
    fields: ApihubVersion_Accreditation_JsonValuesFields,
  },
};

const ApihubVersion_DocumentationFields: FieldMap = {
  externalUri: "external_uri",
};

const ApihubVersion_SourceMetadata_PluginInstanceActionSourceFields: FieldMap = {
  actionId: "action_id",
  pluginInstance: "plugin_instance",
};

const ApihubVersion_SourceMetadataFields: FieldMap = {
  originalResourceCreateTime: "original_resource_create_time",
  originalResourceId: "original_resource_id",
  originalResourceUpdateTime: "original_resource_update_time",
  pluginInstanceActionSource: {
    wireName: "plugin_instance_action_source",
    kind: "object",
    fields: ApihubVersion_SourceMetadata_PluginInstanceActionSourceFields,
  },
  sourceType: "source_type",
};

export interface ApihubVersionConfig {
  /** The attribute values associated with resource. */
  accreditation?: ApihubVersion_Accreditation | Computed<ApihubVersion_Accreditation>;
  /** Output only. The operations contained in the API version. These operations will be added to the version when a new spec is added or when an existing spec is updated. Format is `projects/{project}/locations/{location}/apis/{api}/versions/{version}/operations/{operation}` */
  apiOperations?: string[] | Computed<string[]>;
  /** Optional. The list of user defined attributes associated with the Version resource. The key is the attribute name. It will be of the format: `projects/{project}/locations/{location}/attributes/{attribute}`. The value is the attribute values associated with the resource. */
  attributes?: Record<string, ApihubVersion_Accreditation> | Computed<Record<string, ApihubVersion_Accreditation>>;
  /** The attribute values associated with resource. */
  compliance?: ApihubVersion_Accreditation | Computed<ApihubVersion_Accreditation>;
  /** Output only. The time at which the version was created. */
  createTime?: string | Computed<string>;
  /** Output only. The definitions contained in the API version. These definitions will be added to the version when a new spec is added or when an existing spec is updated. Format is `projects/{project}/locations/{location}/apis/{api}/versions/{version}/definitions/{definition}` */
  definitions?: string[] | Computed<string[]>;
  /** Optional. The deployments linked directly to this API version. Only directly-linked deployments are returned; deployments linked to this version's specs or operations are not included. Note: A particular API version could be deployed to multiple deployments (for dev deployment, UAT deployment, etc) Format is `projects/{project}/locations/{location}/deployments/{deployment}` */
  deployments?: string[] | Computed<string[]>;
  /** Optional. The description of the version. */
  description?: string | Computed<string>;
  /** Required. The display name of the version. */
  displayName?: string | Computed<string>;
  /** Documentation details. */
  documentation?: ApihubVersion_Documentation | Computed<ApihubVersion_Documentation>;
  /** The attribute values associated with resource. */
  lifecycle?: ApihubVersion_Accreditation | Computed<ApihubVersion_Accreditation>;
  /** Identifier. The name of the version. Format: `projects/{project}/locations/{location}/apis/{api}/versions/{version}` */
  name?: string | Computed<string>;
  /** Optional. The selected deployment for a Version resource. This can be used when special handling is needed on client side for a particular deployment linked to the version. Format is `projects/{project}/locations/{location}/deployments/{deployment}` */
  selectedDeployment?: string | Computed<string>;
  /** Output only. The list of sources and metadata from the sources of the version. */
  sourceMetadata?: ApihubVersion_SourceMetadata[] | Computed<ApihubVersion_SourceMetadata[]>;
  /** Output only. The specs associated with this version. Note that an API version can be associated with multiple specs. Format is `projects/{project}/locations/{location}/apis/{api}/versions/{version}/specs/{spec}` */
  specs?: string[] | Computed<string[]>;
  /** Output only. The time at which the version was last updated. */
  updateTime?: string | Computed<string>;
}

export interface ApihubVersionAttrs {
  /** The attribute values associated with resource. */
  accreditation: ApihubVersion_Accreditation;
  /** Output only. The operations contained in the API version. These operations will be added to the version when a new spec is added or when an existing spec is updated. Format is `projects/{project}/locations/{location}/apis/{api}/versions/{version}/operations/{operation}` */
  apiOperations: string[];
  /** Optional. The list of user defined attributes associated with the Version resource. The key is the attribute name. It will be of the format: `projects/{project}/locations/{location}/attributes/{attribute}`. The value is the attribute values associated with the resource. */
  attributes: Record<string, ApihubVersion_Accreditation>;
  /** The attribute values associated with resource. */
  compliance: ApihubVersion_Accreditation;
  /** Output only. The time at which the version was created. */
  createTime: string;
  /** Output only. The definitions contained in the API version. These definitions will be added to the version when a new spec is added or when an existing spec is updated. Format is `projects/{project}/locations/{location}/apis/{api}/versions/{version}/definitions/{definition}` */
  definitions: string[];
  /** Optional. The deployments linked directly to this API version. Only directly-linked deployments are returned; deployments linked to this version's specs or operations are not included. Note: A particular API version could be deployed to multiple deployments (for dev deployment, UAT deployment, etc) Format is `projects/{project}/locations/{location}/deployments/{deployment}` */
  deployments: string[];
  /** Optional. The description of the version. */
  description: string;
  /** Required. The display name of the version. */
  displayName: string;
  /** Documentation details. */
  documentation: ApihubVersion_Documentation;
  /** The attribute values associated with resource. */
  lifecycle: ApihubVersion_Accreditation;
  /** Identifier. The name of the version. Format: `projects/{project}/locations/{location}/apis/{api}/versions/{version}` */
  name: string;
  /** Optional. The selected deployment for a Version resource. This can be used when special handling is needed on client side for a particular deployment linked to the version. Format is `projects/{project}/locations/{location}/deployments/{deployment}` */
  selectedDeployment: string;
  /** Output only. The list of sources and metadata from the sources of the version. */
  sourceMetadata: ApihubVersion_SourceMetadata[];
  /** Output only. The specs associated with this version. Note that an API version can be associated with multiple specs. Format is `projects/{project}/locations/{location}/apis/{api}/versions/{version}/specs/{spec}` */
  specs: string[];
  /** Output only. The time at which the version was last updated. */
  updateTime: string;
}

export const ApihubVersion: ResourceBinding<ApihubVersionConfig, ApihubVersionAttrs> = {
  wireType: "google_apihub_apihub_version",
  fields: {
    accreditation: {
      wireName: "accreditation",
      kind: "object",
      fields: ApihubVersion_AccreditationFields,
    },
    apiOperations: "api_operations",
    attributes: {
      wireName: "attributes",
      kind: "map",
      fields: ApihubVersion_AccreditationFields,
    },
    compliance: {
      wireName: "compliance",
      kind: "object",
      fields: ApihubVersion_AccreditationFields,
    },
    createTime: "create_time",
    definitions: "definitions",
    deployments: "deployments",
    description: "description",
    displayName: "display_name",
    documentation: {
      wireName: "documentation",
      kind: "object",
      fields: ApihubVersion_DocumentationFields,
    },
    lifecycle: {
      wireName: "lifecycle",
      kind: "object",
      fields: ApihubVersion_AccreditationFields,
    },
    name: "name",
    selectedDeployment: "selected_deployment",
    sourceMetadata: {
      wireName: "source_metadata",
      kind: "list",
      fields: ApihubVersion_SourceMetadataFields,
    },
    specs: "specs",
    updateTime: "update_time",
  },
};
