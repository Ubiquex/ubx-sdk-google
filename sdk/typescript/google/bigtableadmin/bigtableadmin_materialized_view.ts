...
export interface BigtableadminMaterializedView_ClusterStates {
  replicationState?: string | Computed<string>;
}

const BigtableadminMaterializedView_ClusterStatesFields: FieldMap = {
  replicationState: "replication_state",
};

export interface BigtableadminMaterializedViewConfig {
  ...
  clusterStates?: Record<string, BigtableadminMaterializedView_ClusterStates> | Computed<Record<string, BigtableadminMaterializedView_ClusterStates>>;
...
export interface BigtableadminMaterializedViewAttrs {
  /** Output only. Map from cluster ID to per-cluster materialized view state. If it could not be determined whether or not the materialized view has data in a particular cluster (for example, if its zone is unavailable), then there will be an entry for the cluster with `STATE_NOT_KNOWN` state. Views: `REPLICATION_VIEW`, `FULL`. */
  clusterStates: Record<string, BigtableadminMaterializedView_ClusterStates>;
  /** Set to true to make the MaterializedView protected against deletion. Views: `SCHEMA_VIEW`, `REPLICATION_VIEW`, `FULL`. */
...
export const BigtableadminMaterializedView: ResourceBinding<BigtableadminMaterializedViewConfig, BigtableadminMaterializedViewAttrs> = {
