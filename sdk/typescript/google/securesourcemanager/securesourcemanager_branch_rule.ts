// Code generated by `ubx sdk gen` from google_securesourcemanager_securesourcemanager_branch_rule. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface SecuresourcemanagerBranchRule_RequiredStatusChecks {
  context?: string | Computed<string>;
}

const SecuresourcemanagerBranchRule_RequiredStatusChecksFields: FieldMap = {
  context: "context",
};

export interface SecuresourcemanagerBranchRuleConfig {
  /** Optional. Determines if allow stale reviews or approvals before merging to the branch. */
  allowStaleReviews?: boolean | Computed<boolean>;
  /** Optional. User annotations. These attributes can only be set and used by the user. See https://google.aip.dev/128#annotations for more details such as format and size limitations. */
  annotations?: Record<string, string> | Computed<Record<string, string>>;
  /** Output only. Create timestamp. */
  createTime?: string | Computed<string>;
  /** Optional. Determines if the branch rule is disabled or not. */
  disabled?: boolean | Computed<boolean>;
  /** Optional. This checksum is computed by the server based on the value of other fields, and may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding. */
  etag?: string | Computed<string>;
  /** Optional. The pattern of the branch that can match to this BranchRule. Specified as regex. .* for all branches. Examples: main, (main|release.*). Current MVP phase only support `.*` for wildcard. */
  includePattern?: string | Computed<string>;
  /** Optional. The minimum number of approvals required for the branch rule to be matched. */
  minimumApprovalsCount?: number | Computed<number>;
  /** Optional. The minimum number of reviews required for the branch rule to be matched. */
  minimumReviewsCount?: number | Computed<number>;
  /** Identifier. A unique identifier for a BranchRule. The name should be of the format: `projects/{project}/locations/{location}/repositories/{repository}/branchRules/{branch_rule}` */
  name?: string | Computed<string>;
  /** Optional. Determines if code owners must approve before merging to the branch. */
  requireCodeOwnerApproval?: boolean | Computed<boolean>;
  /** Optional. Determines if require comments resolved before merging to the branch. */
  requireCommentsResolved?: boolean | Computed<boolean>;
  /** Optional. Determines if require linear history before merging to the branch. */
  requireLinearHistory?: boolean | Computed<boolean>;
  /** Optional. Determines if the branch rule requires a pull request or not. */
  requirePullRequest?: boolean | Computed<boolean>;
  /** Optional. List of required status checks before merging to the branch. */
  requiredStatusChecks?: SecuresourcemanagerBranchRule_RequiredStatusChecks[] | Computed<SecuresourcemanagerBranchRule_RequiredStatusChecks[]>;
  /** Output only. Unique identifier of the repository. */
  uid?: string | Computed<string>;
  /** Output only. Update timestamp. */
  updateTime?: string | Computed<string>;
}

export interface SecuresourcemanagerBranchRuleAttrs {
  /** Optional. Determines if allow stale reviews or approvals before merging to the branch. */
  allowStaleReviews: boolean;
  /** Optional. User annotations. These attributes can only be set and used by the user. See https://google.aip.dev/128#annotations for more details such as format and size limitations. */
  annotations: Record<string, string>;
  /** Output only. Create timestamp. */
  createTime: string;
  /** Optional. Determines if the branch rule is disabled or not. */
  disabled: boolean;
  /** Optional. This checksum is computed by the server based on the value of other fields, and may be sent on update and delete requests to ensure the client has an up-to-date value before proceeding. */
  etag: string;
  /** Optional. The pattern of the branch that can match to this BranchRule. Specified as regex. .* for all branches. Examples: main, (main|release.*). Current MVP phase only support `.*` for wildcard. */
  includePattern: string;
  /** Optional. The minimum number of approvals required for the branch rule to be matched. */
  minimumApprovalsCount: number;
  /** Optional. The minimum number of reviews required for the branch rule to be matched. */
  minimumReviewsCount: number;
  /** Identifier. A unique identifier for a BranchRule. The name should be of the format: `projects/{project}/locations/{location}/repositories/{repository}/branchRules/{branch_rule}` */
  name: string;
  /** Optional. Determines if code owners must approve before merging to the branch. */
  requireCodeOwnerApproval: boolean;
  /** Optional. Determines if require comments resolved before merging to the branch. */
  requireCommentsResolved: boolean;
  /** Optional. Determines if require linear history before merging to the branch. */
  requireLinearHistory: boolean;
  /** Optional. Determines if the branch rule requires a pull request or not. */
  requirePullRequest: boolean;
  /** Optional. List of required status checks before merging to the branch. */
  requiredStatusChecks: SecuresourcemanagerBranchRule_RequiredStatusChecks[];
  /** Output only. Unique identifier of the repository. */
  uid: string;
  /** Output only. Update timestamp. */
  updateTime: string;
}

export const SecuresourcemanagerBranchRule: ResourceBinding<SecuresourcemanagerBranchRuleConfig, SecuresourcemanagerBranchRuleAttrs> = {
  wireType: "google_securesourcemanager_securesourcemanager_branch_rule",
  fields: {
    allowStaleReviews: "allow_stale_reviews",
    annotations: "annotations",
    createTime: "create_time",
    disabled: "disabled",
    etag: "etag",
    includePattern: "include_pattern",
    minimumApprovalsCount: "minimum_approvals_count",
    minimumReviewsCount: "minimum_reviews_count",
    name: "name",
    requireCodeOwnerApproval: "require_code_owner_approval",
    requireCommentsResolved: "require_comments_resolved",
    requireLinearHistory: "require_linear_history",
    requirePullRequest: "require_pull_request",
    requiredStatusChecks: {
      wireName: "required_status_checks",
      kind: "list",
      fields: SecuresourcemanagerBranchRule_RequiredStatusChecksFields,
    },
    uid: "uid",
    updateTime: "update_time",
  },
};
