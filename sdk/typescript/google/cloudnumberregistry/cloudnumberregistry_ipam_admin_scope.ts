// Code generated by `ubx sdk gen` from google_cloudnumberregistry_cloudnumberregistry_ipam_admin_scope. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface CloudnumberregistryIpamAdminScopeConfig {
  /** Output only. The time at which the IpamAdminScope was created. */
  createTime?: string | Computed<string>;
  /** Required. Add-on platforms that are enabled for this IpamAdminScope. Cloud Number Registry only discovers the IP addresses from the enabled platforms. */
  enabledAddonPlatforms?: string[] | Computed<string[]>;
  /** Optional. User-defined labels. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Required. Identifier. The resource name of the IpamAdminScope. */
  name?: string | Computed<string>;
  /** Required. Administrative scopes enabled for IP address discovery and management. For example, "organizations/1234567890". Minimum of 1 scope is required. In preview, only one organization scope is allowed. */
  scopes?: string[] | Computed<string[]>;
  /** Output only. State of resource discovery pipeline. */
  state?: string | Computed<string>;
  /** Output only. The time at which the IpamAdminScope was last updated. */
  updateTime?: string | Computed<string>;
}

export interface CloudnumberregistryIpamAdminScopeAttrs {
  /** Output only. The time at which the IpamAdminScope was created. */
  createTime: string;
  /** Required. Add-on platforms that are enabled for this IpamAdminScope. Cloud Number Registry only discovers the IP addresses from the enabled platforms. */
  enabledAddonPlatforms: string[];
  /** Optional. User-defined labels. */
  labels: Record<string, string>;
  /** Required. Identifier. The resource name of the IpamAdminScope. */
  name: string;
  /** Required. Administrative scopes enabled for IP address discovery and management. For example, "organizations/1234567890". Minimum of 1 scope is required. In preview, only one organization scope is allowed. */
  scopes: string[];
  /** Output only. State of resource discovery pipeline. */
  state: string;
  /** Output only. The time at which the IpamAdminScope was last updated. */
  updateTime: string;
}

export const CloudnumberregistryIpamAdminScope: ResourceBinding<CloudnumberregistryIpamAdminScopeConfig, CloudnumberregistryIpamAdminScopeAttrs> = {
  wireType: "google_cloudnumberregistry_cloudnumberregistry_ipam_admin_scope",
  fields: {
    createTime: "create_time",
    enabledAddonPlatforms: "enabled_addon_platforms",
    labels: "labels",
    name: "name",
    scopes: "scopes",
    state: "state",
    updateTime: "update_time",
  },
};
