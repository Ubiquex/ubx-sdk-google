// Code generated by `ubx sdk gen` from google_dataform_v1beta1_dataform_v1beta1_workspace. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface V1beta1DataformV1beta1Workspace_DataEncryptionState {
  /** Required. The KMS key version name with which data of a resource is encrypted. */
  kmsKeyVersionName?: string | Computed<string>;
}

export interface V1beta1DataformV1beta1Workspace_PrivateResourceMetadata {
  /** Output only. If true, this resource is user-scoped, meaning it is either a workspace or sourced from a workspace. */
  userScoped?: boolean | Computed<boolean>;
}

const V1beta1DataformV1beta1Workspace_DataEncryptionStateFields: FieldMap = {
  kmsKeyVersionName: "kms_key_version_name",
};

const V1beta1DataformV1beta1Workspace_PrivateResourceMetadataFields: FieldMap = {
  userScoped: "user_scoped",
};

export interface V1beta1DataformV1beta1WorkspaceConfig {
  /** Output only. The timestamp of when the workspace was created. */
  createTime?: string | Computed<string>;
  /** Describes encryption state of a resource. */
  dataEncryptionState?: V1beta1DataformV1beta1Workspace_DataEncryptionState | Computed<V1beta1DataformV1beta1Workspace_DataEncryptionState>;
  /** Optional. Input only. Immutable. The maximum depth of the Git repository to checkout for this workspace. If defined and greater than 0, the Git repository will be created as a shallow clone with the given depth, otherwise a full clone will be performed. This field is available only for GitHub, Gitlab and 1p repositories with enabled branch management. */
  depth?: number | Computed<number>;
  /** Optional. If set to true, workspaces will not be moved if its linked Repository is moved. Instead, it will be deleted. */
  disableMoves?: boolean | Computed<boolean>;
  /** Immutable. Controls the enablement of branch checkout for the workspace. When set to True, the workspace will be allowed to checkout branches. */
  enableBranchManagement?: boolean | Computed<boolean>;
  /** Output only. All the metadata information that is used internally to serve the resource. For example: timestamps, flags, status fields, etc. The format of this field is a JSON string. */
  internalMetadata?: string | Computed<string>;
  /** Identifier. The workspace's name. */
  name?: string | Computed<string>;
  /** Optional. Input only. Immutable. The name of the default upstream branch for all pull/push operations in the remote repository for this workspace. If empty, the HEAD branch from repository will be used. */
  originalBranch?: string | Computed<string>;
  /** Metadata used to identify if a resource is user scoped. */
  privateResourceMetadata?: V1beta1DataformV1beta1Workspace_PrivateResourceMetadata | Computed<V1beta1DataformV1beta1Workspace_PrivateResourceMetadata>;
  /** Output only. If set to true, the workspace was created as a shallow clone. Will be set to true if the depth field is set to a value greater than 0, otherwise it will be set to false. */
  shallow?: boolean | Computed<boolean>;
}

export interface V1beta1DataformV1beta1WorkspaceAttrs {
  /** Output only. The timestamp of when the workspace was created. */
  createTime: string;
  /** Describes encryption state of a resource. */
  dataEncryptionState: V1beta1DataformV1beta1Workspace_DataEncryptionState;
  /** Optional. Input only. Immutable. The maximum depth of the Git repository to checkout for this workspace. If defined and greater than 0, the Git repository will be created as a shallow clone with the given depth, otherwise a full clone will be performed. This field is available only for GitHub, Gitlab and 1p repositories with enabled branch management. */
  depth: number;
  /** Optional. If set to true, workspaces will not be moved if its linked Repository is moved. Instead, it will be deleted. */
  disableMoves: boolean;
  /** Immutable. Controls the enablement of branch checkout for the workspace. When set to True, the workspace will be allowed to checkout branches. */
  enableBranchManagement: boolean;
  /** Output only. All the metadata information that is used internally to serve the resource. For example: timestamps, flags, status fields, etc. The format of this field is a JSON string. */
  internalMetadata: string;
  /** Identifier. The workspace's name. */
  name: string;
  /** Optional. Input only. Immutable. The name of the default upstream branch for all pull/push operations in the remote repository for this workspace. If empty, the HEAD branch from repository will be used. */
  originalBranch: string;
  /** Metadata used to identify if a resource is user scoped. */
  privateResourceMetadata: V1beta1DataformV1beta1Workspace_PrivateResourceMetadata;
  /** Output only. If set to true, the workspace was created as a shallow clone. Will be set to true if the depth field is set to a value greater than 0, otherwise it will be set to false. */
  shallow: boolean;
}

export const V1beta1DataformV1beta1Workspace: ResourceBinding<V1beta1DataformV1beta1WorkspaceConfig, V1beta1DataformV1beta1WorkspaceAttrs> = {
  wireType: "google_dataform_v1beta1_dataform_v1beta1_workspace",
  fields: {
    createTime: "create_time",
    dataEncryptionState: {
      wireName: "data_encryption_state",
      kind: "object",
      fields: V1beta1DataformV1beta1Workspace_DataEncryptionStateFields,
    },
    depth: "depth",
    disableMoves: "disable_moves",
    enableBranchManagement: "enable_branch_management",
    internalMetadata: "internal_metadata",
    name: "name",
    originalBranch: "original_branch",
    privateResourceMetadata: {
      wireName: "private_resource_metadata",
      kind: "object",
      fields: V1beta1DataformV1beta1Workspace_PrivateResourceMetadataFields,
    },
    shallow: "shallow",
  },
};
