// Code generated by `ubx sdk gen` from google_notebooks_notebooks_instance. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface NotebooksInstance_GceSetup_AcceleratorConfigs {
  coreCount?: string | Computed<string>;
  type?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup_BootDisk {
  /** Optional. Disk encryption method used on the boot and data disks, defaults to GMEK. */
  diskEncryption?: string | Computed<string>;
  /** Optional. The size of the boot disk in GB attached to this instance, up to a maximum of 64000 GB (64 TB). If not specified, this defaults to the recommended value of 150GB. */
  diskSizeGb?: string | Computed<string>;
  /** Optional. Indicates the type of the disk. */
  diskType?: string | Computed<string>;
  /** Optional. The KMS key used to encrypt the disks, only applicable if disk_encryption is CMEK. Format: `projects/{project_id}/locations/{location}/keyRings/{key_ring_id}/cryptoKeys/{key_id}` Learn more about using your own encryption keys. */
  kmsKey?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup_ConfidentialInstanceConfig {
  /** Optional. Defines the type of technology used by the confidential instance. */
  confidentialInstanceType?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup_ContainerImage {
  /** Required. The path to the container image repository. For example: `gcr.io/{project_id}/{image_name}` */
  repository?: string | Computed<string>;
  /** Optional. The tag of the container image. If not specified, this defaults to the latest tag. */
  tag?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup_DataDisks {
  diskEncryption?: string | Computed<string>;
  diskSizeGb?: string | Computed<string>;
  diskType?: string | Computed<string>;
  kmsKey?: string | Computed<string>;
  resourcePolicies?: string[] | Computed<string[]>;
}

export interface NotebooksInstance_GceSetup_GpuDriverConfig {
  /** Optional. Specify a custom Cloud Storage path where the GPU driver is stored. If not specified, we'll automatically choose from official GPU drivers. */
  customGpuDriverPath?: string | Computed<string>;
  /** Optional. Whether the end user authorizes Google Cloud to install GPU driver on this VM instance. If this field is empty or set to false, the GPU driver won't be installed. Only applicable to instances with GPUs. */
  enableGpuDriver?: boolean | Computed<boolean>;
}

export interface NotebooksInstance_GceSetup_NetworkInterfaces_AccessConfigs {
  externalIp?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup_NetworkInterfaces {
  accessConfigs?: NotebooksInstance_GceSetup_NetworkInterfaces_AccessConfigs[] | Computed<NotebooksInstance_GceSetup_NetworkInterfaces_AccessConfigs[]>;
  network?: string | Computed<string>;
  nicType?: string | Computed<string>;
  subnet?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup_ReservationAffinity {
  /** Required. Specifies the type of reservation from which this instance can consume resources: RESERVATION_ANY (default), RESERVATION_SPECIFIC, or RESERVATION_NONE. See Consuming reserved instances for examples. */
  consumeReservationType?: string | Computed<string>;
  /** Optional. Corresponds to the label key of a reservation resource. To target a RESERVATION_SPECIFIC by name, use compute.googleapis.com/reservation-name as the key and specify the name of your reservation as its value. */
  key?: string | Computed<string>;
  /** Optional. Corresponds to the label values of a reservation resource. This can be either a name to a reservation in the same project or "projects/different-project/reservations/some-reservation-name" to target a shared reservation in the same zone but in a different project. */
  values?: string[] | Computed<string[]>;
}

export interface NotebooksInstance_GceSetup_ServiceAccounts {
  email?: string | Computed<string>;
  scopes?: string[] | Computed<string[]>;
}

export interface NotebooksInstance_GceSetup_ShieldedInstanceConfig {
  /** Optional. Defines whether the VM instance has integrity monitoring enabled. Enables monitoring and attestation of the boot integrity of the VM instance. The attestation is performed against the integrity policy baseline. This baseline is initially derived from the implicitly trusted boot image when the VM instance is created. */
  enableIntegrityMonitoring?: boolean | Computed<boolean>;
  /** Optional. Defines whether the VM instance has Secure Boot enabled. Secure Boot helps ensure that the system only runs authentic software by verifying the digital signature of all boot components, and halting the boot process if signature verification fails. Disabled by default. */
  enableSecureBoot?: boolean | Computed<boolean>;
  /** Optional. Defines whether the VM instance has the vTPM enabled. */
  enableVtpm?: boolean | Computed<boolean>;
}

export interface NotebooksInstance_GceSetup_VmImage {
  /** Optional. Use this VM image family to find the image; the newest image in this family will be used. */
  family?: string | Computed<string>;
  /** Output only. A human-readable description of the image running on the instance (for example, "Debian 11, Python 3.10"), derived at read time from the image release configuration (the source of truth). Set to "Custom" for unrecognized boot-disk images. */
  imageDescription?: string | Computed<string>;
  /** Optional. Use VM image name to find the image. */
  name?: string | Computed<string>;
  /** Required. The name of the Google Cloud project that this VM image belongs to. Format: `{project_id}` */
  project?: string | Computed<string>;
}

export interface NotebooksInstance_GceSetup {
  /** Optional. The hardware accelerators used on this instance. If you use accelerators, make sure that your configuration has [enough vCPUs and memory to support the `machine_type` you have selected](https://cloud.google.com/compute/docs/gpus/#gpus-list). Currently supports only one accelerator configuration. */
  acceleratorConfigs?: NotebooksInstance_GceSetup_AcceleratorConfigs[] | Computed<NotebooksInstance_GceSetup_AcceleratorConfigs[]>;
  /** The definition of a boot disk. */
  bootDisk?: NotebooksInstance_GceSetup_BootDisk | Computed<NotebooksInstance_GceSetup_BootDisk>;
  /** A set of Confidential Instance options. */
  confidentialInstanceConfig?: NotebooksInstance_GceSetup_ConfidentialInstanceConfig | Computed<NotebooksInstance_GceSetup_ConfidentialInstanceConfig>;
  /** Definition of a container image for starting a notebook instance with the environment installed in a container. */
  containerImage?: NotebooksInstance_GceSetup_ContainerImage | Computed<NotebooksInstance_GceSetup_ContainerImage>;
  /** Optional. Data disks attached to the VM instance. Currently supports only one data disk. */
  dataDisks?: NotebooksInstance_GceSetup_DataDisks[] | Computed<NotebooksInstance_GceSetup_DataDisks[]>;
  /** Optional. If true, no external IP will be assigned to this VM instance. */
  disablePublicIp?: boolean | Computed<boolean>;
  /** Optional. Flag to enable ip forwarding or not, default false/off. https://cloud.google.com/vpc/docs/using-routes#canipforward */
  enableIpForwarding?: boolean | Computed<boolean>;
  /** A GPU driver configuration */
  gpuDriverConfig?: NotebooksInstance_GceSetup_GpuDriverConfig | Computed<NotebooksInstance_GceSetup_GpuDriverConfig>;
  /** Output only. The unique ID of the Compute Engine instance resource. */
  instanceId?: string | Computed<string>;
  /** Optional. The machine type of the VM instance. https://cloud.google.com/compute/docs/machine-resource */
  machineType?: string | Computed<string>;
  /** Optional. Custom metadata to apply to this instance. */
  metadata?: Record<string, string> | Computed<Record<string, string>>;
  /** Optional. The minimum CPU platform to use for this instance. The list of valid values can be found in https://cloud.google.com/compute/docs/instances/specify-min-cpu-platform#availablezones */
  minCpuPlatform?: string | Computed<string>;
  /** Optional. The network interfaces for the VM. Supports only one interface. */
  networkInterfaces?: NotebooksInstance_GceSetup_NetworkInterfaces[] | Computed<NotebooksInstance_GceSetup_NetworkInterfaces[]>;
  /** A reservation that an instance can consume from. */
  reservationAffinity?: NotebooksInstance_GceSetup_ReservationAffinity | Computed<NotebooksInstance_GceSetup_ReservationAffinity>;
  /** Optional. The service account that serves as an identity for the VM instance. Currently supports only one service account. */
  serviceAccounts?: NotebooksInstance_GceSetup_ServiceAccounts[] | Computed<NotebooksInstance_GceSetup_ServiceAccounts[]>;
  /** A set of Shielded Instance options. See [Images using supported Shielded VM features](https://cloud.google.com/compute/docs/instances/modifying-shielded-vm). Not all combinations are valid. */
  shieldedInstanceConfig?: NotebooksInstance_GceSetup_ShieldedInstanceConfig | Computed<NotebooksInstance_GceSetup_ShieldedInstanceConfig>;
  /** Optional. The Compute Engine network tags to add to runtime (see [Add network tags](https://cloud.google.com/vpc/docs/add-remove-network-tags)). */
  tags?: string[] | Computed<string[]>;
  /** Definition of a custom Compute Engine virtual machine image for starting a notebook instance with the environment installed directly on the VM. */
  vmImage?: NotebooksInstance_GceSetup_VmImage | Computed<NotebooksInstance_GceSetup_VmImage>;
}

export interface NotebooksInstance_UpgradeHistory {
  action?: string | Computed<string>;
  containerImage?: string | Computed<string>;
  createTime?: string | Computed<string>;
  framework?: string | Computed<string>;
  snapshot?: string | Computed<string>;
  state?: string | Computed<string>;
  targetVersion?: string | Computed<string>;
  version?: string | Computed<string>;
  vmImage?: string | Computed<string>;
}

const NotebooksInstance_GceSetup_AcceleratorConfigsFields: FieldMap = {
  coreCount: "core_count",
  type: "type",
};

const NotebooksInstance_GceSetup_BootDiskFields: FieldMap = {
  diskEncryption: "disk_encryption",
  diskSizeGb: "disk_size_gb",
  diskType: "disk_type",
  kmsKey: "kms_key",
};

const NotebooksInstance_GceSetup_ConfidentialInstanceConfigFields: FieldMap = {
  confidentialInstanceType: "confidential_instance_type",
};

const NotebooksInstance_GceSetup_ContainerImageFields: FieldMap = {
  repository: "repository",
  tag: "tag",
};

const NotebooksInstance_GceSetup_DataDisksFields: FieldMap = {
  diskEncryption: "disk_encryption",
  diskSizeGb: "disk_size_gb",
  diskType: "disk_type",
  kmsKey: "kms_key",
  resourcePolicies: "resource_policies",
};

const NotebooksInstance_GceSetup_GpuDriverConfigFields: FieldMap = {
  customGpuDriverPath: "custom_gpu_driver_path",
  enableGpuDriver: "enable_gpu_driver",
};

const NotebooksInstance_GceSetup_NetworkInterfaces_AccessConfigsFields: FieldMap = {
  externalIp: "external_ip",
};

const NotebooksInstance_GceSetup_NetworkInterfacesFields: FieldMap = {
  accessConfigs: {
    wireName: "access_configs",
    kind: "list",
    fields: NotebooksInstance_GceSetup_NetworkInterfaces_AccessConfigsFields,
  },
  network: "network",
  nicType: "nic_type",
  subnet: "subnet",
};

const NotebooksInstance_GceSetup_ReservationAffinityFields: FieldMap = {
  consumeReservationType: "consume_reservation_type",
  key: "key",
  values: "values",
};

const NotebooksInstance_GceSetup_ServiceAccountsFields: FieldMap = {
  email: "email",
  scopes: "scopes",
};

const NotebooksInstance_GceSetup_ShieldedInstanceConfigFields: FieldMap = {
  enableIntegrityMonitoring: "enable_integrity_monitoring",
  enableSecureBoot: "enable_secure_boot",
  enableVtpm: "enable_vtpm",
};

const NotebooksInstance_GceSetup_VmImageFields: FieldMap = {
  family: "family",
  imageDescription: "image_description",
  name: "name",
  project: "project",
};

const NotebooksInstance_GceSetupFields: FieldMap = {
  acceleratorConfigs: {
    wireName: "accelerator_configs",
    kind: "list",
    fields: NotebooksInstance_GceSetup_AcceleratorConfigsFields,
  },
  bootDisk: {
    wireName: "boot_disk",
    kind: "object",
    fields: NotebooksInstance_GceSetup_BootDiskFields,
  },
  confidentialInstanceConfig: {
    wireName: "confidential_instance_config",
    kind: "object",
    fields: NotebooksInstance_GceSetup_ConfidentialInstanceConfigFields,
  },
  containerImage: {
    wireName: "container_image",
    kind: "object",
    fields: NotebooksInstance_GceSetup_ContainerImageFields,
  },
  dataDisks: {
    wireName: "data_disks",
    kind: "list",
    fields: NotebooksInstance_GceSetup_DataDisksFields,
  },
  disablePublicIp: "disable_public_ip",
  enableIpForwarding: "enable_ip_forwarding",
  gpuDriverConfig: {
    wireName: "gpu_driver_config",
    kind: "object",
    fields: NotebooksInstance_GceSetup_GpuDriverConfigFields,
  },
  instanceId: "instance_id",
  machineType: "machine_type",
  metadata: "metadata",
  minCpuPlatform: "min_cpu_platform",
  networkInterfaces: {
    wireName: "network_interfaces",
    kind: "list",
    fields: NotebooksInstance_GceSetup_NetworkInterfacesFields,
  },
  reservationAffinity: {
    wireName: "reservation_affinity",
    kind: "object",
    fields: NotebooksInstance_GceSetup_ReservationAffinityFields,
  },
  serviceAccounts: {
    wireName: "service_accounts",
    kind: "list",
    fields: NotebooksInstance_GceSetup_ServiceAccountsFields,
  },
  shieldedInstanceConfig: {
    wireName: "shielded_instance_config",
    kind: "object",
    fields: NotebooksInstance_GceSetup_ShieldedInstanceConfigFields,
  },
  tags: "tags",
  vmImage: {
    wireName: "vm_image",
    kind: "object",
    fields: NotebooksInstance_GceSetup_VmImageFields,
  },
};

const NotebooksInstance_UpgradeHistoryFields: FieldMap = {
  action: "action",
  containerImage: "container_image",
  createTime: "create_time",
  framework: "framework",
  snapshot: "snapshot",
  state: "state",
  targetVersion: "target_version",
  version: "version",
  vmImage: "vm_image",
};

export interface NotebooksInstanceConfig {
  /** Output only. Instance creation time. */
  createTime?: string | Computed<string>;
  /** Output only. Email address of entity that sent original CreateInstance request. */
  creator?: string | Computed<string>;
  /** Optional. If true, the notebook instance will not register with the proxy. */
  disableProxyAccess?: boolean | Computed<boolean>;
  /** Optional. If true, deletion protection will be enabled for this Workbench Instance. If false, deletion protection will be disabled for this Workbench Instance. */
  enableDeletionProtection?: boolean | Computed<boolean>;
  /** Optional. Flag to enable managed end user credentials for the instance. */
  enableManagedEuc?: boolean | Computed<boolean>;
  /** Optional. Flag that specifies that a notebook can be accessed with third party identity provider. */
  enableThirdPartyIdentity?: boolean | Computed<boolean>;
  /** The definition of how to configure a VM instance outside of Resources and Identity. */
  gceSetup?: NotebooksInstance_GceSetup | Computed<NotebooksInstance_GceSetup>;
  /** Output only. Additional information about instance health. Example: healthInfo": { "docker_proxy_agent_status": "1", "docker_status": "1", "jupyterlab_api_status": "-1", "jupyterlab_status": "-1", "updated": "2020-10-18 09:40:03.573409" } */
  healthInfo?: Record<string, string> | Computed<Record<string, string>>;
  /** Output only. Instance health_state. */
  healthState?: string | Computed<string>;
  /** Output only. Unique ID of the resource. */
  id?: string | Computed<string>;
  /** Optional. The owner of this instance after creation. Format: `alias@example.com` Currently supports one owner only. If not specified, all of the service account users of your VM instance's service account can use the instance. */
  instanceOwners?: string[] | Computed<string[]>;
  /** Optional. Labels to apply to this instance. These can be later modified by the UpdateInstance method. */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** Output only. Identifier. The name of this notebook instance. Format: `projects/{project_id}/locations/{location}/instances/{instance_id}` */
  name?: string | Computed<string>;
  /** Output only. The proxy endpoint that is used to access the Jupyter notebook. */
  proxyUri?: string | Computed<string>;
  /** Output only. Reserved for future use for Zone Isolation. */
  satisfiesPzi?: boolean | Computed<boolean>;
  /** Output only. Reserved for future use for Zone Separation. */
  satisfiesPzs?: boolean | Computed<boolean>;
  /** Output only. The state of this instance. */
  state?: string | Computed<string>;
  /** Output only. The workforce pools proxy endpoint that is used to access the Jupyter notebook. */
  thirdPartyProxyUrl?: string | Computed<string>;
  /** Output only. Instance update time. */
  updateTime?: string | Computed<string>;
  /** Output only. The upgrade history of this instance. */
  upgradeHistory?: NotebooksInstance_UpgradeHistory[] | Computed<NotebooksInstance_UpgradeHistory[]>;
}

export interface NotebooksInstanceAttrs {
  /** Output only. Instance creation time. */
  createTime: string;
  /** Output only. Email address of entity that sent original CreateInstance request. */
  creator: string;
  /** Optional. If true, the notebook instance will not register with the proxy. */
  disableProxyAccess: boolean;
  /** Optional. If true, deletion protection will be enabled for this Workbench Instance. If false, deletion protection will be disabled for this Workbench Instance. */
  enableDeletionProtection: boolean;
  /** Optional. Flag to enable managed end user credentials for the instance. */
  enableManagedEuc: boolean;
  /** Optional. Flag that specifies that a notebook can be accessed with third party identity provider. */
  enableThirdPartyIdentity: boolean;
  /** The definition of how to configure a VM instance outside of Resources and Identity. */
  gceSetup: NotebooksInstance_GceSetup;
  /** Output only. Additional information about instance health. Example: healthInfo": { "docker_proxy_agent_status": "1", "docker_status": "1", "jupyterlab_api_status": "-1", "jupyterlab_status": "-1", "updated": "2020-10-18 09:40:03.573409" } */
  healthInfo: Record<string, string>;
  /** Output only. Instance health_state. */
  healthState: string;
  /** Output only. Unique ID of the resource. */
  id: string;
  /** Optional. The owner of this instance after creation. Format: `alias@example.com` Currently supports one owner only. If not specified, all of the service account users of your VM instance's service account can use the instance. */
  instanceOwners: string[];
  /** Optional. Labels to apply to this instance. These can be later modified by the UpdateInstance method. */
  labels: Record<string, string>;
  /** Output only. Identifier. The name of this notebook instance. Format: `projects/{project_id}/locations/{location}/instances/{instance_id}` */
  name: string;
  /** Output only. The proxy endpoint that is used to access the Jupyter notebook. */
  proxyUri: string;
  /** Output only. Reserved for future use for Zone Isolation. */
  satisfiesPzi: boolean;
  /** Output only. Reserved for future use for Zone Separation. */
  satisfiesPzs: boolean;
  /** Output only. The state of this instance. */
  state: string;
  /** Output only. The workforce pools proxy endpoint that is used to access the Jupyter notebook. */
  thirdPartyProxyUrl: string;
  /** Output only. Instance update time. */
  updateTime: string;
  /** Output only. The upgrade history of this instance. */
  upgradeHistory: NotebooksInstance_UpgradeHistory[];
}

export const NotebooksInstance: ResourceBinding<NotebooksInstanceConfig, NotebooksInstanceAttrs> = {
  wireType: "google_notebooks_notebooks_instance",
  fields: {
    createTime: "create_time",
    creator: "creator",
    disableProxyAccess: "disable_proxy_access",
    enableDeletionProtection: "enable_deletion_protection",
    enableManagedEuc: "enable_managed_euc",
    enableThirdPartyIdentity: "enable_third_party_identity",
    gceSetup: {
      wireName: "gce_setup",
      kind: "object",
      fields: NotebooksInstance_GceSetupFields,
    },
    healthInfo: "health_info",
    healthState: "health_state",
    id: "id",
    instanceOwners: "instance_owners",
    labels: "labels",
    name: "name",
    proxyUri: "proxy_uri",
    satisfiesPzi: "satisfies_pzi",
    satisfiesPzs: "satisfies_pzs",
    state: "state",
    thirdPartyProxyUrl: "third_party_proxy_url",
    updateTime: "update_time",
    upgradeHistory: {
      wireName: "upgrade_history",
      kind: "list",
      fields: NotebooksInstance_UpgradeHistoryFields,
    },
  },
};
