// Code generated by `ubx sdk gen` from google_biglake_biglake_database. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface BiglakeDatabase_HiveOptions {
  /** Cloud Storage folder URI where the database data is stored, starting with "gs://". */
  locationUri?: string | Computed<string>;
  /** Stores user supplied Hive database parameters. */
  parameters?: Record<string, string> | Computed<Record<string, string>>;
}

const BiglakeDatabase_HiveOptionsFields: FieldMap = {
  locationUri: "location_uri",
  parameters: "parameters",
};

export interface BiglakeDatabaseConfig {
  /** Output only. The creation time of the database. */
  createTime?: string | Computed<string>;
  /** Output only. The deletion time of the database. Only set after the database is deleted. */
  deleteTime?: string | Computed<string>;
  /** Output only. The time when this database is considered expired. Only set after the database is deleted. */
  expireTime?: string | Computed<string>;
  /** Options of a Hive database. */
  hiveOptions?: BiglakeDatabase_HiveOptions | Computed<BiglakeDatabase_HiveOptions>;
  /** Output only. The resource name. Format: projects/{project_id_or_number}/locations/{location_id}/catalogs/{catalog_id}/databases/{database_id} */
  name?: string | Computed<string>;
  /** The database type. */
  type?: string | Computed<string>;
  /** Output only. The last modification time of the database. */
  updateTime?: string | Computed<string>;
}

export interface BiglakeDatabaseAttrs {
  /** Output only. The creation time of the database. */
  createTime: string;
  /** Output only. The deletion time of the database. Only set after the database is deleted. */
  deleteTime: string;
  /** Output only. The time when this database is considered expired. Only set after the database is deleted. */
  expireTime: string;
  /** Options of a Hive database. */
  hiveOptions: BiglakeDatabase_HiveOptions;
  /** Output only. The resource name. Format: projects/{project_id_or_number}/locations/{location_id}/catalogs/{catalog_id}/databases/{database_id} */
  name: string;
  /** The database type. */
  type: string;
  /** Output only. The last modification time of the database. */
  updateTime: string;
}

export const BiglakeDatabase: ResourceBinding<BiglakeDatabaseConfig, BiglakeDatabaseAttrs> = {
  wireType: "google_biglake_biglake_database",
  fields: {
    createTime: "create_time",
    deleteTime: "delete_time",
    expireTime: "expire_time",
    hiveOptions: {
      wireName: "hive_options",
      kind: "object",
      fields: BiglakeDatabase_HiveOptionsFields,
    },
    name: "name",
    type: "type",
    updateTime: "update_time",
  },
};
