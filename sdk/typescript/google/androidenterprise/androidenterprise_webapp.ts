// Code generated by `ubx sdk gen` from google_androidenterprise_androidenterprise_webapp. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface AndroidenterpriseWebapp_Icons {
  imageData?: string | Computed<string>;
}

const AndroidenterpriseWebapp_IconsFields: FieldMap = {
  imageData: "image_data",
};

export interface AndroidenterpriseWebappConfig {
  /** The display mode of the web app. Possible values include: - "minimalUi", the device's status bar, navigation bar, the app's URL, and a refresh button are visible when the app is open. For HTTP URLs, you can only select this option. - "standalone", the device's status bar and navigation bar are visible when the app is open. - "fullScreen", the app opens in full screen mode, hiding the device's status and navigation bars. All browser UI elements, page URL, system status bar and back button are not visible, and the web app takes up the entirety of the available display area. */
  displayMode?: string | Computed<string>;
  /** A list of icons representing this website. If absent, a default icon (for create) or the current icon (for update) will be used. */
  icons?: AndroidenterpriseWebapp_Icons[] | Computed<AndroidenterpriseWebapp_Icons[]>;
  /** A flag whether the app has been published to the Play store yet. */
  isPublished?: boolean | Computed<boolean>;
  /** The start URL, i.e. the URL that should load when the user opens the application. */
  startUrl?: string | Computed<string>;
  /** The title of the web app as displayed to the user (e.g., amongst a list of other applications, or as a label for an icon). */
  title?: string | Computed<string>;
  /** The current version of the app. Note that the version can automatically increase during the lifetime of the web app, while Google does internal housekeeping to keep the web app up-to-date. */
  versionCode?: string | Computed<string>;
  /** The ID of the application. A string of the form "app:<package name>" where the package name always starts with the prefix "com.google.enterprise.webapp." followed by a random id. */
  webAppId?: string | Computed<string>;
}

export interface AndroidenterpriseWebappAttrs {
  /** The display mode of the web app. Possible values include: - "minimalUi", the device's status bar, navigation bar, the app's URL, and a refresh button are visible when the app is open. For HTTP URLs, you can only select this option. - "standalone", the device's status bar and navigation bar are visible when the app is open. - "fullScreen", the app opens in full screen mode, hiding the device's status and navigation bars. All browser UI elements, page URL, system status bar and back button are not visible, and the web app takes up the entirety of the available display area. */
  displayMode: string;
  /** A list of icons representing this website. If absent, a default icon (for create) or the current icon (for update) will be used. */
  icons: AndroidenterpriseWebapp_Icons[];
  /** A flag whether the app has been published to the Play store yet. */
  isPublished: boolean;
  /** The start URL, i.e. the URL that should load when the user opens the application. */
  startUrl: string;
  /** The title of the web app as displayed to the user (e.g., amongst a list of other applications, or as a label for an icon). */
  title: string;
  /** The current version of the app. Note that the version can automatically increase during the lifetime of the web app, while Google does internal housekeeping to keep the web app up-to-date. */
  versionCode: string;
  /** The ID of the application. A string of the form "app:<package name>" where the package name always starts with the prefix "com.google.enterprise.webapp." followed by a random id. */
  webAppId: string;
}

export const AndroidenterpriseWebapp: ResourceBinding<AndroidenterpriseWebappConfig, AndroidenterpriseWebappAttrs> = {
  wireType: "google_androidenterprise_androidenterprise_webapp",
  fields: {
    displayMode: "display_mode",
    icons: {
      wireName: "icons",
      kind: "list",
      fields: AndroidenterpriseWebapp_IconsFields,
    },
    isPublished: "is_published",
    startUrl: "start_url",
    title: "title",
    versionCode: "version_code",
    webAppId: "web_app_id",
  },
};
