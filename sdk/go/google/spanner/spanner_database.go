...
type SpannerDatabase_EncryptionConfig struct {
...
type SpannerDatabase_EncryptionInfo_EncryptionStatus struct {
	Code any
	Details any
	Message any
}

type SpannerDatabase_EncryptionInfo struct {
	EncryptionStatus any
	EncryptionType any
	KmsKeyVersion any
}

type SpannerDatabase_QuorumInfo_QuorumType_SingleRegion struct {
...
type SpannerDatabase_QuorumInfo_QuorumType struct {
...
type SpannerDatabase_QuorumInfo struct {
...
type SpannerDatabase_RestoreInfo_BackupInfo struct {
	// Name of the backup.
	Backup any
...
type SpannerDatabase_RestoreInfo struct {
...
var SpannerDatabase_EncryptionConfigFields = ubx.FieldMap{
...
type SpannerDatabaseAttrs struct {
	// Required. A `CREATE DATABASE` statement, which specifies the ID of the new database. The database ID must conform to the regular expression `a-z*[a-z0-9]` and be between 2 and 30 characters in length. If the database ID is a reserved word or if it contains a hyphen, the database ID must be enclosed in backticks (`` ` ``).
	CreateStatement any
...
var SpannerDatabase = ubx.ResourceBinding{
