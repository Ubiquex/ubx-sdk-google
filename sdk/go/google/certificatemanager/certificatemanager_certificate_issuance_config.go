// Code generated by `ubx sdk gen` from google_certificatemanager_certificatemanager_certificate_issuance_config. DO NOT EDIT.
package certificatemanager

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type CertificatemanagerCertificateIssuanceConfig_CertificateAuthorityConfig_CertificateAuthorityServiceConfig struct {
	// Required. A CA pool resource used to issue a certificate. The CA pool string has a relative resource path following the form "projects/{project}/locations/{location}/caPools/{ca_pool}".
	CaPool any
}

type CertificatemanagerCertificateIssuanceConfig_CertificateAuthorityConfig struct {
	// Contains information required to contact CA service.
	CertificateAuthorityServiceConfig any
}

var CertificatemanagerCertificateIssuanceConfig_CertificateAuthorityConfig_CertificateAuthorityServiceConfigFields = ubx.FieldMap{
		"CaPool": ubx.FieldSpec{WireName: "ca_pool"},
	}

var CertificatemanagerCertificateIssuanceConfig_CertificateAuthorityConfigFields = ubx.FieldMap{
		"CertificateAuthorityServiceConfig": ubx.FieldSpec{
			WireName: "certificate_authority_service_config",
			Kind: "object",
			Fields: CertificatemanagerCertificateIssuanceConfig_CertificateAuthorityConfig_CertificateAuthorityServiceConfigFields,
		},
	}

type CertificatemanagerCertificateIssuanceConfigConfig struct {
	// The CA that issues the workload certificate. It includes CA address, type, authentication to CA service, etc.
	CertificateAuthorityConfig any
	// Output only. The creation timestamp of a CertificateIssuanceConfig.
	CreateTime any
	// Optional. One or more paragraphs of text description of a CertificateIssuanceConfig.
	Description any
	// Required. The key algorithm to use when generating the private key.
	KeyAlgorithm any
	// Optional. Set of labels associated with a CertificateIssuanceConfig.
	Labels any
	// Required. Workload certificate lifetime requested.
	Lifetime any
	// Identifier. A user-defined name of the certificate issuance config. CertificateIssuanceConfig names must be unique globally and match pattern `projects/*/locations/*/certificateIssuanceConfigs/*`.
	Name any
	// Required. Specifies the percentage of elapsed time of the certificate lifetime to wait before renewing the certificate. Must be a number between 1-99, inclusive.
	RotationWindowPercentage any
	// Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing"
	Tags any
	// Output only. The last update timestamp of a CertificateIssuanceConfig.
	UpdateTime any
}

type CertificatemanagerCertificateIssuanceConfigAttrs struct {
	// The CA that issues the workload certificate. It includes CA address, type, authentication to CA service, etc.
	CertificateAuthorityConfig any
	// Output only. The creation timestamp of a CertificateIssuanceConfig.
	CreateTime any
	// Optional. One or more paragraphs of text description of a CertificateIssuanceConfig.
	Description any
	// Required. The key algorithm to use when generating the private key.
	KeyAlgorithm any
	// Optional. Set of labels associated with a CertificateIssuanceConfig.
	Labels any
	// Required. Workload certificate lifetime requested.
	Lifetime any
	// Identifier. A user-defined name of the certificate issuance config. CertificateIssuanceConfig names must be unique globally and match pattern `projects/*/locations/*/certificateIssuanceConfigs/*`.
	Name any
	// Required. Specifies the percentage of elapsed time of the certificate lifetime to wait before renewing the certificate. Must be a number between 1-99, inclusive.
	RotationWindowPercentage any
	// Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing"
	Tags any
	// Output only. The last update timestamp of a CertificateIssuanceConfig.
	UpdateTime any
}

var CertificatemanagerCertificateIssuanceConfig = ubx.ResourceBinding{
	WireType: "google_certificatemanager_certificatemanager_certificate_issuance_config",
	Fields: ubx.FieldMap{
		"CertificateAuthorityConfig": ubx.FieldSpec{
			WireName: "certificate_authority_config",
			Kind: "object",
			Fields: CertificatemanagerCertificateIssuanceConfig_CertificateAuthorityConfigFields,
		},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"KeyAlgorithm": ubx.FieldSpec{WireName: "key_algorithm"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Lifetime": ubx.FieldSpec{WireName: "lifetime"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"RotationWindowPercentage": ubx.FieldSpec{WireName: "rotation_window_percentage"},
		"Tags": ubx.FieldSpec{WireName: "tags"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
