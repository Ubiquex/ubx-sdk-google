// Code generated by `ubx sdk gen` from google_certificatemanager_certificatemanager_certificate. DO NOT EDIT.
package certificatemanager

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting_Cname struct {
	ExpectedData any
	Name any
	ResolvedData any
}

type CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting_Ips struct {
	Resolved any
	Serving any
	ServingOnAltPorts any
}

type CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting struct {
	Cname any
	Ips any
	Issues any
}

type CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo struct {
	AttemptTime any
	Details any
	Domain any
	FailureReason any
	State any
	Troubleshooting any
}

type CertificatemanagerCertificate_Managed_ProvisioningIssue struct {
	// Output only. Human readable explanation about the issue. Provided to help address the configuration issues. Not guaranteed to be stable. For programmatic access use Reason enum.
	Details any
	// Output only. Reason for provisioning failures.
	Reason any
}

type CertificatemanagerCertificate_Managed struct {
	// Output only. Detailed state of the latest authorization attempt for each domain specified for managed certificate resource.
	AuthorizationAttemptInfo any
	// Optional. Immutable. Authorizations that will be used for performing domain authorization.
	DnsAuthorizations any
	// Optional. Immutable. The domains for which a managed SSL certificate will be generated. Wildcard domains are only supported with DNS challenge resolution.
	Domains any
	// Optional. Immutable. The resource name for a CertificateIssuanceConfig used to configure private PKI certificates in the format `projects/*/locations/*/certificateIssuanceConfigs/*`. If this field is not set, the certificates will instead be publicly signed as documented at https://cloud.google.com/load-balancing/docs/ssl-certificates/google-managed-certs#caa.
	IssuanceConfig any
	// Information about issues with provisioning a Managed Certificate.
	ProvisioningIssue any
	// Output only. State of the managed certificate resource.
	State any
}

type CertificatemanagerCertificate_ManagedIdentity struct {
	// Required. Immutable. SPIFFE ID of the Managed Identity used for this certificate.
	Identity any
	// Information about issues with provisioning a Managed Certificate.
	ProvisioningIssue any
	// Output only. State of the managed certificate resource.
	State any
}

type CertificatemanagerCertificate_SelfManaged struct {
	// Optional. Input only. The PEM-encoded certificate chain. Leaf certificate comes first, followed by intermediate ones if any.
	PemCertificate any
	// Optional. Input only. The PEM-encoded private key of the leaf certificate.
	PemPrivateKey any
}

type CertificatemanagerCertificate_UsedBy struct {
	Name any
}

var CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting_CnameFields = ubx.FieldMap{
		"ExpectedData": ubx.FieldSpec{WireName: "expected_data"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"ResolvedData": ubx.FieldSpec{WireName: "resolved_data"},
	}

var CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting_IpsFields = ubx.FieldMap{
		"Resolved": ubx.FieldSpec{WireName: "resolved"},
		"Serving": ubx.FieldSpec{WireName: "serving"},
		"ServingOnAltPorts": ubx.FieldSpec{WireName: "serving_on_alt_ports"},
	}

var CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_TroubleshootingFields = ubx.FieldMap{
		"Cname": ubx.FieldSpec{
			WireName: "cname",
			Kind: "object",
			Fields: CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting_CnameFields,
		},
		"Ips": ubx.FieldSpec{
			WireName: "ips",
			Kind: "object",
			Fields: CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_Troubleshooting_IpsFields,
		},
		"Issues": ubx.FieldSpec{WireName: "issues"},
	}

var CertificatemanagerCertificate_Managed_AuthorizationAttemptInfoFields = ubx.FieldMap{
		"AttemptTime": ubx.FieldSpec{WireName: "attempt_time"},
		"Details": ubx.FieldSpec{WireName: "details"},
		"Domain": ubx.FieldSpec{WireName: "domain"},
		"FailureReason": ubx.FieldSpec{WireName: "failure_reason"},
		"State": ubx.FieldSpec{WireName: "state"},
		"Troubleshooting": ubx.FieldSpec{
			WireName: "troubleshooting",
			Kind: "object",
			Fields: CertificatemanagerCertificate_Managed_AuthorizationAttemptInfo_TroubleshootingFields,
		},
	}

var CertificatemanagerCertificate_Managed_ProvisioningIssueFields = ubx.FieldMap{
		"Details": ubx.FieldSpec{WireName: "details"},
		"Reason": ubx.FieldSpec{WireName: "reason"},
	}

var CertificatemanagerCertificate_ManagedFields = ubx.FieldMap{
		"AuthorizationAttemptInfo": ubx.FieldSpec{
			WireName: "authorization_attempt_info",
			Kind: "list",
			Fields: CertificatemanagerCertificate_Managed_AuthorizationAttemptInfoFields,
		},
		"DnsAuthorizations": ubx.FieldSpec{WireName: "dns_authorizations"},
		"Domains": ubx.FieldSpec{WireName: "domains"},
		"IssuanceConfig": ubx.FieldSpec{WireName: "issuance_config"},
		"ProvisioningIssue": ubx.FieldSpec{
			WireName: "provisioning_issue",
			Kind: "object",
			Fields: CertificatemanagerCertificate_Managed_ProvisioningIssueFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
	}

var CertificatemanagerCertificate_ManagedIdentityFields = ubx.FieldMap{
		"Identity": ubx.FieldSpec{WireName: "identity"},
		"ProvisioningIssue": ubx.FieldSpec{
			WireName: "provisioning_issue",
			Kind: "object",
			Fields: CertificatemanagerCertificate_Managed_ProvisioningIssueFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
	}

var CertificatemanagerCertificate_SelfManagedFields = ubx.FieldMap{
		"PemCertificate": ubx.FieldSpec{WireName: "pem_certificate"},
		"PemPrivateKey": ubx.FieldSpec{WireName: "pem_private_key"},
	}

var CertificatemanagerCertificate_UsedByFields = ubx.FieldMap{
		"Name": ubx.FieldSpec{WireName: "name"},
	}

type CertificatemanagerCertificateConfig struct {
	// Output only. The creation timestamp of a Certificate.
	CreateTime any
	// Optional. One or more paragraphs of text description of a certificate.
	Description any
	// Output only. The expiry timestamp of a Certificate.
	ExpireTime any
	// Optional. Set of labels associated with a Certificate.
	Labels any
	// Configuration and state of a Managed Certificate. Certificate Manager provisions and renews Managed Certificates automatically, for as long as it's authorized to do so.
	Managed any
	// Configuration and state of a Managed Identity Certificate. Certificate Manager provisions and renews Managed Identity Certificates automatically, for as long as it's authorized to do so.
	ManagedIdentity any
	// Identifier. A user-defined name of the certificate. Certificate names must be unique globally and match pattern `projects/*/locations/*/certificates/*`.
	Name any
	// Output only. The PEM-encoded certificate chain.
	PemCertificate any
	// Output only. The list of Subject Alternative Names of dnsName type defined in the certificate (see RFC 5280 4.2.1.6). Managed certificates that haven't been provisioned yet have this field populated with a value of the managed.domains field.
	SanDnsnames any
	// Optional. Immutable. The scope of the certificate.
	Scope any
	// Certificate data for a SelfManaged Certificate. SelfManaged Certificates are uploaded by the user. Updating such certificates before they expire remains the user's responsibility.
	SelfManaged any
	// Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing"
	Tags any
	// Output only. The last update timestamp of a Certificate.
	UpdateTime any
	// Output only. The list of resources that use this Certificate.
	UsedBy any
}

type CertificatemanagerCertificateAttrs struct {
	// Output only. The creation timestamp of a Certificate.
	CreateTime any
	// Optional. One or more paragraphs of text description of a certificate.
	Description any
	// Output only. The expiry timestamp of a Certificate.
	ExpireTime any
	// Optional. Set of labels associated with a Certificate.
	Labels any
	// Configuration and state of a Managed Certificate. Certificate Manager provisions and renews Managed Certificates automatically, for as long as it's authorized to do so.
	Managed any
	// Configuration and state of a Managed Identity Certificate. Certificate Manager provisions and renews Managed Identity Certificates automatically, for as long as it's authorized to do so.
	ManagedIdentity any
	// Identifier. A user-defined name of the certificate. Certificate names must be unique globally and match pattern `projects/*/locations/*/certificates/*`.
	Name any
	// Output only. The PEM-encoded certificate chain.
	PemCertificate any
	// Output only. The list of Subject Alternative Names of dnsName type defined in the certificate (see RFC 5280 4.2.1.6). Managed certificates that haven't been provisioned yet have this field populated with a value of the managed.domains field.
	SanDnsnames any
	// Optional. Immutable. The scope of the certificate.
	Scope any
	// Certificate data for a SelfManaged Certificate. SelfManaged Certificates are uploaded by the user. Updating such certificates before they expire remains the user's responsibility.
	SelfManaged any
	// Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: "123/environment": "production", "123/costCenter": "marketing"
	Tags any
	// Output only. The last update timestamp of a Certificate.
	UpdateTime any
	// Output only. The list of resources that use this Certificate.
	UsedBy any
}

var CertificatemanagerCertificate = ubx.ResourceBinding{
	WireType: "google_certificatemanager_certificatemanager_certificate",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"ExpireTime": ubx.FieldSpec{WireName: "expire_time"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Managed": ubx.FieldSpec{
			WireName: "managed",
			Kind: "object",
			Fields: CertificatemanagerCertificate_ManagedFields,
		},
		"ManagedIdentity": ubx.FieldSpec{
			WireName: "managed_identity",
			Kind: "object",
			Fields: CertificatemanagerCertificate_ManagedIdentityFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PemCertificate": ubx.FieldSpec{WireName: "pem_certificate"},
		"SanDnsnames": ubx.FieldSpec{WireName: "san_dnsnames"},
		"Scope": ubx.FieldSpec{WireName: "scope"},
		"SelfManaged": ubx.FieldSpec{
			WireName: "self_managed",
			Kind: "object",
			Fields: CertificatemanagerCertificate_SelfManagedFields,
		},
		"Tags": ubx.FieldSpec{WireName: "tags"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
		"UsedBy": ubx.FieldSpec{
			WireName: "used_by",
			Kind: "list",
			Fields: CertificatemanagerCertificate_UsedByFields,
		},
	},
}
