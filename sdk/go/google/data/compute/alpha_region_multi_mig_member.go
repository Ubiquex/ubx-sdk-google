// Code generated by `ubx sdk gen` from google_compute_alpha_region_multi_mig_member. DO NOT EDIT.
package compute

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type AlphaRegionMultiMigMember_Status struct {
	// [Output Only] URL of member instance group manager
	InstanceGroupManager any
}

type AlphaRegionMultiMigMemberConfig struct {
	MultiMig any
	MultiMigMember any
	Project any
	Region any
}

type AlphaRegionMultiMigMemberAttrs struct {
	// Output only. [Output Only] Creation timestamp of this multi-MIG member inRFC3339 text format.
	CreationTimestamp any
	// Output only. [Output only] The unique identifier for this resource type. The server generates this identifier.
	Id any
	// Output only. Type of the resource. Alwayscompute#multiMigMember for a list of multi-MIG members.
	Kind any
	MultiMig any
	MultiMigMember any
	// Output only. [Output Only] Server-defined name for the multi-MIG member.
	Name any
	Project any
	Region any
	// Output only. [Output Only] Server-defined fully-qualified URL for this resource.
	SelfLink any
	Status any
}

var AlphaRegionMultiMigMember = ubx.DataSourceBinding{
	WireType: "google_compute_alpha_region_multi_mig_member",
	Fields: ubx.FieldMap{
		"MultiMig": ubx.FieldSpec{WireName: "multi_mig"},
		"MultiMigMember": ubx.FieldSpec{WireName: "multi_mig_member"},
		"Project": ubx.FieldSpec{WireName: "project"},
		"Region": ubx.FieldSpec{WireName: "region"},
	},
}
