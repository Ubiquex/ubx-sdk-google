// Code generated by `ubx sdk gen` from google_dialogflow_dialogflow_webhook. DO NOT EDIT.
package dialogflow

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DialogflowWebhook_GenericWebService_OauthConfig struct {
	ClientId any
	ClientSecret any
	Scopes any
	SecretVersionForClientSecret any
	TokenEndpoint any
}

type DialogflowWebhook_GenericWebService_SecretVersionsForRequestHeaders struct {
	SecretVersion any
}

type DialogflowWebhook_GenericWebService_ServiceAccountAuthConfig struct {
	ServiceAccount any
}

type DialogflowWebhook_GenericWebService struct {
	AllowedCaCerts any
	HttpMethod any
	OauthConfig any
	ParameterMapping any
	Password any
	RequestBody any
	RequestHeaders any
	SecretVersionForUsernamePassword any
	SecretVersionsForRequestHeaders any
	ServiceAccountAuthConfig any
	ServiceAgentAuth any
	Uri any
	Username any
	WebhookType any
}

type DialogflowWebhook_ServiceDirectory struct {
	GenericWebService any
	Service any
}

var DialogflowWebhook_GenericWebService_OauthConfigFields = ubx.FieldMap{
		"ClientId": ubx.FieldSpec{WireName: "client_id"},
		"ClientSecret": ubx.FieldSpec{WireName: "client_secret"},
		"Scopes": ubx.FieldSpec{WireName: "scopes"},
		"SecretVersionForClientSecret": ubx.FieldSpec{WireName: "secret_version_for_client_secret"},
		"TokenEndpoint": ubx.FieldSpec{WireName: "token_endpoint"},
	}

var DialogflowWebhook_GenericWebService_SecretVersionsForRequestHeadersFields = ubx.FieldMap{
		"SecretVersion": ubx.FieldSpec{WireName: "secret_version"},
	}

var DialogflowWebhook_GenericWebService_ServiceAccountAuthConfigFields = ubx.FieldMap{
		"ServiceAccount": ubx.FieldSpec{WireName: "service_account"},
	}

var DialogflowWebhook_GenericWebServiceFields = ubx.FieldMap{
		"AllowedCaCerts": ubx.FieldSpec{WireName: "allowed_ca_certs"},
		"HttpMethod": ubx.FieldSpec{WireName: "http_method"},
		"OauthConfig": ubx.FieldSpec{
			WireName: "oauth_config",
			Kind: "object",
			Fields: DialogflowWebhook_GenericWebService_OauthConfigFields,
		},
		"ParameterMapping": ubx.FieldSpec{WireName: "parameter_mapping"},
		"Password": ubx.FieldSpec{WireName: "password"},
		"RequestBody": ubx.FieldSpec{WireName: "request_body"},
		"RequestHeaders": ubx.FieldSpec{WireName: "request_headers"},
		"SecretVersionForUsernamePassword": ubx.FieldSpec{WireName: "secret_version_for_username_password"},
		"SecretVersionsForRequestHeaders": ubx.FieldSpec{
			WireName: "secret_versions_for_request_headers",
			Kind: "map",
			Fields: DialogflowWebhook_GenericWebService_SecretVersionsForRequestHeadersFields,
		},
		"ServiceAccountAuthConfig": ubx.FieldSpec{
			WireName: "service_account_auth_config",
			Kind: "object",
			Fields: DialogflowWebhook_GenericWebService_ServiceAccountAuthConfigFields,
		},
		"ServiceAgentAuth": ubx.FieldSpec{WireName: "service_agent_auth"},
		"Uri": ubx.FieldSpec{WireName: "uri"},
		"Username": ubx.FieldSpec{WireName: "username"},
		"WebhookType": ubx.FieldSpec{WireName: "webhook_type"},
	}

var DialogflowWebhook_ServiceDirectoryFields = ubx.FieldMap{
		"GenericWebService": ubx.FieldSpec{
			WireName: "generic_web_service",
			Kind: "object",
			Fields: DialogflowWebhook_GenericWebServiceFields,
		},
		"Service": ubx.FieldSpec{WireName: "service"},
	}

type DialogflowWebhookConfig struct {
	Disabled any
	DisplayName any
	GenericWebService any
	Name any
	ServiceDirectory any
	Timeout any
}

type DialogflowWebhookAttrs struct {
	Disabled any
	DisplayName any
	GenericWebService any
	Name any
	ServiceDirectory any
	Timeout any
}

var DialogflowWebhook = ubx.ResourceBinding{
	WireType: "google_dialogflow_dialogflow_webhook",
	Fields: ubx.FieldMap{
		"Disabled": ubx.FieldSpec{WireName: "disabled"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"GenericWebService": ubx.FieldSpec{
			WireName: "generic_web_service",
			Kind: "object",
			Fields: DialogflowWebhook_GenericWebServiceFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"ServiceDirectory": ubx.FieldSpec{
			WireName: "service_directory",
			Kind: "object",
			Fields: DialogflowWebhook_ServiceDirectoryFields,
		},
		"Timeout": ubx.FieldSpec{WireName: "timeout"},
	},
}
