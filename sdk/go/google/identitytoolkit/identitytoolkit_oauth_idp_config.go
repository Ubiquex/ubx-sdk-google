// Code generated by `ubx sdk gen` from google_identitytoolkit_identitytoolkit_oauth_idp_config. DO NOT EDIT.
package identitytoolkit

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type IdentitytoolkitOauthIdpConfig_ResponseType struct {
	// If true, authorization code is returned from IdP's authorization endpoint.
	Code any
	// If true, ID token is returned from IdP's authorization endpoint.
	IdToken any
	// Do not use. The `token` response type is not supported at the moment.
	Token any
}

var IdentitytoolkitOauthIdpConfig_ResponseTypeFields = ubx.FieldMap{
		"Code": ubx.FieldSpec{WireName: "code"},
		"IdToken": ubx.FieldSpec{WireName: "id_token"},
		"Token": ubx.FieldSpec{WireName: "token"},
	}

type IdentitytoolkitOauthIdpConfigConfig struct {
	// The client id of an OAuth client.
	ClientId any
	// The client secret of the OAuth client, to enable OIDC code flow.
	ClientSecret any
	// The config's display name set by developers.
	DisplayName any
	// True if allows the user to sign in with the provider.
	Enabled any
	// For OIDC Idps, the issuer identifier.
	Issuer any
	// The name of the OAuthIdpConfig resource, for example: 'projects/my-awesome-project/oauthIdpConfigs/oauth-config-id'. Ignored during create requests.
	Name any
	// The response type to request for in the OAuth authorization flow. You can set either `id_token` or `code` to true, but not both. Setting both types to be simultaneously true (`{code: true, id_token: true}`) is not yet supported. See https://openid.net/specs/openid-connect-core-1_0.html#Authentication for a mapping of response type to OAuth 2.0 flow.
	ResponseType any
}

type IdentitytoolkitOauthIdpConfigAttrs struct {
	// The client id of an OAuth client.
	ClientId any
	// The client secret of the OAuth client, to enable OIDC code flow.
	ClientSecret any
	// The config's display name set by developers.
	DisplayName any
	// True if allows the user to sign in with the provider.
	Enabled any
	// For OIDC Idps, the issuer identifier.
	Issuer any
	// The name of the OAuthIdpConfig resource, for example: 'projects/my-awesome-project/oauthIdpConfigs/oauth-config-id'. Ignored during create requests.
	Name any
	// The response type to request for in the OAuth authorization flow. You can set either `id_token` or `code` to true, but not both. Setting both types to be simultaneously true (`{code: true, id_token: true}`) is not yet supported. See https://openid.net/specs/openid-connect-core-1_0.html#Authentication for a mapping of response type to OAuth 2.0 flow.
	ResponseType any
}

var IdentitytoolkitOauthIdpConfig = ubx.ResourceBinding{
	WireType: "google_identitytoolkit_identitytoolkit_oauth_idp_config",
	Fields: ubx.FieldMap{
		"ClientId": ubx.FieldSpec{WireName: "client_id"},
		"ClientSecret": ubx.FieldSpec{WireName: "client_secret"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"Enabled": ubx.FieldSpec{WireName: "enabled"},
		"Issuer": ubx.FieldSpec{WireName: "issuer"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"ResponseType": ubx.FieldSpec{
			WireName: "response_type",
			Kind: "object",
			Fields: IdentitytoolkitOauthIdpConfig_ResponseTypeFields,
		},
	},
}
