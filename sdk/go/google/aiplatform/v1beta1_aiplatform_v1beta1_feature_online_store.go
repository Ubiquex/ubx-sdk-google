// Code generated by `ubx sdk gen` from google_aiplatform_v1beta1_aiplatform_v1beta1_feature_online_store. DO NOT EDIT.
package aiplatform

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable_AutoScaling struct {
	// Optional. A percentage of the cluster's CPU capacity. Can be from 10% to 80%. When a cluster's CPU utilization exceeds the target that you have set, Bigtable immediately adds nodes to the cluster. When CPU utilization is substantially lower than the target, Bigtable removes nodes. If not set will default to 50%.
	CpuUtilizationTarget any
	// Required. The maximum number of nodes to scale up to. Must be greater than or equal to min_node_count, and less than or equal to 10 times of 'min_node_count'.
	MaxNodeCount any
	// Required. The minimum number of nodes to scale down to. Must be greater than or equal to 1.
	MinNodeCount any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable_BigtableMetadata struct {
	// The Cloud Bigtable instance id.
	InstanceId any
	// The Cloud Bigtable table id.
	TableId any
	// Tenant project ID.
	TenantProjectId any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable struct {
	AutoScaling any
	// Metadata of the Bigtable instance. This is used by direct read access to the Bigtable in tenant project.
	BigtableMetadata any
	// Optional. It true, enable direct access to the Bigtable instance.
	EnableDirectBigtableAccess any
	// Optional. The zone where the underlying Bigtable cluster for the primary Bigtable instance will be provisioned. Only the zone must be provided. For example, only "us-central1-a" should be provided.
	Zone any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint_PrivateServiceConnectConfig_PscAutomationConfigs struct {
	ErrorMessage any
	ForwardingRule any
	IpAddress any
	Network any
	ProjectId any
	State any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint_PrivateServiceConnectConfig struct {
	// Required. If true, expose the IndexEndpoint via private service connect.
	EnablePrivateServiceConnect any
	// Optional. If set to true, enable secure private service connect with IAM authorization. Otherwise, private service connect will be done without authorization. Note latency will be slightly increased if authorization is enabled.
	EnableSecurePrivateServiceConnect any
	// A list of Projects from which the forwarding rule will target the service attachment.
	ProjectAllowlist any
	// Optional. List of projects and networks where the PSC endpoints will be created. This field is used by Online Inference(Prediction) only.
	PscAutomationConfigs any
	// Output only. The name of the generated service attachment resource. This is only populated if the endpoint is deployed with PrivateServiceConnect.
	ServiceAttachment any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint struct {
	// Represents configuration for private service connect.
	PrivateServiceConnectConfig any
	// Output only. This field will be populated with the domain name to use for this FeatureOnlineStore
	PublicEndpointDomainName any
	// Output only. The name of the service attachment resource. Populated if private service connect is enabled and after FeatureViewSync is created.
	ServiceAttachment any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_EmbeddingManagement struct {
	// Optional. Immutable. Whether to enable embedding management in this FeatureOnlineStore. It's immutable after creation to ensure the FeatureOnlineStore availability.
	Enabled any
}

type V1beta1AiplatformV1beta1FeatureOnlineStore_EncryptionSpec struct {
	// Required. Resource name of the Cloud KMS key used to protect the resource. The Cloud KMS key must be in the same region as the resource. It must have the format `projects/{project}/locations/{location}/keyRings/{key_ring}/cryptoKeys/{crypto_key}`.
	KmsKeyName any
}

var V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable_AutoScalingFields = ubx.FieldMap{
		"CpuUtilizationTarget": ubx.FieldSpec{WireName: "cpu_utilization_target"},
		"MaxNodeCount": ubx.FieldSpec{WireName: "max_node_count"},
		"MinNodeCount": ubx.FieldSpec{WireName: "min_node_count"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable_BigtableMetadataFields = ubx.FieldMap{
		"InstanceId": ubx.FieldSpec{WireName: "instance_id"},
		"TableId": ubx.FieldSpec{WireName: "table_id"},
		"TenantProjectId": ubx.FieldSpec{WireName: "tenant_project_id"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_BigtableFields = ubx.FieldMap{
		"AutoScaling": ubx.FieldSpec{
			WireName: "auto_scaling",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable_AutoScalingFields,
		},
		"BigtableMetadata": ubx.FieldSpec{
			WireName: "bigtable_metadata",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_Bigtable_BigtableMetadataFields,
		},
		"EnableDirectBigtableAccess": ubx.FieldSpec{WireName: "enable_direct_bigtable_access"},
		"Zone": ubx.FieldSpec{WireName: "zone"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint_PrivateServiceConnectConfig_PscAutomationConfigsFields = ubx.FieldMap{
		"ErrorMessage": ubx.FieldSpec{WireName: "error_message"},
		"ForwardingRule": ubx.FieldSpec{WireName: "forwarding_rule"},
		"IpAddress": ubx.FieldSpec{WireName: "ip_address"},
		"Network": ubx.FieldSpec{WireName: "network"},
		"ProjectId": ubx.FieldSpec{WireName: "project_id"},
		"State": ubx.FieldSpec{WireName: "state"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint_PrivateServiceConnectConfigFields = ubx.FieldMap{
		"EnablePrivateServiceConnect": ubx.FieldSpec{WireName: "enable_private_service_connect"},
		"EnableSecurePrivateServiceConnect": ubx.FieldSpec{WireName: "enable_secure_private_service_connect"},
		"ProjectAllowlist": ubx.FieldSpec{WireName: "project_allowlist"},
		"PscAutomationConfigs": ubx.FieldSpec{
			WireName: "psc_automation_configs",
			Kind: "list",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint_PrivateServiceConnectConfig_PscAutomationConfigsFields,
		},
		"ServiceAttachment": ubx.FieldSpec{WireName: "service_attachment"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpointFields = ubx.FieldMap{
		"PrivateServiceConnectConfig": ubx.FieldSpec{
			WireName: "private_service_connect_config",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpoint_PrivateServiceConnectConfigFields,
		},
		"PublicEndpointDomainName": ubx.FieldSpec{WireName: "public_endpoint_domain_name"},
		"ServiceAttachment": ubx.FieldSpec{WireName: "service_attachment"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_EmbeddingManagementFields = ubx.FieldMap{
		"Enabled": ubx.FieldSpec{WireName: "enabled"},
	}

var V1beta1AiplatformV1beta1FeatureOnlineStore_EncryptionSpecFields = ubx.FieldMap{
		"KmsKeyName": ubx.FieldSpec{WireName: "kms_key_name"},
	}

type V1beta1AiplatformV1beta1FeatureOnlineStoreConfig struct {
	Bigtable any
	// Output only. Timestamp when this FeatureOnlineStore was created.
	CreateTime any
	// The dedicated serving endpoint for this FeatureOnlineStore. Only need to set when you choose Optimized storage type. Public endpoint is provisioned by default.
	DedicatedServingEndpoint any
	// Deprecated: This sub message is no longer needed anymore and embedding management is automatically enabled when specifying Optimized storage type. Contains settings for embedding management.
	EmbeddingManagement any
	// Represents a customer-managed encryption key specification that can be applied to a Vertex AI resource.
	EncryptionSpec any
	// Optional. Used to perform consistent read-modify-write updates. If not set, a blind "overwrite" update happens.
	Etag any
	// Optional. The labels with user-defined metadata to organize your FeatureOnlineStore. Label keys and values can be no longer than 64 characters (Unicode codepoints), can only contain lowercase letters, numeric characters, underscores and dashes. International characters are allowed. See https://goo.gl/xmQnxf for more information on and examples of labels. No more than 64 user labels can be associated with one FeatureOnlineStore(System labels are excluded)." System reserved label keys are prefixed with "aiplatform.googleapis.com/" and are immutable.
	Labels any
	// Identifier. Name of the FeatureOnlineStore. Format: `projects/{project}/locations/{location}/featureOnlineStores/{featureOnlineStore}`
	Name any
	// Optimized storage type
	Optimized any
	// Output only. Reserved for future use.
	SatisfiesPzi any
	// Output only. Reserved for future use.
	SatisfiesPzs any
	// Output only. State of the featureOnlineStore.
	State any
	// Output only. Timestamp when this FeatureOnlineStore was last updated.
	UpdateTime any
}

type V1beta1AiplatformV1beta1FeatureOnlineStoreAttrs struct {
	Bigtable any
	// Output only. Timestamp when this FeatureOnlineStore was created.
	CreateTime any
	// The dedicated serving endpoint for this FeatureOnlineStore. Only need to set when you choose Optimized storage type. Public endpoint is provisioned by default.
	DedicatedServingEndpoint any
	// Deprecated: This sub message is no longer needed anymore and embedding management is automatically enabled when specifying Optimized storage type. Contains settings for embedding management.
	EmbeddingManagement any
	// Represents a customer-managed encryption key specification that can be applied to a Vertex AI resource.
	EncryptionSpec any
	// Optional. Used to perform consistent read-modify-write updates. If not set, a blind "overwrite" update happens.
	Etag any
	// Optional. The labels with user-defined metadata to organize your FeatureOnlineStore. Label keys and values can be no longer than 64 characters (Unicode codepoints), can only contain lowercase letters, numeric characters, underscores and dashes. International characters are allowed. See https://goo.gl/xmQnxf for more information on and examples of labels. No more than 64 user labels can be associated with one FeatureOnlineStore(System labels are excluded)." System reserved label keys are prefixed with "aiplatform.googleapis.com/" and are immutable.
	Labels any
	// Identifier. Name of the FeatureOnlineStore. Format: `projects/{project}/locations/{location}/featureOnlineStores/{featureOnlineStore}`
	Name any
	// Optimized storage type
	Optimized any
	// Output only. Reserved for future use.
	SatisfiesPzi any
	// Output only. Reserved for future use.
	SatisfiesPzs any
	// Output only. State of the featureOnlineStore.
	State any
	// Output only. Timestamp when this FeatureOnlineStore was last updated.
	UpdateTime any
}

var V1beta1AiplatformV1beta1FeatureOnlineStore = ubx.ResourceBinding{
	WireType: "google_aiplatform_v1beta1_aiplatform_v1beta1_feature_online_store",
	Fields: ubx.FieldMap{
		"Bigtable": ubx.FieldSpec{
			WireName: "bigtable",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_BigtableFields,
		},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DedicatedServingEndpoint": ubx.FieldSpec{
			WireName: "dedicated_serving_endpoint",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_DedicatedServingEndpointFields,
		},
		"EmbeddingManagement": ubx.FieldSpec{
			WireName: "embedding_management",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_EmbeddingManagementFields,
		},
		"EncryptionSpec": ubx.FieldSpec{
			WireName: "encryption_spec",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1FeatureOnlineStore_EncryptionSpecFields,
		},
		"Etag": ubx.FieldSpec{WireName: "etag"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"Optimized": ubx.FieldSpec{WireName: "optimized"},
		"SatisfiesPzi": ubx.FieldSpec{WireName: "satisfies_pzi"},
		"SatisfiesPzs": ubx.FieldSpec{WireName: "satisfies_pzs"},
		"State": ubx.FieldSpec{WireName: "state"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
