// Code generated by `ubx sdk gen` from google_aiplatform_v1beta1_aiplatform_v1beta1_memory. DO NOT EDIT.
package aiplatform

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type V1beta1AiplatformV1beta1Memory_Metadata struct {
	BoolValue any
	DoubleValue any
	StringValue any
	TimestampValue any
}

type V1beta1AiplatformV1beta1Memory_StructuredContent struct {
	// Required. Represents the structured value of the memory.
	Data any
	// Required. Represents the schema ID for which this structured memory belongs to.
	SchemaId any
}

type V1beta1AiplatformV1beta1Memory_Topics struct {
	CustomMemoryTopicLabel any
	ManagedMemoryTopic any
}

var V1beta1AiplatformV1beta1Memory_MetadataFields = ubx.FieldMap{
		"BoolValue": ubx.FieldSpec{WireName: "bool_value"},
		"DoubleValue": ubx.FieldSpec{WireName: "double_value"},
		"StringValue": ubx.FieldSpec{WireName: "string_value"},
		"TimestampValue": ubx.FieldSpec{WireName: "timestamp_value"},
	}

var V1beta1AiplatformV1beta1Memory_StructuredContentFields = ubx.FieldMap{
		"Data": ubx.FieldSpec{WireName: "data"},
		"SchemaId": ubx.FieldSpec{WireName: "schema_id"},
	}

var V1beta1AiplatformV1beta1Memory_TopicsFields = ubx.FieldMap{
		"CustomMemoryTopicLabel": ubx.FieldSpec{WireName: "custom_memory_topic_label"},
		"ManagedMemoryTopic": ubx.FieldSpec{WireName: "managed_memory_topic"},
	}

type V1beta1AiplatformV1beta1MemoryConfig struct {
	// Output only. Represents the timestamp when this Memory was created.
	CreateTime any
	// Optional. Represents the description of the Memory.
	Description any
	// Optional. Input only. Indicates whether no revision will be created for this request.
	DisableMemoryRevisions any
	// Optional. Represents the display name of the Memory.
	DisplayName any
	// Optional. Represents the timestamp of when this resource is considered expired. This is *always* provided on output when `expiration` is set on input, regardless of whether `expire_time` or `ttl` was provided.
	ExpireTime any
	// Optional. Represents semantic knowledge extracted from the source content.
	Fact any
	// Optional. Represents the type of the memory. If not set, the `NATURAL_LANGUAGE_COLLECTION` type is used. If `STRUCTURED_COLLECTION` or `STRUCTURED_PROFILE` is used, then `structured_data` must be provided.
	MemoryType any
	// Optional. Represents user-provided metadata for the Memory. This information was provided when creating, updating, or generating the Memory. It was not generated by Memory Bank.
	Metadata any
	// Identifier. Represents the resource name of the Memory. Format: `projects/{project}/locations/{location}/reasoningEngines/{reasoning_engine}/memories/{memory}`
	Name any
	// Optional. Input only. Represents the timestamp of when the revision is considered expired. If not set, the memory revision will be kept until manually deleted.
	RevisionExpireTime any
	// Optional. Input only. Represents the labels to apply to the Memory Revision created as a result of this request.
	RevisionLabels any
	// Optional. Input only. Represents the TTL for the revision. The expiration time is computed: now + TTL.
	RevisionTtl any
	// Required. Immutable. Represents the scope of the Memory. Memories are isolated within their scope. The scope is defined when creating or generating memories. Scope values cannot contain the wildcard character '*'.
	Scope any
	// Represents the structured value of the memory.
	StructuredContent any
	// Optional. Represents the Topics of the Memory.
	Topics any
	// Optional. Input only. Represents the TTL for this resource. The expiration time is computed: now + TTL.
	Ttl any
	// Output only. Represents the timestamp when this Memory was most recently updated.
	UpdateTime any
}

type V1beta1AiplatformV1beta1MemoryAttrs struct {
	// Output only. Represents the timestamp when this Memory was created.
	CreateTime any
	// Optional. Represents the description of the Memory.
	Description any
	// Optional. Input only. Indicates whether no revision will be created for this request.
	DisableMemoryRevisions any
	// Optional. Represents the display name of the Memory.
	DisplayName any
	// Optional. Represents the timestamp of when this resource is considered expired. This is *always* provided on output when `expiration` is set on input, regardless of whether `expire_time` or `ttl` was provided.
	ExpireTime any
	// Optional. Represents semantic knowledge extracted from the source content.
	Fact any
	// Optional. Represents the type of the memory. If not set, the `NATURAL_LANGUAGE_COLLECTION` type is used. If `STRUCTURED_COLLECTION` or `STRUCTURED_PROFILE` is used, then `structured_data` must be provided.
	MemoryType any
	// Optional. Represents user-provided metadata for the Memory. This information was provided when creating, updating, or generating the Memory. It was not generated by Memory Bank.
	Metadata any
	// Identifier. Represents the resource name of the Memory. Format: `projects/{project}/locations/{location}/reasoningEngines/{reasoning_engine}/memories/{memory}`
	Name any
	// Optional. Input only. Represents the timestamp of when the revision is considered expired. If not set, the memory revision will be kept until manually deleted.
	RevisionExpireTime any
	// Optional. Input only. Represents the labels to apply to the Memory Revision created as a result of this request.
	RevisionLabels any
	// Optional. Input only. Represents the TTL for the revision. The expiration time is computed: now + TTL.
	RevisionTtl any
	// Required. Immutable. Represents the scope of the Memory. Memories are isolated within their scope. The scope is defined when creating or generating memories. Scope values cannot contain the wildcard character '*'.
	Scope any
	// Represents the structured value of the memory.
	StructuredContent any
	// Optional. Represents the Topics of the Memory.
	Topics any
	// Optional. Input only. Represents the TTL for this resource. The expiration time is computed: now + TTL.
	Ttl any
	// Output only. Represents the timestamp when this Memory was most recently updated.
	UpdateTime any
}

var V1beta1AiplatformV1beta1Memory = ubx.ResourceBinding{
	WireType: "google_aiplatform_v1beta1_aiplatform_v1beta1_memory",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"DisableMemoryRevisions": ubx.FieldSpec{WireName: "disable_memory_revisions"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"ExpireTime": ubx.FieldSpec{WireName: "expire_time"},
		"Fact": ubx.FieldSpec{WireName: "fact"},
		"MemoryType": ubx.FieldSpec{WireName: "memory_type"},
		"Metadata": ubx.FieldSpec{
			WireName: "metadata",
			Kind: "map",
			Fields: V1beta1AiplatformV1beta1Memory_MetadataFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"RevisionExpireTime": ubx.FieldSpec{WireName: "revision_expire_time"},
		"RevisionLabels": ubx.FieldSpec{WireName: "revision_labels"},
		"RevisionTtl": ubx.FieldSpec{WireName: "revision_ttl"},
		"Scope": ubx.FieldSpec{WireName: "scope"},
		"StructuredContent": ubx.FieldSpec{
			WireName: "structured_content",
			Kind: "object",
			Fields: V1beta1AiplatformV1beta1Memory_StructuredContentFields,
		},
		"Topics": ubx.FieldSpec{
			WireName: "topics",
			Kind: "list",
			Fields: V1beta1AiplatformV1beta1Memory_TopicsFields,
		},
		"Ttl": ubx.FieldSpec{WireName: "ttl"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
