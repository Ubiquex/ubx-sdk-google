// Code generated by `ubx sdk gen` from google_cloudsupport_v2beta_cloudsupport_v2beta_support_event_subscription. DO NOT EDIT.
package cloudsupport

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type V2betaCloudsupportV2betaSupportEventSubscriptionConfig struct {
	// Output only. The time at which the subscription was created.
	CreateTime any
	// Output only. The time at which the subscription was deleted.
	DeleteTime any
	// Output only. Reason why subscription is failing. State of subscription must be FAILING in order for this to have a value.
	FailureReason any
	// Identifier. The resource name of the support event subscription.
	Name any
	// Required. The name of the Pub/Sub topic to publish notifications to. Format: projects/{project}/topics/{topic}
	PubSubTopic any
	// Output only. The time at which the subscription will be purged.
	PurgeTime any
	// Output only. The state of the subscription.
	State any
	// Output only. The time at which the subscription was last updated.
	UpdateTime any
}

type V2betaCloudsupportV2betaSupportEventSubscriptionAttrs struct {
	// Output only. The time at which the subscription was created.
	CreateTime any
	// Output only. The time at which the subscription was deleted.
	DeleteTime any
	// Output only. Reason why subscription is failing. State of subscription must be FAILING in order for this to have a value.
	FailureReason any
	// Identifier. The resource name of the support event subscription.
	Name any
	// Required. The name of the Pub/Sub topic to publish notifications to. Format: projects/{project}/topics/{topic}
	PubSubTopic any
	// Output only. The time at which the subscription will be purged.
	PurgeTime any
	// Output only. The state of the subscription.
	State any
	// Output only. The time at which the subscription was last updated.
	UpdateTime any
}

var V2betaCloudsupportV2betaSupportEventSubscription = ubx.ResourceBinding{
	WireType: "google_cloudsupport_v2beta_cloudsupport_v2beta_support_event_subscription",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DeleteTime": ubx.FieldSpec{WireName: "delete_time"},
		"FailureReason": ubx.FieldSpec{WireName: "failure_reason"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PubSubTopic": ubx.FieldSpec{WireName: "pub_sub_topic"},
		"PurgeTime": ubx.FieldSpec{WireName: "purge_time"},
		"State": ubx.FieldSpec{WireName: "state"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
