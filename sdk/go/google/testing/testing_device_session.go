// Code generated by `ubx sdk gen` from google_testing_testing_device_session. DO NOT EDIT.
package testing

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type TestingDeviceSession_AndroidDevice struct {
	// Required. The id of the Android device to be used. Use the TestEnvironmentDiscoveryService to get supported options.
	AndroidModelId any
	// Required. The id of the Android OS version to be used. Use the TestEnvironmentDiscoveryService to get supported options.
	AndroidVersionId any
	// Required. The locale the test device used for testing. Use the TestEnvironmentDiscoveryService to get supported options.
	Locale any
	// Required. How the device is oriented during the test. Use the TestEnvironmentDiscoveryService to get supported options.
	Orientation any
}

type TestingDeviceSession_StateHistories struct {
	EventTime any
	SessionState any
	StateMessage any
}

var TestingDeviceSession_AndroidDeviceFields = ubx.FieldMap{
		"AndroidModelId": ubx.FieldSpec{WireName: "android_model_id"},
		"AndroidVersionId": ubx.FieldSpec{WireName: "android_version_id"},
		"Locale": ubx.FieldSpec{WireName: "locale"},
		"Orientation": ubx.FieldSpec{WireName: "orientation"},
	}

var TestingDeviceSession_StateHistoriesFields = ubx.FieldMap{
		"EventTime": ubx.FieldSpec{WireName: "event_time"},
		"SessionState": ubx.FieldSpec{WireName: "session_state"},
		"StateMessage": ubx.FieldSpec{WireName: "state_message"},
	}

type TestingDeviceSessionConfig struct {
	// Output only. The timestamp that the session first became ACTIVE.
	ActiveStartTime any
	// A single Android device.
	AndroidDevice any
	// Output only. The time that the Session was created.
	CreateTime any
	// Output only. The title of the DeviceSession to be presented in the UI.
	DisplayName any
	// Optional. If the device is still in use at this time, any connections will be ended and the SessionState will transition from ACTIVE to FINISHED.
	ExpireTime any
	// Output only. The interval of time that this device must be interacted with before it transitions from ACTIVE to TIMEOUT_INACTIVITY.
	InactivityTimeout any
	// Optional. Name of the DeviceSession, e.g. "projects/{project_id}/deviceSessions/{session_id}"
	Name any
	// Output only. Current state of the DeviceSession.
	State any
	// Output only. The historical state transitions of the session_state message including the current session state.
	StateHistories any
	// Optional. The amount of time that a device will be initially allocated for. This can eventually be extended with the UpdateDeviceSession RPC. Default: 15 minutes.
	Ttl any
}

type TestingDeviceSessionAttrs struct {
	// Output only. The timestamp that the session first became ACTIVE.
	ActiveStartTime any
	// A single Android device.
	AndroidDevice any
	// Output only. The time that the Session was created.
	CreateTime any
	// Output only. The title of the DeviceSession to be presented in the UI.
	DisplayName any
	// Optional. If the device is still in use at this time, any connections will be ended and the SessionState will transition from ACTIVE to FINISHED.
	ExpireTime any
	// Output only. The interval of time that this device must be interacted with before it transitions from ACTIVE to TIMEOUT_INACTIVITY.
	InactivityTimeout any
	// Optional. Name of the DeviceSession, e.g. "projects/{project_id}/deviceSessions/{session_id}"
	Name any
	// Output only. Current state of the DeviceSession.
	State any
	// Output only. The historical state transitions of the session_state message including the current session state.
	StateHistories any
	// Optional. The amount of time that a device will be initially allocated for. This can eventually be extended with the UpdateDeviceSession RPC. Default: 15 minutes.
	Ttl any
}

var TestingDeviceSession = ubx.ResourceBinding{
	WireType: "google_testing_testing_device_session",
	Fields: ubx.FieldMap{
		"ActiveStartTime": ubx.FieldSpec{WireName: "active_start_time"},
		"AndroidDevice": ubx.FieldSpec{
			WireName: "android_device",
			Kind: "object",
			Fields: TestingDeviceSession_AndroidDeviceFields,
		},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"ExpireTime": ubx.FieldSpec{WireName: "expire_time"},
		"InactivityTimeout": ubx.FieldSpec{WireName: "inactivity_timeout"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"State": ubx.FieldSpec{WireName: "state"},
		"StateHistories": ubx.FieldSpec{
			WireName: "state_histories",
			Kind: "list",
			Fields: TestingDeviceSession_StateHistoriesFields,
		},
		"Ttl": ubx.FieldSpec{WireName: "ttl"},
	},
}
