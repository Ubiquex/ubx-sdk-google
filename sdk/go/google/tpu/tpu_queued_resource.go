// Code generated by `ubx sdk gen` from google_tpu_tpu_queued_resource. DO NOT EDIT.
package tpu

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type TpuQueuedResource_Guaranteed struct {
	// Optional. Defines the minimum duration of the guarantee. If specified, the requested resources will only be provisioned if they can be allocated for at least the given duration.
	MinDuration any
}

type TpuQueuedResource_QueueingPolicy_ValidInterval struct {
	// Optional. Exclusive end of the interval. If specified, a Timestamp matching this interval will have to be before the end.
	EndTime any
	// Optional. Inclusive start of the interval. If specified, a Timestamp matching this interval will have to be the same or after the start.
	StartTime any
}

type TpuQueuedResource_QueueingPolicy struct {
	// Optional. A relative time after which resources may be created.
	ValidAfterDuration any
	// Optional. An absolute time after which resources may be created.
	ValidAfterTime any
	// Represents a time interval, encoded as a Timestamp start (inclusive) and a Timestamp end (exclusive). The start must be less than or equal to the end. When the start equals the end, the interval is empty (matches no time). When both start and end are unspecified, the interval matches any time.
	ValidInterval any
	// Optional. A relative time after which resources should not be created. If the request cannot be fulfilled by this time the request will be failed.
	ValidUntilDuration any
	// Optional. An absolute time after which resources should not be created. If the request cannot be fulfilled by this time the request will be failed.
	ValidUntilTime any
}

type TpuQueuedResource_State_FailedData_Error struct {
	// The status code, which should be an enum value of google.rpc.Code.
	Code any
	// A list of messages that carry the error details. There is a common set of message types for APIs to use.
	Details any
	// A developer-facing error message, which should be in English. Any user-facing error message should be localized and sent in the google.rpc.Status.details field, or localized by the client.
	Message any
}

type TpuQueuedResource_State_FailedData struct {
	// The `Status` type defines a logical error model that is suitable for different programming environments, including REST APIs and RPC APIs. It is used by [gRPC](https://github.com/grpc). Each `Status` message contains three pieces of data: error code, error message, and error details. You can find out more about this error model and how to work with it in the [API Design Guide](https://cloud.google.com/apis/design/errors).
	Error any
}

type TpuQueuedResource_State struct {
	// Further data for the accepted state.
	AcceptedData any
	// Further data for the active state.
	ActiveData any
	// Further data for the creating state.
	CreatingData any
	// Further data for the deleting state.
	DeletingData any
	// Further data for the failed state.
	FailedData any
	// Further data for the provisioning state.
	ProvisioningData any
	// Output only. State of the QueuedResource request.
	State any
	// Output only. The initiator of the QueuedResources's current state. Used to indicate whether the SUSPENDING/SUSPENDED state was initiated by the user or the service.
	StateInitiator any
	// Further data for the suspended state.
	SuspendedData any
	// Further data for the suspending state.
	SuspendingData any
}

type TpuQueuedResource_Tpu_NodeSpec_MultisliceParams struct {
	NodeCount any
	NodeIdPrefix any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_AcceleratorConfig struct {
	Topology any
	Type any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_BootDiskConfig_CustomerEncryptionKey struct {
	KmsKeyName any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_BootDiskConfig struct {
	CustomerEncryptionKey any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_DataDisks struct {
	Mode any
	SourceDisk any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_NetworkConfig struct {
	CanIpForward any
	EnableExternalIps any
	Network any
	QueueCount any
	Subnetwork any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_NetworkEndpoints_AccessConfig struct {
	ExternalIp any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_NetworkEndpoints struct {
	AccessConfig any
	IpAddress any
	Port any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_SchedulingConfig struct {
	Preemptible any
	Reserved any
	Spot any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_ServiceAccount struct {
	Email any
	Scope any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_ShieldedInstanceConfig struct {
	EnableSecureBoot any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_Symptoms struct {
	CreateTime any
	Details any
	SymptomType any
	WorkerId any
}

type TpuQueuedResource_Tpu_NodeSpec_Node_UpcomingMaintenance struct {
	CanReschedule any
	LatestWindowStartTime any
	MaintenanceStatus any
	Type any
	WindowEndTime any
	WindowStartTime any
}

type TpuQueuedResource_Tpu_NodeSpec_Node struct {
	AcceleratorConfig any
	AcceleratorType any
	ApiVersion any
	BootDiskConfig any
	CidrBlock any
	CreateTime any
	DataDisks any
	Description any
	Health any
	HealthDescription any
	Id any
	Labels any
	Metadata any
	MultisliceNode any
	Name any
	NetworkConfig any
	NetworkConfigs any
	NetworkEndpoints any
	QueuedResource any
	RuntimeVersion any
	SchedulingConfig any
	ServiceAccount any
	ShieldedInstanceConfig any
	State any
	Symptoms any
	Tags any
	UpcomingMaintenance any
}

type TpuQueuedResource_Tpu_NodeSpec struct {
	MultisliceParams any
	Node any
	NodeId any
	Parent any
}

type TpuQueuedResource_Tpu struct {
	// Optional. The TPU node(s) being requested.
	NodeSpec any
}

var TpuQueuedResource_GuaranteedFields = ubx.FieldMap{
		"MinDuration": ubx.FieldSpec{WireName: "min_duration"},
	}

var TpuQueuedResource_QueueingPolicy_ValidIntervalFields = ubx.FieldMap{
		"EndTime": ubx.FieldSpec{WireName: "end_time"},
		"StartTime": ubx.FieldSpec{WireName: "start_time"},
	}

var TpuQueuedResource_QueueingPolicyFields = ubx.FieldMap{
		"ValidAfterDuration": ubx.FieldSpec{WireName: "valid_after_duration"},
		"ValidAfterTime": ubx.FieldSpec{WireName: "valid_after_time"},
		"ValidInterval": ubx.FieldSpec{
			WireName: "valid_interval",
			Kind: "object",
			Fields: TpuQueuedResource_QueueingPolicy_ValidIntervalFields,
		},
		"ValidUntilDuration": ubx.FieldSpec{WireName: "valid_until_duration"},
		"ValidUntilTime": ubx.FieldSpec{WireName: "valid_until_time"},
	}

var TpuQueuedResource_State_FailedData_ErrorFields = ubx.FieldMap{
		"Code": ubx.FieldSpec{WireName: "code"},
		"Details": ubx.FieldSpec{WireName: "details"},
		"Message": ubx.FieldSpec{WireName: "message"},
	}

var TpuQueuedResource_State_FailedDataFields = ubx.FieldMap{
		"Error": ubx.FieldSpec{
			WireName: "error",
			Kind: "object",
			Fields: TpuQueuedResource_State_FailedData_ErrorFields,
		},
	}

var TpuQueuedResource_StateFields = ubx.FieldMap{
		"AcceptedData": ubx.FieldSpec{WireName: "accepted_data"},
		"ActiveData": ubx.FieldSpec{WireName: "active_data"},
		"CreatingData": ubx.FieldSpec{WireName: "creating_data"},
		"DeletingData": ubx.FieldSpec{WireName: "deleting_data"},
		"FailedData": ubx.FieldSpec{
			WireName: "failed_data",
			Kind: "object",
			Fields: TpuQueuedResource_State_FailedDataFields,
		},
		"ProvisioningData": ubx.FieldSpec{WireName: "provisioning_data"},
		"State": ubx.FieldSpec{WireName: "state"},
		"StateInitiator": ubx.FieldSpec{WireName: "state_initiator"},
		"SuspendedData": ubx.FieldSpec{WireName: "suspended_data"},
		"SuspendingData": ubx.FieldSpec{WireName: "suspending_data"},
	}

var TpuQueuedResource_Tpu_NodeSpec_MultisliceParamsFields = ubx.FieldMap{
		"NodeCount": ubx.FieldSpec{WireName: "node_count"},
		"NodeIdPrefix": ubx.FieldSpec{WireName: "node_id_prefix"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_AcceleratorConfigFields = ubx.FieldMap{
		"Topology": ubx.FieldSpec{WireName: "topology"},
		"Type": ubx.FieldSpec{WireName: "type"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_BootDiskConfig_CustomerEncryptionKeyFields = ubx.FieldMap{
		"KmsKeyName": ubx.FieldSpec{WireName: "kms_key_name"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_BootDiskConfigFields = ubx.FieldMap{
		"CustomerEncryptionKey": ubx.FieldSpec{
			WireName: "customer_encryption_key",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_BootDiskConfig_CustomerEncryptionKeyFields,
		},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_DataDisksFields = ubx.FieldMap{
		"Mode": ubx.FieldSpec{WireName: "mode"},
		"SourceDisk": ubx.FieldSpec{WireName: "source_disk"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_NetworkConfigFields = ubx.FieldMap{
		"CanIpForward": ubx.FieldSpec{WireName: "can_ip_forward"},
		"EnableExternalIps": ubx.FieldSpec{WireName: "enable_external_ips"},
		"Network": ubx.FieldSpec{WireName: "network"},
		"QueueCount": ubx.FieldSpec{WireName: "queue_count"},
		"Subnetwork": ubx.FieldSpec{WireName: "subnetwork"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_NetworkEndpoints_AccessConfigFields = ubx.FieldMap{
		"ExternalIp": ubx.FieldSpec{WireName: "external_ip"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_NetworkEndpointsFields = ubx.FieldMap{
		"AccessConfig": ubx.FieldSpec{
			WireName: "access_config",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_NetworkEndpoints_AccessConfigFields,
		},
		"IpAddress": ubx.FieldSpec{WireName: "ip_address"},
		"Port": ubx.FieldSpec{WireName: "port"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_SchedulingConfigFields = ubx.FieldMap{
		"Preemptible": ubx.FieldSpec{WireName: "preemptible"},
		"Reserved": ubx.FieldSpec{WireName: "reserved"},
		"Spot": ubx.FieldSpec{WireName: "spot"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_ServiceAccountFields = ubx.FieldMap{
		"Email": ubx.FieldSpec{WireName: "email"},
		"Scope": ubx.FieldSpec{WireName: "scope"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_ShieldedInstanceConfigFields = ubx.FieldMap{
		"EnableSecureBoot": ubx.FieldSpec{WireName: "enable_secure_boot"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_SymptomsFields = ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Details": ubx.FieldSpec{WireName: "details"},
		"SymptomType": ubx.FieldSpec{WireName: "symptom_type"},
		"WorkerId": ubx.FieldSpec{WireName: "worker_id"},
	}

var TpuQueuedResource_Tpu_NodeSpec_Node_UpcomingMaintenanceFields = ubx.FieldMap{
		"CanReschedule": ubx.FieldSpec{WireName: "can_reschedule"},
		"LatestWindowStartTime": ubx.FieldSpec{WireName: "latest_window_start_time"},
		"MaintenanceStatus": ubx.FieldSpec{WireName: "maintenance_status"},
		"Type": ubx.FieldSpec{WireName: "type"},
		"WindowEndTime": ubx.FieldSpec{WireName: "window_end_time"},
		"WindowStartTime": ubx.FieldSpec{WireName: "window_start_time"},
	}

var TpuQueuedResource_Tpu_NodeSpec_NodeFields = ubx.FieldMap{
		"AcceleratorConfig": ubx.FieldSpec{
			WireName: "accelerator_config",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_AcceleratorConfigFields,
		},
		"AcceleratorType": ubx.FieldSpec{WireName: "accelerator_type"},
		"ApiVersion": ubx.FieldSpec{WireName: "api_version"},
		"BootDiskConfig": ubx.FieldSpec{
			WireName: "boot_disk_config",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_BootDiskConfigFields,
		},
		"CidrBlock": ubx.FieldSpec{WireName: "cidr_block"},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DataDisks": ubx.FieldSpec{
			WireName: "data_disks",
			Kind: "list",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_DataDisksFields,
		},
		"Description": ubx.FieldSpec{WireName: "description"},
		"Health": ubx.FieldSpec{WireName: "health"},
		"HealthDescription": ubx.FieldSpec{WireName: "health_description"},
		"Id": ubx.FieldSpec{WireName: "id"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Metadata": ubx.FieldSpec{WireName: "metadata"},
		"MultisliceNode": ubx.FieldSpec{WireName: "multislice_node"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"NetworkConfig": ubx.FieldSpec{
			WireName: "network_config",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_NetworkConfigFields,
		},
		"NetworkConfigs": ubx.FieldSpec{
			WireName: "network_configs",
			Kind: "list",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_NetworkConfigFields,
		},
		"NetworkEndpoints": ubx.FieldSpec{
			WireName: "network_endpoints",
			Kind: "list",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_NetworkEndpointsFields,
		},
		"QueuedResource": ubx.FieldSpec{WireName: "queued_resource"},
		"RuntimeVersion": ubx.FieldSpec{WireName: "runtime_version"},
		"SchedulingConfig": ubx.FieldSpec{
			WireName: "scheduling_config",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_SchedulingConfigFields,
		},
		"ServiceAccount": ubx.FieldSpec{
			WireName: "service_account",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_ServiceAccountFields,
		},
		"ShieldedInstanceConfig": ubx.FieldSpec{
			WireName: "shielded_instance_config",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_ShieldedInstanceConfigFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
		"Symptoms": ubx.FieldSpec{
			WireName: "symptoms",
			Kind: "list",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_SymptomsFields,
		},
		"Tags": ubx.FieldSpec{WireName: "tags"},
		"UpcomingMaintenance": ubx.FieldSpec{
			WireName: "upcoming_maintenance",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_Node_UpcomingMaintenanceFields,
		},
	}

var TpuQueuedResource_Tpu_NodeSpecFields = ubx.FieldMap{
		"MultisliceParams": ubx.FieldSpec{
			WireName: "multislice_params",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_MultisliceParamsFields,
		},
		"Node": ubx.FieldSpec{
			WireName: "node",
			Kind: "object",
			Fields: TpuQueuedResource_Tpu_NodeSpec_NodeFields,
		},
		"NodeId": ubx.FieldSpec{WireName: "node_id"},
		"Parent": ubx.FieldSpec{WireName: "parent"},
	}

var TpuQueuedResource_TpuFields = ubx.FieldMap{
		"NodeSpec": ubx.FieldSpec{
			WireName: "node_spec",
			Kind: "list",
			Fields: TpuQueuedResource_Tpu_NodeSpecFields,
		},
	}

type TpuQueuedResourceConfig struct {
	// Output only. The time when the QueuedResource was created.
	CreateTime any
	// Guaranteed tier definition.
	Guaranteed any
	// Output only. Immutable. The name of the QueuedResource.
	Name any
	// Defines the policy of the QueuedRequest.
	QueueingPolicy any
	// Optional. Name of the reservation in which the resource should be provisioned. Format: projects/{project}/locations/{zone}/reservations/{reservation}
	ReservationName any
	// Spot tier definition.
	Spot any
	// QueuedResourceState defines the details of the QueuedResource request.
	State any
	// Details of the TPU resource(s) being requested.
	Tpu any
}

type TpuQueuedResourceAttrs struct {
	// Output only. The time when the QueuedResource was created.
	CreateTime any
	// Guaranteed tier definition.
	Guaranteed any
	// Output only. Immutable. The name of the QueuedResource.
	Name any
	// Defines the policy of the QueuedRequest.
	QueueingPolicy any
	// Optional. Name of the reservation in which the resource should be provisioned. Format: projects/{project}/locations/{zone}/reservations/{reservation}
	ReservationName any
	// Spot tier definition.
	Spot any
	// QueuedResourceState defines the details of the QueuedResource request.
	State any
	// Details of the TPU resource(s) being requested.
	Tpu any
}

var TpuQueuedResource = ubx.ResourceBinding{
	WireType: "google_tpu_tpu_queued_resource",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Guaranteed": ubx.FieldSpec{
			WireName: "guaranteed",
			Kind: "object",
			Fields: TpuQueuedResource_GuaranteedFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"QueueingPolicy": ubx.FieldSpec{
			WireName: "queueing_policy",
			Kind: "object",
			Fields: TpuQueuedResource_QueueingPolicyFields,
		},
		"ReservationName": ubx.FieldSpec{WireName: "reservation_name"},
		"Spot": ubx.FieldSpec{WireName: "spot"},
		"State": ubx.FieldSpec{
			WireName: "state",
			Kind: "object",
			Fields: TpuQueuedResource_StateFields,
		},
		"Tpu": ubx.FieldSpec{
			WireName: "tpu",
			Kind: "object",
			Fields: TpuQueuedResource_TpuFields,
		},
	},
}
