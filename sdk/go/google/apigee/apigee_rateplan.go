// Code generated by `ubx sdk gen` from google_apigee_apigee_rateplan. DO NOT EDIT.
package apigee

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ApigeeRateplan_ConsumptionPricingRates_Fee struct {
	CurrencyCode any
	Nanos any
	Units any
}

type ApigeeRateplan_ConsumptionPricingRates struct {
	End any
	Fee any
	Start any
}

type ApigeeRateplan_RevenueShareRates struct {
	End any
	SharePercentage any
	Start any
}

var ApigeeRateplan_ConsumptionPricingRates_FeeFields = ubx.FieldMap{
		"CurrencyCode": ubx.FieldSpec{WireName: "currency_code"},
		"Nanos": ubx.FieldSpec{WireName: "nanos"},
		"Units": ubx.FieldSpec{WireName: "units"},
	}

var ApigeeRateplan_ConsumptionPricingRatesFields = ubx.FieldMap{
		"End": ubx.FieldSpec{WireName: "end"},
		"Fee": ubx.FieldSpec{
			WireName: "fee",
			Kind: "object",
			Fields: ApigeeRateplan_ConsumptionPricingRates_FeeFields,
		},
		"Start": ubx.FieldSpec{WireName: "start"},
	}

var ApigeeRateplan_RevenueShareRatesFields = ubx.FieldMap{
		"End": ubx.FieldSpec{WireName: "end"},
		"SharePercentage": ubx.FieldSpec{WireName: "share_percentage"},
		"Start": ubx.FieldSpec{WireName: "start"},
	}

type ApigeeRateplanConfig struct {
	// Name of the API product that the rate plan is associated with.
	Apiproduct any
	// Frequency at which the customer will be billed.
	BillingPeriod any
	// API call volume ranges and the fees charged when the total number of API calls is within a given range. The method used to calculate the final fee depends on the selected pricing model. For example, if the pricing model is `BANDED` and the ranges are defined as follows: ``` { "start": 1, "end": 100, "fee": 2 }, { "start": 101, "end": 200, "fee": 1.50 }, { "start": 201, "end": 0, "fee": 1 }, } ``` Then the following fees would be charged based on the total number of API calls (assuming the currency selected is `USD`): * 50 calls cost 50 x $2 = $100 * 150 calls cost 100 x $2 + 50 x $1.5 = $275 * 250 calls cost 100 x $2 + 100 x $1.5 + 50 x $1 = $400 * 500 calls cost 100 x $2 + 100 x $1.5 + 300 x $1 = $650
	ConsumptionPricingRates any
	// Pricing model used for consumption-based charges.
	ConsumptionPricingType any
	// Output only. Time that the rate plan was created in milliseconds since epoch.
	CreatedAt any
	// Currency to be used for billing. Consists of a three-letter code as defined by the [ISO 4217](https://en.wikipedia.org/wiki/ISO_4217) standard.
	CurrencyCode any
	// Description of the rate plan.
	Description any
	// Display name of the rate plan.
	DisplayName any
	// Time when the rate plan will expire in milliseconds since epoch. Set to 0 or `null` to indicate that the rate plan should never expire.
	EndTime any
	// Frequency at which the fixed fee is charged.
	FixedFeeFrequency any
	// Represents an amount of money with its currency type.
	FixedRecurringFee any
	// Output only. Time the rate plan was last modified in milliseconds since epoch.
	LastModifiedAt any
	// Output only. Name of the rate plan.
	Name any
	// DEPRECATED: This field is no longer supported and will eventually be removed when Apigee Hybrid 1.5/1.6 is no longer supported. Instead, use the `billingType` field inside `DeveloperMonetizationConfig` resource. Flag that specifies the billing account type, prepaid or postpaid.
	PaymentFundingModel any
	// Details of the revenue sharing model.
	RevenueShareRates any
	// Method used to calculate the revenue that is shared with developers.
	RevenueShareType any
	// Represents an amount of money with its currency type.
	SetupFee any
	// Time when the rate plan becomes active in milliseconds since epoch.
	StartTime any
	// Current state of the rate plan (draft or published).
	State any
}

type ApigeeRateplanAttrs struct {
	// Name of the API product that the rate plan is associated with.
	Apiproduct any
	// Frequency at which the customer will be billed.
	BillingPeriod any
	// API call volume ranges and the fees charged when the total number of API calls is within a given range. The method used to calculate the final fee depends on the selected pricing model. For example, if the pricing model is `BANDED` and the ranges are defined as follows: ``` { "start": 1, "end": 100, "fee": 2 }, { "start": 101, "end": 200, "fee": 1.50 }, { "start": 201, "end": 0, "fee": 1 }, } ``` Then the following fees would be charged based on the total number of API calls (assuming the currency selected is `USD`): * 50 calls cost 50 x $2 = $100 * 150 calls cost 100 x $2 + 50 x $1.5 = $275 * 250 calls cost 100 x $2 + 100 x $1.5 + 50 x $1 = $400 * 500 calls cost 100 x $2 + 100 x $1.5 + 300 x $1 = $650
	ConsumptionPricingRates any
	// Pricing model used for consumption-based charges.
	ConsumptionPricingType any
	// Output only. Time that the rate plan was created in milliseconds since epoch.
	CreatedAt any
	// Currency to be used for billing. Consists of a three-letter code as defined by the [ISO 4217](https://en.wikipedia.org/wiki/ISO_4217) standard.
	CurrencyCode any
	// Description of the rate plan.
	Description any
	// Display name of the rate plan.
	DisplayName any
	// Time when the rate plan will expire in milliseconds since epoch. Set to 0 or `null` to indicate that the rate plan should never expire.
	EndTime any
	// Frequency at which the fixed fee is charged.
	FixedFeeFrequency any
	// Represents an amount of money with its currency type.
	FixedRecurringFee any
	// Output only. Time the rate plan was last modified in milliseconds since epoch.
	LastModifiedAt any
	// Output only. Name of the rate plan.
	Name any
	// DEPRECATED: This field is no longer supported and will eventually be removed when Apigee Hybrid 1.5/1.6 is no longer supported. Instead, use the `billingType` field inside `DeveloperMonetizationConfig` resource. Flag that specifies the billing account type, prepaid or postpaid.
	PaymentFundingModel any
	// Details of the revenue sharing model.
	RevenueShareRates any
	// Method used to calculate the revenue that is shared with developers.
	RevenueShareType any
	// Represents an amount of money with its currency type.
	SetupFee any
	// Time when the rate plan becomes active in milliseconds since epoch.
	StartTime any
	// Current state of the rate plan (draft or published).
	State any
}

var ApigeeRateplan = ubx.ResourceBinding{
	WireType: "google_apigee_apigee_rateplan",
	Fields: ubx.FieldMap{
		"Apiproduct": ubx.FieldSpec{WireName: "apiproduct"},
		"BillingPeriod": ubx.FieldSpec{WireName: "billing_period"},
		"ConsumptionPricingRates": ubx.FieldSpec{
			WireName: "consumption_pricing_rates",
			Kind: "list",
			Fields: ApigeeRateplan_ConsumptionPricingRatesFields,
		},
		"ConsumptionPricingType": ubx.FieldSpec{WireName: "consumption_pricing_type"},
		"CreatedAt": ubx.FieldSpec{WireName: "created_at"},
		"CurrencyCode": ubx.FieldSpec{WireName: "currency_code"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"EndTime": ubx.FieldSpec{WireName: "end_time"},
		"FixedFeeFrequency": ubx.FieldSpec{WireName: "fixed_fee_frequency"},
		"FixedRecurringFee": ubx.FieldSpec{
			WireName: "fixed_recurring_fee",
			Kind: "object",
			Fields: ApigeeRateplan_ConsumptionPricingRates_FeeFields,
		},
		"LastModifiedAt": ubx.FieldSpec{WireName: "last_modified_at"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PaymentFundingModel": ubx.FieldSpec{WireName: "payment_funding_model"},
		"RevenueShareRates": ubx.FieldSpec{
			WireName: "revenue_share_rates",
			Kind: "list",
			Fields: ApigeeRateplan_RevenueShareRatesFields,
		},
		"RevenueShareType": ubx.FieldSpec{WireName: "revenue_share_type"},
		"SetupFee": ubx.FieldSpec{
			WireName: "setup_fee",
			Kind: "object",
			Fields: ApigeeRateplan_ConsumptionPricingRates_FeeFields,
		},
		"StartTime": ubx.FieldSpec{WireName: "start_time"},
		"State": ubx.FieldSpec{WireName: "state"},
	},
}
