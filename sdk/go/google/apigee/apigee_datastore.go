// Code generated by `ubx sdk gen` from google_apigee_apigee_datastore. DO NOT EDIT.
package apigee

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ApigeeDatastore_DatastoreConfig struct {
	// Name of the Cloud Storage bucket. Required for `gcs` target_type.
	BucketName any
	// BigQuery dataset name Required for `bigquery` target_type.
	DatasetName any
	// Path of Cloud Storage bucket Required for `gcs` target_type.
	Path any
	// Required. Google Cloud project in which the datastore exists
	ProjectId any
	// Prefix of BigQuery table Required for `bigquery` target_type.
	TablePrefix any
}

var ApigeeDatastore_DatastoreConfigFields = ubx.FieldMap{
		"BucketName": ubx.FieldSpec{WireName: "bucket_name"},
		"DatasetName": ubx.FieldSpec{WireName: "dataset_name"},
		"Path": ubx.FieldSpec{WireName: "path"},
		"ProjectId": ubx.FieldSpec{WireName: "project_id"},
		"TablePrefix": ubx.FieldSpec{WireName: "table_prefix"},
	}

type ApigeeDatastoreConfig struct {
	// Output only. Datastore create time, in milliseconds since the epoch of 1970-01-01T00:00:00Z
	CreateTime any
	// Configuration detail for datastore
	DatastoreConfig any
	// Required. Display name in UI
	DisplayName any
	// Output only. Datastore last update time, in milliseconds since the epoch of 1970-01-01T00:00:00Z
	LastUpdateTime any
	// Output only. Organization that the datastore belongs to
	Org any
	// Output only. Resource link of Datastore. Example: `/organizations/{org}/analytics/datastores/{uuid}`
	Self any
	// Destination storage type. Supported types `gcs` or `bigquery`.
	TargetType any
}

type ApigeeDatastoreAttrs struct {
	// Output only. Datastore create time, in milliseconds since the epoch of 1970-01-01T00:00:00Z
	CreateTime any
	// Configuration detail for datastore
	DatastoreConfig any
	// Required. Display name in UI
	DisplayName any
	// Output only. Datastore last update time, in milliseconds since the epoch of 1970-01-01T00:00:00Z
	LastUpdateTime any
	// Output only. Organization that the datastore belongs to
	Org any
	// Output only. Resource link of Datastore. Example: `/organizations/{org}/analytics/datastores/{uuid}`
	Self any
	// Destination storage type. Supported types `gcs` or `bigquery`.
	TargetType any
}

var ApigeeDatastore = ubx.ResourceBinding{
	WireType: "google_apigee_apigee_datastore",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DatastoreConfig": ubx.FieldSpec{
			WireName: "datastore_config",
			Kind: "object",
			Fields: ApigeeDatastore_DatastoreConfigFields,
		},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"LastUpdateTime": ubx.FieldSpec{WireName: "last_update_time"},
		"Org": ubx.FieldSpec{WireName: "org"},
		"Self": ubx.FieldSpec{WireName: "self"},
		"TargetType": ubx.FieldSpec{WireName: "target_type"},
	},
}
