// Code generated by `ubx sdk gen` from google_datastream_datastream_private_connection. DO NOT EDIT.
package datastream

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DatastreamPrivateConnection_Error struct {
	// Additional information about the error.
	Details any
	// The time when the error occurred.
	ErrorTime any
	// A unique identifier for this specific error, allowing it to be traced throughout the system in logs and API responses.
	ErrorUuid any
	// A message containing more information about the error that occurred.
	Message any
	// A title that explains the reason for the error.
	Reason any
}

type DatastreamPrivateConnection_PscInterfaceConfig struct {
	// Required. Fully qualified name of the Network Attachment that Datastream will connect to. Format: `projects/{project}/regions/{region}/networkAttachments/{name}`
	NetworkAttachment any
}

type DatastreamPrivateConnection_VpcPeeringConfig struct {
	// Required. A free subnet for peering. (CIDR of /29)
	Subnet any
	// Required. Fully qualified name of the VPC that Datastream will peer to. Format: `projects/{project}/global/{networks}/{name}`
	Vpc any
}

var DatastreamPrivateConnection_ErrorFields = ubx.FieldMap{
		"Details": ubx.FieldSpec{WireName: "details"},
		"ErrorTime": ubx.FieldSpec{WireName: "error_time"},
		"ErrorUuid": ubx.FieldSpec{WireName: "error_uuid"},
		"Message": ubx.FieldSpec{WireName: "message"},
		"Reason": ubx.FieldSpec{WireName: "reason"},
	}

var DatastreamPrivateConnection_PscInterfaceConfigFields = ubx.FieldMap{
		"NetworkAttachment": ubx.FieldSpec{WireName: "network_attachment"},
	}

var DatastreamPrivateConnection_VpcPeeringConfigFields = ubx.FieldMap{
		"Subnet": ubx.FieldSpec{WireName: "subnet"},
		"Vpc": ubx.FieldSpec{WireName: "vpc"},
	}

type DatastreamPrivateConnectionConfig struct {
	// Output only. The create time of the resource.
	CreateTime any
	// Required. Display name.
	DisplayName any
	// Represent a user-facing Error.
	Error any
	// Labels.
	Labels any
	// Output only. Identifier. The resource's name.
	Name any
	// The PSC Interface configuration is used to create PSC Interface between Datastream and the consumer's PSC.
	PscInterfaceConfig any
	// Output only. Reserved for future use.
	SatisfiesPzi any
	// Output only. Reserved for future use.
	SatisfiesPzs any
	// Output only. The state of the Private Connection.
	State any
	// Output only. The update time of the resource.
	UpdateTime any
	// The VPC Peering configuration is used to create VPC peering between Datastream and the consumer's VPC.
	VpcPeeringConfig any
}

type DatastreamPrivateConnectionAttrs struct {
	// Output only. The create time of the resource.
	CreateTime any
	// Required. Display name.
	DisplayName any
	// Represent a user-facing Error.
	Error any
	// Labels.
	Labels any
	// Output only. Identifier. The resource's name.
	Name any
	// The PSC Interface configuration is used to create PSC Interface between Datastream and the consumer's PSC.
	PscInterfaceConfig any
	// Output only. Reserved for future use.
	SatisfiesPzi any
	// Output only. Reserved for future use.
	SatisfiesPzs any
	// Output only. The state of the Private Connection.
	State any
	// Output only. The update time of the resource.
	UpdateTime any
	// The VPC Peering configuration is used to create VPC peering between Datastream and the consumer's VPC.
	VpcPeeringConfig any
}

var DatastreamPrivateConnection = ubx.ResourceBinding{
	WireType: "google_datastream_datastream_private_connection",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"Error": ubx.FieldSpec{
			WireName: "error",
			Kind: "object",
			Fields: DatastreamPrivateConnection_ErrorFields,
		},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PscInterfaceConfig": ubx.FieldSpec{
			WireName: "psc_interface_config",
			Kind: "object",
			Fields: DatastreamPrivateConnection_PscInterfaceConfigFields,
		},
		"SatisfiesPzi": ubx.FieldSpec{WireName: "satisfies_pzi"},
		"SatisfiesPzs": ubx.FieldSpec{WireName: "satisfies_pzs"},
		"State": ubx.FieldSpec{WireName: "state"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
		"VpcPeeringConfig": ubx.FieldSpec{
			WireName: "vpc_peering_config",
			Kind: "object",
			Fields: DatastreamPrivateConnection_VpcPeeringConfigFields,
		},
	},
}
