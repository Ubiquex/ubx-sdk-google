// Code generated by `ubx sdk gen` from google_workloadmanager_workloadmanager_deployment. DO NOT EDIT.
package workloadmanager

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type WorkloadmanagerDeployment_SapSystemS4Config_App struct {
	// Optional. Instance ID for app.
	AppInstanceId any
	// Application service account. Let customers bring their own service account for the application.
	AppServiceAccount any
	// Optional. Customized VM names.
	AppVmNames any
	// Required. Image for the ASCS server.
	AscsImage any
	// Optional. Instance ID for ASCS.
	AscsInstanceId any
	// Required. ASCS machine type.
	AscsMachineType any
	// ASCS service account. Let customers bring their own service account for ASCS.
	AscsServiceAccount any
	// Optional. ASCS VM name.
	AscsVm any
	// Optional. Instance ID for ERS.
	ErsInstanceId any
	// Optional. ERS VM name.
	ErsVm any
	// Required. Image for the app server and ASCS server.
	Image any
	// Required. Machine type.
	MachineType any
	// Required. Secret Manager secret.
	SecretManagerSecret any
	// Optional. Storage location.
	SharedStorage any
	// Required. The SAP SID is a three-digit server-specific unique identification code.
	Sid any
	// Required. VMs multiplier.
	VmsMultiplier any
}

type WorkloadmanagerDeployment_SapSystemS4Config_Database struct {
	// Database service account. Let customers bring their own SA for the database.
	DatabaseServiceAccount any
	// Required. Disk type.
	DiskType any
	// Required. Image for the database server.
	Image any
	// Optional. Instance ID.
	InstanceId any
	// Required. Machine type.
	MachineType any
	// Optional. Primary DB VM name.
	PrimaryDbVm any
	// Optional. Secondary DB VM name.
	SecondaryDbVm any
	// Required. Secret Manager secret.
	SecretManagerSecret any
	// Required. The SID is a three-digit server-specific unique identification code.
	Sid any
}

type WorkloadmanagerDeployment_SapSystemS4Config_Location struct {
	// Optional. Create firewall. If true, creates a firewall for the deployment. This field provides an option to not always create a firewall for the deployment.
	CreateCommsFirewall any
	// Optional. Network tags.
	CustomTags any
	// Optional. When the user skips DNS configuration in the UI, `deployment_dns_enabled` is false; otherwise `deployment_dns_enabled` is true.
	DeploymentDnsEnabled any
	// Optional. DNS zone name.
	DnsZone any
	// Optional. DNS zone name suffix.
	DnsZoneNameSuffix any
	InternetAccess any
	// Optional. Network project.
	NetworkProject any
	// Required. Region name.
	RegionName any
	// Required. Subnet name.
	SubnetName any
	// Required. VPC name.
	VpcName any
	// Required. Zone 1 name.
	Zone1Name any
	// Optional. Zone 2 name.
	Zone2Name any
}

type WorkloadmanagerDeployment_SapSystemS4Config struct {
	AllowStoppingForUpdate any
	// Ansible runner service account. Let customers bring their own service account for the Ansible runner.
	AnsibleRunnerServiceAccount any
	// Message for SAP instance details.
	App any
	// Message for SAP instance details.
	Database any
	// Required. Supports non-HA and HA models.
	DeploymentModel any
	// Required. Deployment environment.
	EnvironmentType any
	// The project that infrastructure is deployed in. Currently only supports the same project where the deployment resource exists.
	GcpProjectId any
	// Message for SAP instance details.
	Location any
	// Required. Media bucket name.
	MediaBucketName any
	// Optional. SAP boot disk image.
	SapBootDiskImage any
	// Required. Supports scale up and scale out.
	ScalingMethod any
	// Required. SAP HANA version.
	Version any
	// VM prefix.
	VmPrefix any
}

type WorkloadmanagerDeployment_SqlServerWorkload_ActiveDirectory struct {
	// Optional. DNS IP address.
	DnsAddress any
	// Optional. Human readable form of a domain such as `example.com`.
	Domain any
	// Optional. Domain username.
	DomainUsername any
	// Required. Secret Manager secret.
	SecretManagerSecret any
	// Required. Active Directory type.
	Type any
}

type WorkloadmanagerDeployment_SqlServerWorkload_Database struct {
	// Required. Disk type.
	DiskType any
	// Optional. Only useful for Linux High Availability setup.
	FloatingIpAddress any
	// Required. Machine type.
	MachineType any
	// Optional. The name of a secondary-sole-tenant node/node group.
	SecondarySoleTenantNode any
	// Optional. The type of a secondary-sole-tenant node/node group. E.g., compute.googleapis.com/node-name.
	SecondarySoleTenantNodeType any
	// Required. Secret Manager secret.
	SecretManagerSecret any
	// Required. Whether simultaneous multithreading is enabled or not.
	Smt any
	// Optional. The name of a primary sole-tenant node/node group.
	SoleTenantNode any
	// Optional. The type of a primary sole-tenant node/node group. E.g., compute.googleapis.com/node-name.
	SoleTenantNodeType any
	// Required. Whether to have TempDB on local SSD.
	TempdbOnSsd any
	// Required. SHARED or SOLE_TENANT.
	TenancyModel any
}

type WorkloadmanagerDeployment_SqlServerWorkload_Location struct {
	// Optional. Create a new DNS zone when the field is empty. Only shown for `Using an existing DNS`. List of existing DNS zones. Terraform variable name: existing_dns_zone_name.
	DnsZone any
	// Required. The project that infrastructure is deployed in. Currently only supports the same project where the deployment resource exists.
	GcpProjectId any
	// Required. Internet Access.
	InternetAccess any
	// Required. Network name.
	Network any
	// Required. Primary zone.
	PrimaryZone any
	// Required. Region name.
	Region any
	// Optional. Secondary zone cannot be the same as primary_zone and is only for High Availability deployment mode.
	SecondaryZone any
	// Required. Subnetwork name.
	Subnetwork any
	// Optional. Tertiary zone cannot be the same as primary_zone and secondary_zone, and it is only for High Availability deployment mode.
	TertiaryZone any
}

type WorkloadmanagerDeployment_SqlServerWorkload_Pacemaker struct {
	// Required. Bucket location for node certificates.
	BucketNameNodeCertificates any
	// Required. Pacemaker cluster name.
	PacemakerCluster any
	// Required. Pacemaker cluster secret name.
	PacemakerClusterSecret any
	// Required. Pacemaker cluster username.
	PacemakerClusterUsername any
	// Required. SQL Pacemaker secret name.
	SqlPacemakerSecret any
	// Required. SQL Pacemaker username.
	SqlPacemakerUsername any
}

type WorkloadmanagerDeployment_SqlServerWorkload struct {
	// Active Directory details.
	ActiveDirectory any
	// Compute Engine service account. Let customers bring their own service account for Compute Engine.
	ComputeEngineServiceAccount any
	// Database details.
	Database any
	// Required. HIGH_AVAILABILITY or SINGLE_INSTANCE.
	DeploymentModel any
	// Required. Deployment environment.
	EnvironmentType any
	// Optional. SHARED_DISK or S2D.
	FciType any
	// Optional. AOAG or FCI. It is only needed for the High Availability deployment mode.
	HaType any
	// Required. SQL licensing type.
	IsSqlPayg any
	// Location and networking details for configuring SQL server workload.
	Location any
	// Required. Name of the media storing SQL server installation files.
	MediaBucket any
	// Required. The type of the operating system the SQL server is going to run on top of.
	OperatingSystemType any
	// Required. The image of the operating system.
	OsImage any
	// Optional. OS image type. It's used to create boot disks for VM instances. When either Windows licensing type or SQL licensing type is BYOL, this option is disabled and defaults to a custom image.
	OsImageType any
	// Pacemaker configuration.
	Pacemaker any
	// Optional. SQL Server Edition type, only applicable when the operating system is Linux.
	SqlServerEdition any
	// Optional. 2017, 2019, or 2022.
	SqlServerVersion any
	// Required. Should be unique in the project.
	VmPrefix any
}

type WorkloadmanagerDeployment_TerraformVariables struct {
	InputValue any
}

var WorkloadmanagerDeployment_SapSystemS4Config_AppFields = ubx.FieldMap{
		"AppInstanceId": ubx.FieldSpec{WireName: "app_instance_id"},
		"AppServiceAccount": ubx.FieldSpec{WireName: "app_service_account"},
		"AppVmNames": ubx.FieldSpec{WireName: "app_vm_names"},
		"AscsImage": ubx.FieldSpec{WireName: "ascs_image"},
		"AscsInstanceId": ubx.FieldSpec{WireName: "ascs_instance_id"},
		"AscsMachineType": ubx.FieldSpec{WireName: "ascs_machine_type"},
		"AscsServiceAccount": ubx.FieldSpec{WireName: "ascs_service_account"},
		"AscsVm": ubx.FieldSpec{WireName: "ascs_vm"},
		"ErsInstanceId": ubx.FieldSpec{WireName: "ers_instance_id"},
		"ErsVm": ubx.FieldSpec{WireName: "ers_vm"},
		"Image": ubx.FieldSpec{WireName: "image"},
		"MachineType": ubx.FieldSpec{WireName: "machine_type"},
		"SecretManagerSecret": ubx.FieldSpec{WireName: "secret_manager_secret"},
		"SharedStorage": ubx.FieldSpec{WireName: "shared_storage"},
		"Sid": ubx.FieldSpec{WireName: "sid"},
		"VmsMultiplier": ubx.FieldSpec{WireName: "vms_multiplier"},
	}

var WorkloadmanagerDeployment_SapSystemS4Config_DatabaseFields = ubx.FieldMap{
		"DatabaseServiceAccount": ubx.FieldSpec{WireName: "database_service_account"},
		"DiskType": ubx.FieldSpec{WireName: "disk_type"},
		"Image": ubx.FieldSpec{WireName: "image"},
		"InstanceId": ubx.FieldSpec{WireName: "instance_id"},
		"MachineType": ubx.FieldSpec{WireName: "machine_type"},
		"PrimaryDbVm": ubx.FieldSpec{WireName: "primary_db_vm"},
		"SecondaryDbVm": ubx.FieldSpec{WireName: "secondary_db_vm"},
		"SecretManagerSecret": ubx.FieldSpec{WireName: "secret_manager_secret"},
		"Sid": ubx.FieldSpec{WireName: "sid"},
	}

var WorkloadmanagerDeployment_SapSystemS4Config_LocationFields = ubx.FieldMap{
		"CreateCommsFirewall": ubx.FieldSpec{WireName: "create_comms_firewall"},
		"CustomTags": ubx.FieldSpec{WireName: "custom_tags"},
		"DeploymentDnsEnabled": ubx.FieldSpec{WireName: "deployment_dns_enabled"},
		"DnsZone": ubx.FieldSpec{WireName: "dns_zone"},
		"DnsZoneNameSuffix": ubx.FieldSpec{WireName: "dns_zone_name_suffix"},
		"InternetAccess": ubx.FieldSpec{WireName: "internet_access"},
		"NetworkProject": ubx.FieldSpec{WireName: "network_project"},
		"RegionName": ubx.FieldSpec{WireName: "region_name"},
		"SubnetName": ubx.FieldSpec{WireName: "subnet_name"},
		"VpcName": ubx.FieldSpec{WireName: "vpc_name"},
		"Zone1Name": ubx.FieldSpec{WireName: "zone1_name"},
		"Zone2Name": ubx.FieldSpec{WireName: "zone2_name"},
	}

var WorkloadmanagerDeployment_SapSystemS4ConfigFields = ubx.FieldMap{
		"AllowStoppingForUpdate": ubx.FieldSpec{WireName: "allow_stopping_for_update"},
		"AnsibleRunnerServiceAccount": ubx.FieldSpec{WireName: "ansible_runner_service_account"},
		"App": ubx.FieldSpec{
			WireName: "app",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SapSystemS4Config_AppFields,
		},
		"Database": ubx.FieldSpec{
			WireName: "database",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SapSystemS4Config_DatabaseFields,
		},
		"DeploymentModel": ubx.FieldSpec{WireName: "deployment_model"},
		"EnvironmentType": ubx.FieldSpec{WireName: "environment_type"},
		"GcpProjectId": ubx.FieldSpec{WireName: "gcp_project_id"},
		"Location": ubx.FieldSpec{
			WireName: "location",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SapSystemS4Config_LocationFields,
		},
		"MediaBucketName": ubx.FieldSpec{WireName: "media_bucket_name"},
		"SapBootDiskImage": ubx.FieldSpec{WireName: "sap_boot_disk_image"},
		"ScalingMethod": ubx.FieldSpec{WireName: "scaling_method"},
		"Version": ubx.FieldSpec{WireName: "version"},
		"VmPrefix": ubx.FieldSpec{WireName: "vm_prefix"},
	}

var WorkloadmanagerDeployment_SqlServerWorkload_ActiveDirectoryFields = ubx.FieldMap{
		"DnsAddress": ubx.FieldSpec{WireName: "dns_address"},
		"Domain": ubx.FieldSpec{WireName: "domain"},
		"DomainUsername": ubx.FieldSpec{WireName: "domain_username"},
		"SecretManagerSecret": ubx.FieldSpec{WireName: "secret_manager_secret"},
		"Type": ubx.FieldSpec{WireName: "type"},
	}

var WorkloadmanagerDeployment_SqlServerWorkload_DatabaseFields = ubx.FieldMap{
		"DiskType": ubx.FieldSpec{WireName: "disk_type"},
		"FloatingIpAddress": ubx.FieldSpec{WireName: "floating_ip_address"},
		"MachineType": ubx.FieldSpec{WireName: "machine_type"},
		"SecondarySoleTenantNode": ubx.FieldSpec{WireName: "secondary_sole_tenant_node"},
		"SecondarySoleTenantNodeType": ubx.FieldSpec{WireName: "secondary_sole_tenant_node_type"},
		"SecretManagerSecret": ubx.FieldSpec{WireName: "secret_manager_secret"},
		"Smt": ubx.FieldSpec{WireName: "smt"},
		"SoleTenantNode": ubx.FieldSpec{WireName: "sole_tenant_node"},
		"SoleTenantNodeType": ubx.FieldSpec{WireName: "sole_tenant_node_type"},
		"TempdbOnSsd": ubx.FieldSpec{WireName: "tempdb_on_ssd"},
		"TenancyModel": ubx.FieldSpec{WireName: "tenancy_model"},
	}

var WorkloadmanagerDeployment_SqlServerWorkload_LocationFields = ubx.FieldMap{
		"DnsZone": ubx.FieldSpec{WireName: "dns_zone"},
		"GcpProjectId": ubx.FieldSpec{WireName: "gcp_project_id"},
		"InternetAccess": ubx.FieldSpec{WireName: "internet_access"},
		"Network": ubx.FieldSpec{WireName: "network"},
		"PrimaryZone": ubx.FieldSpec{WireName: "primary_zone"},
		"Region": ubx.FieldSpec{WireName: "region"},
		"SecondaryZone": ubx.FieldSpec{WireName: "secondary_zone"},
		"Subnetwork": ubx.FieldSpec{WireName: "subnetwork"},
		"TertiaryZone": ubx.FieldSpec{WireName: "tertiary_zone"},
	}

var WorkloadmanagerDeployment_SqlServerWorkload_PacemakerFields = ubx.FieldMap{
		"BucketNameNodeCertificates": ubx.FieldSpec{WireName: "bucket_name_node_certificates"},
		"PacemakerCluster": ubx.FieldSpec{WireName: "pacemaker_cluster"},
		"PacemakerClusterSecret": ubx.FieldSpec{WireName: "pacemaker_cluster_secret"},
		"PacemakerClusterUsername": ubx.FieldSpec{WireName: "pacemaker_cluster_username"},
		"SqlPacemakerSecret": ubx.FieldSpec{WireName: "sql_pacemaker_secret"},
		"SqlPacemakerUsername": ubx.FieldSpec{WireName: "sql_pacemaker_username"},
	}

var WorkloadmanagerDeployment_SqlServerWorkloadFields = ubx.FieldMap{
		"ActiveDirectory": ubx.FieldSpec{
			WireName: "active_directory",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SqlServerWorkload_ActiveDirectoryFields,
		},
		"ComputeEngineServiceAccount": ubx.FieldSpec{WireName: "compute_engine_service_account"},
		"Database": ubx.FieldSpec{
			WireName: "database",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SqlServerWorkload_DatabaseFields,
		},
		"DeploymentModel": ubx.FieldSpec{WireName: "deployment_model"},
		"EnvironmentType": ubx.FieldSpec{WireName: "environment_type"},
		"FciType": ubx.FieldSpec{WireName: "fci_type"},
		"HaType": ubx.FieldSpec{WireName: "ha_type"},
		"IsSqlPayg": ubx.FieldSpec{WireName: "is_sql_payg"},
		"Location": ubx.FieldSpec{
			WireName: "location",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SqlServerWorkload_LocationFields,
		},
		"MediaBucket": ubx.FieldSpec{WireName: "media_bucket"},
		"OperatingSystemType": ubx.FieldSpec{WireName: "operating_system_type"},
		"OsImage": ubx.FieldSpec{WireName: "os_image"},
		"OsImageType": ubx.FieldSpec{WireName: "os_image_type"},
		"Pacemaker": ubx.FieldSpec{
			WireName: "pacemaker",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SqlServerWorkload_PacemakerFields,
		},
		"SqlServerEdition": ubx.FieldSpec{WireName: "sql_server_edition"},
		"SqlServerVersion": ubx.FieldSpec{WireName: "sql_server_version"},
		"VmPrefix": ubx.FieldSpec{WireName: "vm_prefix"},
	}

var WorkloadmanagerDeployment_TerraformVariablesFields = ubx.FieldMap{
		"InputValue": ubx.FieldSpec{WireName: "input_value"},
	}

type WorkloadmanagerDeploymentConfig struct {
	// Output only. Create time stamp.
	CreateTime any
	// Description of the deployment.
	Description any
	// The name of the deployment resource. The format is 'projects/{project_id}/locations/{location_id}/deployments/{deployment_id}'.
	Name any
	// Message for SAP system workload.
	SapSystemS4Config any
	// User-specified Service Account (SA) credentials to be used for Cloud Build. Format: `projects/{projectID}/serviceAccounts/{serviceAccount}` The default Cloud Build SA will be used initially if this field is not set during deployment creation.
	ServiceAccount any
	// Message for MS SQL workload.
	SqlServerWorkload any
	// Output only. Current state of the deployment.
	State any
	// Optional. terraform_variables represents all the Terraform variables for the deployment workload. The key is the name of the Terraform variable, and the value is the TerraformVariable. For example: { "project_id": { "input_value": { "string_value": "my-project-id" } }, "zone": { "input_value": { "string_value": "us-central1-a" } } }
	TerraformVariables any
	// Output only. Update time stamp.
	UpdateTime any
	// Optional. The user-specified Cloud Build worker pool resource in which the Cloud Build job will execute. Format: `projects/{project}/locations/{location}/workerPools/{workerPoolId}`. If this field is unspecified, the default Cloud Build worker pool will be used.
	WorkerPool any
	// Optional. Workload type of the deployment.
	WorkloadType any
}

type WorkloadmanagerDeploymentAttrs struct {
	// Output only. Create time stamp.
	CreateTime any
	// Description of the deployment.
	Description any
	// The name of the deployment resource. The format is 'projects/{project_id}/locations/{location_id}/deployments/{deployment_id}'.
	Name any
	// Message for SAP system workload.
	SapSystemS4Config any
	// User-specified Service Account (SA) credentials to be used for Cloud Build. Format: `projects/{projectID}/serviceAccounts/{serviceAccount}` The default Cloud Build SA will be used initially if this field is not set during deployment creation.
	ServiceAccount any
	// Message for MS SQL workload.
	SqlServerWorkload any
	// Output only. Current state of the deployment.
	State any
	// Optional. terraform_variables represents all the Terraform variables for the deployment workload. The key is the name of the Terraform variable, and the value is the TerraformVariable. For example: { "project_id": { "input_value": { "string_value": "my-project-id" } }, "zone": { "input_value": { "string_value": "us-central1-a" } } }
	TerraformVariables any
	// Output only. Update time stamp.
	UpdateTime any
	// Optional. The user-specified Cloud Build worker pool resource in which the Cloud Build job will execute. Format: `projects/{project}/locations/{location}/workerPools/{workerPoolId}`. If this field is unspecified, the default Cloud Build worker pool will be used.
	WorkerPool any
	// Optional. Workload type of the deployment.
	WorkloadType any
}

var WorkloadmanagerDeployment = ubx.ResourceBinding{
	WireType: "google_workloadmanager_workloadmanager_deployment",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"SapSystemS4Config": ubx.FieldSpec{
			WireName: "sap_system_s4_config",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SapSystemS4ConfigFields,
		},
		"ServiceAccount": ubx.FieldSpec{WireName: "service_account"},
		"SqlServerWorkload": ubx.FieldSpec{
			WireName: "sql_server_workload",
			Kind: "object",
			Fields: WorkloadmanagerDeployment_SqlServerWorkloadFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
		"TerraformVariables": ubx.FieldSpec{
			WireName: "terraform_variables",
			Kind: "map",
			Fields: WorkloadmanagerDeployment_TerraformVariablesFields,
		},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
		"WorkerPool": ubx.FieldSpec{WireName: "worker_pool"},
		"WorkloadType": ubx.FieldSpec{WireName: "workload_type"},
	},
}
