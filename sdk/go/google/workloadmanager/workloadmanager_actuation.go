// Code generated by `ubx sdk gen` from google_workloadmanager_workloadmanager_actuation. DO NOT EDIT.
package workloadmanager

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type WorkloadmanagerActuation_ActuationOutput struct {
	// A link to the Cloud Storage file that stores build logs.
	ActuateLogs any
	// Output only. Error message returned from Ansible.
	AnsibleError any
	// Output only. Failed task name returned from Ansible.
	AnsibleFailedTask any
	// Reference to the Blueprint Controller deployment and revision resource.
	BlueprintId any
	// Cloud Build instance UUID associated with this revision, without any suffix or prefix
	CloudbuildId any
	// Output only. Code describing any errors that may have occurred. If not specified, there is no error in actuation.
	ErrorCode any
	// A link to the actuation Cloud Build log.
	ErrorLogs any
	// Output only. Whether the error message is user facing. If true, the error message will be shown in the UI.
	HasUserFacingErrorMsg any
	// Output only. Error message returned from Terraform.
	TerraformError any
	// Reference to the Terraform template used.
	TerraformTemplate any
}

type WorkloadmanagerActuation_DeploymentOutput struct {
	Name any
	Type any
}

var WorkloadmanagerActuation_ActuationOutputFields = ubx.FieldMap{
		"ActuateLogs": ubx.FieldSpec{WireName: "actuate_logs"},
		"AnsibleError": ubx.FieldSpec{WireName: "ansible_error"},
		"AnsibleFailedTask": ubx.FieldSpec{WireName: "ansible_failed_task"},
		"BlueprintId": ubx.FieldSpec{WireName: "blueprint_id"},
		"CloudbuildId": ubx.FieldSpec{WireName: "cloudbuild_id"},
		"ErrorCode": ubx.FieldSpec{WireName: "error_code"},
		"ErrorLogs": ubx.FieldSpec{WireName: "error_logs"},
		"HasUserFacingErrorMsg": ubx.FieldSpec{WireName: "has_user_facing_error_msg"},
		"TerraformError": ubx.FieldSpec{WireName: "terraform_error"},
		"TerraformTemplate": ubx.FieldSpec{WireName: "terraform_template"},
	}

var WorkloadmanagerActuation_DeploymentOutputFields = ubx.FieldMap{
		"Name": ubx.FieldSpec{WireName: "name"},
		"Type": ubx.FieldSpec{WireName: "type"},
	}

type WorkloadmanagerActuationConfig struct {
	// Message for output of actuation.
	ActuationOutput any
	// Output only. Deployment output.
	DeploymentOutput any
	// Output only. End time stamp.
	EndTime any
	// The name of the actuation resource. The format is projects/{project}/locations/{location}/deployments/{deployment}/actuations/{actuation}.
	Name any
	// Output only. Start time stamp.
	StartTime any
	// Output only. Actuation state.
	State any
}

type WorkloadmanagerActuationAttrs struct {
	// Message for output of actuation.
	ActuationOutput any
	// Output only. Deployment output.
	DeploymentOutput any
	// Output only. End time stamp.
	EndTime any
	// The name of the actuation resource. The format is projects/{project}/locations/{location}/deployments/{deployment}/actuations/{actuation}.
	Name any
	// Output only. Start time stamp.
	StartTime any
	// Output only. Actuation state.
	State any
}

var WorkloadmanagerActuation = ubx.ResourceBinding{
	WireType: "google_workloadmanager_workloadmanager_actuation",
	Fields: ubx.FieldMap{
		"ActuationOutput": ubx.FieldSpec{
			WireName: "actuation_output",
			Kind: "object",
			Fields: WorkloadmanagerActuation_ActuationOutputFields,
		},
		"DeploymentOutput": ubx.FieldSpec{
			WireName: "deployment_output",
			Kind: "list",
			Fields: WorkloadmanagerActuation_DeploymentOutputFields,
		},
		"EndTime": ubx.FieldSpec{WireName: "end_time"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"StartTime": ubx.FieldSpec{WireName: "start_time"},
		"State": ubx.FieldSpec{WireName: "state"},
	},
}
