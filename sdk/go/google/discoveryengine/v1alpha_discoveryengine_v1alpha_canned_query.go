// Code generated by `ubx sdk gen` from google_discoveryengine_v1alpha_discoveryengine_v1alpha_canned_query. DO NOT EDIT.
package discoveryengine

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTexts_SuggestedPrompts struct {
	PromptText any
}

type V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTexts struct {
	// Optional. The prefix that `suggested_prompts` should start with.
	Prefix any
	// Required. The prompts the canned query will offer to the user.
	SuggestedPrompts any
	// Required. The title that is for the end user.
	Title any
}

type V1alphaDiscoveryengineV1alphaCannedQuery_RequiredCapabilities struct {
	ActionName any
}

var V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTexts_SuggestedPromptsFields = ubx.FieldMap{
		"PromptText": ubx.FieldSpec{WireName: "prompt_text"},
	}

var V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTextsFields = ubx.FieldMap{
		"Prefix": ubx.FieldSpec{WireName: "prefix"},
		"SuggestedPrompts": ubx.FieldSpec{
			WireName: "suggested_prompts",
			Kind: "list",
			Fields: V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTexts_SuggestedPromptsFields,
		},
		"Title": ubx.FieldSpec{WireName: "title"},
	}

var V1alphaDiscoveryengineV1alphaCannedQuery_RequiredCapabilitiesFields = ubx.FieldMap{
		"ActionName": ubx.FieldSpec{WireName: "action_name"},
	}

type V1alphaDiscoveryengineV1alphaCannedQueryConfig struct {
	// The text pieces for the canned query, which can be localized.
	DefaultTexts any
	// The display name of the canned query. It must be a UTF-8 encoded string with a length limit of 128 characters.
	DisplayName any
	// Whether this canned query is enabled.
	Enabled any
	// Output only. Whether this is a Google-defined, read-only canned query.
	GoogleDefined any
	// Optional. The translations of the text attributes. The keys should be BCP-47 language codes.
	LocalizedTexts any
	// Immutable. Resource name of the canned query. Format: `projects/{project}/locations/{location}/collections/{collection}/engines/{engine}/assistants/{assistant}/cannedQueries/{canned_query}` It must be a UTF-8 encoded string with a length limit of 1024 characters.
	Name any
	// Optional. The capabilities the Assistant needs to have to use this canned query.
	RequiredCapabilities any
}

type V1alphaDiscoveryengineV1alphaCannedQueryAttrs struct {
	// The text pieces for the canned query, which can be localized.
	DefaultTexts any
	// The display name of the canned query. It must be a UTF-8 encoded string with a length limit of 128 characters.
	DisplayName any
	// Whether this canned query is enabled.
	Enabled any
	// Output only. Whether this is a Google-defined, read-only canned query.
	GoogleDefined any
	// Optional. The translations of the text attributes. The keys should be BCP-47 language codes.
	LocalizedTexts any
	// Immutable. Resource name of the canned query. Format: `projects/{project}/locations/{location}/collections/{collection}/engines/{engine}/assistants/{assistant}/cannedQueries/{canned_query}` It must be a UTF-8 encoded string with a length limit of 1024 characters.
	Name any
	// Optional. The capabilities the Assistant needs to have to use this canned query.
	RequiredCapabilities any
}

var V1alphaDiscoveryengineV1alphaCannedQuery = ubx.ResourceBinding{
	WireType: "google_discoveryengine_v1alpha_discoveryengine_v1alpha_canned_query",
	Fields: ubx.FieldMap{
		"DefaultTexts": ubx.FieldSpec{
			WireName: "default_texts",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTextsFields,
		},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"Enabled": ubx.FieldSpec{WireName: "enabled"},
		"GoogleDefined": ubx.FieldSpec{WireName: "google_defined"},
		"LocalizedTexts": ubx.FieldSpec{
			WireName: "localized_texts",
			Kind: "map",
			Fields: V1alphaDiscoveryengineV1alphaCannedQuery_DefaultTextsFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"RequiredCapabilities": ubx.FieldSpec{
			WireName: "required_capabilities",
			Kind: "list",
			Fields: V1alphaDiscoveryengineV1alphaCannedQuery_RequiredCapabilitiesFields,
		},
	},
}
