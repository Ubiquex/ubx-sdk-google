// Code generated by `ubx sdk gen` from google_discoveryengine_discoveryengine_identity_mapping_store. DO NOT EDIT.
package discoveryengine

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DiscoveryengineIdentityMappingStore_CmekConfig_SingleRegionKeys struct {
	KmsKey any
}

type DiscoveryengineIdentityMappingStore_CmekConfig struct {
	// Output only. The default CmekConfig for the Customer.
	IsDefault any
	// Required. KMS key resource name which will be used to encrypt resources `projects/{project}/locations/{location}/keyRings/{keyRing}/cryptoKeys/{keyId}`.
	KmsKey any
	// Output only. KMS key version resource name which will be used to encrypt resources `/cryptoKeyVersions/{keyVersion}`.
	KmsKeyVersion any
	// Output only. The timestamp of the last key rotation.
	LastRotationTimestampMicros any
	// Required. The name of the CmekConfig of the form `projects/{project}/locations/{location}/cmekConfig` or `projects/{project}/locations/{location}/cmekConfigs/{cmek_config}`.
	Name any
	// Output only. Whether the NotebookLM Corpus is ready to be used.
	NotebooklmState any
	// Optional. Single-regional CMEKs that are required for some VAIS features.
	SingleRegionKeys any
	// Output only. The states of the CmekConfig.
	State any
}

var DiscoveryengineIdentityMappingStore_CmekConfig_SingleRegionKeysFields = ubx.FieldMap{
		"KmsKey": ubx.FieldSpec{WireName: "kms_key"},
	}

var DiscoveryengineIdentityMappingStore_CmekConfigFields = ubx.FieldMap{
		"IsDefault": ubx.FieldSpec{WireName: "is_default"},
		"KmsKey": ubx.FieldSpec{WireName: "kms_key"},
		"KmsKeyVersion": ubx.FieldSpec{WireName: "kms_key_version"},
		"LastRotationTimestampMicros": ubx.FieldSpec{WireName: "last_rotation_timestamp_micros"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"NotebooklmState": ubx.FieldSpec{WireName: "notebooklm_state"},
		"SingleRegionKeys": ubx.FieldSpec{
			WireName: "single_region_keys",
			Kind: "list",
			Fields: DiscoveryengineIdentityMappingStore_CmekConfig_SingleRegionKeysFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
	}

type DiscoveryengineIdentityMappingStoreConfig struct {
	// Configurations used to enable CMEK data encryption with Cloud KMS keys.
	CmekConfig any
	// Input only. The KMS key to be used to protect this Identity Mapping Store at creation time. Must be set for requests that need to comply with CMEK Org Policy protections. If this field is set and processed successfully, the Identity Mapping Store will be protected by the KMS key, as indicated in the cmek_config field.
	KmsKeyName any
	// Immutable. The full resource name of the identity mapping store. Format: `projects/{project}/locations/{location}/identityMappingStores/{identity_mapping_store}`. This field must be a UTF-8 encoded string with a length limit of 1024 characters.
	Name any
}

type DiscoveryengineIdentityMappingStoreAttrs struct {
	// Configurations used to enable CMEK data encryption with Cloud KMS keys.
	CmekConfig any
	// Input only. The KMS key to be used to protect this Identity Mapping Store at creation time. Must be set for requests that need to comply with CMEK Org Policy protections. If this field is set and processed successfully, the Identity Mapping Store will be protected by the KMS key, as indicated in the cmek_config field.
	KmsKeyName any
	// Immutable. The full resource name of the identity mapping store. Format: `projects/{project}/locations/{location}/identityMappingStores/{identity_mapping_store}`. This field must be a UTF-8 encoded string with a length limit of 1024 characters.
	Name any
}

var DiscoveryengineIdentityMappingStore = ubx.ResourceBinding{
	WireType: "google_discoveryengine_discoveryengine_identity_mapping_store",
	Fields: ubx.FieldMap{
		"CmekConfig": ubx.FieldSpec{
			WireName: "cmek_config",
			Kind: "object",
			Fields: DiscoveryengineIdentityMappingStore_CmekConfigFields,
		},
		"KmsKeyName": ubx.FieldSpec{WireName: "kms_key_name"},
		"Name": ubx.FieldSpec{WireName: "name"},
	},
}
