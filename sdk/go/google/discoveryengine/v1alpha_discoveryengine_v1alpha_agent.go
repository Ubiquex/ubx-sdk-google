// Code generated by `ubx sdk gen` from google_discoveryengine_v1alpha_discoveryengine_v1alpha_agent. DO NOT EDIT.
package discoveryengine

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type V1alphaDiscoveryengineV1alphaAgent_A2aAgentDefinition_CloudMarketplaceConfig struct {
	// Required. The Marketplace Entitlement this agent is associated with. Format: `projects/{project}/entitlements/{entitlement}`.
	Entitlement any
	// Output only. The Marketplace Order this agent belongs to. Format: `billingAccounts/{billing_account}/orders/{order}`
	Order any
}

type V1alphaDiscoveryengineV1alphaAgent_A2aAgentDefinition struct {
	// Configuration specific to agents that are deployed from Cloud Marketplace.
	CloudMarketplaceConfig any
	// Optional. The agent card is a JSON string.
	JsonAgentCard any
}

type V1alphaDiscoveryengineV1alphaAgent_AdkAgentDefinition_ProvisionedReasoningEngine struct {
	// Required. The reasoning engine that the agent is connected to. Format: `projects/{project}/locations/{location}/reasoningEngines/{reasoning_engine}`
	ReasoningEngine any
}

type V1alphaDiscoveryengineV1alphaAgent_AdkAgentDefinition struct {
	// Keeps track of the reasoning engine that the agent is connected to. This message is not intended to keep track of agent's lifecycle. Instead it is only used to define parameters to connect to the agent that is already deployed to a reasoning engine.
	ProvisionedReasoningEngine any
}

type V1alphaDiscoveryengineV1alphaAgent_AuthorizationConfig struct {
	// Optional. The authorization that is required to invoke the agent. Auth tokens will be passed to the agent as part of the request auth header.
	AgentAuthorization any
	// Optional. List of required authorizations for agent to access other resources. Auth tokens will be passed to the agent as part of the request body.
	ToolAuthorizations any
}

type V1alphaDiscoveryengineV1alphaAgent_DialogflowAgentDefinition struct {
	// Required. Resource name of the underlying Dialogflow Agent. Format: `projects/{project}/locations/{location}/agents/{agent}`
	DialogflowAgent any
}

type V1alphaDiscoveryengineV1alphaAgent_Icon struct {
	// Base64-encoded image file contents.
	Content any
	// Image URI.
	Uri any
}

type V1alphaDiscoveryengineV1alphaAgent_ObservabilityConfig struct {
	// Optional. Enables observability. If `false`, all other flags are ignored.
	ObservabilityEnabled any
	// Optional. Enables sensitive logging. Sensitive logging includes customer core content (e.g. prompts, responses). If `false`, will sanitize all sensitive fields.
	SensitiveLoggingEnabled any
}

type V1alphaDiscoveryengineV1alphaAgent_SharingConfig struct {
	// Optional. The sharing scope of the agent.
	Scope any
}

type V1alphaDiscoveryengineV1alphaAgent_StarterPrompts struct {
	Text any
}

var V1alphaDiscoveryengineV1alphaAgent_A2aAgentDefinition_CloudMarketplaceConfigFields = ubx.FieldMap{
		"Entitlement": ubx.FieldSpec{WireName: "entitlement"},
		"Order": ubx.FieldSpec{WireName: "order"},
	}

var V1alphaDiscoveryengineV1alphaAgent_A2aAgentDefinitionFields = ubx.FieldMap{
		"CloudMarketplaceConfig": ubx.FieldSpec{
			WireName: "cloud_marketplace_config",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_A2aAgentDefinition_CloudMarketplaceConfigFields,
		},
		"JsonAgentCard": ubx.FieldSpec{WireName: "json_agent_card"},
	}

var V1alphaDiscoveryengineV1alphaAgent_AdkAgentDefinition_ProvisionedReasoningEngineFields = ubx.FieldMap{
		"ReasoningEngine": ubx.FieldSpec{WireName: "reasoning_engine"},
	}

var V1alphaDiscoveryengineV1alphaAgent_AdkAgentDefinitionFields = ubx.FieldMap{
		"ProvisionedReasoningEngine": ubx.FieldSpec{
			WireName: "provisioned_reasoning_engine",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_AdkAgentDefinition_ProvisionedReasoningEngineFields,
		},
	}

var V1alphaDiscoveryengineV1alphaAgent_AuthorizationConfigFields = ubx.FieldMap{
		"AgentAuthorization": ubx.FieldSpec{WireName: "agent_authorization"},
		"ToolAuthorizations": ubx.FieldSpec{WireName: "tool_authorizations"},
	}

var V1alphaDiscoveryengineV1alphaAgent_DialogflowAgentDefinitionFields = ubx.FieldMap{
		"DialogflowAgent": ubx.FieldSpec{WireName: "dialogflow_agent"},
	}

var V1alphaDiscoveryengineV1alphaAgent_IconFields = ubx.FieldMap{
		"Content": ubx.FieldSpec{WireName: "content"},
		"Uri": ubx.FieldSpec{WireName: "uri"},
	}

var V1alphaDiscoveryengineV1alphaAgent_ObservabilityConfigFields = ubx.FieldMap{
		"ObservabilityEnabled": ubx.FieldSpec{WireName: "observability_enabled"},
		"SensitiveLoggingEnabled": ubx.FieldSpec{WireName: "sensitive_logging_enabled"},
	}

var V1alphaDiscoveryengineV1alphaAgent_SharingConfigFields = ubx.FieldMap{
		"Scope": ubx.FieldSpec{WireName: "scope"},
	}

var V1alphaDiscoveryengineV1alphaAgent_StarterPromptsFields = ubx.FieldMap{
		"Text": ubx.FieldSpec{WireName: "text"},
	}

type V1alphaDiscoveryengineV1alphaAgentConfig struct {
	// Stored definition of an agent that uses A2A.
	A2aAgentDefinition any
	// Stores the definition of an agent that uses ADK and is deployed to Agent Engine (formerly known as Reasoning Engine).
	AdkAgentDefinition any
	// Describes the authorizations required.
	AuthorizationConfig any
	// Output only. Timestamp when this Agent was created.
	CreateTime any
	// Output only. The reason why the agent creation (provisioning) failed. Only set if the state is CREATION_FAILED.
	CreationFailureReason any
	// Optional. The custom placeholder text that appears in the text box before the user enters any text.
	CustomPlaceholderText any
	// Output only. The reason why the agent deployment failed. Only set if the state is DEPLOYMENT_FAILED.
	DeploymentFailureReason any
	// Required. Human-readable description of the agent. This might be used by an LLM to automatically select an agent to respond to a user query.
	Description any
	// Stored definition of an agent that uses a Dialogflow agent.
	DialogflowAgentDefinition any
	// Required. Display name of the agent. This might be used by an LLM to automatically select an agent to respond to a user query.
	DisplayName any
	// Represents an image.
	Icon any
	// Optional. The code of the language of the text in the description, display_name and starter_prompts fields.
	LanguageCode any
	// Stores the definition of a Google managed agent.
	ManagedAgentDefinition any
	// Identifier. Resource name of the agent. Format: `projects/{project}/locations/{location}/collections/{collection}/engines/{engine}/assistants/{assistant}/agents/{agent}`
	Name any
	// Observability config for a resource.
	ObservabilityConfig any
	// Output only. The reason why the agent was rejected. Only set if the state is PRIVATE, and got there via rejection.
	RejectionReason any
	// Sharing related configuration.
	SharingConfig any
	// Optional. The starter prompt suggestions to show the user on the landing page of the agent.
	StarterPrompts any
	// Output only. The lifecycle state of the agent.
	State any
	// Output only. The reason why the agent was suspended. Only set if the state is SUSPENDED.
	SuspensionReason any
	// Output only. Timestamp when this Agent was most recently updated.
	UpdateTime any
}

type V1alphaDiscoveryengineV1alphaAgentAttrs struct {
	// Stored definition of an agent that uses A2A.
	A2aAgentDefinition any
	// Stores the definition of an agent that uses ADK and is deployed to Agent Engine (formerly known as Reasoning Engine).
	AdkAgentDefinition any
	// Describes the authorizations required.
	AuthorizationConfig any
	// Output only. Timestamp when this Agent was created.
	CreateTime any
	// Output only. The reason why the agent creation (provisioning) failed. Only set if the state is CREATION_FAILED.
	CreationFailureReason any
	// Optional. The custom placeholder text that appears in the text box before the user enters any text.
	CustomPlaceholderText any
	// Output only. The reason why the agent deployment failed. Only set if the state is DEPLOYMENT_FAILED.
	DeploymentFailureReason any
	// Required. Human-readable description of the agent. This might be used by an LLM to automatically select an agent to respond to a user query.
	Description any
	// Stored definition of an agent that uses a Dialogflow agent.
	DialogflowAgentDefinition any
	// Required. Display name of the agent. This might be used by an LLM to automatically select an agent to respond to a user query.
	DisplayName any
	// Represents an image.
	Icon any
	// Optional. The code of the language of the text in the description, display_name and starter_prompts fields.
	LanguageCode any
	// Stores the definition of a Google managed agent.
	ManagedAgentDefinition any
	// Identifier. Resource name of the agent. Format: `projects/{project}/locations/{location}/collections/{collection}/engines/{engine}/assistants/{assistant}/agents/{agent}`
	Name any
	// Observability config for a resource.
	ObservabilityConfig any
	// Output only. The reason why the agent was rejected. Only set if the state is PRIVATE, and got there via rejection.
	RejectionReason any
	// Sharing related configuration.
	SharingConfig any
	// Optional. The starter prompt suggestions to show the user on the landing page of the agent.
	StarterPrompts any
	// Output only. The lifecycle state of the agent.
	State any
	// Output only. The reason why the agent was suspended. Only set if the state is SUSPENDED.
	SuspensionReason any
	// Output only. Timestamp when this Agent was most recently updated.
	UpdateTime any
}

var V1alphaDiscoveryengineV1alphaAgent = ubx.ResourceBinding{
	WireType: "google_discoveryengine_v1alpha_discoveryengine_v1alpha_agent",
	Fields: ubx.FieldMap{
		"A2aAgentDefinition": ubx.FieldSpec{
			WireName: "a2a_agent_definition",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_A2aAgentDefinitionFields,
		},
		"AdkAgentDefinition": ubx.FieldSpec{
			WireName: "adk_agent_definition",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_AdkAgentDefinitionFields,
		},
		"AuthorizationConfig": ubx.FieldSpec{
			WireName: "authorization_config",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_AuthorizationConfigFields,
		},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"CreationFailureReason": ubx.FieldSpec{WireName: "creation_failure_reason"},
		"CustomPlaceholderText": ubx.FieldSpec{WireName: "custom_placeholder_text"},
		"DeploymentFailureReason": ubx.FieldSpec{WireName: "deployment_failure_reason"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"DialogflowAgentDefinition": ubx.FieldSpec{
			WireName: "dialogflow_agent_definition",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_DialogflowAgentDefinitionFields,
		},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"Icon": ubx.FieldSpec{
			WireName: "icon",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_IconFields,
		},
		"LanguageCode": ubx.FieldSpec{WireName: "language_code"},
		"ManagedAgentDefinition": ubx.FieldSpec{WireName: "managed_agent_definition"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"ObservabilityConfig": ubx.FieldSpec{
			WireName: "observability_config",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_ObservabilityConfigFields,
		},
		"RejectionReason": ubx.FieldSpec{WireName: "rejection_reason"},
		"SharingConfig": ubx.FieldSpec{
			WireName: "sharing_config",
			Kind: "object",
			Fields: V1alphaDiscoveryengineV1alphaAgent_SharingConfigFields,
		},
		"StarterPrompts": ubx.FieldSpec{
			WireName: "starter_prompts",
			Kind: "list",
			Fields: V1alphaDiscoveryengineV1alphaAgent_StarterPromptsFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
		"SuspensionReason": ubx.FieldSpec{WireName: "suspension_reason"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
