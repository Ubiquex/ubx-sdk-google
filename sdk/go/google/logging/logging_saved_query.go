// Code generated by `ubx sdk gen` from google_logging_logging_saved_query. DO NOT EDIT.
package logging

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type LoggingSavedQuery_LoggingQuery_SummaryFields struct {
	Field any
}

type LoggingSavedQuery_LoggingQuery struct {
	// Required. An advanced query using the Logging Query Language (https://docs.cloud.google.com/logging/docs/view/logging-query-language). The maximum length of the filter is 20000 characters.
	Filter any
	// Characters will be counted from the end of the string.
	SummaryFieldEnd any
	// Characters will be counted from the start of the string.
	SummaryFieldStart any
	// Optional. The set of summary fields to display for this saved query.
	SummaryFields any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField_SqlAggregationFunction struct {
	Parameters any
	Type any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField_VirtualField struct {
	UnderlyingFieldSources any
	VirtualFieldType any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField struct {
	Alias any
	Cast any
	Field any
	Operation any
	RegexExtraction any
	SqlAggregationFunction any
	TruncationGranularity any
	VirtualField any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources struct {
	AliasRef any
	ColumnType any
	Field any
	IsJson any
	ParentPath any
	ProjectedField any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_Filter_LeafPredicate struct {
	// The comparison type to use for the filter.
	Comparator any
	// A source that can be used to represent a "field of data" within various parts of a structured query, such as in SELECT, WHERE, or ORDER BY clauses. The term "field of data" is used here because it is not limited to literal fields in the underlying data schema.
	FieldSource any
	// A source that can be used to represent a "field of data" within various parts of a structured query, such as in SELECT, WHERE, or ORDER BY clauses. The term "field of data" is used here because it is not limited to literal fields in the underlying data schema.
	FieldSourceValue any
	// Determines if the NOT flag should be added to the comparator.
	IsNegation any
	// The Value will be used to hold user defined constants set as the Right Hand Side of the filter.
	LiteralValue any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_Filter struct {
	// The children of the filter predicate. This equates to the branches of the filter predicate that could contain further nested leaves.
	ChildPredicates any
	// This is a leaf of the FilterPredicate. Ex: { field: json_payload.message.error_code, filter_value: {numeric_value: 400}, comparator: EQUAL_TO} The field will be schema field that is selected using the . annotation to display the drill down value. The value will be the user inputted text that the filter is comparing against.
	LeafPredicate any
	// The operator type for the filter. Currently there is no support for multiple levels of nesting, so this will be a single value with no joining of different operator types
	OperatorType any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_OrderBys struct {
	FieldSource any
	SortOrderDirection any
}

type LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder struct {
	// Defines the items to include in the query result, analogous to a SQL SELECT clause.
	FieldSources any
	// A filter for a query. This equates to the WHERE clause in SQL.
	Filter any
	// The limit to use for the query. This equates to the LIMIT clause in SQL. A limit of 0 will be treated as not enabled.
	Limit any
	// The sort orders to use for the query. This equates to the ORDER BY clause in SQL.
	OrderBys any
	// Required. The view/resource to query. For now only a single view/resource will be sent, but there are plans to allow multiple views in the future. Marking as repeated for that purpose. Example: - "projects/123/locations/global/buckets/456/views/_Default" - "projects/123/locations/global/metricBuckets/456/views/_Default"
	ResourceNames any
	// The plain text search to use for the query. There is no support for multiple search terms. This uses the SEARCH functionality in BigQuery. For example, a search_term = 'ERROR' would result in the following SQL:SELECT * FROM resource WHERE SEARCH(resource, 'ERROR') LIMIT 100
	SearchTerm any
}

type LoggingSavedQuery_OpsAnalyticsQuery struct {
	// Defines a structured query configuration that can be used instead of writing raw SQL. This configuration represents the components of a SQL query (FROM, SELECT, WHERE, ORDER BY, LIMIT) and is typically converted into an executable query (e.g., BigQuery SQL) by the backend service to retrieve data for analysis or visualization.
	QueryBuilder any
	// Optional. A Log Analytics SQL query in text format.If both sql_query_text and query_builder fields are set, then the sql_query_text will be used, if its non-empty. At least one of the two fields must be set.
	SqlQueryText any
}

var LoggingSavedQuery_LoggingQuery_SummaryFieldsFields = ubx.FieldMap{
		"Field": ubx.FieldSpec{WireName: "field"},
	}

var LoggingSavedQuery_LoggingQueryFields = ubx.FieldMap{
		"Filter": ubx.FieldSpec{WireName: "filter"},
		"SummaryFieldEnd": ubx.FieldSpec{WireName: "summary_field_end"},
		"SummaryFieldStart": ubx.FieldSpec{WireName: "summary_field_start"},
		"SummaryFields": ubx.FieldSpec{
			WireName: "summary_fields",
			Kind: "list",
			Fields: LoggingSavedQuery_LoggingQuery_SummaryFieldsFields,
		},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField_SqlAggregationFunctionFields = ubx.FieldMap{
		"Parameters": ubx.FieldSpec{WireName: "parameters"},
		"Type": ubx.FieldSpec{WireName: "type"},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField_VirtualFieldFields = ubx.FieldMap{
		"UnderlyingFieldSources": ubx.FieldSpec{WireName: "underlying_field_sources"},
		"VirtualFieldType": ubx.FieldSpec{WireName: "virtual_field_type"},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedFieldFields = ubx.FieldMap{
		"Alias": ubx.FieldSpec{WireName: "alias"},
		"Cast": ubx.FieldSpec{WireName: "cast"},
		"Field": ubx.FieldSpec{WireName: "field"},
		"Operation": ubx.FieldSpec{WireName: "operation"},
		"RegexExtraction": ubx.FieldSpec{WireName: "regex_extraction"},
		"SqlAggregationFunction": ubx.FieldSpec{
			WireName: "sql_aggregation_function",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField_SqlAggregationFunctionFields,
		},
		"TruncationGranularity": ubx.FieldSpec{WireName: "truncation_granularity"},
		"VirtualField": ubx.FieldSpec{
			WireName: "virtual_field",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedField_VirtualFieldFields,
		},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSourcesFields = ubx.FieldMap{
		"AliasRef": ubx.FieldSpec{WireName: "alias_ref"},
		"ColumnType": ubx.FieldSpec{WireName: "column_type"},
		"Field": ubx.FieldSpec{WireName: "field"},
		"IsJson": ubx.FieldSpec{WireName: "is_json"},
		"ParentPath": ubx.FieldSpec{WireName: "parent_path"},
		"ProjectedField": ubx.FieldSpec{
			WireName: "projected_field",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSources_ProjectedFieldFields,
		},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_Filter_LeafPredicateFields = ubx.FieldMap{
		"Comparator": ubx.FieldSpec{WireName: "comparator"},
		"FieldSource": ubx.FieldSpec{
			WireName: "field_source",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSourcesFields,
		},
		"FieldSourceValue": ubx.FieldSpec{
			WireName: "field_source_value",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSourcesFields,
		},
		"IsNegation": ubx.FieldSpec{WireName: "is_negation"},
		"LiteralValue": ubx.FieldSpec{WireName: "literal_value"},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FilterFields = ubx.FieldMap{
		"ChildPredicates": ubx.FieldSpec{WireName: "child_predicates"},
		"LeafPredicate": ubx.FieldSpec{
			WireName: "leaf_predicate",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_Filter_LeafPredicateFields,
		},
		"OperatorType": ubx.FieldSpec{WireName: "operator_type"},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_OrderBysFields = ubx.FieldMap{
		"FieldSource": ubx.FieldSpec{
			WireName: "field_source",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSourcesFields,
		},
		"SortOrderDirection": ubx.FieldSpec{WireName: "sort_order_direction"},
	}

var LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilderFields = ubx.FieldMap{
		"FieldSources": ubx.FieldSpec{
			WireName: "field_sources",
			Kind: "list",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FieldSourcesFields,
		},
		"Filter": ubx.FieldSpec{
			WireName: "filter",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_FilterFields,
		},
		"Limit": ubx.FieldSpec{WireName: "limit"},
		"OrderBys": ubx.FieldSpec{
			WireName: "order_bys",
			Kind: "list",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilder_OrderBysFields,
		},
		"ResourceNames": ubx.FieldSpec{WireName: "resource_names"},
		"SearchTerm": ubx.FieldSpec{WireName: "search_term"},
	}

var LoggingSavedQuery_OpsAnalyticsQueryFields = ubx.FieldMap{
		"QueryBuilder": ubx.FieldSpec{
			WireName: "query_builder",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQuery_QueryBuilderFields,
		},
		"SqlQueryText": ubx.FieldSpec{WireName: "sql_query_text"},
	}

type LoggingSavedQueryConfig struct {
	// Output only. The timestamp when the saved query was created.
	CreateTime any
	// Optional. A human readable description of the saved query.
	Description any
	// Required. The user specified title for the SavedQuery.
	DisplayName any
	// Describes a Cloud Logging query that can be run in Logs Explorer UI or via the logging API.In addition to the query itself, additional information may be stored to capture the display configuration and other UI state used in association with analysis of query results.
	LoggingQuery any
	// Output only. Resource name of the saved query.In the format: "projects/[PROJECT_ID]/locations/[LOCATION_ID]/savedQueries/[QUERY_ID]" For a list of supported locations, see Supported Regions (https://docs.cloud.google.com/logging/docs/region-support#bucket-regions)After the saved query is created, the location cannot be changed.If the user doesn't provide a QUERY_ID, the system will generate an alphanumeric ID.
	Name any
	// Describes a query that can be run in Log Analytics.
	OpsAnalyticsQuery any
	// Output only. The timestamp when the saved query was last updated.
	UpdateTime any
	// Required. The visibility status of this query, which determines its ownership.
	Visibility any
}

type LoggingSavedQueryAttrs struct {
	// Output only. The timestamp when the saved query was created.
	CreateTime any
	// Optional. A human readable description of the saved query.
	Description any
	// Required. The user specified title for the SavedQuery.
	DisplayName any
	// Describes a Cloud Logging query that can be run in Logs Explorer UI or via the logging API.In addition to the query itself, additional information may be stored to capture the display configuration and other UI state used in association with analysis of query results.
	LoggingQuery any
	// Output only. Resource name of the saved query.In the format: "projects/[PROJECT_ID]/locations/[LOCATION_ID]/savedQueries/[QUERY_ID]" For a list of supported locations, see Supported Regions (https://docs.cloud.google.com/logging/docs/region-support#bucket-regions)After the saved query is created, the location cannot be changed.If the user doesn't provide a QUERY_ID, the system will generate an alphanumeric ID.
	Name any
	// Describes a query that can be run in Log Analytics.
	OpsAnalyticsQuery any
	// Output only. The timestamp when the saved query was last updated.
	UpdateTime any
	// Required. The visibility status of this query, which determines its ownership.
	Visibility any
}

var LoggingSavedQuery = ubx.ResourceBinding{
	WireType: "google_logging_logging_saved_query",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"LoggingQuery": ubx.FieldSpec{
			WireName: "logging_query",
			Kind: "object",
			Fields: LoggingSavedQuery_LoggingQueryFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"OpsAnalyticsQuery": ubx.FieldSpec{
			WireName: "ops_analytics_query",
			Kind: "object",
			Fields: LoggingSavedQuery_OpsAnalyticsQueryFields,
		},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
		"Visibility": ubx.FieldSpec{WireName: "visibility"},
	},
}
