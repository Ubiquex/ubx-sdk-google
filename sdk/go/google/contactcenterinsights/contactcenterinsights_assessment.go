// Code generated by `ubx sdk gen` from google_contactcenterinsights_contactcenterinsights_assessment. DO NOT EDIT.
package contactcenterinsights

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ContactcenterinsightsAssessment_AgentInfo struct {
	// A user-specified string representing the agent.
	AgentId any
	// The agent type, e.g. HUMAN_AGENT.
	AgentType any
	// The agent's deployment display name. Only applicable to automated agents.
	DeploymentDisplayName any
	// The agent's deployment ID. Only applicable to automated agents.
	DeploymentId any
	// The agent's name.
	DisplayName any
	// A user-provided string indicating the outcome of the agent's segment of the call.
	DispositionCode any
	// The entry subagent's display name.
	EntrySubagentDisplayName any
	// The entry subagent's ID.
	EntrySubagentId any
	// The agent's location.
	Location any
	// A user-specified string representing the agent's team. Deprecated in favor of the `teams` field.
	Team any
	// User-specified strings representing the agent's teams.
	Teams any
	// The agent's version display name. Only applicable to automated agents.
	VersionDisplayName any
	// The agent's version ID. Only applicable to automated agents.
	VersionId any
}

var ContactcenterinsightsAssessment_AgentInfoFields = ubx.FieldMap{
		"AgentId": ubx.FieldSpec{WireName: "agent_id"},
		"AgentType": ubx.FieldSpec{WireName: "agent_type"},
		"DeploymentDisplayName": ubx.FieldSpec{WireName: "deployment_display_name"},
		"DeploymentId": ubx.FieldSpec{WireName: "deployment_id"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"DispositionCode": ubx.FieldSpec{WireName: "disposition_code"},
		"EntrySubagentDisplayName": ubx.FieldSpec{WireName: "entry_subagent_display_name"},
		"EntrySubagentId": ubx.FieldSpec{WireName: "entry_subagent_id"},
		"Location": ubx.FieldSpec{WireName: "location"},
		"Team": ubx.FieldSpec{WireName: "team"},
		"Teams": ubx.FieldSpec{WireName: "teams"},
		"VersionDisplayName": ubx.FieldSpec{WireName: "version_display_name"},
		"VersionId": ubx.FieldSpec{WireName: "version_id"},
	}

type ContactcenterinsightsAssessmentConfig struct {
	// Information about an agent involved in the conversation.
	AgentInfo any
	// Output only. The time at which the assessment was created.
	CreateTime any
	// Identifier. The resource name of the assessment. Format: projects/{project}/locations/{location}/conversations/{conversation}/assessments/{assessment}
	Name any
	// Output only. The state of the assessment.
	State any
	// Output only. The time at which the assessment was last updated.
	UpdateTime any
}

type ContactcenterinsightsAssessmentAttrs struct {
	// Information about an agent involved in the conversation.
	AgentInfo any
	// Output only. The time at which the assessment was created.
	CreateTime any
	// Identifier. The resource name of the assessment. Format: projects/{project}/locations/{location}/conversations/{conversation}/assessments/{assessment}
	Name any
	// Output only. The state of the assessment.
	State any
	// Output only. The time at which the assessment was last updated.
	UpdateTime any
}

var ContactcenterinsightsAssessment = ubx.ResourceBinding{
	WireType: "google_contactcenterinsights_contactcenterinsights_assessment",
	Fields: ubx.FieldMap{
		"AgentInfo": ubx.FieldSpec{
			WireName: "agent_info",
			Kind: "object",
			Fields: ContactcenterinsightsAssessment_AgentInfoFields,
		},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"State": ubx.FieldSpec{WireName: "state"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
