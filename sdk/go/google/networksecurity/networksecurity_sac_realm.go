// Code generated by `ubx sdk gen` from google_networksecurity_networksecurity_sac_realm. DO NOT EDIT.
package networksecurity

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type NetworksecuritySacRealm_PairingKey struct {
	// Output only. Timestamp in UTC of when this resource is considered expired. It expires 7 days after creation.
	ExpireTime any
	// Output only. Key value.
	Key any
}

var NetworksecuritySacRealm_PairingKeyFields = ubx.FieldMap{
		"ExpireTime": ubx.FieldSpec{WireName: "expire_time"},
		"Key": ubx.FieldSpec{WireName: "key"},
	}

type NetworksecuritySacRealmConfig struct {
	// Output only. Timestamp when the realm was created.
	CreateTime any
	// Optional. Optional list of labels applied to the resource.
	Labels any
	// Identifier. Resource name, in the form `projects/{project}/locations/global/sacRealms/{sacRealm}`.
	Name any
	// Key to be shared with SSE service provider to establish global handshake.
	PairingKey any
	// Immutable. SSE service provider associated with the realm.
	SecurityService any
	// Output only. State of the realm.
	State any
	// Output only. Timestamp when the realm was last updated.
	UpdateTime any
}

type NetworksecuritySacRealmAttrs struct {
	// Output only. Timestamp when the realm was created.
	CreateTime any
	// Optional. Optional list of labels applied to the resource.
	Labels any
	// Identifier. Resource name, in the form `projects/{project}/locations/global/sacRealms/{sacRealm}`.
	Name any
	// Key to be shared with SSE service provider to establish global handshake.
	PairingKey any
	// Immutable. SSE service provider associated with the realm.
	SecurityService any
	// Output only. State of the realm.
	State any
	// Output only. Timestamp when the realm was last updated.
	UpdateTime any
}

var NetworksecuritySacRealm = ubx.ResourceBinding{
	WireType: "google_networksecurity_networksecurity_sac_realm",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PairingKey": ubx.FieldSpec{
			WireName: "pairing_key",
			Kind: "object",
			Fields: NetworksecuritySacRealm_PairingKeyFields,
		},
		"SecurityService": ubx.FieldSpec{WireName: "security_service"},
		"State": ubx.FieldSpec{WireName: "state"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
