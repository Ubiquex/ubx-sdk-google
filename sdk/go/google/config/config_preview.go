// Code generated by `ubx sdk gen` from google_config_config_preview. DO NOT EDIT.
package config

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ConfigPreview_ErrorStatus struct {
	// The status code, which should be an enum value of google.rpc.Code.
	Code any
	// A list of messages that carry the error details. There is a common set of message types for APIs to use.
	Details any
	// A developer-facing error message, which should be in English. Any user-facing error message should be localized and sent in the google.rpc.Status.details field, or localized by the client.
	Message any
}

type ConfigPreview_PreviewArtifacts struct {
	// Output only. Location of artifacts in Google Cloud Storage. Format: `gs://{bucket}/{object}`
	Artifacts any
	// Output only. Location of a blueprint copy and other content in Google Cloud Storage. Format: `gs://{bucket}/{object}`
	Content any
}

type ConfigPreview_ProviderConfig struct {
	// Optional. ProviderSource specifies the source type of the provider.
	SourceType any
}

type ConfigPreview_TerraformBlueprint_ExternalValues_DeploymentSource struct {
	Deployment any
	OutputName any
}

type ConfigPreview_TerraformBlueprint_ExternalValues struct {
	DeploymentSource any
}

type ConfigPreview_TerraformBlueprint_GitSource struct {
	// Subdirectory inside the repository. Example: 'staging/my-package'
	Directory any
	// Git reference (e.g. branch or tag).
	Ref any
	// Repository URL. Example: 'https://github.com/kubernetes/examples.git'
	Repo any
}

type ConfigPreview_TerraformBlueprint_InputValues struct {
	InputValue any
}

type ConfigPreview_TerraformBlueprint struct {
	// Optional. Map of input variable names in this blueprint to configurations for importing values from external sources.
	ExternalValues any
	// URI of an object in Google Cloud Storage. Format: `gs://{bucket}/{object}` URI may also specify an object version for zipped objects. Format: `gs://{bucket}/{object}#{version}`
	GcsSource any
	// A set of files in a Git repository.
	GitSource any
	// Optional. Input variable values for the Terraform blueprint.
	InputValues any
}

type ConfigPreview_TfErrors struct {
	Error any
	ErrorDescription any
	HttpResponseCode any
	ResourceAddress any
}

var ConfigPreview_ErrorStatusFields = ubx.FieldMap{
		"Code": ubx.FieldSpec{WireName: "code"},
		"Details": ubx.FieldSpec{WireName: "details"},
		"Message": ubx.FieldSpec{WireName: "message"},
	}

var ConfigPreview_PreviewArtifactsFields = ubx.FieldMap{
		"Artifacts": ubx.FieldSpec{WireName: "artifacts"},
		"Content": ubx.FieldSpec{WireName: "content"},
	}

var ConfigPreview_ProviderConfigFields = ubx.FieldMap{
		"SourceType": ubx.FieldSpec{WireName: "source_type"},
	}

var ConfigPreview_TerraformBlueprint_ExternalValues_DeploymentSourceFields = ubx.FieldMap{
		"Deployment": ubx.FieldSpec{WireName: "deployment"},
		"OutputName": ubx.FieldSpec{WireName: "output_name"},
	}

var ConfigPreview_TerraformBlueprint_ExternalValuesFields = ubx.FieldMap{
		"DeploymentSource": ubx.FieldSpec{
			WireName: "deployment_source",
			Kind: "object",
			Fields: ConfigPreview_TerraformBlueprint_ExternalValues_DeploymentSourceFields,
		},
	}

var ConfigPreview_TerraformBlueprint_GitSourceFields = ubx.FieldMap{
		"Directory": ubx.FieldSpec{WireName: "directory"},
		"Ref": ubx.FieldSpec{WireName: "ref"},
		"Repo": ubx.FieldSpec{WireName: "repo"},
	}

var ConfigPreview_TerraformBlueprint_InputValuesFields = ubx.FieldMap{
		"InputValue": ubx.FieldSpec{WireName: "input_value"},
	}

var ConfigPreview_TerraformBlueprintFields = ubx.FieldMap{
		"ExternalValues": ubx.FieldSpec{
			WireName: "external_values",
			Kind: "map",
			Fields: ConfigPreview_TerraformBlueprint_ExternalValuesFields,
		},
		"GcsSource": ubx.FieldSpec{WireName: "gcs_source"},
		"GitSource": ubx.FieldSpec{
			WireName: "git_source",
			Kind: "object",
			Fields: ConfigPreview_TerraformBlueprint_GitSourceFields,
		},
		"InputValues": ubx.FieldSpec{
			WireName: "input_values",
			Kind: "map",
			Fields: ConfigPreview_TerraformBlueprint_InputValuesFields,
		},
	}

var ConfigPreview_TfErrorsFields = ubx.FieldMap{
		"Error": ubx.FieldSpec{
			WireName: "error",
			Kind: "object",
			Fields: ConfigPreview_ErrorStatusFields,
		},
		"ErrorDescription": ubx.FieldSpec{WireName: "error_description"},
		"HttpResponseCode": ubx.FieldSpec{WireName: "http_response_code"},
		"ResourceAddress": ubx.FieldSpec{WireName: "resource_address"},
	}

type ConfigPreviewConfig struct {
	// Optional. Arbitrary key-value metadata storage e.g. to help client tools identify preview during automation. See https://google.aip.dev/148#annotations for details on format and size limitations.
	Annotations any
	// User-defined location of Cloud Build logs, artifacts, and in Google Cloud Storage. Format: `gs://{bucket}/{folder}` A default bucket will be bootstrapped if the field is not set or empty Default Bucket Format: `gs://--blueprint-config` Constraints: - The bucket needs to be in the same project as the deployment - The path cannot be within the path of `gcs_source` If omitted and deployment resource ref provided has artifacts_gcs_bucket defined, that artifact bucket is used.
	ArtifactsGcsBucket any
	// Output only. Cloud Build instance UUID associated with this preview.
	Build any
	// Output only. Time the preview was created.
	CreateTime any
	// Optional. Deployment reference. If specified, the preview will be performed using the provided deployment's current state and use any relevant fields from the deployment unless explicitly specified in the preview create request.
	Deployment any
	// Output only. Code describing any errors that may have occurred.
	ErrorCode any
	// Output only. Link to tf-error.ndjson file, which contains the full list of the errors encountered during a Terraform preview. Format: `gs://{bucket}/{object}`.
	ErrorLogs any
	// The `Status` type defines a logical error model that is suitable for different programming environments, including REST APIs and RPC APIs. It is used by [gRPC](https://github.com/grpc). Each `Status` message contains three pieces of data: error code, error message, and error details. You can find out more about this error model and how to work with it in the [API Design Guide](https://cloud.google.com/apis/design/errors).
	ErrorStatus any
	// Optional. User-defined labels for the preview.
	Labels any
	// Output only. Location of preview logs in `gs://{bucket}/{object}` format.
	Logs any
	// Identifier. Resource name of the preview. Resource name can be user provided or server generated ID if unspecified. Format: `projects/{project}/locations/{location}/previews/{preview}`
	Name any
	// Artifacts created by preview.
	PreviewArtifacts any
	// Optional. Current mode of preview.
	PreviewMode any
	// ProviderConfig contains the provider configurations.
	ProviderConfig any
	// Required. User-specified Service Account (SA) credentials to be used when previewing resources. Format: `projects/{projectID}/serviceAccounts/{serviceAccount}`
	ServiceAccount any
	// Output only. Current state of the preview.
	State any
	// TerraformBlueprint describes the source of a Terraform root module which describes the resources and configs to be deployed.
	TerraformBlueprint any
	// Output only. Summary of errors encountered during Terraform preview. It has a size limit of 10, i.e. only top 10 errors will be summarized here.
	TfErrors any
	// Output only. The current Terraform version set on the preview. It is in the format of "Major.Minor.Patch", for example, "1.3.10".
	TfVersion any
	// The user-specified Terraform version constraint. Example: "=1.3.10".
	TfVersionConstraint any
	// The user-specified Worker Pool resource in which the Cloud Build job will execute. Format projects/{project}/locations/{location}/workerPools/{workerPoolId} If this field is unspecified, the default Cloud Build worker pool will be used. If omitted and deployment resource ref provided has worker_pool defined, that worker pool is used.
	WorkerPool any
}

type ConfigPreviewAttrs struct {
	// Optional. Arbitrary key-value metadata storage e.g. to help client tools identify preview during automation. See https://google.aip.dev/148#annotations for details on format and size limitations.
	Annotations any
	// User-defined location of Cloud Build logs, artifacts, and in Google Cloud Storage. Format: `gs://{bucket}/{folder}` A default bucket will be bootstrapped if the field is not set or empty Default Bucket Format: `gs://--blueprint-config` Constraints: - The bucket needs to be in the same project as the deployment - The path cannot be within the path of `gcs_source` If omitted and deployment resource ref provided has artifacts_gcs_bucket defined, that artifact bucket is used.
	ArtifactsGcsBucket any
	// Output only. Cloud Build instance UUID associated with this preview.
	Build any
	// Output only. Time the preview was created.
	CreateTime any
	// Optional. Deployment reference. If specified, the preview will be performed using the provided deployment's current state and use any relevant fields from the deployment unless explicitly specified in the preview create request.
	Deployment any
	// Output only. Code describing any errors that may have occurred.
	ErrorCode any
	// Output only. Link to tf-error.ndjson file, which contains the full list of the errors encountered during a Terraform preview. Format: `gs://{bucket}/{object}`.
	ErrorLogs any
	// The `Status` type defines a logical error model that is suitable for different programming environments, including REST APIs and RPC APIs. It is used by [gRPC](https://github.com/grpc). Each `Status` message contains three pieces of data: error code, error message, and error details. You can find out more about this error model and how to work with it in the [API Design Guide](https://cloud.google.com/apis/design/errors).
	ErrorStatus any
	// Optional. User-defined labels for the preview.
	Labels any
	// Output only. Location of preview logs in `gs://{bucket}/{object}` format.
	Logs any
	// Identifier. Resource name of the preview. Resource name can be user provided or server generated ID if unspecified. Format: `projects/{project}/locations/{location}/previews/{preview}`
	Name any
	// Artifacts created by preview.
	PreviewArtifacts any
	// Optional. Current mode of preview.
	PreviewMode any
	// ProviderConfig contains the provider configurations.
	ProviderConfig any
	// Required. User-specified Service Account (SA) credentials to be used when previewing resources. Format: `projects/{projectID}/serviceAccounts/{serviceAccount}`
	ServiceAccount any
	// Output only. Current state of the preview.
	State any
	// TerraformBlueprint describes the source of a Terraform root module which describes the resources and configs to be deployed.
	TerraformBlueprint any
	// Output only. Summary of errors encountered during Terraform preview. It has a size limit of 10, i.e. only top 10 errors will be summarized here.
	TfErrors any
	// Output only. The current Terraform version set on the preview. It is in the format of "Major.Minor.Patch", for example, "1.3.10".
	TfVersion any
	// The user-specified Terraform version constraint. Example: "=1.3.10".
	TfVersionConstraint any
	// The user-specified Worker Pool resource in which the Cloud Build job will execute. Format projects/{project}/locations/{location}/workerPools/{workerPoolId} If this field is unspecified, the default Cloud Build worker pool will be used. If omitted and deployment resource ref provided has worker_pool defined, that worker pool is used.
	WorkerPool any
}

var ConfigPreview = ubx.ResourceBinding{
	WireType: "google_config_config_preview",
	Fields: ubx.FieldMap{
		"Annotations": ubx.FieldSpec{WireName: "annotations"},
		"ArtifactsGcsBucket": ubx.FieldSpec{WireName: "artifacts_gcs_bucket"},
		"Build": ubx.FieldSpec{WireName: "build"},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Deployment": ubx.FieldSpec{WireName: "deployment"},
		"ErrorCode": ubx.FieldSpec{WireName: "error_code"},
		"ErrorLogs": ubx.FieldSpec{WireName: "error_logs"},
		"ErrorStatus": ubx.FieldSpec{
			WireName: "error_status",
			Kind: "object",
			Fields: ConfigPreview_ErrorStatusFields,
		},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Logs": ubx.FieldSpec{WireName: "logs"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PreviewArtifacts": ubx.FieldSpec{
			WireName: "preview_artifacts",
			Kind: "object",
			Fields: ConfigPreview_PreviewArtifactsFields,
		},
		"PreviewMode": ubx.FieldSpec{WireName: "preview_mode"},
		"ProviderConfig": ubx.FieldSpec{
			WireName: "provider_config",
			Kind: "object",
			Fields: ConfigPreview_ProviderConfigFields,
		},
		"ServiceAccount": ubx.FieldSpec{WireName: "service_account"},
		"State": ubx.FieldSpec{WireName: "state"},
		"TerraformBlueprint": ubx.FieldSpec{
			WireName: "terraform_blueprint",
			Kind: "object",
			Fields: ConfigPreview_TerraformBlueprintFields,
		},
		"TfErrors": ubx.FieldSpec{
			WireName: "tf_errors",
			Kind: "list",
			Fields: ConfigPreview_TfErrorsFields,
		},
		"TfVersion": ubx.FieldSpec{WireName: "tf_version"},
		"TfVersionConstraint": ubx.FieldSpec{WireName: "tf_version_constraint"},
		"WorkerPool": ubx.FieldSpec{WireName: "worker_pool"},
	},
}
