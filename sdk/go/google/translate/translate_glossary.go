// Code generated by `ubx sdk gen` from google_translate_translate_glossary. DO NOT EDIT.
package translate

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type TranslateGlossary_InputConfig_GcsSource struct {
	// Required. Source data URI. For example, `gs://my_bucket/my_object`.
	InputUri any
}

type TranslateGlossary_InputConfig struct {
	// The Google Cloud Storage location for the input content.
	GcsSource any
}

type TranslateGlossary_LanguageCodesSet struct {
	// Optional. The ISO-639 language code(s) for terms defined in the glossary. All entries are unique. The list contains at least two entries. Expected to be an exact match for GlossaryTerm.language_code.
	LanguageCodes any
}

type TranslateGlossary_LanguagePair struct {
	// Required. The ISO-639 language code of the input text, for example, "en-US". Expected to be an exact match for GlossaryTerm.language_code.
	SourceLanguageCode any
	// Required. The ISO-639 language code for translation output, for example, "zh-CN". Expected to be an exact match for GlossaryTerm.language_code.
	TargetLanguageCode any
}

var TranslateGlossary_InputConfig_GcsSourceFields = ubx.FieldMap{
		"InputUri": ubx.FieldSpec{WireName: "input_uri"},
	}

var TranslateGlossary_InputConfigFields = ubx.FieldMap{
		"GcsSource": ubx.FieldSpec{
			WireName: "gcs_source",
			Kind: "object",
			Fields: TranslateGlossary_InputConfig_GcsSourceFields,
		},
	}

var TranslateGlossary_LanguageCodesSetFields = ubx.FieldMap{
		"LanguageCodes": ubx.FieldSpec{WireName: "language_codes"},
	}

var TranslateGlossary_LanguagePairFields = ubx.FieldMap{
		"SourceLanguageCode": ubx.FieldSpec{WireName: "source_language_code"},
		"TargetLanguageCode": ubx.FieldSpec{WireName: "target_language_code"},
	}

type TranslateGlossaryConfig struct {
	// Optional. The display name of the glossary.
	DisplayName any
	// Output only. When the glossary creation was finished.
	EndTime any
	// Output only. The number of entries defined in the glossary.
	EntryCount any
	// Input configuration for glossaries.
	InputConfig any
	// Used with equivalent term set glossaries.
	LanguageCodesSet any
	// Used with unidirectional glossaries.
	LanguagePair any
	// Identifier. The resource name of the glossary. Glossary names have the form `projects/{project-number-or-id}/locations/{location-id}/glossaries/{glossary-id}`.
	Name any
	// Output only. When CreateGlossary was called.
	SubmitTime any
}

type TranslateGlossaryAttrs struct {
	// Optional. The display name of the glossary.
	DisplayName any
	// Output only. When the glossary creation was finished.
	EndTime any
	// Output only. The number of entries defined in the glossary.
	EntryCount any
	// Input configuration for glossaries.
	InputConfig any
	// Used with equivalent term set glossaries.
	LanguageCodesSet any
	// Used with unidirectional glossaries.
	LanguagePair any
	// Identifier. The resource name of the glossary. Glossary names have the form `projects/{project-number-or-id}/locations/{location-id}/glossaries/{glossary-id}`.
	Name any
	// Output only. When CreateGlossary was called.
	SubmitTime any
}

var TranslateGlossary = ubx.ResourceBinding{
	WireType: "google_translate_translate_glossary",
	Fields: ubx.FieldMap{
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"EndTime": ubx.FieldSpec{WireName: "end_time"},
		"EntryCount": ubx.FieldSpec{WireName: "entry_count"},
		"InputConfig": ubx.FieldSpec{
			WireName: "input_config",
			Kind: "object",
			Fields: TranslateGlossary_InputConfigFields,
		},
		"LanguageCodesSet": ubx.FieldSpec{
			WireName: "language_codes_set",
			Kind: "object",
			Fields: TranslateGlossary_LanguageCodesSetFields,
		},
		"LanguagePair": ubx.FieldSpec{
			WireName: "language_pair",
			Kind: "object",
			Fields: TranslateGlossary_LanguagePairFields,
		},
		"Name": ubx.FieldSpec{WireName: "name"},
		"SubmitTime": ubx.FieldSpec{WireName: "submit_time"},
	},
}
