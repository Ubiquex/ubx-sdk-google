// Code generated by `ubx sdk gen` from google_beyondcorp_beyondcorp_security_gateway. DO NOT EDIT.
package beyondcorp

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type BeyondcorpSecurityGateway_Hubs_InternetGateway struct {
	AssignedIps any
}

type BeyondcorpSecurityGateway_Hubs struct {
	InternetGateway any
}

type BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders_DeviceInfo struct {
	// Optional. The output type details for the delegated device.
	OutputType any
}

type BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders struct {
	// The delegated device information configuration.
	DeviceInfo any
	// The delegated dispatch information configuration.
	DispatchInfo any
	// The delegated group configuration details.
	GroupInfo any
	// Optional. Default output type for all enabled headers.
	OutputType any
	// The configuration information for the delegated user.
	UserInfo any
}

type BeyondcorpSecurityGateway_ProxyProtocolConfig struct {
	// Optional. List of the allowed client header names.
	AllowedClientHeaders any
	// Optional. Client IP configuration. The client IP address is included if true.
	ClientIp any
	// Contextual headers configuration.
	ContextualHeaders any
	// Optional. The security gateway identity configuration.
	GatewayIdentity any
	// Optional. Custom resource specific headers along with the values. The names should conform to RFC 9110: >Field names can contain alphanumeric characters, hyphens, and periods, can contain only ASCII-printable characters and tabs, and must start with a letter.
	MetadataHeaders any
}

type BeyondcorpSecurityGateway_ServiceDiscovery_ApiGateway_ResourceOverride struct {
	// Optional. Contains the URI path fragment where HTTP request is sent.
	Path any
}

type BeyondcorpSecurityGateway_ServiceDiscovery_ApiGateway struct {
	// API operation descriptor.
	ResourceOverride any
}

type BeyondcorpSecurityGateway_ServiceDiscovery struct {
	// If Service Discovery is done through API, defines its settings.
	ApiGateway any
}

var BeyondcorpSecurityGateway_Hubs_InternetGatewayFields = ubx.FieldMap{
		"AssignedIps": ubx.FieldSpec{WireName: "assigned_ips"},
	}

var BeyondcorpSecurityGateway_HubsFields = ubx.FieldMap{
		"InternetGateway": ubx.FieldSpec{
			WireName: "internet_gateway",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_Hubs_InternetGatewayFields,
		},
	}

var BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders_DeviceInfoFields = ubx.FieldMap{
		"OutputType": ubx.FieldSpec{WireName: "output_type"},
	}

var BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeadersFields = ubx.FieldMap{
		"DeviceInfo": ubx.FieldSpec{
			WireName: "device_info",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders_DeviceInfoFields,
		},
		"DispatchInfo": ubx.FieldSpec{
			WireName: "dispatch_info",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders_DeviceInfoFields,
		},
		"GroupInfo": ubx.FieldSpec{
			WireName: "group_info",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders_DeviceInfoFields,
		},
		"OutputType": ubx.FieldSpec{WireName: "output_type"},
		"UserInfo": ubx.FieldSpec{
			WireName: "user_info",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeaders_DeviceInfoFields,
		},
	}

var BeyondcorpSecurityGateway_ProxyProtocolConfigFields = ubx.FieldMap{
		"AllowedClientHeaders": ubx.FieldSpec{WireName: "allowed_client_headers"},
		"ClientIp": ubx.FieldSpec{WireName: "client_ip"},
		"ContextualHeaders": ubx.FieldSpec{
			WireName: "contextual_headers",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ProxyProtocolConfig_ContextualHeadersFields,
		},
		"GatewayIdentity": ubx.FieldSpec{WireName: "gateway_identity"},
		"MetadataHeaders": ubx.FieldSpec{WireName: "metadata_headers"},
	}

var BeyondcorpSecurityGateway_ServiceDiscovery_ApiGateway_ResourceOverrideFields = ubx.FieldMap{
		"Path": ubx.FieldSpec{WireName: "path"},
	}

var BeyondcorpSecurityGateway_ServiceDiscovery_ApiGatewayFields = ubx.FieldMap{
		"ResourceOverride": ubx.FieldSpec{
			WireName: "resource_override",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ServiceDiscovery_ApiGateway_ResourceOverrideFields,
		},
	}

var BeyondcorpSecurityGateway_ServiceDiscoveryFields = ubx.FieldMap{
		"ApiGateway": ubx.FieldSpec{
			WireName: "api_gateway",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ServiceDiscovery_ApiGatewayFields,
		},
	}

type BeyondcorpSecurityGatewayConfig struct {
	// Output only. Timestamp when the resource was created.
	CreateTime any
	// Output only. Service account used for operations that involve resources in consumer projects.
	DelegatingServiceAccount any
	// Optional. An arbitrary user-provided name for the SecurityGateway. Cannot exceed 64 characters.
	DisplayName any
	// Output only. IP addresses that will be used for establishing connection to the endpoints.
	ExternalIps any
	// Optional. Map of Hubs that represents regional data path deployment with GCP region as a key.
	Hubs any
	// Configuration for Cloud Logging.
	Logging any
	// Identifier. Name of the resource.
	Name any
	// The configuration for the proxy.
	ProxyProtocolConfig any
	// Settings related to the Service Discovery.
	ServiceDiscovery any
	// Output only. The operational state of the SecurityGateway.
	State any
	// Output only. Timestamp when the resource was last modified.
	UpdateTime any
}

type BeyondcorpSecurityGatewayAttrs struct {
	// Output only. Timestamp when the resource was created.
	CreateTime any
	// Output only. Service account used for operations that involve resources in consumer projects.
	DelegatingServiceAccount any
	// Optional. An arbitrary user-provided name for the SecurityGateway. Cannot exceed 64 characters.
	DisplayName any
	// Output only. IP addresses that will be used for establishing connection to the endpoints.
	ExternalIps any
	// Optional. Map of Hubs that represents regional data path deployment with GCP region as a key.
	Hubs any
	// Configuration for Cloud Logging.
	Logging any
	// Identifier. Name of the resource.
	Name any
	// The configuration for the proxy.
	ProxyProtocolConfig any
	// Settings related to the Service Discovery.
	ServiceDiscovery any
	// Output only. The operational state of the SecurityGateway.
	State any
	// Output only. Timestamp when the resource was last modified.
	UpdateTime any
}

var BeyondcorpSecurityGateway = ubx.ResourceBinding{
	WireType: "google_beyondcorp_beyondcorp_security_gateway",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DelegatingServiceAccount": ubx.FieldSpec{WireName: "delegating_service_account"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"ExternalIps": ubx.FieldSpec{WireName: "external_ips"},
		"Hubs": ubx.FieldSpec{
			WireName: "hubs",
			Kind: "map",
			Fields: BeyondcorpSecurityGateway_HubsFields,
		},
		"Logging": ubx.FieldSpec{WireName: "logging"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"ProxyProtocolConfig": ubx.FieldSpec{
			WireName: "proxy_protocol_config",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ProxyProtocolConfigFields,
		},
		"ServiceDiscovery": ubx.FieldSpec{
			WireName: "service_discovery",
			Kind: "object",
			Fields: BeyondcorpSecurityGateway_ServiceDiscoveryFields,
		},
		"State": ubx.FieldSpec{WireName: "state"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
