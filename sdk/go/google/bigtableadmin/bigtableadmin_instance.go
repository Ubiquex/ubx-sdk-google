// Code generated by `ubx sdk gen` from google_bigtableadmin_bigtableadmin_instance. DO NOT EDIT.
package bigtableadmin

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig_AutoscalingLimits struct {
	MaxServeNodes any
	MinServeNodes any
}

type BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig_AutoscalingTargets struct {
	CpuUtilizationPercent any
	StorageUtilizationGibPerNode any
}

type BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig struct {
	AutoscalingLimits any
	AutoscalingTargets any
}

type BigtableadminInstance_Clusters_ClusterConfig struct {
	ClusterAutoscalingConfig any
}

type BigtableadminInstance_Clusters_EncryptionConfig struct {
	KmsKeyName any
}

type BigtableadminInstance_Clusters struct {
	ClusterConfig any
	DefaultStorageType any
	EncryptionConfig any
	Location any
	Name any
	NodeScalingFactor any
	ServeNodes any
	State any
}

type BigtableadminInstance_Instance struct {
	// Output only. A commit timestamp representing when this Instance was created. For instances created before this field was added (August 2021), this value is `seconds: 0, nanos: 1`.
	CreateTime any
	// Required. The descriptive name for this instance as it appears in UIs. Can be changed at any time, but should be kept globally unique to avoid confusion.
	DisplayName any
	// Optional. The edition of the instance. See Edition for details.
	Edition any
	// Output only. The region where Knowledge Catalog data is synced to and stored, including user-created aspects.
	KnowledgeCatalogRegion any
	// Labels are a flexible and lightweight mechanism for organizing cloud resources into groups that reflect a customer's organizational needs and deployment strategies. They can be used to filter resources and aggregate metrics. * Label keys must be between 1 and 63 characters long and must conform to the regular expression: `\p{Ll}\p{Lo}{0,62}`. * Label values must be between 0 and 63 characters long and must conform to the regular expression: `[\p{Ll}\p{Lo}\p{N}_-]{0,63}`. * No more than 64 labels can be associated with a given resource. * Keys and values must both be under 128 bytes. Labels and Tags (below) are both used to bind metadata to resources, with different use-cases. See https://cloud.google.com/resource-manager/docs/tags/tags-overview for an in-depth overview on the difference between tags and labels.
	Labels any
	// The unique name of the instance. Values are of the form `projects/{project}/instances/a-z+[a-z0-9]`.
	Name any
	// Output only. Reserved for future use.
	SatisfiesPzi any
	// Output only. Reserved for future use.
	SatisfiesPzs any
	// Output only. The current state of the instance.
	State any
	// Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: - "123/environment": "production", - "123/costCenter": "marketing" Tags and Labels (above) are both used to bind metadata to resources, with different use-cases. See https://cloud.google.com/resource-manager/docs/tags/tags-overview for an in-depth overview on the difference between tags and labels.
	Tags any
	// The type of the instance. Defaults to `PRODUCTION`.
	Type any
}

var BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig_AutoscalingLimitsFields = ubx.FieldMap{
		"MaxServeNodes": ubx.FieldSpec{WireName: "max_serve_nodes"},
		"MinServeNodes": ubx.FieldSpec{WireName: "min_serve_nodes"},
	}

var BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig_AutoscalingTargetsFields = ubx.FieldMap{
		"CpuUtilizationPercent": ubx.FieldSpec{WireName: "cpu_utilization_percent"},
		"StorageUtilizationGibPerNode": ubx.FieldSpec{WireName: "storage_utilization_gib_per_node"},
	}

var BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfigFields = ubx.FieldMap{
		"AutoscalingLimits": ubx.FieldSpec{
			WireName: "autoscaling_limits",
			Kind: "object",
			Fields: BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig_AutoscalingLimitsFields,
		},
		"AutoscalingTargets": ubx.FieldSpec{
			WireName: "autoscaling_targets",
			Kind: "object",
			Fields: BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfig_AutoscalingTargetsFields,
		},
	}

var BigtableadminInstance_Clusters_ClusterConfigFields = ubx.FieldMap{
		"ClusterAutoscalingConfig": ubx.FieldSpec{
			WireName: "cluster_autoscaling_config",
			Kind: "object",
			Fields: BigtableadminInstance_Clusters_ClusterConfig_ClusterAutoscalingConfigFields,
		},
	}

var BigtableadminInstance_Clusters_EncryptionConfigFields = ubx.FieldMap{
		"KmsKeyName": ubx.FieldSpec{WireName: "kms_key_name"},
	}

var BigtableadminInstance_ClustersFields = ubx.FieldMap{
		"ClusterConfig": ubx.FieldSpec{
			WireName: "cluster_config",
			Kind: "object",
			Fields: BigtableadminInstance_Clusters_ClusterConfigFields,
		},
		"DefaultStorageType": ubx.FieldSpec{WireName: "default_storage_type"},
		"EncryptionConfig": ubx.FieldSpec{
			WireName: "encryption_config",
			Kind: "object",
			Fields: BigtableadminInstance_Clusters_EncryptionConfigFields,
		},
		"Location": ubx.FieldSpec{WireName: "location"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"NodeScalingFactor": ubx.FieldSpec{WireName: "node_scaling_factor"},
		"ServeNodes": ubx.FieldSpec{WireName: "serve_nodes"},
		"State": ubx.FieldSpec{WireName: "state"},
	}

var BigtableadminInstance_InstanceFields = ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"DisplayName": ubx.FieldSpec{WireName: "display_name"},
		"Edition": ubx.FieldSpec{WireName: "edition"},
		"KnowledgeCatalogRegion": ubx.FieldSpec{WireName: "knowledge_catalog_region"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"SatisfiesPzi": ubx.FieldSpec{WireName: "satisfies_pzi"},
		"SatisfiesPzs": ubx.FieldSpec{WireName: "satisfies_pzs"},
		"State": ubx.FieldSpec{WireName: "state"},
		"Tags": ubx.FieldSpec{WireName: "tags"},
		"Type": ubx.FieldSpec{WireName: "type"},
	}

type BigtableadminInstanceConfig struct {
	// Required. The clusters to be created within the instance, mapped by desired cluster ID, e.g., just `mycluster` rather than `projects/myproject/instances/myinstance/clusters/mycluster`. Fields marked `OutputOnly` must be left blank.
	Clusters any
	// A collection of Bigtable Tables and the resources that serve them. All tables in an instance are served from all Clusters in the instance.
	Instance any
	// Required. The ID to be used when referring to the new instance within its project, e.g., just `myinstance` rather than `projects/myproject/instances/myinstance`.
	InstanceId any
	// Required. The unique name of the project in which to create the new instance. Values are of the form `projects/{project}`.
	Parent any
}

type BigtableadminInstanceAttrs struct {
	// Required. The clusters to be created within the instance, mapped by desired cluster ID, e.g., just `mycluster` rather than `projects/myproject/instances/myinstance/clusters/mycluster`. Fields marked `OutputOnly` must be left blank.
	Clusters any
	// Output only. A commit timestamp representing when this Instance was created. For instances created before this field was added (August 2021), this value is `seconds: 0, nanos: 1`.
	CreateTime any
	// Required. The descriptive name for this instance as it appears in UIs. Can be changed at any time, but should be kept globally unique to avoid confusion.
	DisplayName any
	// Optional. The edition of the instance. See Edition for details.
	Edition any
	// A collection of Bigtable Tables and the resources that serve them. All tables in an instance are served from all Clusters in the instance.
	Instance any
	// Required. The ID to be used when referring to the new instance within its project, e.g., just `myinstance` rather than `projects/myproject/instances/myinstance`.
	InstanceId any
	// Output only. The region where Knowledge Catalog data is synced to and stored, including user-created aspects.
	KnowledgeCatalogRegion any
	// Labels are a flexible and lightweight mechanism for organizing cloud resources into groups that reflect a customer's organizational needs and deployment strategies. They can be used to filter resources and aggregate metrics. * Label keys must be between 1 and 63 characters long and must conform to the regular expression: `\p{Ll}\p{Lo}{0,62}`. * Label values must be between 0 and 63 characters long and must conform to the regular expression: `[\p{Ll}\p{Lo}\p{N}_-]{0,63}`. * No more than 64 labels can be associated with a given resource. * Keys and values must both be under 128 bytes. Labels and Tags (below) are both used to bind metadata to resources, with different use-cases. See https://cloud.google.com/resource-manager/docs/tags/tags-overview for an in-depth overview on the difference between tags and labels.
	Labels any
	// The unique name of the instance. Values are of the form `projects/{project}/instances/a-z+[a-z0-9]`.
	Name any
	// Required. The unique name of the project in which to create the new instance. Values are of the form `projects/{project}`.
	Parent any
	// Output only. Reserved for future use.
	SatisfiesPzi any
	// Output only. Reserved for future use.
	SatisfiesPzs any
	// Output only. The current state of the instance.
	State any
	// Optional. Input only. Immutable. Tag keys/values directly bound to this resource. For example: - "123/environment": "production", - "123/costCenter": "marketing" Tags and Labels (above) are both used to bind metadata to resources, with different use-cases. See https://cloud.google.com/resource-manager/docs/tags/tags-overview for an in-depth overview on the difference between tags and labels.
	Tags any
	// The type of the instance. Defaults to `PRODUCTION`.
	Type any
}

var BigtableadminInstance = ubx.ResourceBinding{
	WireType: "google_bigtableadmin_bigtableadmin_instance",
	Fields: ubx.FieldMap{
		"Clusters": ubx.FieldSpec{
			WireName: "clusters",
			Kind: "map",
			Fields: BigtableadminInstance_ClustersFields,
		},
		"Instance": ubx.FieldSpec{
			WireName: "instance",
			Kind: "object",
			Fields: BigtableadminInstance_InstanceFields,
		},
		"InstanceId": ubx.FieldSpec{WireName: "instance_id"},
		"Parent": ubx.FieldSpec{WireName: "parent"},
	},
}
