// Code generated by `ubx sdk gen` from google_managedidentities_v1alpha1_managedidentities_v1alpha1_backup. DO NOT EDIT.
package managedidentities

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type V1alpha1ManagedidentitiesV1alpha1BackupConfig struct {
	// Output only. The time the backups was created.
	CreateTime any
	// Optional. A short description of the backup.
	Description any
	// Optional. Resource labels to represent user provided metadata.
	Labels any
	// Output only. The unique name of the Backup in the form of projects/{project_id}/locations/global/domains/{domain_name}/backups/{name}
	Name any
	// Output only. The current state of the backup.
	State any
	// Output only. Additional information about the current status of this backup, if available.
	StatusMessage any
	// Output only. Indicates whether it’s an on-demand backup or scheduled.
	Type any
	// Output only. Last update time.
	UpdateTime any
}

type V1alpha1ManagedidentitiesV1alpha1BackupAttrs struct {
	// Output only. The time the backups was created.
	CreateTime any
	// Optional. A short description of the backup.
	Description any
	// Optional. Resource labels to represent user provided metadata.
	Labels any
	// Output only. The unique name of the Backup in the form of projects/{project_id}/locations/global/domains/{domain_name}/backups/{name}
	Name any
	// Output only. The current state of the backup.
	State any
	// Output only. Additional information about the current status of this backup, if available.
	StatusMessage any
	// Output only. Indicates whether it’s an on-demand backup or scheduled.
	Type any
	// Output only. Last update time.
	UpdateTime any
}

var V1alpha1ManagedidentitiesV1alpha1Backup = ubx.ResourceBinding{
	WireType: "google_managedidentities_v1alpha1_managedidentities_v1alpha1_backup",
	Fields: ubx.FieldMap{
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Description": ubx.FieldSpec{WireName: "description"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"State": ubx.FieldSpec{WireName: "state"},
		"StatusMessage": ubx.FieldSpec{WireName: "status_message"},
		"Type": ubx.FieldSpec{WireName: "type"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
