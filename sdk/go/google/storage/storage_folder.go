// Code generated by `ubx sdk gen` from google_storage_storage_folder. DO NOT EDIT.
package storage

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type StorageFolder_PendingRenameInfo struct {
	// The ID of the rename folder operation.
	OperationId any
}

var StorageFolder_PendingRenameInfoFields = ubx.FieldMap{
		"OperationId": ubx.FieldSpec{WireName: "operation_id"},
	}

type StorageFolderConfig struct {
	// The name of the bucket containing this folder.
	Bucket any
	// The creation time of the folder in RFC 3339 format.
	CreateTime any
	// The ID of the folder, including the bucket name, folder name.
	Id any
	// The kind of item this is. For folders, this is always storage#folder.
	Kind any
	// The version of the metadata for this folder. Used for preconditions and for detecting changes in metadata.
	Metageneration any
	// The name of the folder. Required if not specified by URL parameter.
	Name any
	// Only present if the folder is part of an ongoing rename folder operation. Contains information which can be used to query the operation status.
	PendingRenameInfo any
	// The link to this folder.
	SelfLink any
	// The modification time of the folder metadata in RFC 3339 format.
	UpdateTime any
}

type StorageFolderAttrs struct {
	// The name of the bucket containing this folder.
	Bucket any
	// The creation time of the folder in RFC 3339 format.
	CreateTime any
	// The ID of the folder, including the bucket name, folder name.
	Id any
	// The kind of item this is. For folders, this is always storage#folder.
	Kind any
	// The version of the metadata for this folder. Used for preconditions and for detecting changes in metadata.
	Metageneration any
	// The name of the folder. Required if not specified by URL parameter.
	Name any
	// Only present if the folder is part of an ongoing rename folder operation. Contains information which can be used to query the operation status.
	PendingRenameInfo any
	// The link to this folder.
	SelfLink any
	// The modification time of the folder metadata in RFC 3339 format.
	UpdateTime any
}

var StorageFolder = ubx.ResourceBinding{
	WireType: "google_storage_storage_folder",
	Fields: ubx.FieldMap{
		"Bucket": ubx.FieldSpec{WireName: "bucket"},
		"CreateTime": ubx.FieldSpec{WireName: "create_time"},
		"Id": ubx.FieldSpec{WireName: "id"},
		"Kind": ubx.FieldSpec{WireName: "kind"},
		"Metageneration": ubx.FieldSpec{WireName: "metageneration"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"PendingRenameInfo": ubx.FieldSpec{
			WireName: "pending_rename_info",
			Kind: "object",
			Fields: StorageFolder_PendingRenameInfoFields,
		},
		"SelfLink": ubx.FieldSpec{WireName: "self_link"},
		"UpdateTime": ubx.FieldSpec{WireName: "update_time"},
	},
}
