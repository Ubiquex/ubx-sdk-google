...
type AlphaHaController_NetworkingAutoConfiguration struct {
...
type AlphaHaController_Status_FailoverProgress_LastFailoverAttempt struct {
...
type AlphaHaController_Status_FailoverProgress struct {
...
type AlphaHaControllerAttrs struct {
	// Advanced configuration option. If specified, these Backend Services need to be pre-created. Currently, only one backend service can be specified, and it must be L4 Internal Load Balancer (ILB).
	BackendServices any
	// Output only. [Output Only] Creation timestamp in RFC3339 text format.
	CreationTimestamp any
